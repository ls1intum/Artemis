 ...
 *   - Artemis state tokens via the ARBITRARY form (collision-safe while Bootstrap is still loaded unlayered —
 *     a named `text-danger` would lose to Bootstrap's unlayered `!important` utility): text-(--danger),
 *     bg-(--success), border-(--warning), text-(--info); or `[style.color]="'var(--danger)'"` when dynamic;
 *     or a PrimeNG `severity` component (p-message / p-tag / p-button). info is CYAN: use --info, never blue.
 *
 * Scope: static `class="..."`, `[class.<palette>]`, the bound source of `[class]`/`[ngClass]`, and the bound
 * source of any binding (to catch `var(--p-*-NNN)` in `[style.*]`/`[color]`). Brand-bound semantic `-(--token)`
 * arbitrary values (text-(--danger), bg-(--surface-100)) are intentionally allowed.
...
export default {
    meta: {
        ...
        messages: {
            rawPalette:
                "Raw Tailwind color class '{{cls}}' is not allowed — it is not brand-bound and breaks theming/dark mode. Use a semantic token (text-primary / text-surface-* / text-muted-color / text-(--danger)|(--success)|(--warning)|(--info)). See client-development.mdx (### Styling).",
            primitivePalette:
                "PrimeNG primitive palette token '{{cls}}' is used to express meaning — primitives encode VALUE (red-500), not INTENT. Use the SEMANTIC token: text-(--danger)/(--success)/(--warning)/(--info) (info is cyan), or a PrimeNG severity component. See client-development.mdx (### Styling).",
        },
        schema: [],
    },
    create(context) {
