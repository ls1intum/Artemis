...
public class OpenApiConfiguration {
    ...
    public SpringfoxCustomizer noApiFirstCustomizer() {
        return docket -> docket.select().apis(RequestHandlerSelectors.basePackage("de.tum.in.www1.artemis.gateway.web.api").negate());
    }
    ...
    public Docket apiFirstDocket(JHipsterProperties jHipsterProperties) {
        ...
        return new Docket(DocumentationType.OAS_30).groupName("openapi").host(properties.getHost()).protocols(new HashSet<>(Arrays.asList(properties.getProtocols())))
                ...
                .apis(RequestHandlerSelectors.basePackage("de.tum.in.www1.artemis.gateway.web.api")).paths(regex(properties.getDefaultIncludePattern())).build();
    }
}
