...
if [ "$SKIP_CLIENT" = false ]; then
    echo ""
    echo -e "${BLUE}Step 2b: Starting client (bun run start)...${NC}"
    ...
    if [ "$DEBUG" = true ]; then
        bun run start > >(tee "$LOCAL_DIR/client.log") 2>&1 &
    else
        bun run start > "$LOCAL_DIR/client.log" 2>&1 &
    fi
    CLIENT_PID=$!
...
TEST_CMD=(bunx playwright test "${BASE_ARGS[@]}" --project=fast-tests --project=slow-tests --workers="$TEST_WORKERS")
