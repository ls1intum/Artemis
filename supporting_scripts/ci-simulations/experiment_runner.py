...
from utils import add_user_to_course, authenticate_user
from admin_operations import (
    create_single_programming_exercise,
...
class ExperimentRunner:
    ...
    def authenticate_single_user(self, user_index: int) -> tuple[int, Session]:
        ...
        try:
            authenticate_user(username, password, session, SERVER_URL)
    ...
    def register_students_to_course(self, user_count: int):
        ...
        def register_user(user_index):
            add_user_to_course(
                self.admin_session, self.course_id, "students", USER_NAME_PATTERN.format(user_index), SERVER_URL
            )
    ...
    def setup_experiment(self, experiment_config: ExperimentConfig, number_of_students: int, experiment_id: str = str(uuid4())):
        ...
        authenticate_user(admin_user, admin_password, self.admin_session, SERVER_URL)
