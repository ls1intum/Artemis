...
PACKAGE_FILES=("package.json" "documentation/package.json" "src/test/playwright/package.json")
...
check_dep() {
  ...
  if [[ "$CUR_VERSION" =~ ^\^([0-9]+)\.([0-9]+)\.([0-9]+)$ ]]; then
    ...
    if (( LATEST_MAJOR > CUR_MAJOR )); then
      echo "[$SOURCE_FILE] $DEP_NAME: Current -> $CUR_VERSION, Latest -> $LATEST_VERSION"
...
for PACKAGE_JSON in "${PACKAGE_FILES[@]}"; do
  if [ ! -f "$PACKAGE_JSON" ]; then
    echo "Skipping $PACKAGE_JSON (not found)" >&2
    continue
  fi
  ...
  for PACKAGE in $OVERRIDES; do
    ...
    if [[ "$CUR_VALUE" =~ "{" ]]; then
      ...
      for SUB_DEP in $SUB_PACKAGES; do
        ...
        check_dep "$SUB_DEP" "$CUR_SUB_VERSION" "$PACKAGE_JSON"
      done
    else
      # Direct dependency.
      check_dep "$PACKAGE" "$CUR_VALUE" "$PACKAGE_JSON"
    fi
  done
done
