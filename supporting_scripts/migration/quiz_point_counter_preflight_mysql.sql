...
-- Archive the complete mysql client output as the migration preflight artifact.
...
WITH totals AS (
    ...
    SELECT t.exercise_id,
           ...
           GREATEST(COALESCE(c.rated_counter, 0), 0) AS normalized_rated_counter,
           GREATEST(COALESCE(c.un_rated_counter, 0), 0) AS normalized_unrated_counter
    FROM quiz_statistic_counter c
    ...
    SELECT n.*,
           COALESCE(
               MIN(CASE WHEN source_points = normalized_points THEN source_id END) OVER (PARTITION BY statistic_id, normalized_points),
...
SELECT exercise_id,
       ...
       overall_points
FROM with_target_id
WHERE NOT (source_points <=> normalized_points)
...
WITH normalized AS (
    SELECT qs.id AS statistic_id,
           ...
           GREATEST(COALESCE(c.rated_counter, 0), 0) AS rated_counter,
           GREATEST(COALESCE(c.un_rated_counter, 0), 0) AS unrated_counter
    FROM quiz_statistic qs
...
HAVING NOT (participants_rated <=> COALESCE(SUM(rated_counter), 0))
    OR NOT (participants_unrated <=> COALESCE(SUM(unrated_counter), 0))
