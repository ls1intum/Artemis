...
`frontendUrl` and pass a required check. Each allowlisted occurrence is replaced by a NUL, which is
a non-word character, so removing it can never splice two fragments into a new match.
...
SELF_TEST_CASES = (
    ...
    # The bypass the allowlist must not open: an allowlisted name cannot shield a coined one that
