...
# into any bun-managed file; instead we re-run the prebuild after bumping so a
# developer's local checkout shows the new number immediately without a full Gradle
# build. Skip this with --no-prebuild-refresh if bun/Node aren't available.
#
# Usage:
...
bump_version() {
    ...
    # Refresh the client-side __VERSION__ constant so `bun start` / `bun run webapp:build`
    ...
    if [[ "$REFRESH_PREBUILD" == true ]]; then
        if command -v bun >/dev/null 2>&1; then
            echo "Refreshing client environment.override.ts via bun run prebuild..."
            APP_VERSION="$new" bun run prebuild >/dev/null
            echo "Client __VERSION__ now reflects $new."
        else
            echo "Note: bun not on PATH; skipping client prebuild refresh." >&2
            echo "      Run \`APP_VERSION=$new bun run prebuild\` (or any bun build) to update the client constant." >&2
        fi
    fi
}
