...
# =============================================================================
# The distributed data provider is chosen by a single Artemis property. Everything else about the stack is identical, so
...
# artemis.continuous-integration.data-store in a deployment's own config from deciding the provider behind your back.
services:
    artemis-app-node-1:
        env_file:
...
if [ "$SKIP_UP" = false ]; then
    ...
    if [ "$MIDDLEWARE" = "hazelcast" ]; then
        ...
        # not reach that node and the run is silently testing the wrong provider.
