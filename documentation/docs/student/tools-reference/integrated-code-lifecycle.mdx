...
ICL combines version control, secure communication, and continuous integration to provide a seamless experience for students.
...
How to navigate through the repository and view the code is explained in the following sections.
...
To access your submissions, you need to navigate to the exercise, press the '_Code_' button, and click on <Image src={openRepositoryButtonImg} alt={"Open Repository"} size={ImageSize.small} inline />.
...
the '_Code_' button to clone the repository to your local machine and see the full commit history by pressing the '_Open Commit History_' button. Finally, you can
download the repository as a ZIP file by pressing the '_Download Repository_' button.

<Image src={currentRepositoryContentImg} alt={'Current Repository Content'} size={ImageSize.large} />

The _Commit History_ page shows all the commits that you uploaded to the repository. The commits are ordered from the latest to the oldest.
...
As a student, token authentication normally uses a participation token.
Artemis automatically generates one token for each programming exercise repository you participate in and provides it through the code button dialog.
Instructor and course-staff token behavior is documented separately in the [instructor Integrated Code Lifecycle documentation](/instructor/integrations/integrated-code-lifecycle#repository-access-and-automatic-tokens-for-course-staff).
...
You can review and revoke the participation tokens you own at any time under **User Settings → Access Tokens**.
Revoking a token invalidates it immediately; a new one is created automatically the next time you open the clone dialog for that exercise.
...
The tokens for student repositories are automatically created and provided through the code button dialog.
For every programming exercise you participate in, Artemis automatically generates an access token associated only with that repository.
You do not need to create this token manually.
