...
| Render links to exercises, lectures, other chats, lecture-units and slides with correct icon |  | <AVAILABLE /> | <AVAILABLE /> | <AVAILABLE /> |
...
and the content of these lectures. All references are then prepended with icons which are unique to the reference's type,
...
Note that only the author of the message as well as tutors and instructors can perform this action.
