...
3. The **organizational unit IDs** of your departments or faculties in CAMPUSOnline (managed via the admin API).
...
Organizational units are managed dynamically via the admin API (see below) and stored in the database, so no YAML configuration is needed for them.
...
3. Select the organizational unit from the dropdown
4. Type the course name in the search field (minimum 3 characters)
5. Select the correct course from the dropdown (shows instructor, department, and semester for disambiguation)
6. Save the course
...
| `PUT` | `/api/core/admin/campus-online/org-units/{orgUnitId}` | Update an organizational unit |
| `DELETE` | `/api/core/admin/campus-online/org-units/{orgUnitId}` | Delete an organizational unit |
| `POST` | `/api/core/admin/campus-online/org-units/import` | Bulk-import organizational units from CSV |
...
| `GET` | `/api/core/admin/campus-online/courses/search?query={name}&orgUnitId={id}&semester={sem}` | Search courses by name (typeahead) |
...
- Verify that at least one organizational unit has been created via the admin API
- Ensure you have selected the correct organizational unit from the dropdown
