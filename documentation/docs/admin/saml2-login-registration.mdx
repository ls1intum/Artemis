...
# External client redirect (VS Code extension, mobile apps)

By default, after a successful SAML2 login Artemis redirects to the web UI (`/`).
External clients such as the VS Code extension or the mobile apps can instead receive a JWT directly:
when the SAML2 flow is started with an additional `redirect_uri` query parameter
(e.g. `{baseUrl}/saml2/authenticate/{registrationId}?redirect_uri=vscode://artemis/callback`),
Artemis redirects to that URI with the JWT appended as a `jwt` query parameter after the login succeeds.

The feature is **disabled by default** and opt-in.
Configure the URI schemes you trust under `saml2.allowed-redirect-schemes`; an empty list keeps the feature disabled.

```yaml
saml2:
    # Empty list (default) disables the feature. http and https are always rejected.
    allowed-redirect-schemes: ['vscode', 'artemis-ios', 'artemis-android']
    # Lifetime of the JWT minted via the external redirect (minutes). Default: 60.
    external-redirect-token-ttl-minutes: 60
```

If a `redirect_uri` is provided while the feature is disabled, it is ignored and the user is redirected to `/`.

**Security note (accepted residual risk):**
The JWT is delivered as a query parameter on a custom-scheme URI.
A malicious application registered for the same scheme on the user's machine could intercept it,
and the JWT may appear in non-default reverse-proxy access logs.
`http` and `https` schemes are always rejected, which prevents classic remote open-redirect token theft,
and the server-side nonce is single-use with a 5-minute expiry.
JWTs minted for the redirect flow have an independently configurable, short lifetime (default 60 minutes) to limit the blast radius of an interception.
Enable this feature only if you accept this residual risk for the custom schemes you allowlist.
