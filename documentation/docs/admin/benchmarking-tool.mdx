...
| # | Students | Mode | Purpose |
| --- | --- | --- | --- |
| S0 | 10 | `CREATE_COURSE_AND_EXAM` | Smoke test. Proves the wiring, the users, and the build agents work before anything larger is attempted. |
| S1 | 100 | `CREATE_COURSE_AND_EXAM` | Baseline. Every later run is compared against these percentiles. |
| S2 | 500 | `CREATE_COURSE_AND_EXAM` | First point where queueing usually becomes visible. |
| S3 | 1000 | `CREATE_COURSE_AND_EXAM` | Realistic large course. |
| S4 | 2000 | `CREATE_COURSE_AND_EXAM` | The size Artemis is publicly described as handling. This is the run that substantiates that claim. |
| S5 | 3000+ | `CREATE_COURSE_AND_EXAM` | Only if S4 stays comfortable. Finds the actual limit rather than confirming a target. |
