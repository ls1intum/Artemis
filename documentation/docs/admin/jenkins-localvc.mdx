...
sidebar_label: Jenkins and LocalVC Setup
---
...
1. You can now first build and deploy Jenkins, then you can also start the other services which weren't started yet:
   ...
   ```bash
   JAVA_OPTS=-Djenkins.install.runSetupWizard=false docker compose --env-file .env -f docker/<Jenkins setup to be launched>.yml up --build -d jenkins
   docker compose --env-file .env -f docker/<Jenkins setup to be launched>.yml up -d
   ```
...
2. If you're using `docker compose`, you can simply use the following command and skip the next steps.
   ...
   ```bash
   docker compose --env-file .env -f docker/<Jenkins setup to be launched>.yml up --build -d
   ```
