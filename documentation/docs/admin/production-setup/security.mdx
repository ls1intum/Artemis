...
Build agents clone repositories from the core nodes. Three mechanisms exist, and a clone is only served when the caller is a known agent, is calling from an address that agent is connected from, and is asking for a repository it currently has a build job for.

| Mechanism | Configuration | Scope |
|---|---|---|
| **Build job clone token** (default over HTTPS) | none | the repositories of one running build job |
| **SSH key** (recommended) | `build-agent-use-ssh: true` plus `ssh-template-clone-url` and `ssh-private-key-folder-path` | the repositories of the agent's running build jobs |
| **Shared git credentials** (**refused with local CI**) | `build-agent-git-username` and `build-agent-git-password` | *every* repository in the installation |

<Callout variant={CalloutVariant.warning}>
    **A node that runs local CI refuses to start with either shared credential configured.** Clear
    `build-agent-git-username` and `build-agent-git-password`; the shipped configuration no longer sets them.
