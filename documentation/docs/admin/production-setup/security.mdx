...
<Callout variant={CalloutVariant.info}>
    ...
    **The binding works on both providers, by two routes.** It compares where an agent was seen with where its clone
    ...
    provider regardless**, since both are checked against the address of the request itself.
</Callout>
