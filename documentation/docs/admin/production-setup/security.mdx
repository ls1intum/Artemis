...
<Callout variant={CalloutVariant.info}>
    ...
    The address binding applies to an agent whose node has a middleware connection to observe, on either backend.
    Hazelcast names a build agent client by its own short name; Redis has no member/client split, so a node's client
    name is its node identity and Artemis maps it back through the member address the agent publishes. An agent whose
    node opens no client connection at all — one sharing a JVM with a core node under Hazelcast — has no origin to
    check and is left unbound rather than refused, as is every agent when the middleware cannot report client addresses.
