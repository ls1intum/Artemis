...
<Callout variant={CalloutVariant.warning}>
    Set `artemis.version-control.build-agent-use-ssh` to the same value on the build agents *and* on every core node. It decides both which mechanism an agent uses and which one a core node accepts, so a mismatch fails every clone: agents that authenticate over HTTPS against a core node configured for SSH are rejected, and vice versa. A fleet in which only some agents use SSH cannot be served — the switch is installation-wide.
