...
| **Shared git credentials** (**deprecated**) | `build-agent-git-username` and `build-agent-git-password` | *every* repository in the installation |

On a node that runs local CI, leave both shared credentials empty. They are only still needed by a client that is not an Artemis build agent and therefore has neither a key nor a build job, which in practice means [Jenkins with LocalVC](/admin/jenkins-localvc).

<Callout variant={CalloutVariant.warning}>
    The shared credential pair is deprecated. One static secret, held by every build agent and every core node and
    usually written into several configuration files, opens every repository in the installation, and it does so ahead
    of the rate limit, the authorization checks and the access log, which makes a leak both unlimited and invisible.

    Clear `build-agent-git-username` and `build-agent-git-password` unless Jenkins clones from this installation. Build
    agents do not need them: over HTTPS an agent authenticates with the clone token of the job it is running, and with
    `build-agent-use-ssh: true` it authenticates with a key it generates at startup and never sends anywhere.
</Callout>
...
- A **clone token** is minted when a build job is queued and travels with it. It is accepted only for the repositories that job declares — the participant's assignment repository, the tests, the solution where the exercise checks it out, and any auxiliary repositories — and only from the agent that claimed the job. It has no expiry of its own and needs none: a job leaves the processing list when it finishes, is cancelled, or hits the build timeout, and the token stops working at that moment.
...
<Callout variant={CalloutVariant.info}>
    ...
    The address binding applies to an agent that has a cluster connection of its own to observe. An agent sharing a
    JVM with a core node opens none, and neither does one whose middleware client name is not its
    `build-agent.short-name` — under Redis that name is the node identity, so the two normally differ. Those agents are
    left unbound rather than refused, as is every agent when the middleware cannot report client addresses at all.
    Artemis says which of the two it is in the log, once, and the allowlist and the build job scoping apply either way.
</Callout>
