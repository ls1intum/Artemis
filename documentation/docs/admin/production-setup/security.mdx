...
artemis:
    ...
    version-control:
        # Deprecated. Omit both unless Jenkins clones from this installation:
        # build agents authenticate per build job, see "Build Agent Authentication" below.
...
<Callout variant={CalloutVariant.warning}>
    ...
    address is equally believed, so narrow `server.tomcat.remoteip.internal-proxies` to your proxies if the private
    network is not a space you fully control.
...
`artemis.version-control.build-agent-use-ssh` does different things on the two node roles. On a build agent it selects the mechanism the agent uses. On a core node it controls only whether the shared credential pair still grants repository-wide read. Core nodes always accept a registered build agent's public key, whatever the property says there, because a key is per-agent and only reaches a core node through an agent that has joined the cluster.
...
<Callout variant={CalloutVariant.warning}>
    Do not do it in the other order. A core node with `build-agent-use-ssh: true` no longer grants the shared credential
    pair repository-wide read, and `buildjob_user` is not a real account, so an agent that still relies on that pair
    fails to clone from it. Agents that use build job clone tokens are unaffected either way.

    Leave the credential pair configured when Jenkins is your CI backend. The [Jenkins with LocalVC](/admin/jenkins-localvc)
    setup has Jenkins clone over HTTPS with it, and Jenkins is not an Artemis build agent, so it has neither a key nor a
    build job to fall back to.
</Callout>
