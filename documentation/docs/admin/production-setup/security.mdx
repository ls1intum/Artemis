...
Both paths write a VCS access log entry naming the agent and the build job. The old shared-credential shortcut wrote nothing at all, which made the most privileged reader in the installation the only one that could not be audited.
...
<Callout variant={CalloutVariant.info}>
    ...
    If your middleware cannot report client connection addresses, Artemis logs this at startup and skips the address
    binding rather than refusing every build. The allowlist and the build job scoping still apply.
</Callout>
