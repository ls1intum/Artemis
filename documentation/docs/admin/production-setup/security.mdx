...
Under the `prod` profile, Artemis refuses to start when one of these properties still holds a value that is published in this repository: the JWT signing key, the internal admin password (also when it is identical to the username), and the build-agent git password. The startup error names the property and how to supply a value. Generate the signing key with `openssl rand -base64 64`.
...
```env
JHIPSTER_SECURITY_AUTHENTICATION_JWT_BASE64SECRET="<openssl rand -base64 64>"
