...
- `trusted-proxies` names the reverse proxies whose `X-Forwarded-For` header may be believed for HTTP git requests. Any client can set that header, so it is only consulted when the request's TCP peer is one of these addresses. Leave it empty when Artemis is reached directly.
