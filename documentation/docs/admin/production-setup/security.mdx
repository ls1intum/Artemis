...
Every build agent registers the address it connects to the cluster from. Core nodes observe that address on the agent's own cluster connection rather than taking the agent's word for it, and a clone is only served from an address the presenting agent is currently connected from.
...
<Callout variant={CalloutVariant.info}>
    ...
    **The address binding needs Hazelcast.** It compares where the middleware saw an agent connect with where its clone
    arrives, and those are the same path only when the agent's middleware connection terminates on a core node.
    Hazelcast clients connect to the cluster members, which are exactly those nodes. Redis is a separate service, so an
    agent's connection to it says nothing about its route to a core node — with Redis in a container and the nodes on
    the host, one side observes the docker bridge gateway and the other loopback. Artemis therefore skips the binding
    on Redis rather than refusing every clone, and says so in the log at startup.

    An agent that opens no client connection at all — one sharing a JVM with a core node under Hazelcast — is likewise
    unbound. **`allowed-ranges` and the per-build-job scoping apply on every backend either way**, because both are
    checked against the address of the request itself rather than against an observation.
</Callout>
