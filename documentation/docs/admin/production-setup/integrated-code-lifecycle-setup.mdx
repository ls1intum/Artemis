---
id: integrated-code-lifecycle-setup
title: Integrated Code Lifecycle Setup
sidebar_label: Integrated Code Lifecycle Setup
---
...
- Start the database: [Database Setup](/developer/setup#database-setup) (ignore if previously done, you should either have a database running locally or in a container)
...
By default, every build runs in a Docker container on the build agent, which is what the rest of this page assumes.
A production installation can instead let each build run as a Kubernetes Job; see
[LocalCI Build Runners](/admin/production-setup/build-runners).
...
Please read [Run the server via a run configuration in IntelliJ](/developer/setup#run-the-server-via-a-run-configuration-in-intellij) for more details.
...
```bash
docker compose --env-file .env -f docker/artemis-dev-local-vc-local-ci-mysql.yml up
```
