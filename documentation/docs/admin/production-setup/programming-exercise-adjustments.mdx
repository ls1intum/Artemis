...
Either key can be set independently. When set, the corresponding host directory is bind-mounted into every build
container at `/root/.m2` or `/root/.gradle` respectively, which is `$HOME/.m2` / `$HOME/.gradle` for the default
root-based Java build images shipped with Artemis. When unset (the default), no bind mounts are added and the
...
- **Cache poisoning between submissions (security trade-off).** A student submission that successfully resolves a
  dependency writes the artifact into the shared host cache. Subsequent submissions — for the same student, a
  different student, or a different exercise on the same agent — will consume the cached copy without re-validating
  against Maven Central. A malicious build could deliberately seed the cache with a manipulated artifact under a
  coordinate that a future submission declares, which would then be silently consumed by that build. The pre-baked
  dependencies in the official Artemis build images (e.g. `ls1tum/artemis-maven-template`) cover the common case and
  are not affected because they are baked into the image layer, not the bind-mounted cache. **Recommended deployment
  pattern:** use bind-mount caching only on agents that run general programming exercises without secret tests, or
  in combination with the network-restriction patterns described under [Security Considerations](#security-considerations-for-secret-tests).
  For exam-grade isolation, the [Docker volumes approach below](#caching-docker-volumes) — which mounts the cache
  read-only for student submissions and read-write only for the trusted solution warm-up — remains the stronger
  option. A future iteration can add a per-submission read-only mode to this mechanism; tracked as follow-up.
