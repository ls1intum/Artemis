...
- They must be writable by the build-container user. The official Java build images run as root, so mode `0777` on
  the host is the simplest choice and also accommodates custom images that use a different non-root UID.
...
- **Gradle file locks.** Gradle's `~/.gradle/caches` uses file locks; concurrent Gradle builds on the same agent will
  ...
  large `concurrent-build-size`, consider switching the Gradle cache to a read-only mount populated by a warm-up
  build (`GRADLE_RO_DEP_CACHE`) — out of scope for the default configuration.
...
- **No isolation between submissions.** All builds on the same agent share the same on-disk cache. If you rely on
  cache isolation for security-sensitive secret tests, prefer the [Docker volumes approach below](#caching-docker-volumes)
  with read-only mounts for student submissions.
