...
default root-based Java build images shipped with Artemis. When unset (the default), no bind mounts are added and
...
- `read-only: true` — both caches are mounted with `:ro`. Untrusted student submissions cannot mutate the shared
  cache, so cache poisoning is structurally impossible. The trade-off is that the cache must be populated out of
  band — typically by a trusted "warm-up" build the operator runs separately (`docker run --rm -v /var/cache/artemis-buildagent/m2:/root/.m2 ls1tum/artemis-maven-template:… mvn -f /path/to/seed-pom.xml dependency:resolve`)
  or by occasionally flipping the flag back to `false` long enough to repopulate. An empty or partial read-only
  cache will fail builds because Maven/Gradle cannot write fetched artifacts to a read-only local repository —
  warm-up is required before enabling read-only.
...
- The cache grows over time. A weekly cron such as
  `find <path> -atime +30 -type f -delete && find <path> -type d -empty -delete` keeps both the file footprint and
  the empty-directory residue bounded. Verify that the host's filesystem is not mounted with `noatime` before
  relying on access-time-based pruning, otherwise switch to `-mtime`.
