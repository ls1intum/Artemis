...
3. Set up the Maven proxy repository:
   a. Create a new repository (_Repository - Repositories - Create repository_) of type `maven2 (proxy)` with name `maven-proxy`.
   b. The remote URL is https://repo1.maven.org/maven2/.
4. Optionally create a new cleanup policy under _Repository - Cleanup Policies_
    1. Format: `maven2`
...
This setting is relevant only when using [Integrated Code Lifecycle Setup](https://docs.artemis.cit.tum.de/dev/setup/integrated-code-lifecycle.html).
