...
Ensure that the values are consistent across these files. In a [multi-node setup](https://docs.artemis.cit.tum.de/admin/setup/distributed.html), also verify that both core nodes and build agent nodes use the same values.
...
Ensure that these values remain consistent across all nodes in a [multi-node setup](https://docs.artemis.cit.tum.de/admin/setup/distributed.html) to maintain uniform resource allocation.
