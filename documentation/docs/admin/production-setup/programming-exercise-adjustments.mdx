...
Set the following two optional keys in `application-buildagent.yml` on each build agent:
...
- The cache grows over time. A weekly cron such as
  `find <path> -atime +30 -type f -delete && find <path> -mindepth 1 -type d -empty -delete` keeps both the file
  footprint and the empty-directory residue bounded. The default Linux mount option (`relatime`) updates `atime`
  often enough that this predicate behaves intuitively; on `noatime` mounts switch to `-mtime` instead, but note
  that `-mtime` reflects download time and will re-download an old-but-hot artefact after each prune.
- Pair the prune with a disk-space alert on the cache filesystem. The exact mechanism depends on your monitoring
