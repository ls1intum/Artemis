...
- The cache grows over time. A weekly cron such as `find <path> -atime +30 -type f -delete` keeps it bounded; verify
  that the host's filesystem is not mounted with `noatime` before relying on access-time-based pruning, otherwise
  switch to `-mtime`.
