...
Core nodes serve the main functionality of Artemis. In ICL, this additionally includes the [CI Management](https://docs.artemis.tum.de/developer/system-design#ci-management),
responsible for managing and interacting with the build job queue (adding, cancelling and viewing build jobs), and the [Local VC system](https://docs.artemis.tum.de/developer/setup#integrated-code-lifecycle-setup).

For ICL, the run configuration for core nodes need to include the additional profiles `core`, `localvc` and `localci`, e.g.:

```bash
--spring.profiles.active=prod,core,ldap,localvc,localci,athena,scheduling,iris
```

To enable LTI integration, add `--artemis.lti.enabled=true` to your configuration.

Core nodes do not require further adjustments to the `application-prod.yml`, as long as you have added the necessary variables as described in the [Integrated Code Lifecycle Setup](https://docs.artemis.tum.de/developer/setup#integrated-code-lifecycle-setup).
