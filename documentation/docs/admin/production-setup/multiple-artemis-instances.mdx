---
id: multiple-artemis-instances
title: Multiple Artemis Instances
sidebar_label: Multiple Artemis Instances
---
...
Artemis synchronizes distributed data through either [Hazelcast](https://hazelcast.com), which is the default, or a
Redis-compatible store. All instances must use the same provider. The
[distributed-data administration guide](/admin/production-setup/distributed-data) explains the operational differences
and the required upgrade procedure for Redis and Valkey.
...
artemis:
    ...
    failed-build-logs-path: {{ artemis_repo_basepath }}/failed-build-logs
```

Where `{{ artemis_repo_basepath }}` is the path to the shared folder

The two build log folders have to be shared as well, because the instance that writes a build log is rarely the one
that later serves it: `build-logs-path` holds the downloadable log of every build job, and `failed-build-logs-path`
holds the logs of failed builds that the build output panel, Iris and Hyperion read.
...
Artemis is **not** deployed as a rolling update. A new version is deployed by stopping the cluster
and bringing it back up in a defined order, so that no instance ever serves traffic against a schema
it was not built for:

1. Stop every Artemis instance.
2. Start the first instance on its own, and wait until it has fully initialized. This is the instance
   that applies the Liquibase migrations of the new version.
3. Once the migrations have finished and the first instance is up, start the remaining instances.

Starting several instances at once is what this order avoids. Liquibase takes a lock on its own
changelog table, so a second instance starting mid-migration waits for that lock rather than
corrupting anything, but it waits without a useful timeout and reports nothing about why. Waiting for
the first instance is both faster and easier to diagnose.

<Callout variant={CalloutVariant.info}>
  Because the cluster is stopped for the migration, a changelog never has to be compatible with both
  the old and the new version of the application at the same time. A migration that adds a column,
  backfills it, and adds a constraint over it can do all three in one release.
</Callout>

<Callout variant={CalloutVariant.warning}>
  Stopping the cluster removes the need for two versions to share a schema, but it does not make
  **rolling back** safe. Starting the previous version again does not undo the migration: the schema
  it meets is the new one. A column added as `NOT NULL` without a default breaks every `INSERT` the
  previous version issues against that table, even though it never selects the column, and a
  constraint the new version tightened rejects writes the previous version considers valid.

  So a version is only revertible on its own if the new schema still accepts everything the previous
  version reads and writes. Where a migration breaks that — and adding a required column or
  tightening a constraint usually does — plan the way back before deploying: take a backup
  immediately before the migration and restore it as part of the rollback, or keep the schema
  backward compatible and complete the tightening in a later release.
</Callout>
...
Core nodes serve the main functionality of Artemis. In ICL, this additionally includes the [CI Management](/developer/system-design#ci-management),
responsible for managing and interacting with the build job queue (adding, cancelling and viewing build jobs), and the [Local VC system](/developer/system-design#version-control-subsystem).
...
Core nodes do not require further adjustments to the `application-prod.yml`, as long as you have added the necessary variables as described in the [Integrated Code Lifecycle Setup](/admin/production-setup/integrated-code-lifecycle-setup).
...
4. Configuration related to the execution of build jobs:
    ...
    artemis:
        continuous-integration:
            build-runner: docker                                # Supported values: docker, kubernetes. See Build Runners.
...
The settings above configure the default Docker build runner, which executes each build in a container on the build agent host.
Artemis also ships a proof-of-concept Kubernetes runner that executes each build as a namespaced Kubernetes Job instead, so the
build-agent pods need no Docker socket. [LocalCI Build Runners](/admin/production-setup/build-runners) compares both and documents
how to configure and deploy them.
...
3. Start the registry service, e.g., by running `docker compose --env-file .env -f docker/broker-registry.yml up`.
...
The shipped `prod-multinode.env` describes a local test installation and sets `INFO_TESTSERVER=true`, so its core
nodes would start without installation metadata. The operator, administrator, and institution it sets appear on the
About page; as a test server, the stack sends no telemetry. Standalone build agents ignore them. For a real deployment, set `ARTEMIS_ENV_FILE` to an external replacement containing the installation's actual
`INFO_OPERATORNAME`, `INFO_OPERATORADMINNAME`, and `INFO_UNIVERSITYNAME`, along with its credentials.
Set `INFO_TESTSERVER=false` for a production installation: its core nodes refuse to start without the three values, and
its scheduling node includes them in the [telemetry](/admin/telemetry) report, the administrator's name only when
`artemis.telemetry.sendAdminDetails` is enabled.
Keep these shared values in that file rather than in `node1.env`, so every core node receives the same identity.
...
<Callout variant={CalloutVariant.info}>
    ...
    `docker compose --env-file .env -f docker/test-server-multi-node-mysql-localci.yml up mysql jhipster-registry activemq-broker artemis-app-node-1`.
    After these containers are up and running, you can start the remaining containers:
    `docker compose --env-file .env -f docker/test-server-multi-node-mysql-localci.yml up artemis-app-node-2 artemis-app-node-3 nginx`.
</Callout>
...
1. When running the Artemis container on a Unix system, you will have to give the user running in the container
    ...
    group by running `getent group docker | cut -d: -f3`. Afterwards, add the following line to the `.env` file in the
    repository root, which is the file the start command below loads:
