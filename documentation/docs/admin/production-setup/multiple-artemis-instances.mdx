...
To enable Athena-based feedback suggestions, add `--artemis.athena.enabled=true` to your configuration. After enabling, AI feedback features must also be activated per course by an instructor or administrator via the course settings. The Athena modules for each exercise type (text, programming, modeling) are configured at the application level via `artemis.athena.modules.*` and apply to all courses. See [Athena Service](/admin/extensions-setup#athena-service) for the full configuration reference.
