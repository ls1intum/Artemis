...
3. Furthermore, you will need some configuration related to version control and continuous integration.
    ...
    These shared credentials are deprecated and can be omitted: a build agent authenticates with the clone token of the build job it is running, or with an SSH key as described after the example below. Configure them only if Jenkins clones from this installation.
