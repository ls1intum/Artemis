...
The core nodes read the operator, administrator, and institution from this shared file; standalone build agents
ignore them. For a real deployment, set `ARTEMIS_ENV_FILE` to an external replacement containing the installation's actual
...
`node1.env`, so every core node receives the same required identity.
