...
3. Furthermore, you will need some configuration related to version control and continuous integration.
    ...
    HTTPS authentication for build agents is deprecated; prefer SSH keys as described after the example below.
    ...
    Preferably, set `artemis.version-control.build-agent-use-ssh: true` to have every agent generate an SSH key pair at startup and authenticate with it, which removes the need for a shared credential entirely; both properties above may then be omitted. Set it on the build agents first — localvc nodes accept a registered agent's key regardless of their own setting, so builds keep working — and only afterwards on the localvc nodes, where it stops that pair from granting build agents read access to every repository. See [Build Agent Authentication](/admin/production-setup/security#build-agent-authentication).
