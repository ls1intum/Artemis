...
3. Furthermore, you will need some configuration related to version control and continuous integration.
    ...
    Alternatively, set `artemis.version-control.build-agent-use-ssh: true` to have every agent generate an SSH key pair at startup and authenticate with it, which removes the need for a shared credential. Set the property to the same value on the build agents *and* on every localvc node: it decides both which mechanism an agent uses and which one a localvc node accepts, and a node with SSH enabled rejects the git username and password rather than leaving a repository-wide read shortcut open. See [Build Agent Authentication](/admin/production-setup/security#build-agent-authentication).
