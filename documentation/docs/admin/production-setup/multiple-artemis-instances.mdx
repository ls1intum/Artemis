...
responsible for managing and interacting with the build job queue (adding, cancelling and viewing build jobs), and the [Local VC system](/developer/setup#integrated-code-lifecycle-setup).
...
Core nodes do not require further adjustments to the `application-prod.yml`, as long as you have added the necessary variables as described in the [Integrated Code Lifecycle Setup](/developer/setup#configure-artemis-for-integrated-code-lifecycle).
