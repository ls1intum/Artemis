...
nodes would start without installation metadata; the operator, administrator, and institution it sets only fill the
About page. Standalone build agents ignore them. For a real deployment, set `ARTEMIS_ENV_FILE` to an external replacement containing the installation's actual
`INFO_OPERATORNAME`, `INFO_OPERATORADMINNAME`, and `INFO_UNIVERSITYNAME`, along with its credentials.
Set `INFO_TESTSERVER=false` for a production installation, whose core nodes refuse to start without the three values.
