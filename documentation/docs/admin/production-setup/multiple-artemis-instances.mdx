...
3. Furthermore, you will need some configuration related to version control and continuous integration.
    ...
    Add the shared credential pair **only** if [Jenkins with LocalVC](/admin/jenkins-localvc) clones from this installation. Jenkins is not an Artemis build agent, so it has neither a key nor a build job and this pair is the only mechanism left for it. It is deprecated, and a core node accepts it for read access to *every* repository, so leave it out otherwise:

    ```yaml
    artemis:
        version-control:
            # Deprecated, and only needed for Jenkins with LocalVC. Set the same values on the Jenkins side.
            build-agent-git-username: buildjob_user # Replace with more secure credentials for production
            build-agent-git-password: buildjob_password # Replace with more secure credentials for production
    ```

    Preferably, set `artemis.version-control.build-agent-use-ssh: true` to have every agent generate an SSH key pair at startup and authenticate with it. Set it on the build agents first — localvc nodes accept a registered agent's key regardless of their own setting, so builds keep working — and only afterwards on the localvc nodes, where it stops the shared pair from granting read access to every repository.
