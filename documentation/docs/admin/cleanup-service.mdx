...
Both values must be positive and are validated when the configuration is bound, so a typo such as `0` or a negative number fails startup instead of being applied — this job has no kill switch and its deletions are irreversible, so a cutoff in the future would delete records written minutes ago.

The job runs at 03:10 every night on the node with the scheduling profile active. It deletes in batches of 5 000 rows, up to 200 batches per retention period per night, so a backlog that has accumulated over years is drained over several nights rather than in one very long transaction. The log line reports how many rows each period removed, and says explicitly when the per-run cap was reached and rows remain.

### Pruning a large backlog manually

On an instance upgrading from a version without this job, the table may hold years of accumulated rows, and letting the nightly job drain it takes as many nights as it takes. The same work can be done in one maintenance window with plain SQL (MySQL 8). The event type list below is the login record; everything else falls into the long-retention bucket.

**1. Create the index the deletes need.** Without it each statement scans the whole table. Liquibase creates the same index guarded by a `not indexExists` precondition, so creating it here simply makes that changeset a no-op on the next upgrade. This is an online operation in MySQL (no exclusive lock) but takes time proportional to table size.

```sql
ALTER TABLE jhi_persistent_audit_event ADD INDEX idx_persistent_audit_event_date (event_date);
```

**2. See how much is affected**, before deleting anything.

```sql
SELECT COUNT(*) AS expired_login_records
...
WHERE event_type IN ('AUTHENTICATION_SUCCESS', 'AUTHENTICATION_PASSKEY_SUCCESS', 'SAML2_AUTHENTICATION_SUCCESS', 'AUTHENTICATION_FAILURE', 'LOGOUT_SUCCESS')
  AND event_date < NOW() - INTERVAL 365 DAY;

SELECT COUNT(*) AS expired_other_events
FROM jhi_persistent_audit_event
WHERE (event_type IS NULL
  ...
  AND event_date < NOW() - INTERVAL 1825 DAY;
```

**3. Delete the login record older than one year.** The data rows go first, because `jhi_persistent_audit_evt_data`'s foreign key is `ON DELETE RESTRICT` and the database rejects deleting a parent whose data rows still exist.

```sql
DELETE d
FROM jhi_persistent_audit_evt_data d
    JOIN jhi_persistent_audit_event e ON d.event_id = e.event_id
WHERE e.event_type IN ('AUTHENTICATION_SUCCESS', 'AUTHENTICATION_PASSKEY_SUCCESS', 'SAML2_AUTHENTICATION_SUCCESS', 'AUTHENTICATION_FAILURE', 'LOGOUT_SUCCESS')
  AND e.event_date < NOW() - INTERVAL 365 DAY;
...
**4. Delete everything else older than five years.** Note the `IS NULL`: `event_type` is nullable and `NULL NOT IN (...)` is unknown rather than true, so without it those rows would never be removed.
...
WHERE (e.event_type IS NULL
    OR e.event_type NOT IN ('AUTHENTICATION_SUCCESS', 'AUTHENTICATION_PASSKEY_SUCCESS', 'SAML2_AUTHENTICATION_SUCCESS', 'AUTHENTICATION_FAILURE', 'LOGOUT_SUCCESS'))
  AND e.event_date < NOW() - INTERVAL 1825 DAY;
...
**If the counts from step 2 are large**, delete in batches instead, so no single statement holds locks and undo log for millions of rows or hands a replica one enormous transaction. Repeat this pair until it reports 0 rows affected:
...
Both statements order by `event_id`, which is unique, so they select exactly the same rows and the foreign key is never violated; ordering by `event_date` would not guarantee that when rows share a timestamp. The inner query is wrapped in a derived table because MySQL does not allow `LIMIT` directly inside `IN (SELECT ...)`. The same pair with the `IS NULL`/`NOT IN` predicate and 1825 days batches the other bucket.

:::note
Deleting rows returns their space to InnoDB for reuse but not to the filesystem. If the space has to be reclaimed, rebuild the tables afterwards with `ALTER TABLE jhi_persistent_audit_event ENGINE = InnoDB;`, which is a far heavier operation and needs its own maintenance window.
:::
