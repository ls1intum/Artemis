...
The confirmation dialog then lists the affected entity types with their counts (courses, users, feedback, and so on) before anything is executed, and its button is named after what the operation does: **Warn** for the two email-only phases, **Reset** for the old-course reset, and **Delete** everywhere else.
