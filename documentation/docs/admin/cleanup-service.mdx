...
Because these operations have no "from"/"to" pickers, the page states their scope in a grey line under each operation name, resolved against the cutoffs this instance is configured with. For example, "Courses that ended before Mar 4, 2021 (5 years retention)" for the plagiarism-case cleanup, or "Courses whose instructors were warned before Feb 2, 2026 (30 days grace period)" for the reset. The dates are the ones the operation would apply if it ran now, so they move with the configuration and with the current date; there is no need to reconstruct them from the YAML.

The confirmation dialog then lists the affected entity types with their counts (courses, users, feedback, and so on) before anything is executed, and its button is named after what the operation does: **Warn** for the two warning phases (archive and email for courses, email for users), **Reset** for the old-course reset, and **Delete** everywhere else.
