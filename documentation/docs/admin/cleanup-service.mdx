...
On an instance upgrading from a version without this job, the table may hold years of accumulated rows, and letting the nightly job drain it takes as many nights as it takes. To do it in one maintenance window instead, use `supporting_scripts/database/prune_audit_events.sql` (MySQL 8), which applies exactly the same retention scheme in batches you control:

```bash
# on a host that can reach the database
mysql Artemis < supporting_scripts/database/prune_audit_events.sql
```

As shipped, the script only reports: the current table size, the number of rows each retention period would remove, and it creates the index the pruning needs (`idx_persistent_audit_event_date`) if it is missing. Review that output, then uncomment the two `CALL` statements at the end and run it again:
...
The arguments are the bucket, the retention in days, the batch size, the maximum number of batches, and the pause between batches in seconds. Each batch commits on its own, so the run is safe to interrupt and safe to repeat: it reports either `done` or `batch cap reached, re-run to continue`. Rows inside the retention period are never touched.

Two notes:

- Creating the index is an online operation in MySQL (no exclusive lock), but it still takes time proportional to the table size. It is worth doing first regardless, because without it every batch would scan and sort the whole table. Liquibase creates the same index guarded by a `not indexExists` precondition, so creating it here simply makes that changeset a no-op on the next upgrade.
- Deleting rows returns their space to InnoDB for reuse but not to the filesystem. If the space has to be reclaimed, rebuild the tables afterwards (`ALTER TABLE jhi_persistent_audit_event ENGINE = InnoDB;`), which is a far heavier operation and needs its own maintenance window.
