...
Every repository of a programming exercise is stored as its own ZIP inside the exercise export. The template, solution, test and auxiliary repositories keep their full Git history, including every branch and tag, so extracting one of them yields a usable repository. Student repositories are archived as a snapshot of the submitted working tree without a `.git` directory, matching what students and tutors get when they download a single repository from Artemis.
...
| | Programming exercises | One ZIP per repository (template, solution, test, auxiliary and student repos) |
...
2. **Notify stakeholders** - Inform instructors about the reset
