---
id: upgrade-guide
title: Upgrade Guide
sidebar_label: Upgrade Guide
---

import Callout from "../../src/components/Callout/Callout";
import {CalloutVariant} from "../../src/components/Callout/Callout.types";

# Upgrade Guide

This page documents the required upgrade path for Artemis. Due to database migration consolidation at each major version, administrators **must deploy specific intermediate versions** before upgrading to the next major release.

## Why Intermediate Versions Are Required

With every major release (e.g., 8.0.0, 9.0.0), Artemis consolidates all incremental database changelogs from the previous release cycle into a single initial schema. This dramatically improves application startup time (especially for tests) and simplifies the migration history.

However, this means the new major version's schema **assumes all previous incremental migrations have already been applied**. If you skip the required intermediate version, the consolidated schema will conflict with your existing database and Artemis will refuse to start.

<Callout variant={CalloutVariant.warning}>
  <strong>Important:</strong> If Artemis detects that the required intermediate version was not deployed, it will exit with error code 15 and a message indicating which version must be deployed first.
</Callout>

## Upgrade Path Table

The table below shows the required intermediate version for each major release. You must deploy and start the **Required Version** at least once before upgrading to the **Target Version**.

| Target Version | Required Intermediate Version | Valid Until |
|----------------|-------------------------------|-------------|
| 6.0.0          | 5.12.9                        | 7.0.0       |
| 7.0.0          | 6.9.6                         | 8.0.0       |
| 8.0.0          | 7.10.5                        | 9.0.0       |
| 9.0.0          | 8.8.6                         | 10.0.0      |

### How to Read This Table

- **Target Version**: The major version you want to upgrade to.
- **Required Intermediate Version**: The minimum version your database must have been running before upgrading to the target version. Deploy this version, start Artemis, wait for it to fully initialize, then upgrade to the target version.
- **Valid Until**: The migration path is removed at this version. For example, the 5.12.9 → 6.0.0 path is available until 7.0.0, after which upgrading from 5.x is no longer supported.

## Upgrade Scenarios

### Fresh Installation

A fresh installation (empty database) can always use the latest version directly. No intermediate versions are needed.

### Upgrading Within a Major Version (e.g., 8.5.0 → 8.8.6)

Minor and patch version upgrades within the same major version require **no special steps**. Simply deploy the new version. All incremental Liquibase changelogs will be applied automatically.

### Upgrading Across One Major Version (e.g., 8.8.6 → 9.0.0)

1. Deploy and start the **required intermediate version** (e.g., 8.8.6) to ensure all incremental migrations are applied.
2. Wait for Artemis to fully start (check logs for "Started ArtemisApp").
3. Stop Artemis.
4. Deploy and start the new major version (e.g., 9.0.0).
5. The migration system will automatically:
   - Detect the previous version from the `artemis_version` table.
   - Nullify the initial schema checksum so Liquibase recalculates it.
   - Apply a cleanup changeset that removes old changelog entries from `DATABASECHANGELOG`.
   - Apply any new incremental changelogs.
   - Update the `artemis_version` table.

### Upgrading Across Multiple Major Versions (e.g., 6.5.0 → 9.0.0)

You must upgrade through each intermediate version sequentially:

1. **6.5.0 → 6.9.6** (deploy 6.9.6, start, wait for full initialization)
2. **6.9.6 → 7.10.5** (deploy 7.10.5, start, wait for full initialization)
3. **7.10.5 → 8.8.6** (deploy 8.8.6, start, wait for full initialization)
4. **8.8.6 → 9.0.0** (deploy 9.0.0, start, wait for full initialization)

<Callout variant={CalloutVariant.info}>
  Each intermediate version must be fully started at least once so that all database migrations are applied before proceeding to the next version.
</Callout>

### Skipping a Required Version

If you attempt to start a major version without having deployed the required intermediate version, Artemis will:

1. Log an error message: *"Cannot start Artemis because the migration path was not followed. Please deploy and start the release X.Y.Z first, otherwise the migration will fail"*
2. Exit with code **15**.
3. No database changes will be made.

To resolve this, deploy the required intermediate version first, start Artemis, wait for it to fully initialize, stop it, and then deploy the desired major version.

## Version-Specific Migration Steps

Some releases change behavior in a way that requires a one-time action on existing data. These steps are additional to the upgrade scenarios above.

### 9.9: Accounts Provisioned Through SAML2

The sign-in page asks for the identifier first and then decides, from the account's internal flag, whether to offer a password field or to send the user to the configured identity provider. Only an account marked as internal authenticates against a password stored in Artemis; every other account is sent to the identity provider.

Accounts that Artemis provisioned through SAML2 before version 9.9 are stored as internal accounts, so they are offered the password form instead of the identity provider. Accounts provisioned from LDAP are not affected, because they have always been stored as externally managed.

Clearing the internal flag takes away everything that authenticates against a password stored in Artemis: the password form on the sign-in page, **Change password**, the password reset function, and Git over HTTPS with a password. VCS access tokens and SSH keys keep working, because neither is checked against the password.

What this means for your users depends on `info.saml2.enablePassword`:

- **Unset or `false` for the whole life of the instance**: every SAML2-provisioned account holds a random password that was never given to its owner, so the owner cannot sign in at all and nothing about the account depends on the password. These are the accounts the change below is for.
- **`true` at any point**: every account provisioned while it was `true` was sent a mail with a link to set an Artemis password, and the password form works for whoever used that link. Artemis records that it provisioned the account, but not whether the owner completed the setup, so you cannot tell from Artemis alone which of these accounts still depends on its password. Treat this group as manual review rather than a bulk update.

#### Identifying the Affected Accounts

<Callout variant={CalloutVariant.warning}>
  The row in `jhi_user` does not record where an account came from, and it carries no usable signal either. An account provisioned through SAML2 and an account that is genuinely internal are both activated, both hold a password hash, and both hold a reset key, because every account is created with a generated password and a fresh reset key. Never read a password hash or a reset key as evidence that the owner set a password. Do not decide this by email domain either: the same domain also covers accounts that users registered themselves, accounts created by an LTI launch, and the internal administrator.
</Callout>

Artemis records a `SAML2_ACCOUNT_CREATE` audit event whenever it provisions an account on a first SAML2 login, and the event names the account by its `login`. Match on `login` in every query on this page. It is the column Artemis constrains to be unique, and it is what Artemis itself looks the account up by when an assertion arrives: the login is derived from `saml2.username-pattern`, lowercased, and compared against `jhi_user.login`.

On version 9.9 these events are stored in `jhi_persistent_audit_event`, so the affected accounts can be listed exactly:

```sql
SELECT u.login, u.email
FROM jhi_user u
WHERE u.is_internal = TRUE
  AND u.login IN (SELECT d.event_data
                  FROM jhi_persistent_audit_evt_data d
                       JOIN jhi_persistent_audit_event e ON e.event_id = d.event_id
                  WHERE d.name = 'user' AND e.event_type = 'SAML2_ACCOUNT_CREATE');
```

From version 10.0 on, the same events live in `security_audit_event`, with their key-value data in `security_audit_evt_data`. Substitute both table names in every query below.

If the audit log does not reach back far enough, build the list from the identity provider instead, and still match on `login`: export the accounts the identity provider manages, apply your configured `saml2.username-pattern` to each one to get the login Artemis would have stored, lowercase the result, and match `jhi_user.login` against that list.

<Callout variant={CalloutVariant.warning}>
  Do not fall back to matching on `email`. Artemis identifies a SAML2 user by the login that `saml2.username-pattern` produces and never by email, and `jhi_user.email` is not unique: an address stored on an older release can be shared between accounts or recycled from a former owner, so an email match can select more than one row, or the wrong one. Clearing the internal flag on the wrong row changes that account's authentication route and locks its owner out of the password form, the password reset function, and password-based Git access. Where the identity provider gives you nothing but an email address, leave that account out of the update and confirm its identity in **Administration → User management** first. See [User Email Uniqueness](./user-email-uniqueness) for finding and resolving the duplicates an instance already has, and point `saml2.username-pattern` at a stable, globally unique identity provider attribute so that the login remains a reliable key.
</Callout>

#### Clearing the Flag

**For a few accounts**, open **Administration → User management**, edit the user, and clear the **Internal** checkbox.

**For many accounts**, update `jhi_user.is_internal` directly — but only for accounts that are safe to change in bulk, which means only while `info.saml2.enablePassword` has been unset or `false` for the whole life of the instance. Confirm that the `SELECT` above returns what you expect and that you have a backup, then run the update in a transaction:

```sql
BEGIN;
UPDATE jhi_user SET is_internal = FALSE
WHERE is_internal = TRUE
  AND login IN (SELECT d.event_data
                FROM jhi_persistent_audit_evt_data d
                     JOIN jhi_persistent_audit_event e ON e.event_id = d.event_id
                WHERE d.name = 'user' AND e.event_type = 'SAML2_ACCOUNT_CREATE');
COMMIT;
```

If `info.saml2.enablePassword` has ever been `true`, do not run that statement as it stands. `SAML2_ACCOUNT_CREATE` says only that Artemis provisioned the account; it records neither the setting that was in force at the time nor whether the owner completed the password setup. Narrow the set before you change anything:

- Restrict it by provisioning date. `event_date` on the audit event holds when the account was provisioned, so adding a condition such as `AND e.event_date < '2026-01-01'` to the subquery limits the set to the period in which your own configuration history says the setting was `false`.
- Exclude every account that has a password event of its own. From version 9.9 on, Artemis records `COMPLETE_PASSWORD_RESET`, `CHANGE_OWN_PASSWORD` and `ADMIN_CHANGE_USER_PASSWORD`, each naming the account the same way `SAML2_ACCOUNT_CREATE` does:

  ```sql
  SELECT u.login, u.email
  FROM jhi_user u
  WHERE u.is_internal = TRUE
    AND u.login IN (SELECT d.event_data
                    FROM jhi_persistent_audit_evt_data d
                         JOIN jhi_persistent_audit_event e ON e.event_id = d.event_id
                    WHERE d.name = 'user' AND e.event_type = 'SAML2_ACCOUNT_CREATE')
    AND u.login NOT IN (SELECT d.event_data
                        FROM jhi_persistent_audit_evt_data d
                             JOIN jhi_persistent_audit_event e ON e.event_id = d.event_id
                        WHERE d.name = 'user'
                          AND e.event_type IN ('COMPLETE_PASSWORD_RESET', 'CHANGE_OWN_PASSWORD', 'ADMIN_CHANGE_USER_PASSWORD'));
  ```

  Artemis writes these three events from version 9.9 on, so on a database that ran an earlier release they cover only the time since that upgrade, and immediately after the upgrade they cover nothing at all. A missing password event narrows the review list; it is not proof that the owner never set a password.

Whatever remains after those two steps is a manual decision. Settle it with the account owners, or leave the accounts internal: an account that stays internal is offered the password form, which is recoverable, whereas an account wrongly turned external loses its password access until an administrator sets the flag back.

Keep the internal administrator configured in `artemis.user-management.internal-admin.username` internal, and keep every other account that does authenticate against a password stored in Artemis internal as well. Accounts that Artemis provisions after the upgrade are stored correctly.

## Database Compatibility

Artemis supports both **MySQL** and **PostgreSQL**. The migration system works identically on both databases. The initial schema uses database-agnostic Liquibase types with additional database-specific changesets for:

- **MySQL**: Columns with `enum` types for data integrity and storage optimization.
- **PostgreSQL**: Columns with `varchar`/`text`/`bytea` types as appropriate for PostgreSQL.

<Callout variant={CalloutVariant.info}>
  Ensure your database is backed up before performing any major version upgrade.
</Callout>

## Technical Details

The migration path logic is implemented in `DatabaseMigration.java`. It runs before Liquibase on every application startup and checks the `artemis_version` table to determine the previous version. The `MigrationPath` class defines version boundaries:

```
MigrationPath("8.8.6")
  → requiredVersion = 8.8.6
  → upgradeVersion  = 9.0.0  (next major)
  → nextUpgradeVersion = 10.0.0  (major after that)
```

When `currentVersion ∈ [upgradeVersion, nextUpgradeVersion)` and `previousVersion >= requiredVersion`, the system nullifies the initial schema checksum (once) to allow Liquibase to recalculate it against the consolidated schema. This runs only on the first startup after the schema consolidation — subsequent startups detect that the consolidation is already complete and skip this step.
