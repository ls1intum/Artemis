...
Every Artemis pull request applies the baseline and the changelogs it summarises to empty MySQL and PostgreSQL databases and compares the two resulting schemas, so a difference between the databases cannot reach a release.
