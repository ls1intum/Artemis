...
<Callout variant={CalloutVariant.warning}>
  `info.saml2.enablePassword` predates this separation. It mails a newly provisioned SAML2 user a link to set an Artemis password, which gives an account the identity provider manages a second credential stored in Artemis. Do not enable it. Where an instance has used it, still clear the internal flag, and tell the affected users to sign in through the identity provider and to replace a password they used for Git with a VCS access token or an SSH key.
</Callout>
