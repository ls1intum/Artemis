...
|---|---|
| Found in the Artemis database | The existing account is used; its details are refreshed from the directory if it is configured |
...
That is the intended effect: deactivation refuses authentication everywhere and revokes the account's
tokens, SSH keys and passkeys. Only an administrator can activate the account again, and the revoked
credentials have to be created anew afterwards.
