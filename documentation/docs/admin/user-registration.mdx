...
account through this factory. Whether it still owes that account the password dialog is recorded by
`UserLti.initialized` rather than by `activated`, and the account is activated when the dialog is shown, so it
never needs the key either.
:::
...
clears it too — the administrative activate action, and the LTI launch handing over the generated password —
so an activated account never carries one.
