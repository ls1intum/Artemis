...
carries one, and records separately whether the password dialog is still owed — see
[LTI account provisioning](#lti-account-provisioning) below.
:::
...
clears it too — the administrative activate action — and an LTI-provisioned account has its key discarded
when the launch creates it, so an activated account never carries one.
