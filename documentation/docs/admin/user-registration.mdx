...
account through this factory, and it reads `activated` as its own record of whether the password dialog is
still owed. Its account is activated when that dialog is shown, so it never needs the key either.
:::
