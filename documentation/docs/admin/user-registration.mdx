...
internal account has no way to redeem a key. Narrowing account creation to match is a separate change: the
LTI launch is the only path that creates an internal account through this factory, and it reads `activated`
as its own record of whether the password dialog is still owed.
:::
...
- Nothing lets an account holder reverse a deactivation through an authentication provider. Only an
