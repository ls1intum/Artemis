...
<Callout variant={CalloutVariant.info}>
    Ensure all nodes use the same SSH key set and restrict access to configuration files for system users only.
</Callout>

## Rate Limiting

Artemis provides **cluster-wide rate limiting** powered by Bucket4j (token buckets) and Hazelcast (distributed state).
