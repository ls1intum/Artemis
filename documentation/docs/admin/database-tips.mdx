...
```python
   ...
   def _to_public(line: str, schema: str) -> str:
       return line.replace(f"COPY {schema}.", "COPY public.", 1).replace(
           f"setval('{schema}.", "setval('public.", 1
       )

   def _extract_data(data_file_path: Path, schema: str) -> None:
       ...
       with open(data_file_path, encoding="utf-8") as data_file:
           ...
           for line in data_file:
               ...
               if table is not None:
                   ...
                   if not skipping:
                       print(_to_public(line, schema), end="")
                   continue
   ...
   def _merge_files(*, schema_file_path: Path, data_file_path: Path) -> None:
       schema = _detect_schema(data_file_path)
       ...
       with open(schema_file_path, encoding="utf-8") as schema_file:
           ...
           _extract_data(data_file_path, schema)
...
```sql
-- every table's row count, to compare against the MySQL side
SELECT relname, n_live_tup FROM pg_stat_user_tables ORDER BY relname;

-- any identity sequence that would hand out an id already in use
...
For each sequence the second query returns, `nextval` has to be greater than the largest id in the table. pgloader repositions the sequences and `merge.py` carries the new positions over, so this should hold everywhere; if it does not, the first write to that table after go-live fails with a duplicate key error.
