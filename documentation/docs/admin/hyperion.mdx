...
<Callout variant={CalloutVariant.warning}>
    <strong>Agentic whole-exercise generation and adaptation require the integrated LocalCI/LocalVC lifecycle (the <code>localci</code> profile).</strong> They drive a hardened sandbox on a LocalCI build agent, persist through LocalVC with a LocalCI trigger, and verify with the production LocalCI test/SCA parsers, so they are automatically disabled on Jenkins deployments (the endpoints are not registered and the client hides the entry points). The other Hyperion features — consistency check and problem-statement rewriting — do not depend on LocalCI and work under any supported CI backend.
</Callout>
...
Agentic exercise generation runs the agent's work inside a **hardened, CI-sized Docker sandbox on a LocalCI build agent** — the same hosts that run student and exam builds. A generation session is long-lived (several minutes) and consumes real CPU, memory, and Docker resources for its whole duration.
...
artemis:
  continuous-integration:
    build-agent:
      # 0 = this agent never hosts a generation sandbox (default). Set > 0 only on agents you dedicate to generation.
      max-concurrent-generation-sessions: 0
```

<Callout variant={CalloutVariant.warning}>
    Leave <code>max-concurrent-generation-sessions</code> at <strong>0</strong> on every exam-critical build agent. Enable it (a small positive number) only on spare agents you are comfortable dedicating to generation. On a single-node integrated deployment (core + build agent on one host) the co-located node runs generation directly, so set the value there if you use the feature.
</Callout>

Additional controls:
- **Pausing a build agent** (from the LocalCI build-agent admin page) now also stops it accepting **new** generation sessions, so you can drain an agent before maintenance or an exam. In-flight sessions finish and tear down cleanly.
- Orphaned sandbox containers are reaped automatically after `generation-container-expiry-minutes` (default 90).
...
- **Exam safety:** Because generation shares build-agent hosts with exam builds, keep `max-concurrent-generation-sessions` at 0 on exam agents (see *Build-agent placement*).
