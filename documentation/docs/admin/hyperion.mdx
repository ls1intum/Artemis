...
Hyperion extends Artemis with AI-assisted authoring features. For programming exercises, it offers consistency checks for problem statements and exercise artefacts and can rewrite instructions with the help of generative AI. For quiz exercises, it generates Multiple-Choice questions from a topic or, in courses with competencies, from the course's competency graph and linked lecture content ([Quiz Exercise: Generate Multiple-Choice Questions with AI](/instructor/exercises/quiz-exercise#generate-multiple-choice-questions-with-ai)), and it can refine existing Multiple-Choice questions from a natural-language instruction, either individually or in bulk ([Quiz Exercise: Refine Multiple-Choice Questions with AI](/instructor/exercises/quiz-exercise#refine-multiple-choice-questions-with-ai)). The functionality is provided by Artemis and Spring AI, so no EduTelligence service needs to be deployed; the only exception is competency-graph quiz generation, which additionally uses Pyris for semantic lecture search when Iris/Pyris is deployed, and falls back gracefully to the other context sources if it is not.
