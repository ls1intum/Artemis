...
|---|---|
| On-premises GPU available | Self-hosted Qwen3-Embedding-8B via OpenAI-compatible API (e.g., TUM GPU server) |
...
Edit the relevant env file (`docker/weaviate/default.env` for the keyword-only setup, or `docker/weaviate/embeddings.env` for the sidecar setup) and make the following changes:
