...
<Callout variant={CalloutVariant.info}>
  <p>Transcription itself is performed by <strong>Pyris/Iris</strong> (not by the Nebula service). The <code>artemis.nebula.*</code> namespace is retained for URL-detection helpers (TUM-Live playlist resolution, YouTube URL validation) that run inside Artemis. The actual webhook that triggers transcription is the lecture ingestion webhook, delivered to Pyris.</p>
</Callout>

Artemis supports automatic transcription for videos from the following sources:

- **TUM-Live (GoCast):** Artemis resolves the public video URL to an HLS playlist via the TUM-Live API and forwards the playlist URL to Pyris. Pyris downloads the audio with FFmpeg and transcribes it with Whisper.
- **YouTube (public and unlisted):** Artemis recognizes the YouTube URL, passes it through unchanged to Pyris, and tags the request so Pyris knows to use `yt-dlp` for the download. **Private YouTube videos are not supported** because Pyris cannot authenticate with YouTube to access them.

The lecture ingestion webhook payload includes a `videoSourceType` field (`TUM_LIVE` or `YOUTUBE`) so that Pyris can select the appropriate download strategy. The field is omitted when the source cannot be identified, in which case Pyris falls back to its default behavior.

### Artemis Configuration

#### Prerequisites

- A running Artemis instance with the `scheduling` Spring profile enabled.
- A shared secret that will be used both in Artemis and in the Nebula gateway.
- *(For TUM-Live videos)* A running instance of <a href="https://github.com/TUM-Dev/gocast">GoCast (TUM-Live)</a> where the lecture recordings are hosted in a public course.
- *(For YouTube videos)* Pyris workers must have outbound HTTPS access to `*.youtube.com` and `*.googlevideo.com`, and `yt-dlp` must be available in the Pyris runtime. No YouTube API key is required.
...
With those pieces in place, instructors can automatically transcribe lecture recordings from TUM-Live and YouTube without manual copy/paste workflows.
