...
Nebula can transcribe videos from the following sources:

- **TUM-Live (GoCast):** Artemis resolves the public video URL to an HLS playlist via the TUM-Live API. Nebula downloads the audio from the playlist and transcribes it.
- **YouTube (public and unlisted):** Artemis passes the YouTube video URL directly to Nebula, which downloads the audio track using `yt-dlp` and transcribes it. **Private YouTube videos are not supported** because Nebula cannot authenticate with YouTube to access them.

When Artemis sends a transcription request to Nebula, it includes a `videoSourceType` field (`TUM_LIVE` or `YOUTUBE`) so that Nebula knows which download strategy to use.
...
- *(For YouTube videos)* Nebula workers must have outbound HTTPS access to `*.youtube.com` and `*.googlevideo.com`. No YouTube API key is required.
