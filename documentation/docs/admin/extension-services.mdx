...
Artemis can automatically transcribe lecture recordings by delegating the work to Pyris via the lecture ingestion webhook. Artemis detects the video source, forwards the URL to Pyris, and tags the request with a `videoSourceType` field (`TUM_LIVE` or `YOUTUBE`) so Pyris can pick the appropriate download strategy.
...
- *(For TUM-Live videos)* A running instance of <a href="https://github.com/TUM-Dev/gocast">GoCast (TUM-Live)</a> where the lecture recordings are hosted in a public course. Set `artemis.tum-live.api-base-url` to the TUM-Live API endpoint (include the `/api/v2` suffix).
- *(For YouTube videos)* Pyris workers must have outbound HTTPS access to `youtu.be`, `*.youtube.com`, `*.youtube-nocookie.com`, and `*.googlevideo.com`, and `yt-dlp` must be available in the Pyris runtime. No YouTube API key is required.
