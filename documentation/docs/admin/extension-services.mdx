...
For Pyris deployment and configuration, refer to the <a href="https://ls1intum.github.io/edutelligence/iris/">Iris admin documentation</a>. For the Artemis request, callback, WebSocket, ingestion, search, and memory boundaries, see the [AI Pipelines catalog](../developer/ai-pipelines#iris-and-pyris); service-owned execution and retrieval details are documented in the [Iris pipeline system](https://ls1intum.github.io/edutelligence/iris/docs/developer/pipeline-system) and [Iris RAG pipeline](https://ls1intum.github.io/edutelligence/iris/docs/developer/rag-pipeline).
...
Athena is an EduTelligence assessment service that Artemis uses for configured feedback-suggestion modules and related assessment workflows. To enable Athena, special configuration is required. The [AI Pipelines catalog](../developer/ai-pipelines#athena) describes the Artemis-side selection, forwarding, scheduling, and repository-export boundaries; the [Athena module structure](https://ls1intum.github.io/edutelligence/athena/docs/dev/module/structure/) documents the service-owned module contract.
...
Configure the Athena endpoint and shared secret in the active deployment-specific configuration. The module used for each supported exercise type (text, programming, modeling) is configured at the application level and applies to all courses; it is not configurable per course or per exercise.
...
artemis:
    athena:
        ...
        # Optional: override the default module name for each exercise type.
        modules:
            text: module_text_llm
            programming: module_programming_llm
            modeling: module_modeling_llm
```

The `modules.*` keys are optional; the defaults shown above are used when omitted. For a list of available modules, refer to the <a href="https://github.com/ls1intum/edutelligence/tree/main/athena">Athena documentation</a>. Use a deployment-managed shared secret. For the service-side module layout and decorator contracts, refer to the [Athena module structure](https://ls1intum.github.io/edutelligence/athena/docs/dev/module/structure/).

Enabling Athena at the deployment level makes AI-assisted feedback available; it must still be activated per course by an instructor or administrator. See [AI Assisted Feedback](/instructor/course-management/course-configuration#ai-assisted-feedback) in the Course Configuration documentation.
...
<a href="https://search.sharing-codeability.uibk.ac.at/">Sharing Platform</a> is an open platform for sharing teaching materials related to programming. It is operated by the
<a href="https://www.uibk.ac.at/en/">University of Innsbruck</a>. While primarily designed as an open exchange platform, it also provides features such as private group exchanges
and the ability to restrict public access to certain content, such as the solution repository of an Artemis exercise.
