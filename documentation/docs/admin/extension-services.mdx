...
- *(For YouTube videos)* Pyris workers must have outbound HTTPS access to `youtu.be`, `*.youtube.com`, and `*.googlevideo.com`, and `yt-dlp` must be available in the Pyris runtime. No YouTube API key is required.
