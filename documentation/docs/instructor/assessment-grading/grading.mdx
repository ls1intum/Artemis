...
- `Plagiarism Grade` (default `U`) is assigned when a student has received a [Plagiarism verdict](/instructor/assessment-grading/plagiarism-check) in one or more exercises.
...
> **Note:** You can check the [grading key](/instructor/exams/grade-and-publish#grading-key) for more detailed information on how to create grading keys for courses and exams.
...
Instructors can create bonus configurations for exams with `Grade` type grading keys by clicking on <Image src={bonusButtonImg} alt={"bonus"} size={ImageSize.small} hideBorder={true} inline /> on the exam detail page.
