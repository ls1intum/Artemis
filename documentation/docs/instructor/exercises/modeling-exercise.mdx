...
**Create Modeling Exercise** sits on the course's **Exercises** page in course management.
...
<Callout variant={CalloutVariant.info}>
  ...
  Tick **Use in Assessment Training** while creating an example submission for it to feed the matching. An example
  submission without that box ticked is documentation for assessors, not training data.

</Callout>

<Image src={useInAssessmentTraining} alt="The Use in Assessment Training option on an example submission" size={ImageSize.large} caption="Marking an example submission as training data" />

### Import an Existing Exercise

**Import Modeling Exercise** copies an exercise you already have. Pick one from the list and Artemis opens the
creation form with its fields filled in, ready to adjust.
