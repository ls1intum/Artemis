...
**Create Modeling Exercise** sits on the course's **Exercises** page in course management. The form comes in three
parts.

The first part is what every exercise has — **Title**, **Categories**, **Difficulty**, and the **Mode**, individual
or team, which cannot be changed later — plus the dates that shape its life: **Release Date** makes it visible,
**Due Date** stops students working, and **Assessment Due Date** publishes the feedback. **Points** and **Bonus
Points** set what it is worth, and **Inclusion in course score calculation** decides whether it counts.

**Diagram Type** is the field specific to this type, and the one to get right first.

<Image src={createModelingExerciseForm1} alt="First part of the modeling exercise form, with the diagram type" size={ImageSize.large} caption="Creating a modeling exercise: the first part of the form" />
...
The second part carries the content and the two automation switches. **Problem Statement** is the task students
read and **Assessment Instructions** is the rubric assessors work from. **Enable automatic assessment suggestions**
reuses assessments from submissions already graded in this exercise; **Enable feedback suggestions from Athena**
generates them with the Athena service instead.

<Image src={createModelingExerciseForm2} alt="Second part of the form, with the problem statement and the automation switches" size={ImageSize.large} caption="The problem statement and the assessment options" />
...
The third part is the example solution: the diagram itself, an **Example Solution Explanation** saying why it is
built that way, and an **Example Solution Publication Date**. Leave the date empty and the solution stays with the
tutors rather than reaching students.

<Image src={createModelingExerciseForm3} alt="Third part of the form, with the example solution" size={ImageSize.large} caption="The example solution" />
...
The exercise appears on the course's exercise list, where each row offers the same actions: **Edit** reopens the
form and the problem statement, **Scores** shows what students achieved, **Participations** lists who took part,
**Submissions** lists what they handed in, **Example Submissions** manages the training data, and **Delete**
removes it. The title opens the exercise itself.
...
to be marked, and **Submissions** on the modeling exercise opens its queue.
