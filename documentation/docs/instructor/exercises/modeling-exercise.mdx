...
import createModelingExerciseFormProblem from './assets/modeling/create-modeling-exercise-form-problem.png';
...
<Image src={createModelingExerciseFormProblem} alt="Create Modeling Exercise Form Problem Statement" size={ImageSize.large} caption="Problem Statement" />
...
<Callout variant={CalloutVariant.info}>
  ...
  If you are not clear about any of the fields, you can access additional hints by hovering over the <img src={hintIcon} alt="Hint Icon" style={{display: 'inline', verticalAlign: 'middle', height: '1rem'}} /> icon for many of them.
...
<Callout variant={CalloutVariant.info}>
  ...
  Artemis uses semi-automatic grading of modeling exercises using machine learning. You can train the model by selecting the <em>Use in Assessment Training</em> checkbox while creating an example submission.
...
When **Grading Feedback Suggestions** is enabled at the course level, Artemis automatically generates assessment suggestions for modeling submissions using the Athena service.

<Callout variant={CalloutVariant.info}>

  Grading feedback suggestions are enabled per course by an instructor or administrator. See <a href="/instructor/course-management/course-configuration#ai-assisted-feedback">AI Assisted Feedback</a> in the Course Configuration documentation. To learn how to set up the Athena service, refer to <a href="/admin/artemis-intelligence">Artemis Intelligence</a>.
...
When **Formative Feedback Requests** is enabled at the course level, students can request preliminary AI feedback for their modeling submissions before the due date. The feedback is generated through the Athena Service, which analyzes both the **structure** and **layout** of the diagrams and produces feedback based on the provided **Grading Instructions**, **Problem Statement**, and **Sample Solution**.
...
1. **Requesting Feedback**
   - 1.1. Navigate to a **Modeling Exercise** in a course with **Formative Feedback Requests** enabled
...
<TumLiveVideo src="https://tum.live/w/artemisintro/55922" title="Video tutorial of the automated assessment of modeling exercises" />
