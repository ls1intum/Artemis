...
Artemis can use Hyperion to generate starter code, reference solutions, and tests for Java programming exercises. Code generation is currently only available for Java. The generated files are written to the exercise repositories, committed, and built through the configured CI setup. Treat the generated result as a draft: review the changed files, inspect the build result, and adapt the exercise before releasing it to students.
