...
<Callout variant={CalloutVariant.info}>
    ...
    **Only available with [Integrated Code Lifecycle](/instructor/integrations/integrated-code-lifecycle)**, and only when the administrator has enabled both Hyperion (<code>artemis.hyperion.enabled=true</code>) and whole-exercise generation (<code>artemis.hyperion.exercise-generation.enabled=true</code>). Starting a run requires at least the <strong>Editor</strong> role within the course.
