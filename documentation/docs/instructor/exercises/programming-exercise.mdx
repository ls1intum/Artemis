...
Artemis integrates AI-assisted authoring capabilities into the programming exercise editor through the Hyperion module. These features help instructors generate, refine, and analyze problem statements without leaving the editor. Every change they propose is shown in a diff view that you accept or discard, so nothing is applied without your approval. [Whole-exercise generation](#ai-assisted-exercise-generation) works differently and is described further below.
...
Problem statement generation, refinement, quality fixes, and difficulty adaptations are all presented in a diff view before they are applied. The left column shows the current problem statement with deletions highlighted in red, the right column shows the proposed revision with additions highlighted in green, and a summary bar reports the net change. You can accept, discard, or toggle between split and inline views.
...
<Callout variant={CalloutVariant.info}>
    ...
    **Only available with [Integrated Code Lifecycle](/instructor/integrations/integrated-code-lifecycle)**, and only when the administrator has enabled Hyperion, enabled whole-exercise generation, and allocated generation capacity on at least one build agent — see the [Hyperion setup guide](/admin/hyperion#enable-hyperion). Starting a run requires at least the <strong>Editor</strong> role within the course.

</Callout>

Hyperion can author a complete Java programming exercise for you — problem statement, template repository, solution repository, and tests — or adapt an existing draft. Unlike the problem statement features above, it does not propose a diff for you to accept. It writes the result into the exercise's own repositories, and only after the automated checks described below have passed.

Nothing reaches students. Runs are only permitted on unreleased draft exercises, and a run never changes an exercise's dates or release state. What you get back is a draft that you review and release yourself.

#### Requirements

Because Hyperion writes directly to the repositories, both generation and adaptation require an exercise that:

- uses **Java** with a Maven project type. Other programming languages, Gradle, and the Maven Blackbox project type are not supported.
- has **no auxiliary repositories**.
- has **no student participations**.
- is **not yet available to students**. Artemis looks at the date on which students could first start working: the exercise's start date if it has one, otherwise its release date; for exam exercises, the exam's start date. That date must be set and lie in the future — an exercise with no such date counts as already available and is rejected.

If the exercise is already available to students, copy it or move that date into the future before asking Hyperion to rewrite it.

#### Starting a run

There are two entry points:

- **During exercise creation:** for a new Java programming exercise, choose **Generate with AI**. Artemis creates the exercise and its repositories, opens the online editor, and starts generation immediately.
- **From the online editor:** describe the requirements in the problem statement (at least 40 characters), then use **Artemis Intelligence** → **Generate exercise**. For an exercise that already has content, **Adapt exercise…** applies selected review feedback together with any extra instructions you give it.
...
Save or discard your unsaved editor changes first — a run cannot start on top of them. Artemis then asks you to confirm, restating that verified output is saved automatically. Only one run at a time is possible per exercise.

While a run is in progress, the **AI activity** panel streams its progress and the paths of the files it changes. You can leave the page and come back; reloading reconnects to the running job. You can cancel while Hyperion is still authoring or checking the exercise. Once the automated checks have passed, Artemis finishes saving the verified work rather than discarding it.

<Callout variant={CalloutVariant.warning}>

    Do not edit or push to the exercise repositories while a run is in progress. Artemis makes the online editor read-only and rejects LocalVC pushes over both HTTP and SSH, along with the exercise changes it guards, until the run releases its lock. Do not work around this by changing the repository storage directly.

</Callout>

#### What the automated checks cover

Hyperion only saves to the exercise after a verification run confirms that:

- the reference solution passes the generated tests,
- the template fails the tests it is meant to fail,
- every task in the problem statement resolves to real test cases, and
- the protected test-harness files are unchanged.

These are mechanical checks: they establish that the exercise builds and grades coherently. They say nothing about whether it teaches what you intended, sits at the right difficulty, or covers everything you asked for. A separate automated quality review looks at those questions, but its findings are advisory — they never block a mechanically sound exercise from being saved.

#### Reviewing and undoing the result

A run that neither fails nor is cancelled finishes with one of three outcomes, shown in the activity panel.

**Saved.** The exercise passed the mechanical checks and the quality review raised nothing that needs your attention. Review it before releasing it.

**Needs review.** The exercise passed the same mechanical checks and was saved, but the quality review either raised concerns or could not run. This is a valid authoring candidate that needs your judgement — not a broken exercise. The findings are attached as review comments in the editor. Read them, then keep the exercise as it is, edit it yourself, or run **Adapt exercise…** against the findings. Artemis deliberately saves a mechanically valid exercise rather than discarding your draft over an advisory finding.

**Not fully saved.** The run stopped part-way through saving, so some changes may already be in the exercise and others not. Inspect every repository and the problem statement before editing further. Undo is not available in this state.

**Review saved changes** opens the exercise version history for the problem statement, or the exact commit Hyperion created for the solution, template, and tests repositories.

A run that saved successfully offers **Undo generation** or **Undo adaptation**, which restores the problem statement and the repositories to the commits captured before the run started. Undo is one-way: you cannot get the generated version back afterwards. It is also temporary, and stops being offered once:

- a repository has changed since the run — undo refuses to overwrite a commit that is no longer the one Hyperion created;
- the next run starts writing;
- a week has passed; or
- the Artemis server that recorded it restarts, on a single-node deployment.

So review the result promptly, and undo promptly if you are going to.

#### Steering a run with review comments

Use **Apply with AI** on an open [review comment](#review-comments) thread to include it in the next adaptation. The action is offered on consistency-review threads in the template, solution, and tests repositories; resolved and outdated threads, and threads on other exercises, are excluded.
...
Selected threads are tagged, and the action changes to **Remove Selection**.
...
**Adapt exercise…** then lists the selected findings and lets you add free-form instructions alongside them.
