...
<strong>Note:</strong> Only some templates for Local CI support Sequential Test Runs. Static Code Analysis for some exercises is also only supported for Local CI. Instructors can
still extend the generated programming exercises with additional features that are not available in a specific template.

</Callout>

<Callout variant={CalloutVariant.success}>
    We encourage instructors to contribute improvements to the existing templates or to provide new templates. Please contact [Stephan
    Krusche](https://www.professoren.tum.de/krusche-stephan) and/or create Pull Requests in the [GitHub repository](https://github.com/ls1intum/Artemis).
</Callout>
...
<TumLiveVideo src="45966" title="Video tutorial for creating a new programming exercise on TUM-Live" />
...
<Image
    src={collaborativeCodeEditor}
    alt="Two instructors editing the same file with visible cursors"
    size={ImageSize.large}
    caption="Live collaborative code editing with cursor awareness"
/>
...
<Image
    src={collaborativeProblemStatement}
    alt="Two instructors editing the problem statement simultaneously"
    size={ImageSize.large}
    caption="Live collaborative problem statement editing"
/>
...
Artemis integrates AI-assisted authoring capabilities into the programming exercise editor through the Hyperion module. These features help instructors generate, refine, and analyze problem statements without leaving the editor. Problem-statement-only generation and refinement are review-first: proposed changes appear in a diff before you apply them. Whole-exercise generation and adaptation use the automatic persistence workflow described below.
...
When creating a new exercise, the **Problem** step provides a requirements input field. Enter a textual prompt describing the desired exercise (up to 1,000 characters) and click **Generate Draft Problem Statement**. The system inserts a student-facing Markdown draft into the editor for review. Final task-to-test annotations are added only during whole-exercise generation, after tests exist.

<Image
    src={hyperionGenerate}
    alt="Problem statement generation interface"
    size={ImageSize.large}
    caption="AI-assisted problem statement generation with requirements input, character counter, and the generated draft in the Markdown editor"
/>
...
<Image
    src={hyperionRefinement}
    alt="Problem statement refinement interface"
    size={ImageSize.large}
    caption="Global refinement mode — the action changes to 'Refine Problem Statement' when the editor already contains content"
/>
...
<Image
    src={hyperionTargetedRefinementButton}
    alt="Refine Selection button next to selected text"
    size={ImageSize.large}
    caption="Targeted refinement entry point — the button appears when text is selected in the editor"
/>

<Image
    src={hyperionTargetedRefinementPrompt}
    alt="Inline instruction field for targeted refinement"
    size={ImageSize.large}
    caption="Inline instruction field for describing the desired change to the selected region"
/>

#### Diff View

AI-assisted problem-statement refinements, quality fixes, and difficulty adaptations are presented in a diff view before application. Whole-exercise generation uses a separate informed confirmation, saves only after automated build and grading checks, and then offers exact saved-change review and undo actions in the editor. In the problem-statement diff, the left column shows deletions in red and the right column shows additions in green. You can accept, discard, or toggle between split and inline views.

<Image
    src={hyperionDiffView}
    alt="Diff view comparing original and AI suggestion"
    size={ImageSize.large}
    caption="Diff view for reviewing AI-suggested modifications — accept or discard before changes are applied"
/>
...
<Image
    src={hyperionChecklistQuality}
    alt="Quality assessment panel with radar chart and detected issues"
    size={ImageSize.large}
    caption="Quality assessment panel showing scores, detected issues, and controls for individual or batch correction"
/>
...
<Image
    src={hyperionDifficultyAssessment}
    alt="Difficulty assessment panel showing mismatch indicator"
    size={ImageSize.large}
    caption="Difficulty assessment with suggested level, mismatch indicator, reasoning, and adaptation controls"
/>
...
<Image
    src={hyperionLearningGoals}
    alt="Learning goal estimation panel with knowledge area badges"
    size={ImageSize.large}
    caption="Estimated learning goals with knowledge area badges, taxonomy levels, evidence, and competency linking controls"
/>

<Image
    src={hyperionLinkedCompetencies}
    alt="Linked Competencies section populated from estimated goals"
    size={ImageSize.large}
    caption="Linked Competencies section showing course competencies created from the estimated learning goals"
/>
...
<Callout variant={CalloutVariant.info}>
    ...
    This feature requires Hyperion (<code>artemis.hyperion.enabled=true</code>) and the Local CI/Local VC setup. It is not available with Jenkins. Starting a run requires at least the <strong>Editor</strong> role.

</Callout>

Hyperion can generate or adapt a complete Java Maven programming exercise: problem statement, template, solution, and tests. The action is limited to supported unreleased draft exercises without student participations. Output is saved to the exercise's normal repositories after mechanical verification. If automated quality review still finds concerns after the repair attempts, Artemis saves the mechanically valid exercise as **Needs review** instead of discarding the instructor's draft. Review every artifact before release; use review comments and **Adapt exercise** for follow-up changes.

There are two entry points:

- **During exercise creation:** For a new Java programming exercise, choose **Generate with AI**. Artemis creates the exercise and repositories, opens the online editor, and starts generation.
- **From the online editor:** Describe the exercise requirements in the problem statement, then use **Artemis Intelligence** → **Generate exercise**. For an existing exercise, **Adapt exercise** can apply selected consistency-review feedback plus optional instructions.

<Image
    src={hyperionGenerateWithAi}
    alt="Generate with AI button during programming exercise creation"
    size={ImageSize.large}
    caption="Generate with AI action during programming exercise creation"
/>

#### Human Feedback Support

Use **Apply with AI** on active consistency-review threads to include them in the next adaptation. Resolved, outdated, unrelated, and unauthorized threads are excluded. The adaptation dialog shows the selected findings and accepts additional instructions.
...
<Image src={hyperionSelectionTag} alt="Review thread selected for exercise adaptation" size={ImageSize.large} caption="Selected review thread indicator" />
...
Before a manually started generation or adaptation, Artemis explains that mechanically verified output is saved automatically. The activity panel streams progress and changed-file paths. After persistence, **Review saved changes** opens the problem-statement version history or the exact Hyperion commit diff for each touched repository. A reload reconnects to the retained job state. You can cancel while Hyperion is authoring or checking the exercise; once mechanical verification has completed, Artemis finishes saving the verified work instead of discarding it.

Hyperion saves generated content to the exercise's normal branch only after an authoritative mechanical verification run confirms that the solution passes, the template fails the required tests, task bindings resolve, and protected test infrastructure remains unchanged. These checks do not establish pedagogical quality or semantic completeness. Quality findings are shown as **Needs review**, but the mechanically valid result remains available in the exercise so the instructor can inspect it, leave review comments, adapt it again, or revert the generation. A `PARTIAL` result means that saving across the exercise repositories did not complete and requires manual inspection before further editing.

A successfully persisted generation or adaptation offers **Undo generation** or **Undo adaptation** while its in-memory baseline remains available, provided the repositories have not changed again. Undo restores the captured pre-run commits and problem statement. The action is not crash-durable: a server restart or conflicting repository change can make it unavailable. Review and undo promptly when needed.

<Callout variant={CalloutVariant.warning}>

    Do not edit or push to the exercise repositories while generation is running. The online editor is read-only while it knows the status of a run, but external Git pushes and other repository writers are not globally locked.
...
<TumLiveVideo src="https://live.rbg.tum.de/w/artemisintro/73628" title="Video tutorial for review comments in programming exercises on TUM-Live" />
...
<Image
    src={reviewConsistencyThreadWidget}
    alt="AI-created consistency issue review comment with severity and category labels"
    size={ImageSize.large}
    caption="Consistency check result shown as a review comment"
/>
...
<Callout variant={CalloutVariant.info}>
    ...
    Review comments stay attached to saved exercise content. If the referenced line changes later, Artemis may mark the thread as outdated. Re-check the referenced location before resolving the thread or applying an AI-suggested fix.
...
<strong>Important for Java and Kotlin exercises:</strong> Maven Central rate-limits download requests. Because all builds of an Artemis instance originate from the same IP
addresses, builds can suddenly fail with <strong>HTTP 429 (Too Many Requests)</strong> errors — especially during high-load periods such as exams, when many students submit at the
same time. We strongly recommend configuring a Maven repository mirror as the <strong>first</strong> repository in the test repository's build configuration, as described below.
...
<strong>The repository order matters:</strong> Repositories are queried in the order they are declared. The mirror must come <strong>before</strong> Maven Central so that all
dependencies are resolved from the mirror and Maven Central is not contacted at all. If the mirror is declared after Maven Central, builds will still hit the rate limit.
...
Controlling when tests _run_ (via build-phase inclusion conditions) is separate from controlling when results are _visible_ to students (via test-case visibility in the grading page). To achieve the desired outcome, both must be configured accordingly:
...
For Java exercises, the _staticCodeAnalysisConfig_ folder contains configuration files for each tool.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Swift</td>
            <td>
                <a href="https://realm.github.io/SwiftLint/">SwiftLint</a>
            </td>
            <td>.swiftlint.yml</td>
        </tr>
        <tr>
            ...
            <td>Rust</td>
            <td>
                <a href="https://doc.rust-lang.org/clippy/">Clippy</a>
            </td>
            <td>clippy.toml</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>Dart</td>
            <td>
                <a href="https://dart.dev/tools/analysis">dart analyze</a>
            </td>
...
- <img src={gradingReset} alt="Reset" style={{ display: 'inline', verticalAlign: 'middle', height: '2rem' }} /> **Reset**: Reset the current grading configuration of the open tab
  to the default values. For Test Case Tab, all test cases are set to weight 1, bonus multiplier 1, and bonus points 0. For the Code Analysis Tab, the default configuration depends
  on the selected programming language.
- <img src={gradingReevaluateAll} alt="Re-evaluate All" style={{ display: 'inline', verticalAlign: 'middle', height: '2rem' }} /> **Re-evaluate All**: Re-evaluate scores using
  currently saved settings and existing feedback
- <img src={gradingTriggerAll} alt="Trigger All" style={{ display: 'inline', verticalAlign: 'middle', height: '2rem' }} /> **Trigger All**: Trigger all build plans to create new
...
<TumLiveVideo src="40748" title="Video tutorial for configuring test case grading on TUM-Live" />
...
<strong>Example 1:</strong> Given an exercise with 3 test cases, maximum points of 10 and 10 achievable bonus points. The highest achievable score is 200%. Test Case (TC) A has
weight 2, TC B and TC C have weight 1 (bonus multipliers 1 and bonus points 0 for all test cases). A student that only passes TC A will receive 50% of the maximum points (5
points).
...
<strong>Example 2:</strong> Given the configuration of Example 1 with an additional bonus multiplier of 2 for TC A. Passing TC A accounts for (2 × 2) / (2 + 1 + 1) × 100 = 100% of
the maximum points (10). Passing TC B or TC C accounts for 1/4 × 100 = 25% of the maximum points (2.5). If the student passes all test cases they will receive a score of 150%,
which amounts to 10 points and 5 bonus points.

</Callout>

<Callout variant={CalloutVariant.info}>

<strong>Example 3:</strong> Given the configuration of Example 2 with additional bonus points of 5 for TC B. The points achieved for passing TC A and TC C do not change. Passing TC
B now accounts for 2.5 points plus 5 bonus points (7.5). If the student passes all test cases they will receive 10 (TC A) + 7.5 (TC B) + 2.5 (TC C) points, which amounts to 10
points and 10 bonus points and a score of 200%.
...
<table>
    ...
    <tbody>
        <tr>
            <td rowSpan={12}>
                <strong>
                    <a href="https://checkstyle.org/checks/coding/index.html">Coding</a> / Code Style
                </strong>
            </td>
        ...
        <tr>
            <td rowSpan={4}>
                <strong>
                    <a href="https://checkstyle.org/checks/design/index.html">Class Design</a> / Design
                </strong>
            </td>
            <td>FinalClass</td>
            <td></td>
        ...
        <tr>
            <td rowSpan={3}>
                <strong>
                    <a href="https://checkstyle.org/checks/blocks/index.html">Block Checks</a> / Code Style
                </strong>
            </td>
        ...
        <tr>
            <td rowSpan={2}>
                <strong>
                    <a href="https://checkstyle.org/checks/modifier/index.html">Modifiers</a> / Code Style
                </strong>
            </td>
            <td>ModifierOrder</td>
            <td></td>
        ...
        <tr>
            <td rowSpan={4}>
                <strong>
                    <a href="https://checkstyle.org/checks/sizes/index.html">Size Violations</a> / Code Metrics
                </strong>
            </td>
            <td>MethodLength</td>
            <td></td>
        ...
        <tr>
            <td rowSpan={3}>
                <strong>
                    <a href="https://checkstyle.org/checks/imports/index.html">Imports</a> / Naming &amp; Formatting
                </strong>
            </td>
            <td>IllegalImport</td>
            <td></td>
        ...
        <tr>
            <td rowSpan={7}>
                <strong>
                    <a href="https://checkstyle.org/checks/naming/index.html">Naming Conventions</a> / Naming &amp; Formatting
                </strong>
            </td>
            <td>ConstantName</td>
            <td></td>
        ...
        <tr>
            <td rowSpan={10}>
                <strong>
                    <a href="https://checkstyle.org/checks/whitespace/index.html">Whitespace</a> / Naming &amp; Formatting
                </strong>
            </td>
        ...
        <tr>
            <td rowSpan={7}>
                <strong>
                    <a href="https://checkstyle.org/checks/javadoc/index.html">Javadoc Comments</a> / Documentation
                </strong>
            </td>
        ...
        <tr>
            <td rowSpan={4}>
                <strong>
                    <a href="https://checkstyle.org/checks/misc/index.html">Miscellaneous</a> / Miscellaneous
                </strong>
            </td>
...
<table>
    ...
    <tbody>
        <tr>
            <td rowSpan={7}>
                <strong>Best Practices / Bad Practice</strong>
            </td>
        ...
        <tr>
            <td rowSpan={10}>
                <strong>Code Style / Code Style</strong>
            </td>
        ...
        <tr>
            <td rowSpan={17}>
                <strong>Error Prone / Potential Bugs</strong>
            </td>
        ...
        <tr>
            <td rowSpan={3}>
                <strong>Multithreading / Potential Bugs</strong>
            </td>
        ...
        <tr>
            <td rowSpan={1}>
                <strong>Performance / Performance</strong>
            </td>
            <td>BigIntegerInstantiation</td>
        </tr>
        <tr>
            <td rowSpan={1}>
                <strong>Security / Security</strong>
            </td>
            <td>All rules</td>
        </tr>
    </tbody>
...
<table>
    ...
    <tbody>
        <tr>
            <td rowSpan={4}>
                <strong>Memory Management / Potential Bugs</strong>
            </td>
        ...
        <tr>
            <td rowSpan={8}>
                <strong>Undefined Behavior / Potential Bugs</strong>
            </td>
            <td>double-free</td>
        </tr>
        <tr>
        ...
        <tr>
            <td rowSpan={3}>
                <strong>Bad Practice / Bad Practice</strong>
            </td>
            <td>double-fclose</td>
        </tr>
        <tr>
        ...
        <tr>
            <td rowSpan={1}>
                <strong>Miscellaneous / Miscellaneous</strong>
            </td>
...
<strong>Important:</strong> Only method/class bodies should fail to compile, not complete test classes. Anything outside method/nested class bodies must compile, including method
signatures, return types, parameter types, and lambdas. Use nested classes for fields/methods with student class types that might not compile.
...
<strong>Definition:</strong> One submission = one push to the exercise repository by the participant that triggers automatic tests resulting in feedback. Automatic test runs
triggered by instructors are not counted as submissions.
