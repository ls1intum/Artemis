...
The **Template** repository of several languages ships a `.vscode/tasks.json` and a `.vscode/launch.json`, so students can press **Run** or start a debug session in the online IDE without setting anything up first. Student repositories inherit both files when they are created from the template.
...
**Run** works in any online IDE image. **Debug** additionally requires the matching debug extension to be preinstalled in the image.
...
Not every template ships a run configuration. The Python template, for example, contains sorting algorithms but no entry point that orchestrates them. To enable **Run** and **Debug** for such an exercise, add an entry point (e.g. `main.py`) to the **Template** repository and create the two configuration files next to your sources.
...
The same pattern applies to any language: put the run command into `tasks.json` and the entry point into `launch.json`, both inside a `.vscode` directory in the **Template** repository.
