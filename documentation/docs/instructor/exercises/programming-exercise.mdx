...
Hyperion can generate or adapt a complete Java Maven or Gradle programming exercise: problem statement, template, solution, and tests. The action is limited to supported unreleased draft exercises without student participations. Output is saved automatically only after mechanical verification and the automated full-artifact review complete without blocking findings. Treat every result as a draft and review every artifact before release.
