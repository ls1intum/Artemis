...
Hyperion can generate or adapt a complete Java programming exercise: problem statement, template, solution, and tests. The action is limited to unreleased draft exercises without student participations. Treat every result as a draft and review it before release.
...
- **From the online editor:** Use **Artemis Intelligence** → **Generate exercise**. For an existing exercise, **Adapt exercise** can apply selected consistency-review feedback plus optional instructions.
...
The activity panel streams progress and changed-file previews. A reload reconnects to the retained job state. You can cancel a running job; cancellation is cooperative and can take effect after the current model or build operation returns.

Hyperion accepts generated content only after an isolated verification run confirms that the solution passes, the template fails the required tests, task bindings resolve, and protected test infrastructure remains unchanged. Only accepted output can update the live exercise. A rejected but recoverable draft can instead be saved to isolated review branches; review and merge all three repositories with an external Git client because Artemis does not expose custom branches. A partial live update is reported as requiring manual review and cannot be reverted automatically.

A successfully persisted adaptation offers **Undo adaptation** for up to seven days while the in-memory baseline remains available, provided the repositories have not changed again. Undo restores the captured pre-adaptation commits and problem statement; a full cluster restart can make the action unavailable earlier.
