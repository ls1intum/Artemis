...
3. [Adapt the build phases (optional)](#adapt-build-phases-optional)
...
To do this, check the **Customize Build Plan** option in the exercise creation, edit, or import screen. All changes apply to all builds (template, solution, and student submissions).
...
- **Inclusion condition**: The condition can be either `Always` or `After due date`. If a phase is configured to run after the due date, Artemis automatically schedules all submissions to be rebuilt one minute (configurable) after the due date (or after the exam's due date, including grace period and student time extensions).
...
1. **Immediate Full Feedback:** Set all phases to `Always`. All test results are shown immediately after submission.
2. **Build Status Only:** Remove all phases that expect tests. Students only see if their code compiled successfully.
3. **Tests After Due Date:** Keep the compilation phase as `Always` and set any phases expecting tests to `After due date`. Test output remains hidden until the deadline passes.
4. **Partial Immediate Feedback:** Set specific test phases (e.g., static code analysis) to `Always`, and other test phases to `After due date`. Students receive immediate feedback on some aspects and delayed feedback on others.

**Default behavior:**
- **Creating a course programming exercise**: Phases are set to always run. Test results are set to always be visible.
- **Creating a programming exercise in exam mode**: Phases that expect tests are automatically set to run after the due date. This means only compilation will occur during the exam, while tests will run after the exam concludes.
...
Update the build plan accordingly (see the [Adapt Build Configuration](#adapt-build-phases-optional) section).
