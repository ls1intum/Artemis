...
<Callout variant={CalloutVariant.info}>
    ...
    This feature requires the Hyperion module to be enabled by the administrator (<code>artemis.hyperion.enabled=true</code>). Exercise generation requires at least the <strong>Editor</strong> role within the course.

</Callout>

Artemis can use Hyperion to generate or adapt complete Java programming exercises, including the template, solution, tests, and problem statement. Exercise generation is currently only available for unreleased Java draft exercises without student participations. Accepted results are written to the exercise repositories, committed, and built through the configured CI setup. Treat the generated result as a draft: review the changed files, inspect the build result, and adapt the exercise before releasing it to students.

There are two ways to start exercise generation:

**During exercise creation:** For new Java programming exercises, click **Generate with AI** instead of the regular generate action. Artemis creates the exercise with prepared repositories, opens the online editor, and starts generation for the exercise.
...
**From the online editor:** Open the exercise with <img src={editInEditor} alt="Edit in Editor" style={{display: 'inline', verticalAlign: 'middle', height: '2rem'}} /> and use **Artemis Intelligence** → **Generate exercise**. Hyperion streams progress and file snapshots while the exercise is generated or adapted.

<Image
    src={hyperionGenerateCode}
    alt="Artemis Intelligence menu with Generate exercise action"
    size={ImageSize.small}
    caption="Generate exercise action in the Artemis Intelligence menu"
/>
...
After selecting a thread, the thread header shows that it is selected for exercise adaptation and the action changes to **Remove Selection**.

<Image src={hyperionSelectionTag} alt="Review thread marked as selected for exercise adaptation" size={ImageSize.large} caption="Selected review thread indicator" />
...
Selected feedback is passed to Hyperion when you start **Adapt exercise**. Hyperion uses the selected thread comments together with any additional instructions as targeted human feedback while updating the problem statement, template, solution, and tests.
...
<Image
    src={hyperionGenerationStatus}
    alt="Generation status popover with repository states and file activity"
    size={ImageSize.large}
    caption="Live generation status with repository states and file activity"
/>
...
<Callout variant={CalloutVariant.warning}>
    ...
    Avoid editing the same exercise repositories manually while exercise generation is running. If another generation job is already active for the exercise, Artemis blocks the new request until the active job has finished.

</Callout>

<Image
    src={hyperionGeneratingCode}
    alt="Exercise generation in progress dialog"
    size={ImageSize.small}
    caption="Dialog shown when exercise generation is already running for the exercise"
/>
