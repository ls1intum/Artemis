...
Hyperion can generate or adapt a complete Java Maven or Gradle programming exercise: problem statement, template, solution, and tests. The action is limited to supported unreleased draft exercises without student participations. Accepted output is saved automatically after mechanical verification. Treat every result as a draft and review every artifact before release.
...
Before a manually started generation or adaptation, Artemis explains that accepted output is saved automatically. The activity panel streams progress and changed-file previews. After persistence, **Review saved changes** opens the problem-statement version history or the exact Hyperion commit diff for each touched repository. A reload reconnects to the retained job state. You can cancel a running job; cancellation is cooperative and can take effect after the current model or build operation returns.

Hyperion accepts generated content only after an isolated mechanical verification run confirms that the solution passes, the template fails the required tests, task bindings resolve, and protected test infrastructure remains unchanged. These checks do not establish pedagogical quality or semantic completeness; instructor review remains required. Only accepted output can update the live exercise. A rejected but recoverable draft can instead be saved to isolated review branches; review and merge all three repositories with an external Git client because Artemis does not expose custom branches. A `PARTIAL` result means that live persistence was incomplete or that recoverable output exists only on isolated recovery branches. It is not fully saved, requires manual review, and cannot be reverted automatically.
...
3. **Configure Import Options**
    ...
    <Image src={programmingImportOptions} alt="Import Options" size={ImageSize.large} caption="Import Options" />- **Recreate Build Plans**: Create new build plans instead of
    copying from imported exercise - **Update Template**: Update template files in repositories. Useful for outdated exercises. For Java, replaces JUnit4 with Ares (JUnit5) and
    updates dependencies. May require test case adaptation
