...
For some languages — primarily Java, Kotlin, Swift and Go — you can specify the package that the exercise template uses, which defines the package structure students must follow. This keeps submissions consistent, which matters when tests locate classes by their fully qualified name.
...
Currently supported for OCaml and Haskell only.
...
Each has a name, a checkout directory and a description, and is created together with the exercise. If a checkout directory is set, the repository is checked out there during every build. See [Auxiliary Repositories](/instructor/exercises/programming-exercise/create-an-exercise#auxiliary-repositories) for how to configure them.
