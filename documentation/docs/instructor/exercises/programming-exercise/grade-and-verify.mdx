...
<Callout variant={CalloutVariant.warning}>
  ...
  A reset cannot be undone. Deleting participations, submissions and results without also recreating the repositories and build plans can leave orphaned artifacts behind on the version control and CI systems, so review the selection carefully before confirming.
