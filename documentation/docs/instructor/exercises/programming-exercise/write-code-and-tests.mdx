...
import instructorEditor from '../assets/programming/code-editor-instructor.png';
...
<Image src={instructorEditor} alt="Online editor on the template repository, with the tasks beside the code" size={ImageSize.large} caption="Editing the template repository" />
...
<Callout variant={CalloutVariant.info}>
    ...
    <ul>
        <li>Click <strong>Submit</strong> frequently to ensure your work is committed to the repository.</li>
...
Edit it either with **Edit** on the exercise, or with **Edit in Editor** to have it side by side with the code. Both editors synchronise in real time when several people work at once — see [Collaborative Editing](#collaborative-editing).
