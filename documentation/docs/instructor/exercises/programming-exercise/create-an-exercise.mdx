...
- **Preview**: Shows the repository and build plan names that will be generated from the short names.
...
Define up to two categories per exercise. Categories are visible to students, so use them consistently to group related exercises.
...
Auxiliary repositories are additional repositories beyond the template, solution and test repositories. Each one has a name, a checkout directory and a description, and is created together with the exercise. **Cannot be changed after creation.**
...
- **Customize Build Configuration**: Lets you adapt the build plan. Available for all languages with Local CI and Jenkins, and also adjustable after creation. See [Configure the Build](/instructor/exercises/programming-exercise/configure-the-build).
...
- **Assessment Type**: **Automatic** grading from the test results, or **Manual**, where tutors review submissions afterwards.
...
- **Template** (BASE): Builds the template against the tests. Student build plans are copied from it.
- **Solution** (SOLUTION): Builds the solution against the tests, which is how you verify the exercise is set up correctly.
...
2. Click **Import** in the page header. The **Manage Exercises** dialog opens on its **Import** tab; click the **Programming** card, then pick the exercise to import.
   ...
   <Callout variant={CalloutVariant.info}>
     ...
     You can import from any course in which you are an instructor.
