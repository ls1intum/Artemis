...
Test results normally reach Artemis as XML files in JUnit format, collected from the result paths configured for each build phase. Any language can therefore report results, as long as it writes JUnit XML.

### Custom Feedback

When a test result cannot be expressed as a JUnit case, write a **JSON** file into one of the phase's result paths instead. With Integrated Code Lifecycle, Artemis parses every `.json` file it collects as custom feedback, using this schema:
