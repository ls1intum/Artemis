...
A `Collection` or `Map` of objects. It is the inverse of a **ManyToOne** relationship. Example: one `Result` has a list of `Feedback` elements. For ordered OneToMany relations see the ordered collections section below.
...
Propagates all operations mentioned below from the parent object to the to child object.
