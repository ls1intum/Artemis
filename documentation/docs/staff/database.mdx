...
Please prepare for the meeting and enter your PR and a short description of the changes in the [meeting agenda](https://outline.aet.cit.tum.de/doc/meetings-stDhTnsRvf).
...
- **Deployment Restrictions:** Database-affecting deployments cannot proceed without prior approval from designated reviewers to ensure accountability and prevent unauthorized changes. For test server deployments, approval is required from [Timor Morrien](https://github.com/Hialus) or [Benjamin Schmitz](https://github.com/bensofficial).
...
<Callout variant={CalloutVariant.info}>
    <p>
        Additional effort to use <code>FetchType.LAZY</code> does not count as a strong argument.
    </p>
</Callout>
...
A `Collection` or `Map` of objects. It is the inverse of a **ManyToOne** relationship. Example: one `Result` has a list of `Feedback` elements.
For ordered OneToMany relations see the [ordered collections section](#choosing-collection-types) below.
...
<Callout variant={CalloutVariant.danger}>
  For <strong>OneToMany</strong>, <strong>ManyToOne</strong>, and <strong>ManyToMany</strong> relationships you must not forget to mark the associated elements with <code>@JsonIgnoreProperties()</code>. Without this, the object serialization process will be stuck in an endless loop and throw an error. For more information check out the examples listed above and see: <a href="https://www.concretepage.com/jackson-api/jackson-jsonignore-jsonignoreproperties-and-jsonignoretype">Jackson and JsonIgnoreType</a>.
</Callout>
...
<Callout variant={CalloutVariant.info}>
  This cascade type is not used in Artemis yet.
</Callout>
...
<Callout variant={CalloutVariant.danger}>
  Do NOT skip the reload step when using <code>CascadeType.REMOVE</code> with L2 cache. The stale cache
  references will cause <code>EntityNotFoundException</code> during cascade processing.
</Callout>
...
### 3. DynamicSpecificationRepository
...
<details>
<summary>Example: DynamicSpecificationRepository Usage</summary>
...
A `Set` comes with certain advantages such as ensuring that there are no duplicates and null values in your collection. There are also performance arguments to use a `Set`, especially for `@ManyToMany` relationships. For more information see this [stackoverflow thread](https://stackoverflow.com/questions/4655392/which-java-type-do-you-use-for-jpa-collections-and-why).

<details>
<summary>Example: Unordered Collection with Set</summary>
...
<details>
<summary>Example: TreeSet for Ordered Collection without Duplicates</summary>
...
<details>
<summary>Example: Eagerly Fetch with LEFT JOIN FETCH</summary>
...
### JpaSystemException with @OrderColumn
...
1. Save the child entity (e.g., [Feedback](https://github.com/ls1intum/Artemis/blob/develop/src/main/java/de/tum/cit/aet/artemis/domain/Feedback.java)) without connection to the parent entity (e.g., [Result](https://github.com/ls1intum/Artemis/blob/develop/src/main/java/de/tum/cit/aet/artemis/domain/Result.java))
...
<Callout variant={CalloutVariant.info}>
  For more information see the ordered collections section above.
</Callout>

### Null Values in Ordered Collections

There are `null` values in your ordered collection: You must annotate the ordered collection with `CascadeType.ALL` and `orphanRemoval = true`.

<details>
<summary>Example: Prevent Null Values in Ordered Collections</summary>
