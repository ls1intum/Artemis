...
<Callout variant={CalloutVariant.info}>
    For production setups with cloud providers (OpenAI, Azure OpenAI), refer to the [Hyperion Admin Setup](https://docs.artemis.cit.tum.de/admin/setup/hyperion/) in the legacy documentation.
</Callout>
...
3. Create a programming exercise and open it in the editor (click "Edit in Editor")
4. Click the **"Check Consistency"** button in the Hyperion toolbar
5. Observe the logs:

**Artemis Server logs** should include:

```
SpringAIConfiguration      : Found Chat Model: openai/gpt-oss-20b
```

**LMS Server logs** can be viewed using:

```bash
lms log stream
```

Or by opening the LM Studio GUI and activating the Developer view.

The LMS logs will show the prompt sent from Artemis, progress status, and the generated prediction.
...
This is a common issue on macOS with Apple Silicon when using MLX models like `gpt-oss-20b`:
...
- [Hyperion Admin Setup](https://docs.artemis.cit.tum.de/admin/setup/hyperion/) - Production setup with cloud providers
- [Server Setup](https://docs.artemis.cit.tum.de/dev/setup/server/) - General Artemis server configuration
