...
Server integration tests run against PostgreSQL by default (matching CI) using Docker containers managed by Testcontainers. The Gradle test task automatically pulls the necessary Docker images and starts the containers.
...
### PostgreSQL Tests (recommended, matches CI)

```bash
./gradlew testPostgres -x webapp
```
