...
- Declare an application-wide service with `@Service()` instead of `@Injectable({ providedIn: 'root' })`. It is the same provider, it is what `ng generate service` emits,
  and it rejects constructor injection at compile time. Keep `@Injectable` for a service with other provider metadata, and for a class that carries another Angular
  decorator, such as a pipe that is also injected as a service: `@Service()` cannot share a class with `@Pipe`. The autofixable `@angular-eslint/prefer-service-decorator`
  rule enforces this.
