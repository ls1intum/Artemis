...
<Callout variant={CalloutVariant.danger}>
    New UI uses PrimeNG components with Tailwind utilities, as specified in `CLAUDE.md`. The private [tum-ui package](/developer/guidelines/tum-ui-kit) is an extraction boundary
    for existing Artemis-owned components; it does not replace this policy. Do not create a custom widget when PrimeNG, Angular CDK, Angular Aria, or native HTML already provides
    the required behavior. Never introduce new Bootstrap.
</Callout>
