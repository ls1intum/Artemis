...
**We are migrating from Bootstrap to [Tailwind CSS v4](https://tailwindcss.com/) (utilities) + [PrimeNG](https://primeng.org/) (components).** All new UI elements must use PrimeNG components for behavior and Tailwind utilities for layout/spacing/color. Do not introduce new Bootstrap components or Bootstrap CSS classes. Existing Bootstrap usage is migrated module by module.
</Callout>
...
* **PrimeNG component** for anything with behavior, state, accessibility semantics, or that Bootstrap modeled as a *component*: `p-button`, `p-tag` (badges), `p-message` (alerts), `p-card`, `p-table`, `p-tabs`, `p-paginator`, `p-tooltip`, `p-progressbar`, dialogs (`DialogService`), form inputs (`pInputText`/`p-select`/`p-checkbox`), `p-menu`, `p-datepicker`.
...
Why the **arbitrary** form `text-(--danger)` rather than a named `text-danger`? While Bootstrap is still loaded **unlayered**, its `.text-danger { … !important }` beats any layered Tailwind utility unconditionally, so a named `text-danger` would silently render Bootstrap's color. The escaped class `.text-\(--danger\)` shares no selector with Bootstrap, so it wins cleanly. At the Bootstrap-removal exit gate, add `@theme inline { --color-danger: var(--danger); … }` and `sed` `text-(--danger)` → `text-danger` — a pure rename with zero visual change.
