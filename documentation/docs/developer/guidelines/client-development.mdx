...
<Callout variant={CalloutVariant.info}>
    ...
    Artemis-specific implementation details. For the decisions a surface makes before any markup exists — information hierarchy, the states it must survive, how it prints an
    uncertain number, and when it may animate — see **[Client UX Patterns](/developer/guidelines/client-ux)**.
</Callout>
...
**A Tailwind utility whose name Bootstrap also defines loses — including inside a fully migrated module.** Locking a module stops it from _writing_ Bootstrap classes; it does not unload
Bootstrap's stylesheet, which stays global until the last module migrates. Bootstrap's utilities are unlayered and carry `!important`, and an unlayered `!important` declaration beats
anything in `@layer utilities` regardless of source order — so layering Bootstrap later would not change this either.

</Callout>

The colliding names are `border`, `border-0`, `rounded`, `text-primary`, `bg-primary`, `border-primary`, `text-white`, `text-center`, `text-end`, `bg-transparent`, and `shadow-lg`.
Most of them happen to resolve to the same value, which is what makes the class dangerous rather than merely dead — the failure is invisible until the two definitions drift. Three
diverge today:

| Written                       | Renders                                                                                                               | Write instead                                                  |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `border`                      | Bootstrap's shorthand with `--bs-border-color`, overriding the deliberately quieter dark hairline                     | a directional width (`border-b`, `border-x`) or component SCSS |
| `border-0`                    | `border: 0 !important`, which also deletes a sibling `border-b-2` — an underline written this way is simply not drawn | `border-none`, or no border utility at all                     |
| `text-primary` / `bg-primary` | Bootstrap's brand color in both themes, plus a third value from the dark theme's own override                         | a TUM UI `severity`, or the `--primary` semantic property      |

**Headings do not take their typography from utilities either.** Bootstrap's unlayered `h1`–`h6` type scale and `global.scss`'s `body, h1, h2, h3, h4 { font-weight: 400 }` beat
`@layer utilities` the same way, so `<h2 class="text-lg font-semibold">` silently renders at the Bootstrap size and weight. Do not answer this with `!` utilities: use a package
component that owns the typography of its own heading. `tum-ui-card-title` is the pattern: it takes the heading `level` as an input and publishes `role="heading"` with `aria-level` on
its own element, so it still joins the page's heading outline while no Bootstrap `h*` element rule can reach it.
