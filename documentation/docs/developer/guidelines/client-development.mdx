...
An ESLint rule (`localRules/prefer-signal-reactivity-over-ngonchanges`) **warns** on `ngOnChanges` in `@Component` / `@Directive` classes across **all** client modules — the entire client is migrated to signals, so the rule is applied globally (the same scope as `localRules/enforce-signal-apis`). It is intentionally a warning, not an error.

The goal is to **remove `ngOnChanges` entirely**: migrate every usage to `computed()` (derived state) or `effect()` (genuine side effects). For the rare cases that still genuinely need `ngOnChanges` (the bottom two rows above — `SimpleChanges.previousValue` / `isFirstChange()`, or logic that must run before child components initialise), add a **detailed comment** explaining why a signal-based replacement is not yet possible. Do **not** silence the rule with `eslint-disable`: the warning is intentionally left in place so the remaining cases stay visible until a signal-based replacement is found.
