...
Markup the application did not author itself is rendered inside a **shadow root**, never directly into this document.
The server-side problem statement renderer is the case this exists for: a problem statement is authored by course
staff, passes a jsoup safelist on the server and DOMPurify in the browser, and is then injected into the shadow root of
the content component.

The shadow root isolates the statement's **CSS**, not its scripts: it is same-origin, so it is not a security boundary.
What keeps a sanitizer bypass from reaching the application is the sanitization itself, not any origin barrier. The
statement is therefore protected by two independent sanitizers, and the client assembly (`assembleShadowContent` in
`app/programming/shared/instructions-render/ssr/problem-statement-frame.util.ts`) lets nothing executable through:

- **The fragment is sanitized with DOMPurify** after the server's jsoup safelist, which also covers the SVG a diagram
  emits that the server safelist deliberately does not see. Every `<script>` is removed, and no event-handler attribute
  survives, so no author markup runs.
- **Nothing about the viewer goes into the shadow root.** `data-feedback` and `data-result` (the reader's own results)
  are stripped before injection: without an origin boundary the statement shares the page's credentials, so observable
  data must not sit in a DOM a bypass could read.
- **Same-origin images the server could not inline are replaced by their alt text**, and retained external images get
  `referrerpolicy="no-referrer"`. The client requests `inlineImages: true`, so local files arrive as `data:` URIs.
- **Clicks and keyboard activation are handled by the content component**, not by native navigation: every statement
  anchor (HTML and SVG) has its default cancelled and is opened only if it is one of the hrefs the render actually
  contained, so a click can never navigate the application away or open an arbitrary URL.

An earlier version of this renderer put the statement in a sandboxed iframe for script and origin isolation on top of
the sanitization. That was dropped in favour of the double jsoup + DOMPurify sanitization: the iframe added a
cross-document `postMessage` protocol and a per-engine cookie caveat of its own, and the same attack surface (a single
sanitizer bypass equals a compromise) already exists in the legacy `[innerHtml]` renderer that ships today. The
trade-off is deliberate and documented in the pull request. jsdom implements no layout and its shadow-root focus
handling is incomplete, so the rendering, highlighting and interactivity are asserted in the Playwright layout spec
...
Charts and diagrams are instantiated with PrimeNG's [`p-chart`](https://primeng.org/chart), which wraps
[Chart.js](https://www.chartjs.org/docs/latest/). Persistent value labels on top of the marks come from
[`chartjs-plugin-datalabels`](https://chartjs-plugin-datalabels.netlify.app/).

Do not hand-write Chart.js option objects. `app/shared-ui/chart` holds the shared layer that keeps every chart
consistent, and you should build on it:

| Helper                                                                                                | Purpose                                                                                |
| ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `barChartOptions`, `lineChartOptions`, `doughnutChartOptions` (`chart-options.ts`)                    | Option factories: axes, legend, tooltips, data labels, percent scales                  |
| `singleSeriesChartData`, `multiSeriesToStackedBarData`, `multiSeriesToLineData` (`chart-adapters.ts`) | Turn Artemis' `ChartSeriesEntry` / `ChartMultiSeriesEntry` models into Chart.js `data` |
| `ChartColorService.resolvedColors` (`chart-color.service.ts`)                                         | Resolves CSS custom properties to concrete colors and re-resolves on theme switch      |
| `toChartSelectEvent` (`chart-options.ts`)                                                             | Maps a `(onDataSelect)` payload back to the entry that was clicked                     |

A typical component looks like this — a signal for the data, `computed` option and data objects, and an explicit
height on the chart's container:
...
readonly chartOptions = computed(() =>
    barChartOptions({
        ...
        tooltip: { label: (item) => `${this.translateService.instant('statistics.amountOfStudents')}: ${item.parsed.y}` },
...
1. **Give the chart's container an explicit height.** Chart.js is configured with `maintainAspectRatio: false`, so
   it sizes the canvas from its parent. A parent without a definite height leaves the canvas at its 150px default,
   which overflows the box and overlaps whatever follows. If the parent is a flex item, `flex: 1; min-height: 0`
   works too; a plain block needs `style="height: …"` or a `[style.height.px]` binding. For a horizontal bar chart,
...
2. **Every chart needs an accessible name.** `p-chart` renders into a `<canvas role="img">`. Everything Chart.js
   draws — axis labels, data labels, the values themselves — is rasterised pixels: it cannot be selected, copied,
   searched, or read. Without an accessible name, assistive technology announces nothing but an unlabelled image,
   so the whole chart is silent. Supply one of:

    ```html
    <!-- Preferred: reuse the translation key of the chart's own visible heading -->
    <p-chart type="line" [data]="chartData()" [ariaLabel]="'artemisApp.courseStatistics.activeStudents' | artemisTranslate" />

    <!-- Or name it from the visible heading, when that heading has a stable unique id -->
    <p-chart type="bar" [data]="chartData()" [ariaLabelledBy]="'active-students-heading'" />

    <!-- Or, ONLY if the chart is decorative because every figure it shows is also rendered as text next to it -->
    <p-chart type="doughnut" [data]="chartData()" aria-hidden="true" />
    ```
    ...
    Note that `aria-label` is a _name_, not a _description_: it says what the chart is, not what it shows. Chart.js
    has no SVG renderer, so exposing the actual figures still requires a DOM text alternative — prefer rendering the
    key numbers as text next to the chart.

3. **Resolve colors through `ChartColorService`, never inline hex values.** Chart.js needs concrete colors, but
   canvas cannot read CSS custom properties. `resolvedColors` resolves the tokens you pass and re-resolves them when
   the user switches between the light and the dark theme; a hardcoded color would break one of the two.

4. **Keep axis labels, ticks and tooltips translation-sensitive.** Anything drawn inside the canvas is set from
   TypeScript, so it does not re-render on a language change by itself. Either build the option object in a
   `computed` that reads a signal fed by `onLangChange`, or subscribe and recompute:
...
5. **Do not override library internals with `::ng-deep`.** Use the documented `p-chart` inputs, the shared option
   factories, or CSS custom properties. If there is no supported customization point, reconsider the requirement or
   contribute one upstream instead of coupling Artemis to private markup.
