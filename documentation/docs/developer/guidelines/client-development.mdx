...
<Callout variant={CalloutVariant.danger}>
    **TUM UI is the target Artemis component system. Bootstrap, ng-bootstrap, and PrimeNG are migration-only dependencies.** Prefer a component from
    [`@tumaet/ui-angular`](/developer/tum-ui-reference) whenever one exists, and do not introduce Bootstrap or ng-bootstrap in new work. PrimeNG is a temporary fallback for a
    capability TUM UI does not yet provide, not the destination for new component design.
</Callout>
...
The target is **TUM UI for reusable components and [Tailwind CSS v4](https://tailwindcss.com/) for
application layout**. Bootstrap, ng-bootstrap, and PrimeNG remain only while existing screens and
missing TUM UI capabilities are migrated. Tailwind is wired through
`src/main/webapp/tailwind.css` and enabled module by module while Bootstrap remains global.

**Decision rule — what to reach for:**

- **Native HTML** for established platform semantics such as links, headings, lists, and basic form
  structure.
- **TUM UI component** for a reusable control or visual contract. Inspect the
  [component reference](/developer/tum-ui-reference) before adding another abstraction.
- **Tailwind utility** for application-owned layout, spacing, sizing, typography, and an Artemis
  semantic color utility where one exists (`flex`, `grid`, `gap-2`, `text-end`, `font-semibold`,
  `w-full`).
- **Angular CDK primitive** inside TUM UI when reusable behavior needs focus, overlay, directionality,
  selection, or virtual-scrolling infrastructure.
- **Artemis component** for domain composition. It may combine TUM UI components but must not move
  routes, requests, or feature policy into the package.
- **PrimeNG fallback** only for a missing TUM UI capability that cannot be delivered with the
  current change. Do not add Bootstrap or ng-bootstrap.
...
3. Do not override library internals with `::ng-deep`. Use documented inputs, CSS custom properties,
   or a supported wrapper contract. If the library exposes no stable customization point, reconsider
   the requirement or contribute one upstream instead of coupling Artemis to private markup.
