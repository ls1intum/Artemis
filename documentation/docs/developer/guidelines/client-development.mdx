...
| Need `SimpleChanges.previousValue` or `isFirstChange()` | keep `ngOnChanges` (signals don't expose these directly) |
| Logic that must run **before** child components initialise | keep `ngOnChanges` (it runs before `ngOnInit`; an `effect()` runs afterwards) |
...
An ESLint rule (`localRules/prefer-signal-reactivity-over-ngonchanges`) **warns** on `ngOnChanges` in `@Component` / `@Directive` classes. It is enabled for modules that are fully migrated to signals **and** already free of `ngOnChanges` (see `signalReactivityModules` in `eslint.config.mjs`); that list grows as the remaining migrated modules are cleaned up. The rule is intentionally a warning, not an error — for the legitimate cases above, keep `ngOnChanges` and silence the rule on the line with a short justification:
