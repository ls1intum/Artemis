...
applicable component, justify the gap in the pull request before introducing stable Angular CDK behavior. Do not
implement custom keyboard, focus, overlay, or selection behavior when a maintained library supplies
it.

The private TUM UI package contains existing cross-application components. It does not change the
PrimeNG policy. When maintaining an existing `@tumaet/ui-angular` use or contributing an agreed
cross-application contract, follow the [TUM UI package guide](/developer/guidelines/tum-ui-kit).
