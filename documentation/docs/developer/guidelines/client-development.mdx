...
Markup the application did not author itself is rendered inside a **sandboxed iframe**, never into this document. The
server-side problem statement renderer is the case this exists for: a problem statement is authored by course staff,
passes a jsoup safelist on the server and DOMPurify in the browser, and is then put into a frame that makes a bypass of
either one harmless rather than merely unlikely.

The rules, all of them load-bearing:

- The frame's `sandbox` is **`allow-scripts` and nothing else**. Adding `allow-same-origin` hands the markup back the
  cookies, the storage and the parent DOM the frame exists to keep it away from, and voids every other guarantee here.
  Angular refuses to let `sandbox` be a binding at all (NG0910), so it is always a static attribute.
- The frame carries **its own CSP** with a per-render nonce, written as a `<meta http-equiv>` as the first element in
  its `<head>`. This is not defence in depth. A `srcdoc` document inherits the embedder's policy, and the application
  policy carries `'unsafe-inline'`, so the inherited one stops nothing; and the sandbox alone is not enough in every
  engine (WebKit sends the `SameSite=Lax` JWT cookie from a sandboxed opaque-origin frame, where Chromium and Firefox
  send none). `connect-src 'none'`, `default-src 'none'` and the absent `'unsafe-eval'` are what close that.
- **Nothing about the viewer goes into the frame.** Attributes describing the reader's own results are stripped before
  assembly, because observable data is the only thing worth exfiltrating.
- **No same-origin URL may remain** in the frame's markup. A request from an opaque origin carries no credentials, so
  an authenticated asset would not load anyway; it is inlined by the server or replaced by its alt text.
- The frame talks to the application only over `postMessage`, over a small closed protocol with a per-render
  generation token, and the parent bounds-checks everything it accepts.

Because these guarantees differ between engines, the tests that assert them run in Chromium, Firefox **and** WebKit,
under the `@cross-engine` tag in the Playwright configuration. jsdom implements neither sandboxing nor CSP, so a Vitest
spec would report all of them as passing whatever the code did. See
`app/programming/shared/instructions-render/ssr/problem-statement-frame-policy.ts`.
