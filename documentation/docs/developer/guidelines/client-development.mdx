...
| `me-*` / `ms-*` / `mb-*` / `p-*` / `gap-*` | identical Tailwind utilities (the spacing scale matches 1:1) |
...
Enforced by `localRules/no-raw-tailwind-color-palette` (bans raw palette, hex, **and** primitive `--p-<color>-<n>` tokens used for meaning — in classes and in `[style.*]`/`[color]` bindings) across all templates, `localRules/no-bootstrap-classes` (bans Bootstrap color/component classes) on migrated modules, and a stylelint hex/`--bs-` ban per module.
