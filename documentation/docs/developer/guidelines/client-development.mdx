...
`@theme inline` is load-bearing: each generated utility emits a `var(--danger)` **reference** that resolves at the element, so dark mode keeps working (dark themes reassign `--danger` and the utility follows). A non-`inline` `@theme` would snapshot the light value into a new global and break dark mode. There is no exit-gate rename — these tokens survive Bootstrap removal as-is.
