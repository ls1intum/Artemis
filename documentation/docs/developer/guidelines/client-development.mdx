...
<Callout variant={CalloutVariant.danger}>
**Core rule: every component member that the template reads and that can change at runtime must be a signal (or `computed` / `input` / `model`).**

Only values that are **constant** for the lifetime of the component (icons, enum references, injected services, configuration literals) may stay plain class fields. Everything else that the template binds to — and that you ever reassign or mutate — must be signal-backed, so the view re-renders automatically when it changes.
</Callout>
