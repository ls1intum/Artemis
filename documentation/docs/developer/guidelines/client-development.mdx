...
**Colors must be brand-bound.** Use the semantic tokens from `tailwindcss-primeui` (`bg-primary`, `text-surface-*`, `text-muted-color`, `text-primary-contrast`) and the Artemis state tokens (`text-success`/`bg-danger`/…), never a raw Tailwind palette (`text-green-500`) or arbitrary hex (`bg-[#f00]`). This is enforced by the `localRules/no-raw-tailwind-color-palette` ESLint rule (all templates) and a stylelint hex/`--bs-` ban (per migrated module). Dark mode resolves automatically via the PrimeNG `darkModeSelector` (the Tailwind `dark:` variant is aligned to it).
