...
PrimeNG is the component library for new Artemis UI. Use Tailwind utilities for layout, spacing, and
semantic color. Use native HTML where no component abstraction is needed. If PrimeNG has no
applicable component, document the gap before introducing Angular CDK or Angular Aria behavior. Do
not implement custom keyboard, focus, overlay, or selection behavior when a maintained library
supplies it.

The private TUM UI package isolates existing reusable components. It does not change the PrimeNG
policy, and adding a new Artemis component to that package requires an explicit reusable-package
need. When working with it, browse the{' '}<Link to="pathname:///developer/tum-ui/?path=/docs/actions-button--docs" target="_self">Existing TUM UI components</Link>{' '}
and follow the [TUM UI package guide](/developer/guidelines/tum-ui-kit).

For dialogs, use PrimeNG `DialogService` from `primeng/dynamicdialog`. Pass inputs through its `data`
configuration instead of assigning `componentInstance` fields.

Do not add Bootstrap classes, Bootstrap components, or `@ng-bootstrap/ng-bootstrap` APIs. When
working in a module that still uses Bootstrap, follow the module-level migration process in
[Styling](#styling) instead of mixing frameworks within a component.

See the [PrimeNG documentation](https://primeng.org/) for component APIs.
