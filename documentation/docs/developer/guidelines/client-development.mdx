...
| `text-danger` / `text-success` / `text-warning` / `text-info`   | `text-(--danger)` / `text-(--success)` / `text-(--warning)` / `text-(--info)` (arbitrary form — see **Colors** below)                                                                                                                                             |
...
| state on **plain markup** (static)      | the arbitrary semantic utility `text-(--danger)` / `bg-(--success)` / `border-(--warning)` / `text-(--info)`                  | `text-danger` (Bootstrap) or `text-(--p-red-500)` (primitive) |
...
Why the **arbitrary** form `text-(--danger)` rather than a named `text-danger`? While Bootstrap is still loaded **unlayered**, its `.text-danger { … !important }` beats any layered Tailwind utility unconditionally (an unlayered `!important` rule wins over any layered rule regardless of layer order — so even wrapping Bootstrap in a `@layer` does not help; only removing Bootstrap's colour utilities does). The escaped class `.text-\(--danger\)` shares no selector with Bootstrap, so it sidesteps the collision and resolves the same theme-aware token. `text-(--danger)` is therefore the **deliberate coexistence form**, not technical debt to refactor piecemeal — keep it consistent.

**Do not** pre-emptively introduce a namespaced `@theme` (e.g. `--color-state-danger` → `text-state-danger`) to "look idiomatic" now: flat validation text and status glyphs do not need the autocomplete / `/opacity` features that `@theme` utilities add, and a namespace bakes in a throwaway name that has to be renamed twice. The idiomatic end state arrives in **one** pass at the Bootstrap-removal exit gate: add `@theme inline { --color-danger: var(--danger); … }` (the `inline` keyword forwards to the html-level theme-aware var so dark mode keeps working) and `sed` `text-(--danger)` → `text-danger` — a pure rename with zero visual change, because the tokens already exist.
...
**Coexistence with Bootstrap (transitional).** Bootstrap is still loaded globally for not-yet-migrated modules. To avoid global breakage during coexistence, `tailwind.css` deliberately does **not** import Tailwind's Preflight (Bootstrap's Reboot stays the authoritative reset) and uses `source(none)` with an explicit `@source` allowlist (one line per migrated module) so Tailwind only emits utilities for migrated code. A migrated module must be **fully** converted — never use a Tailwind utility to override a Bootstrap-styled element, because Tailwind's layer loses the cascade to unlayered Bootstrap. Once a module is Bootstrap-free, lock it with a `localRules/no-bootstrap-classes` ESLint block scoped to its templates (see `eslint.config.mjs`).

**Exit criteria (global Bootstrap removal — separate effort).** Once Bootstrap is removed app-wide: switch `tailwind.css` to `@import 'tailwindcss';` (re-enables Preflight), configure the PrimeNG `cssLayer` ordering (`tailwind-base, primeng, tailwind-utilities`) in `app.config.ts`, drop `source(none)` + the `@source` allowlist, and remove the `bootstrap` dependency.
