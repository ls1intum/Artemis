...
<Callout variant={CalloutVariant.info}>
    Artemis used to render charts with `ngx-charts` (SVG). Those components are gone — if you find a
    `<ngx-charts-*>` element or an `ngxData`/`[scheme]`/`[view]` binding, it is dead code.
</Callout>
...
2. **Every chart needs an accessible name.** `p-chart` renders into a `<canvas role="img">`. Everything Chart.js
   ...
   so the whole chart is silent. Supply one of:
    ...
    Note that `aria-label` is a *name*, not a *description*: it says what the chart is, not what it shows. Chart.js
