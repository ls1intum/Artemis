...
<Callout variant={CalloutVariant.danger}>
  <strong>Steps 15–17 are destructive and irreversible.</strong> They remove persisted statistics and the source rows for authored content and selections. Run the migration
  against a copy of the production database and verify representative quizzes before the real cutover — especially on MySQL, where the backfill SQL is not covered by the
  automated test suite (see Caveats).
</Callout>

**Statistics objects dropped (steps 15–16):**

`exercise.quiz_point_statistic_id`, `quiz_question.quiz_question_statistic_id`, `quiz_statistic_counter`, and `quiz_statistic`.

The owner foreign keys and unique constraints are removed before the columns. The counter table is dropped before the statistic table and before the authored-content tables whose
rows its historical foreign keys referenced. The initial schema omits the same objects for fresh installations.
...
<Callout variant={CalloutVariant.danger}>
  <strong>The statistics cutover is coordinated and irreversible.</strong> Stop or upgrade every old application node before this migration runs. An old node still expects the
  removed mappings and cannot coexist with the final schema. The dropped aggregate rows and ids cannot be reconstructed; results and submitted-answer selections are the source
  of truth after deployment.
</Callout>
...
- **No aggregate identity.** Statistics DTO ids are intentionally `null`; callers must not use former statistic or counter row ids.
...
- **Irreversible.** Once the legacy child and statistics tables are dropped there is no rollback path; authored content/selections and Results are authoritative.
...
| Tables fully removable | could drop `quiz_statistic` **and** `quiz_statistic_counter` entirely | both are removed directly by this migration |
