...
mysql --host "$DB_HOST" --user "$DB_USER" --password "$DB_NAME" < supporting_scripts/migration/quiz_point_counter_preflight_mysql.sql > quiz-point-counter-preflight.txt
```

The report contains:

1. structurally invalid `PC` ownership rows (this result must be empty);
2. every source row that will be normalized or merged, including its preserved/target id and `exercise.max_points` versus the current question-point sum;
3. owner-level differences between relational participant summaries and the normalized counter sums (reported only; the migration never changes the summary columns).
