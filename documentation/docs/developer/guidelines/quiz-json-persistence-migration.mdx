...
| `drag_and_drop_mapping.{drag_item_id, drop_location_id}` **where `submitted_answer_id` is set** | `selection.mappings[].{dragItemId, dropLocationId}` (no id — submission mappings have no identity) |
...
| `short_answer_submitted_text.{spot_id, text, is_correct}` | `selection.submittedTexts[].{spotId, text, isCorrect}` (row `id` dropped — no identity) |
...
| `multiple_choice_submitted_answer_selected_options.selected_options_id` | `selection.selectedOptionIds[]` |
...
| `drop_locations_order`, `drag_items_order`, `spots_order`, `solutions_order`, `answer_options_order` | Array element order encodes it (preserved via `ORDER BY` on PostgreSQL; see the MySQL ordering caveat) |
