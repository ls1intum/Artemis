...
| Old column                                                                                  | New location                                                                |
| ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `quiz_statistic_counter.{id, points, rated_counter, un_rated_counter}` (discriminator `PC`) | `quiz_statistic.counters[].{id, points, ratedCounter, unRatedCounter}`      |
| `quiz_statistic_counter.quiz_point_statistic_id`                                            | Ownership by containment in the `QuizPointStatistic` row's `counters` array |

Existing point-counter ids are preserved. New counters use statistic-scoped `max(existingIds) + 1`, matching the local-id pattern used by quiz-question JSON components.
...
| `quiz_statistic_counter.{drag_and_drop_question_statistic_id, short_answer_question_statistic_id, multiple_choice_question_statistic_id, quiz_point_statistic_id}` | Statistic ownership is represented by containment in `quiz_statistic.counters`                 |

## Migration steps

The migration runs the original question-type phases followed by the PointCounter cutover. The `content` / `selection` columns are added once (steps 1a–1d), the `counters` column once (steps 5a–5b); every statistic subtype then reuses that column.
...
| Point-statistics follow-up | Normalize and backfill `PC` rows into `quiz_statistic.counters`, validate parity, then drop `quiz_statistic_counter`      |

Each dialect has its own changeset: PostgreSQL uses `jsonb_build_object` / `jsonb_agg` (with `ORDER BY`), MySQL uses `JSON_OBJECT` / `JSON_ARRAYAGG`. Backfills are filtered by the owner discriminator (`DD` / `SA` / `MC` / `QP`) so each row gets exactly the right shape, and use an empty array when an owner has no counters.

## Cleanup: what is dropped and what is kept

<Callout variant={CalloutVariant.danger}>
    <strong>The cleanup and point-statistics follow-up are destructive and irreversible.</strong> They delete the source rows and drop the legacy tables, so the JSON backfills
    become the single source of truth. Run the migration against a copy of the production database and verify the generated preflight report <em>before</em> the real cutover —
    especially on MySQL, where the backfill SQL is not covered by the automated test suite (see Caveats).
</Callout>

### PointCounter preflight report

Run the dialect-specific, read-only preflight script against the production snapshot and redirect the complete client output to an immutable deployment artifact:

```bash
psql "$DATABASE_URL" --file supporting_scripts/migration/quiz_point_counter_preflight_postgresql.sql > quiz-point-counter-preflight.txt
mysql --host "$DB_HOST" --user "$DB_USER" --password --database "$DB_NAME" < supporting_scripts/migration/quiz_point_counter_preflight_mysql.sql > quiz-point-counter-preflight.txt
```

The report contains:

1. structurally invalid `PC` ownership rows (this result must be empty);
2. every `QP` statistic without exactly one exercise owner (this result must be empty);
3. every source row that will be normalized or merged, including its preserved/target id and `exercise.max_points` versus the current question-point sum;
4. owner-level differences between relational participant summaries and the normalized counter sums (reported only; the migration never changes the summary columns).

Stop writers before generating the final report. Keep them stopped while the migration, post-backfill checks, and smoke test run. A failed precondition leaves `quiz_statistic_counter` intact; after the drop, recovery is forward-only or from the pre-cutover backup.
...
<Callout variant={CalloutVariant.info}>
    <strong>`quiz_statistic` is kept; `quiz_statistic_counter` is dropped.</strong> All four statistic subtypes keep their relational owner row and use the discriminator-dependent{' '}
    <code>counters</code> JSON column. The point-statistics follow-up preserves existing PointCounter ids, normalizes invalid numeric legacy values, merges collisions
    deterministically, validates the resulting arrays, and only then drops the remaining counter table.
</Callout>
...
- **Ids are preserved.** Old primary keys become local ids inside the JSON. PointCounter ids stay in REST and WebSocket payloads; newly created PointCounters allocate ids within their `QuizPointStatistic` using `max(existingIds) + 1`.
- **Discriminator-scoped.** Each backfill only touches rows of its own statistic type, so the four subtypes that share `quiz_statistic` never cross-contaminate.
...
- **Irreversible.** Once step 17 and the point-statistics follow-up drop the tables there is no rollback path for the child data; the JSON columns are authoritative.
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Statistics location    | folded **all** statistics, incl. point stats, into `quiz_question.statistics` + `quiz_exercise.statistics` (2 extra columns) | keeps every statistic owner row in `quiz_statistic`; all discriminator-specific counter arrays use its single `counters` column |
| Tables fully removable | could drop `quiz_statistic` **and** `quiz_statistic_counter` entirely                                                        | keeps `quiz_statistic` for relational summaries and ownership; drops `quiz_statistic_counter` after the point-counter follow-up |
...
| Table drops            | present but **commented out** (never executed)                                                                               | executed in steps 15–17 and the PointCounter follow-up                                                                          |
