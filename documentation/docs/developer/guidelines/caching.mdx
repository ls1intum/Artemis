...
Artemis runs in a multi-node cluster (3 production nodes behind nginx with round-robin load balancing and no sticky sessions), serving thousands of concurrent users. The two caching layers in use are:

* **Spring `@Cacheable`**, resolved against the `RoutingCacheManager` in `CacheManagerConfiguration`. **Allowed**, but only with explicit eviction logic. Caches whose values are large blobs are per-node; all others are shared across nodes through the distributed data provider, so they work on Hazelcast and on Redis alike. See [Distributed Data](/developer/guidelines/distributed-data) for the split and for why no code may name a provider.
...
### No service-level `@Transactional`
...
### Multi-node cluster, no sticky sessions

Cross-node cache invalidation propagates asynchronously through the distributed store. Under load the staleness window is observable as user-visible bugs: a writer commits on node A while a reader on node B serves a still-cached, stale value. With round-robin load balancing the next request from the same user often hits a different node, so the inconsistency appears intermittent and is hard to reproduce in single-node testing.
...
Spring's caching abstraction backed by the distributed data provider is permitted for DTOs and computed values that are read-heavy with low write throughput. Representative existing caches (not exhaustive — check `@Cacheable`/`@CacheConfig` references in the codebase for the current set):

* Title caches: `courseTitle`, `exerciseTitle`, `lectureTitle`, `examTitle`, `tutorialGroupTitle`, `competencyTitle`, `diagramTitle`, `organizationTitle`. Used by breadcrumbs / navigation. Served **per node** by `TitleCacheConfiguration`, not by the distributed store: each answers with one column of one row found by primary key, so a request to another machine is not obviously cheaper than asking the database.
...
* `userCourseNotificationSettingPreset` and `userCourseNotificationSettingSpecification` — per-user/per-course notification settings, keyed by `setting_preset_<userId>_<courseId>` / `setting_specifications_<userId>_<courseId>`. Read once per recipient while a notification is filtered, which is what makes them worth caching. The values are the preset id and a record of the channel flags, never the settings entities.
...
* `savedPosts` — saved-post lookups, keyed by `saved_post_count_<userId>` / `saved_post_type_<type>_<userId>` / `saved_post_status_<status>_<userId>`. The values are a count, a list of ids, and a list of `SavedPostDTO`.

`RoutingCacheManager` decides where each of these lives, by name, so the choice is made in one place and cannot drift as caches are added:

* **Per node**, served by a bounded Caffeine cache: the blobs (`files`, `plantUmlPng`, `plantUmlSvg`) and the titles. A network round trip buys nothing for a rendered diagram or for one column found by primary key. Each node keeps its own copy, so eviction has to reach every node: `PerNodeCacheEvictionService` broadcasts it over a distributed topic, and every per-node cache also expires its entries, so a missed broadcast self-corrects instead of serving a stale value forever. That expiry window is the price of moving a cache off the shared store, which is why a cache whose staleness would be visible for long belongs in the distributed manager instead.
* **Distributed**, served by the configured `DistributedDataProvider`: everything else, which is what a cache whose entries have to be identical on every node needs.

Most caches ship with explicit eviction logic that runs on writes: `@CacheEvict` annotations on the writer repository or service for the notification and saved-post caches, and the Hibernate event listener pattern below for the title caches handled by `TitleCacheEvictionService` (currently `Course`, `Exercise`, `Lecture`, `Organization`, `ApollonDiagram`, `Exam`, `ExerciseGroup`).
...
## What a cached value may be

**A cached value must not be a JPA entity.** Project the query into a record of the fields the caller actually reads. `ArchitectureTest.testNoEntityAsCacheValue` fails the build on a `@Cacheable` or `@CachePut` method whose return type is, or contains, an entity, so `List<SavedPost>` is rejected as well as a bare `SavedPost`. Enums and records from a domain package are fine: the rule asks the JPA annotations, not the package name, because an enum constant has no associations to drag along.

Three reasons, in order of how much they cost:

1. **An entity carries whatever its associations reach.** A cached list of bookmarks used to answer with `SavedPost`, whose `@ManyToOne` user is eager, so the store held a full `User`: the password hash, and through `pushNotificationDeviceConfigurations` the device token and secret key of every bookmarking user. `@JsonIgnore` does not help, because map values are encoded by Java serialization and Jackson never runs.
2. **The stored shape becomes a wire format.** Because the reachable graph is what gets recorded, refactoring an unrelated entity changes it, and `DistributedDataSurfaceTest` then reports a schema change that has to be reviewed and possibly migrated. Projecting the query cut the recorded surface from 206 types to 80.
3. **A hit is not necessarily faster than the query.** Deserializing an object graph over a network hop can cost more than the projected query it replaces, which is the opposite of the reason for adding a cache.

Where a caller has to write the row back, read the entity separately and leave the cache to the readers. `UserCourseNotificationSettingPresetRepository` is the pattern: the `@Cacheable` method answers with the preset value, and an uncached sibling answers with the entity for the one code path that saves it. The `@CacheEvict` annotations stay on the write methods, keyed off the entity, so eviction is unaffected by the projection.
...
public class TitleCacheEvictionService implements PostUpdateEventListener, PostDeleteEventListener {
    ...
    public TitleCacheEvictionService(EntityManagerFactory entityManagerFactory, CacheManager cacheManager,
            PerNodeCacheEvictionService perNodeCacheEvictionService) {
...
4. **It evicts on every node, not just its own.** The title caches are per node, so a local eviction would leave the other nodes answering with the previous title. Passing the id through `PerNodeCacheEvictionService` broadcasts it. Note that the key keeps its own type: the entry is keyed by id, so evicting a stringified id would silently match nothing.
...
4. **Answers with a record of scalars, never an entity.** See [What a cached value may be](#what-a-cached-value-may-be). Enforced by `ArchitectureTest.testNoEntityAsCacheValue`.
...
* Spring `@Cacheable` paths are coherent across nodes regardless of L2 state, so the notification and file caches behave identically on both sides.

<Callout variant={CalloutVariant.warning}>
  <strong>The title caches are the exception while the change that made them per-node rolls out.</strong> A node on the old build reads a title from the distributed cache; a node on the new build reads it from its own Caffeine cache and announces the eviction on a topic the old build does not listen to. A rename during that window can therefore leave the other side answering with the previous title: an old node until it is replaced, a new node until the five minute expiry. Only the titles are affected, and the exposure ends with the deployment. An instance for which a stale title during a deploy is not acceptable should roll all nodes at once. Clearing the distributed title caches afterwards is not a remedy: once every node runs the new build nothing reads that map any more, and clearing it cannot reach the entry a new node already holds in its own Caffeine cache. What ends that entry is the five minute expiry, or a restart of the node holding it.
</Callout>

<Callout variant={CalloutVariant.warning}>
  <strong>The version prefix does not protect a cache that names its own key.</strong> <code>PrefixedKeyGenerator</code> puts the commit id in front of the key so entries written by different builds cannot collide, but Spring uses a key generator only when a <code>@Cacheable</code> gives no <code>key</code>. Every cache listed above except the PlantUML ones specifies <code>key = "..."</code>, so its entries are <em>not</em> scoped to a build, and a release can read what the previous release wrote. That is why a value type reachable from a cache needs a declared <code>serialVersionUID</code>: a computed one covers the public methods as well as the fields, so adding a method to it makes the next release reject the entry with an <code>InvalidClassException</code>. Prefer a small record whose shape you control.
</Callout>
