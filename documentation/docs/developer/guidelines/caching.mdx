...
* **`#12574` — cross-node stale `selectedOptions` in multiple-choice quiz submissions.** A `@ManyToMany(fetch = EAGER)` collection cached with `NONSTRICT_READ_WRITE` returned partial subsets of options on different nodes during the async-invalidation window. Users saw their submitted answers "lose" selections on refresh.
* **`#12584` — `ObjectNotFoundException` from stale entity-cache hits** during quiz-submission merge. Hibernate's `internalLoad` resolved a reference from the L2 region that pointed at an entity already deleted on another node.
...
This boots the production-faithful stack (Postgres, JHipster Registry / Eureka, ActiveMQ, three Artemis nodes, nginx load balancer, containerised Playwright) and is the canonical reproduction surface for cache-coherence regressions. Bug `#12574` was originally surfaced by this kind of multi-node exercise; do not skip it for cache-related changes.
