...
**Docker is required** to run server tests. Tests run against PostgreSQL via Testcontainers by default to match the production database engine.

You can execute server tests with the following commands:

* Execute all tests (PostgreSQL, default): `./gradlew test -x webapp`
* Execute tests with a coverage report: `./gradlew test jacocoTestReport -x webapp`
* Run a single test: `./gradlew test --tests ExamIntegrationTest -x webapp`
* Execute all tests against H2 (in-memory, no Docker needed, for quick local iteration): `./gradlew test -x webapp -Dzonky.test.database.type=H2`
...
To run the server tests against MySQL in a Docker test container, use:

* Execute tests with MySQL container: `./gradlew testMysql -x webapp`
...
class TestClass {
    ...
    void testQueryCount() throws Exception {
        var courses = assertThatDb(() -> request.get("/api/course/courses/for-dashboard", HttpStatus.OK, CoursesForDashboardDTO.class)).hasBeenCalledAtMostTimes(8);
...
We enforce a maximum of **11 server starts** during test execution via the GitHub Action in `supporting_scripts/extract_number_of_server_starts.sh`.
A context is only worth its start-up cost if it shortens the critical path of a run by more than it adds, which is what splitting a bucket in two does and what an accidental context does not.
...
|-----------------|-------------------|---------|
| `AbstractSpringIntegrationIndependentTestBase` | N/A | Shared mocks and setup for independent test buckets |
| `AbstractSpringIntegrationIndependentTest` | `Artemis_independent` | Independent tests without CI/CD integration |
| `AbstractSpringIntegrationIndependentBatchTest` | `Artemis_independent_batch` | Second independent bucket for balanced parallel execution |
...
| `AbstractSpringIntegrationLocalCILocalVCTestBase` | N/A | Shared mocks and setup for the two local CI + LocalVC buckets |
| `AbstractSpringIntegrationLocalCILocalVCTest` | `Artemis_localci_localvc` | Local CI + LocalVC integration |
| `AbstractSpringIntegrationLocalCILocalVCBatchTest` | `Artemis_localci_localvc_batch` | Second local CI + LocalVC bucket for balanced parallel execution |
...
1. **Never add `@MockitoSpyBean` or `@MockitoBean` to concrete test classes.** Add them to the appropriate base class instead.
2. **Never add `@TestPropertySource` to concrete test classes.** Add properties to the appropriate base class.
3. **Never add `@ActiveProfiles` to concrete test classes.** The profiles are defined in base classes.
4. **Never use `@DirtiesContext`.** Clean up test state manually instead of dirtying the context.
5. **Never add `@ContextConfiguration`, `@SpringBootTest`, or `@Import` to concrete test classes.** These are defined in base classes.
6. **Never use `@Profile` or `@Conditional*` on test classes.** These are meant for Spring beans, not test classes.
7. If you need a new spy bean or property, add it to the appropriate base class and update the `resetSpyBeans()` method.
8. **Every `@MockitoBean` and `@MockitoSpyBean` must be reset after each test.** A mock lives on the shared Spring
   context, so stubbing it is global state: whatever one test sets stays set for every test that follows in that
   context. Tests that stub the same bean themselves hide this, so it surfaces as an order-dependent failure in a
   class that never touches the bean — it inherits a stub from a class that ran earlier. `LocalVCFetchAndPushIntegrationTest`
   left `doReturn(true).when(ldapTemplate).compare(...)` behind, which had LDAP accepting every password for the rest
   of the run.
9. An architecture test (`SpringContextConfigurationArchitectureTest`) enforces these rules automatically, including
   the reset above: a bean counts as reset when the class declaring it reads it in an `@AfterEach` method, or in one
   named `resetSpyBeans`.
