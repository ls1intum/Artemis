...
**Docker is required** to run server tests. CI runs all tests against PostgreSQL via Testcontainers to match the production database engine.

You can execute server tests with the following commands:

* Execute all tests against PostgreSQL (recommended, matches CI): `./gradlew testPostgres -x webapp`
* Execute tests with a coverage report: `./gradlew testPostgres jacocoTestReport -x webapp`
* Run a single test against PostgreSQL: `./gradlew test --tests ExamIntegrationTest -x webapp -Dzonky.test.database.type=POSTGRES`
* Execute all tests against H2 (in-memory, no Docker needed, for quick local iteration): `./gradlew test -x webapp`
