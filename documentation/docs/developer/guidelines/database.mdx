...
Every `@OneToOne`, `@OneToMany` and `@ManyToMany` must declare `fetch = FetchType.LAZY`. The ArchUnit rule `ArchitectureTest.testNoEagerFetching` fails the build for any that does not; the associations that are still eager are listed in `FIELDS_ALLOWED_TO_FETCH_EAGERLY`, which may only shrink.
