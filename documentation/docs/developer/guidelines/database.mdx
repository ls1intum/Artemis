...
<Callout variant={CalloutVariant.info}>
  <strong>Important:</strong> For REST API design and Data Transfer Objects (DTOs), see the <a href="/developer/guidelines/rest-api">REST API Guidelines</a>. Entities must never be used directly in REST API request or response bodies.
</Callout>

<Callout variant={CalloutVariant.warning}>
  Hibernate second-level cache (<code>@Cache</code>) is disabled cluster-wide in Artemis. Do not add <code>@Cache</code> annotations on entities or associations — an ArchUnit rule fails the build if you do. See <a href="/developer/guidelines/caching">Caching Guidelines</a> for the rationale and the canonical Spring <code>@Cacheable</code> eviction pattern.
</Callout>

## Structural Migration with Liquibase

To prevent accidental irreversible database modifications, we use [Liquibase](https://docs.liquibase.com/home.html) to prepare changes when developing that can be confirmed in the review process.
...
1. Get the current time in the format `YYYYMMDDHHmmss`.
2. Create a new file in `/changelog` named `<formatted-time>_changelog.xml` and include this file at the bottom of the `master.xml` as every other file.
3. Add your changelog in your newly created file. Take other changes and the Liquibase documentation as inspiration.
...
<Callout variant={CalloutVariant.warning}>
  Especially for manually written SQL statements (<code>&lt;sql&gt;…&lt;/sql&gt;</code> blocks in the changelog) you have to make sure they are compatible with both database types.
</Callout>
...
<Callout variant={CalloutVariant.info}>
  Only use those preconditions in exceptional cases where it definitely cannot be avoided. They increase the risk of migrations that are inconsistent between the two different database types.
</Callout>

```xml title="Pattern for database changes with preconditions depending on the database type"
<databaseChangelog>
    <!-- … -->
...
<Callout variant={CalloutVariant.warning}>
  This type of migration should only be used if the logic cannot be implemented in SQL as part of a Liquibase changelog. Java migrations are <strong>significantly slower</strong> since they need to load and convert data from the DBMS into Java objects, apply the changes, and then write the new data back to the database.
</Callout>
...
1. Get the current time in the format `YYYYMMDD_HHmmss`.
2. Create a new file in `/entries` named `MigrationEntry<formatted-time>.java` containing a class extending `MigrationEntry`.
3. Implement the required methods in your class and follow the JavaDoc.
4. Add the class to the `migrationEntryMap` attribute in the `MigrationRegistry.java` together with the next Integer key inside the constructor.
...
A configuration that must not outlive what it configures holds the **parent's** key, not the other way round. A foreign
key constrains the row that holds it: with the key on the parent, the database can require that parent's configuration
to exist, but nothing can require the configuration's parent to exist. Deleting the parent by any route other than the
application's own cascade then leaves the configuration behind, and the schema has no way to refuse it.
...
So the parent does not carry the configuration at all - there is no field, mapped or otherwise. Only the configuration
knows the relation:
...
Which relationship an entity *belongs to*, and whether the database guarantees it, is recorded per
entity in [Entity Ownership](/developer/guidelines/entity-ownership). Read it before adding an
entity: a nullable owning foreign key is how a row ends up unreachable but undeleted, which also
puts it outside every data privacy cleanup.
...
Default to a `Set` unless presentation order is semantically meaningful — see the [ordered collections section](#choosing-collection-types) below for when (and how) to use a `List`.
...
<Callout variant={CalloutVariant.info}>
  Hibernate second-level cache is disabled cluster-wide in Artemis (see <a href="/developer/guidelines/caching">Caching Guidelines</a>), so the historic <code>EntityNotFoundException</code>-during-cascade failure caused by stale L2 collections no longer applies. The explicit-reload pattern remains good practice because it makes the post-deletion collection state explicit at the call site and avoids confusion in the persistence context.
</Callout>
...
If you genuinely need a `List` whose database position is meaningful and you cannot derive the order from a domain field (e.g., a timestamp, a `displayOrder` int, a numbered `spotNr`), Hibernate offers `@OrderColumn`. **Treat it as a sharp tool**: it has caused multiple production incidents in this codebase (issues [#12574](https://github.com/ls1intum/Artemis/issues/12574), [#12584](https://github.com/ls1intum/Artemis/issues/12584)). Read the rules below before using it, and prefer `Set` or `@OrderBy` on a domain field whenever you can.

<Callout variant={CalloutVariant.danger}>
    <strong>Mandatory rules for <code>@OrderColumn</code>:</strong>
    <ul>
        <li><strong>Bidirectional only.</strong> The relationship must be <code>@OneToMany(mappedBy = "...")</code> with the child entity owning the FK via its own <code>@ManyToOne</code> + explicit <code>@JoinColumn(name = "...")</code>. The unidirectional shape — <code>@OneToMany + @JoinColumn + @OrderColumn</code> — causes Hibernate to <code>DELETE</code>+<code>INSERT</code> the entire child collection on every parent save, regenerating primary keys and breaking in-flight references (this is the exact #12584 ID-regeneration bug).</li>
        <li><strong>One owning side.</strong> If the child has a <code>@ManyToOne</code> back-reference, the parent <em>must not</em> additionally declare <code>@JoinColumn(name = "...")</code> on its <code>@OneToMany</code>; both sides claiming the same column corrupts the FK on save.</li>
        <li><strong>Always set the back-reference</strong> when adding a child. Use a parent helper (<code>parent.addChild(child)</code> that calls <code>child.setParent(this)</code>) — never <code>parent.getChildren().add(child)</code> from outside.</li>
        <li><strong>Defensive <code>@PrePersist</code> / <code>@PreUpdate</code> hook</strong> on the parent that re-asserts <code>child.setParent(this)</code> on every element. This catches every test factory and DTO mapper that bypasses the helper. See the four quiz-domain entities (<code>MultipleChoiceQuestion</code>, <code>DragAndDropQuestion</code>, <code>ShortAnswerQuestion</code>, <code>QuizExercise</code>) for the canonical implementation.</li>
        <li><strong>No L2 cache</strong> on the collection (<code>@Cache</code> is forbidden cluster-wide anyway, see <a href="caching">caching guidelines</a>).</li>
        <li><strong>Always load the full collection before mutating it.</strong> A partial fetch (<code>JOIN FETCH ... WHERE ...</code> that filters child rows) returns a list with <code>null</code> padding at the missing index slots.</li>
        <li><strong>Name the order column explicitly:</strong> <code>@OrderColumn(name = "answer_options_order")</code>. Don't rely on Hibernate's default-naming heuristic surviving future changes.</li>
        <li><strong>Direct field assignment in setters.</strong> A defensive <code>setX(list)</code> that calls <code>this.x.clear()</code> on the existing collection will throw <code>LazyInitializationException</code> when callers like <code>filterSensitiveInformation</code> swap out a managed <code>PersistentList</code> after the session has closed. Keep <code>setX</code> as a plain field assignment; rely on the <code>@PrePersist</code>/<code>@PreUpdate</code> hook above for back-reference fixup.</li>
    </ul>
</Callout>

<details>
<summary>Example: Bidirectional <code>@OrderColumn</code> (the canonical pattern)</summary>
...
<Callout variant={CalloutVariant.info}>
    Hibernate manages the order column for you, but the column must already exist in the database — it is not created automatically. Match <code>@OrderColumn(name = "...")</code> to the existing physical column.
</Callout>
...
- `@OrderBy` resolves at SQL `ORDER BY` time, so partial fetches just return the matching rows in order with no `null` padding.
- Concurrent writes do not race on a single index column — each writer updates its own row's `displayOrder`.
- The relationship can be a `Set` (typically `LinkedHashSet`), which avoids the [`MultipleBagFetchException`](https://stackoverflow.com/questions/4334970/hibernate-throws-multiplebagfetchexception-cannot-simultaneously-fetch-multipl) when multiple ordered collections are fetched together.
...
<details>
<summary>Example: Set + explicit displayOrder</summary>
...
- dedupes Cartesian products that appear when callers `JOIN FETCH` the collection alongside another collection;
- avoids `MultipleBagFetchException` if a sibling collection later drops `@OrderColumn`;
- matches the conceptual model and reads honestly in code.
...
**Fix it by overriding `hashCode()` on the child to a class constant**, mirroring the [`Feedback`](https://github.com/ls1intum/Artemis/blob/develop/src/main/java/de/tum/cit/aet/artemis/assessment/domain/Feedback.java) pattern. All instances collide into a single bucket; the id-based `equals` still discriminates them; the `Set` lookup degrades to a linear scan, which is free for the small collections this pattern targets.

<details>
<summary>Example: Set + constant <code>hashCode()</code></summary>
...
<Callout variant={CalloutVariant.info}>
    Apply the constant <code>hashCode()</code> override <em>only</em> on entities that are the child side of a <code>Set</code>-mapped <code>@OneToMany</code> and may be constructed transient by application code (importers, DTO mappers, factories). Entities used as keys in large <code>HashMap</code>s or caches should keep the id-based default, since the linear-scan degradation matters at scale.
</Callout>

## Queries That Behave Differently on MySQL and PostgreSQL

Artemis runs on both MySQL and PostgreSQL. A query can work on one and fail on the other, or, worse, return
different rows without any error. Integration tests run against PostgreSQL only, so a MySQL-only difference
never shows up locally, and a difference that changes the result rather than raising an error shows up in
neither.
...
Lower case both sides rather than relying on a collation. In JPQL, keep it inside the query so that no caller
can defeat it:
...
<Callout variant={CalloutVariant.warning}>
  Lower casing one side only is the worst of the three, because it hides itself: <code>LOWER(column) LIKE '%Max%'</code> still matches on MySQL, whose collation ignores case anyway, and matches nothing on PostgreSQL. Whenever the column is wrapped, the parameter has to be wrapped with it.
</Callout>
...
A pattern with a leading wildcard cannot use an index on either database, so a full-text search is the answer
where that becomes the bottleneck.
...
A page window only means something if the order is total. Rows an order cannot tell apart are free to change
places between two queries, so one of them can appear on two pages and another on none. Append the id to
whatever order the caller asked for rather than replacing it, so that the requested order still decides the
page.

### Name Every Grouped Column

MySQL accepts a selected column that is neither aggregated nor named in the `GROUP BY`; PostgreSQL rejects it.
Name every non-aggregated column of the select list in the `GROUP BY`.
...
`JpaSystemException: null index column for collection` (or `IllegalArgumentException: Illegal null value for list index`) is almost always a symptom of one of the rules in the [ordered collections section](#ordered-collection-with-duplicates-list) being violated. Before adding workarounds, check:

1. **Is the relationship unidirectional?** If the parent has both `@OneToMany ... @JoinColumn(name = "...")` *and* `@OrderColumn`, convert it to bidirectional (`mappedBy`) and let the child own the FK via its own `@ManyToOne` + `@JoinColumn`. This is the root cause of the #12584 ID-regeneration class of bugs and resolves the null-index error in nearly every case.
2. **Is the FK column claimed twice?** If the child also has a `@ManyToOne` to the parent without an explicit `@JoinColumn`, JPA defaults to the same column the parent's `@JoinColumn` claims. Pick one owner — the child — and remove the parent's `@JoinColumn`.
3. **Is the back-reference set on every child before save?** Add a `@PrePersist`/`@PreUpdate` hook on the parent that asserts `child.setParent(this)` on every element. See the four quiz-domain entities for the pattern.
4. **Is the collection partially fetched?** A `JOIN FETCH ... WHERE ...` that filters child rows returns a list with `null` slots. Either fetch the full collection or stop using `@OrderColumn` and migrate to `Set` + `@OrderBy` on a domain field.
5. **Is some code path saving children directly?** A `childRepository.save(...)` or `@Modifying @Query` that inserts/deletes child rows without going through the parent's collection desyncs the order column. Route through `parent.addChild(child)` + `parentRepository.save(parent)`.
...
- The collection was partially fetched with a `WHERE` clause that filtered some children out (Hibernate keeps the indices stable by padding with `null`); or
- A previous save under unidirectional `@OrderColumn` raced and left an index gap.

Either fetch the full collection before mutating it, or — preferably — replace `@OrderColumn` with `Set` + `@OrderBy` on a domain field (see [the alternative pattern](#preferred-alternative-set--explicit-displayorder--orderby) above). Defensive Java-side filtering (e.g., `removeNullResults`) is a smell, not a fix.
