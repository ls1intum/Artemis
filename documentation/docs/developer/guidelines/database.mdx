...
Liquibase can generate the SQL required to roll back a set of changesets in **offline mode**, i.e., without connecting to the target database. This is the recommended workflow when preparing a rollback script for the MySQL production database: generate the SQL locally, review it, and apply it manually.

<Callout variant={CalloutVariant.warning}>
  The <code>rollback-count-sql</code> command only <em>generates</em> SQL — it does not execute it. Always review the produced <code>rollback.sql</code> carefully and coordinate with the project lead before running it against a production database.
</Callout>

1. Download the MySQL Connector/J JAR from the [MySQL downloads page](https://dev.mysql.com/downloads/connector/j/).
2. Install Liquibase, e.g., via [Homebrew](https://formulae.brew.sh/formula/liquibase):
   ```bash
   brew install liquibase
   ```
3. Export the `DATABASECHANGELOG` table from the target database as CSV.
4. In the exported CSV, replace every occurrence of `config/liquibase` with `src/main/resources/config/liquibase` so the changelog paths resolve against the checked-out repository. Save the file as `databasechangelog.csv` in your current working directory.
5. Run Liquibase in offline mode from the repository root to generate the rollback SQL:

    ```bash
    liquibase \
        --url="offline:mysql?changelog-file=./databasechangelog.csv" \
        --driver=com.mysql.cj.jdbc.Driver \
        --classpath="[PATH]/mysql-connector-j-[VERSION].jar" \
        --changelog-file=src/main/resources/config/liquibase/changelog/[CHANGELOG].xml \
        rollback-count-sql [NUMBER_OF_MIGRATIONS_TO_ROLL_BACK] \
        > rollback.sql
    ```

    Replace `[PATH]` with the directory holding the connector JAR, `[VERSION]` with the version of the downloaded connector JAR (e.g., `9.4.0`), `[CHANGELOG]` with the changelog file to roll back from (usually `master.xml`), and `[NUMBER_OF_MIGRATIONS_TO_ROLL_BACK]` with the number of most recently applied changesets to revert. The generated `rollback.sql` is what you review and — after approval — apply to the target database.
