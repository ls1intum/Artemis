...
3. **Is the back-reference set on every child before save?** Add a `@PrePersist`/`@PreUpdate` hook on the parent that asserts `child.setParent(this)` on every initialized element. Guard the hook with `Hibernate.isInitialized(...)` so an unrelated flush does not force-load lazy child collections. See the four quiz-domain entities for the pattern.
