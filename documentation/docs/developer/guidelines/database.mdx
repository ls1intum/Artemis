...
5. Run Liquibase in offline mode from the repository root to generate the rollback SQL:
    ...
    liquibase \
        ...
        --classpath="[PATH]/mysql-connector-j-9.4.0.jar" \
    ...
    Replace `[PATH]` with the directory holding the connector JAR, `[CHANGELOG]` with the changelog file to roll back from (usually `master.xml`), and `[NUMBER_OF_MIGRATIONS_TO_ROLL_BACK]` with the number of most recently applied changesets to revert. The generated `rollback.sql` is what you review and — after approval — apply to the target database.
