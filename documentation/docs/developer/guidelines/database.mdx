...
<Callout variant={CalloutVariant.warning}>
  Lower casing one side only is worse than lower casing neither: <code>LOWER(column) LIKE '%Max%'</code> can never match. If the column is wrapped, the parameter has to be wrapped too.
</Callout>

A comparison wrapped in `LOWER` cannot use a plain index on the column. Where a search runs against a large
table, add a functional index for it, for example `CREATE INDEX user_login_lower ON jhi_user (LOWER(login))`.
