...
| `ExerciseVariantGroup` | Course `course.exerciseVariantGroups` join column | Held by the parent | **Yes** | Required in the database; the course owns the join column, so the group has no field for it. |
...
neither. Nineteen such rows exist, all from 2019 and 2020, before conversations existed.
...
## What a production database contains

Counted on a production dump from September 2026:

| Orphan | Rows | Why |
| --- | --- | --- |
| `Submission` with no participation | 84,066 | 82,464 quiz, 1,221 modeling, 381 text. Historical rather than ongoing: of the quiz rows, 48,837 were never submitted at all and 33,624 date from 2019, leaving three that are newer. Across all 84,066, exactly one carries a result |
...
| `Slide` with no attachment video unit | 12,380 | `SlideSplitterService` detaches superseded slides deliberately, and `ON DELETE SET NULL` detaches the rest when their unit is deleted |
| `LearnerProfile` no user points at | 469 | Personal data, unreachable, outside every deletion path |
| `Feedback` with no result | 34 | |
| `Post` with neither a conversation nor a plagiarism case | 19 | User-authored content, all from 2019 and 2020 |
| `QuizQuestion` with no exercise | 13 | 2,318 submitted answers still reference them |
| `SubmissionPolicy`, `TeamAssignmentConfig`, `PlagiarismDetectionConfig` no exercise points at | 4, 3, 3 | |

Every column that is now required came back with no violations before it was constrained. The counts
are worth re-running before acting on any of the rest, since a shape that is empty today is only one
unguarded write path away from not being.

The `Slide` and `LearnerProfile` rows illustrate the two ends of the problem. Slides are detached by
the application on purpose, so the schema cannot be fixed before that behaviour is decided. Learner
profiles hold per-course study preferences for a named account and are reachable only from
...
violations repaired first, and a repair is only writable where the parent can be derived.

**Add a check constraint over alternative parents.** The right tool where an entity genuinely has two
possible owners. It costs two null tests on INSERT and UPDATE, nothing on read, and it is the only
way to express "exactly one of these".
...
**Let the child own its position.** `AuxiliaryRepository` cannot be required while the exercise's
collection owns the `@OrderColumn`. Replacing it with a plain position column the application sets
would make the repository writable on its own, and removal an explicit delete.

**Remove a redundant second foreign key.** `PlagiarismSubmission` is reachable from its comparison
through `submission_a_id` and `submission_b_id`, and points back through `plagiarism_comparison_id`.
The two make a cycle, so deleting a comparison forces the database to null one of them, which is why
the submission's parent cannot be required. Giving the submission a side column and dropping the
comparison's two columns would leave one direction and one required parent, but it touches the
plagiarism result payload and belongs in its own change.
