...
| `CourseAthenaConfig` | Course `course_id` | Required | No |  |
| `CourseConfiguration` | Course `course_id` | Required | No |  |
...
| `TeamAssignmentConfig` | Exercise `exercise_id` | Required | No |  |
...
| `OnlineCourseConfiguration` | Course `course_id` | Required | No |  |
...
| `PlagiarismDetectionConfig` | Exercise `exercise_id` | Required | No |  |
...
| `TutorialGroupsConfiguration` | Course `course_id` | Required | No |  |
...
Of the 147 tables behind the 186 entities, 85 have a required parent, 24 are roots by design, 18 are
join entities whose parents are all required and five carry a check constraint over alternative
parents. That leaves **15 tables where the schema permits an orphan**, in two shapes.

### An owning foreign key that is nullable

`Participation`, `Submission`, `Feedback`, `QuizQuestion`, `PlagiarismSubmissionElement` and
`KnowledgeArea`.

`KnowledgeArea` is held open by the model rather than by old data: it is a tree, so a null parent
means a root node. The other five hold rows today that carry no evidence of which parent they
belonged to, so a constraint needs a decision about those rows before a migration.

Two of them come with a decision beyond the rows. Requiring `Feedback.result` would mean feedback
can no longer be orphaned at all, which retires the cleanup that removes orphaned feedback rather
than only constraining a column. `QuizQuestion.exercise` cannot be settled on its own either: the
questions without an exercise are reachable through submitted answers whose submissions have
themselves lost their participation, so they go with `Submission.participation`.
