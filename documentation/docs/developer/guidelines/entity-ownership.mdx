...
That retires the deletion but not the count. `deleteOrphanFeedback` can only ever delete nothing once
the column is `NOT NULL`, so it goes with the constraint. The admin cleanup page keeps
`countOrphanFeedback`, and the long feedback text count that asks the same question, so the category
reads as clean rather than vanishing from the inventory; on any database the migration has run, both
report zero.
