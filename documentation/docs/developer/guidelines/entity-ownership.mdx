...
decision beyond the rows: requiring `Feedback.result` means feedback can no longer be orphaned at
all, which retires the cleanup that removed orphaned feedback rather than only constraining a
column, and the questions without an exercise were reachable through submitted answers whose
submissions had themselves lost their participation, so settling them meant settling those rows too.
