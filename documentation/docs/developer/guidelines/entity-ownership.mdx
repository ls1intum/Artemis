...
| `ScienceCourseConsent` | User `user_id` and Course `course_id` | Required (both) | No | One decision per account and course; cascades on both. |
| `ScienceEnabledCourse` | Course `course_id` | Required | No | Whether a course collects science data; cascades on the course. |
| `ScienceEvent` | — | Root | No | Analytics event; `resource_id` is an untyped pointer with no foreign key, and `course_id` is optional because events predating course-level collection have no course to name. |
| `ScienceResearchExportAudit` | — | Root | No | Immutable record of a research export, which outlives every course and event it describes. |
