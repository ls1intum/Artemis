...
Check out the top of `_default-variables.scss` to see the available default colors. While you should not
...
Either add one of the Bootstrap alert classes to your box, such as `alert alert-danger`, or use our globally
defined colors:

```scss
var(--artemis-alert-XXX-color); // The text color of the alert
var(--artemis-alert-XXX-background); // The less intensive background color of the alert
...
event for each theme change from then on until you unsubscribe.
