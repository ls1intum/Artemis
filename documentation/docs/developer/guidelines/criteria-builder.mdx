...
1. **Metamodel:** The metamodel is used to refer to the columns of a table in an object-oriented way.
For this, each entity needs to have a corresponding metamodel class. (Artemis already fulfills this requirement)
...
1. **Dependent Sub-Query:**
    In an SQL database query, a correlated sub-query is a sub-query (a query nested inside another query) that uses values from the outer query.
    But with a dependent sub-query you might run into performance problems because a dependent sub-query typically needs to be run once for each row in the outer query, e.g. if your outer query has 1000 rows, the sub-query will be run 1000 times.

2. **Independent Sub-Query:**
    An independent sub-query is a sub-query that can be run on its own, without the main (sub-)query.
    Therefore, an independent sub-query typically only needs to be evaluated once.
...
- Specification that matches all selected courses:
    ...
    public static Specification<User> getAllUsersMatchingCourses(Set<Long> courseIds) {
        return (root, query, criteriaBuilder) -> {
            Join<User, UserCourseRole> courseRole = root.join(User_.COURSE_ROLES, JoinType.LEFT);

            // Only count memberships in one of the selected courses
            courseRole.on(criteriaBuilder.in(courseRole.get(UserCourseRole_.COURSE).get(Course_.ID)).value(courseIds));

            // A user matches only if they are a member of every selected course
            query.groupBy(root.get(User_.ID))
                    .having(criteriaBuilder.equal(criteriaBuilder.countDistinct(courseRole.get(UserCourseRole_.COURSE)), courseIds.size()));
    ...
    ```sql
    SELECT DISTINCT user FROM jhi_user user
    LEFT OUTER JOIN user_course_role course_role ON user.id = course_role.user_id
    AND course_role.course_id IN ( ? )
    WHERE (user.login LIKE ?
        OR user.email LIKE ?
        OR user.first_name LIKE ?
        OR user.last_name LIKE ?)
    GROUP BY user.id
    HAVING Count(DISTINCT course_role.course_id) = ?
