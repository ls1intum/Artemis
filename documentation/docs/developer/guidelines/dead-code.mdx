...
settled by provenance, like every other one here. `@AutoConfigureAfter` and the `@ConditionalOnBean`
family on a non-auto-configuration class are meaningless or order-dependent, and are a reliable sign of
a generated leftover.
