...
So on Redis, `RedisDiscoveryEnvironmentPostProcessor` sets `eureka.client.enabled=false` before the context is built,
and Spring Cloud's own conditions then keep every Eureka auto-configuration out of it. A Redis node creates no Eureka
client, registers nothing, runs no heartbeat thread and contributes no registry health indicator — verified on a
three-node cluster: zero Eureka threads, zero live `com.netflix.*` objects, zero references to port 8761.
