...
**How its calls count.** A `GET` is a view and everything else an action, unless the endpoint is `@Internal`, which makes
it a system call. Add `@UsageInteraction` where that is wrong:
...
* `SYSTEM` for an endpoint another system calls, such as a build agent or a calendar app.
