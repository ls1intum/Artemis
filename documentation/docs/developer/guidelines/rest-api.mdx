...
* `GET` endpoints must not declare a `@RequestBody` — a GET request body has no defined semantics (RFC 9110) and is dropped by many proxies and clients; pass data via path or query parameters instead. Enforced by `AbstractModuleResourceArchitectureTest#getEndpointsShouldNotDeclareRequestBody`.
* Endpoints must return `ResponseEntity`, never `ModelAndView`. `ModelAndView` is being phased out (the two remaining server-side `forward:` endpoints are baselined); any new usage is rejected. Enforced by `AbstractModuleResourceArchitectureTest#restEndpointsShouldNotReturnModelAndView`.
