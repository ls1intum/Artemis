...
Existing rows may still hold a path-shaped value, and a node running a previous release keeps writing one during a rolling deployment. Reduce every stored value to its last path segment on read, so a legacy value, a canonical value and a bare filename all resolve alike. The reduction is idempotent, which means a cleanup migration is data hygiene rather than a precondition, and the two can ship in either order.
