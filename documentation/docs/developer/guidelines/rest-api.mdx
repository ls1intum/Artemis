...
* A value that is not a column of its own. `DragItem.pictureFilePath` lives inside the `quiz_question.content` JSON document, and rewriting it would mean engine-specific JSON surgery on every drag-and-drop question. It is deliberately left as it is and read tolerantly instead, until the next edit of the question stores the reduced value through the ordinary write path.
