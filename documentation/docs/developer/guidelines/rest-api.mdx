...
<Callout variant={CalloutVariant.info}>
  Existing deviations are tracked in <code>AbstractModuleResourceArchitectureTest#PATH_VARIABLE_COLLECTION_BASELINE</code>. This is a <strong>shrinking baseline</strong>: new violations fail the build, and each entry must be removed as the path is migrated to the canonical shape. Do not add entries without a follow-up plan.
</Callout>
