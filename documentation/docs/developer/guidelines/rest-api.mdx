...
**How it works:**
* Requests without a tool claim (e.g., browser requests) can access all endpoints meeting role-based authorization rules
* Requests with a tool claim (e.g., `{"tool": "SCORPIO"}`) can only access endpoints annotated with `@AllowedTools(ToolTokenType.__)`
