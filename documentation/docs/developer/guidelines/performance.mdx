...
An architecture test enforces the annotation, with no exception list: `@Transactional` on a method must be
declared in a repository interface, and no class outside one may carry it. Programmatic transaction
management is not yet covered by a test — a handful of services still construct a `TransactionTemplate`
from an injected `PlatformTransactionManager`, so the rule cannot be turned on until those are gone. Treat
that as debt to pay down rather than as permission: it is the same boundary with a different spelling, and
