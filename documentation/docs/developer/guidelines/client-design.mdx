...
For color design principles and standards, refer to the **[AET UI/UX Guidelines - Colors](https://ls1intum.github.io/ui-ux-guidelines/docs/colors)**. The guidelines below cover Artemis-specific theming implementation.
</Callout>
...
| Color | Use case | Usage |
|-------|----------|-------|
| `bs-body-color` | The default font color; will be black in light mode and white in dark mode. | CSS: `var(--bs-body-color)` |
| `bs-body-bg` | The body background color; will be something bright in light mode and something darker in dark mode.<br/><br/>It can be used for smaller boxes in main content, as it's usually pretty distinguishable from the background color used in Artemis' primary content area. | CSS: `var(--bs-body-bg)` |
| `artemis-dark` | A dark color that is typical for some of Artemis' UI elements, for example the navbar background. | CSS: `var(--artemis-dark)` |
| `primary` | A blue-ish default color to indicate a primary action. Also used as link color. Use this to indicate the primary next step for the user (or one of them). | CSS: `var(--primary)`<br/><br/>For text: `<span class="text-primary">`<br/><br/>For buttons: `<button class="btn btn-primary">` |
| `secondary` | A gray color. Use this for secondary action buttons and hint texts. | CSS: `var(--secondary)`<br/><br/>For text: `<span class="text-secondary">`<br/><br/>For buttons: `<button class="btn btn-secondary">` |
| `success` | A green color indicating a successful operation, state, or safe action. | CSS: `var(--success)`<br/><br/>For text: `<span class="text-success">`<br/><br/>For buttons: `<button class="btn btn-success">` |
| `danger` | A red color indicating a failed operation, error-state, or dangerous action. | CSS: `var(--danger)`<br/><br/>For text: `<span class="text-danger">`<br/><br/>For buttons: `<button class="btn btn-danger">` |
| `warning` | An orange color indicating a partly failed operation, a warning, or an unsafe, yet not ultra-dangerous action. | CSS: `var(--warning)`<br/><br/>For text: `<span class="text-warning">`<br/><br/>For buttons: `<button class="btn btn-warning">` |
| `info` | An teal-ish color indicating an informational element. | CSS: `var(--info)`<br/><br/>For text: `<span class="text-info">`<br/><br/>For buttons: `<button class="btn btn-info">` |
...
3. Select a value for each theme.
    ...
    **Re-use preset colors where possible!** For example, choose `gray-700` in light mode and `gray-400` in dark mode.
    ...
    If you need a **slightly modified preset color**, use [SCSS functions](https://sass-lang.com/documentation/modules/color) to derive it.
