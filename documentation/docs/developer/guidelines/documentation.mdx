...
used here: MDX reads the braces as a JSX expression and the build fails with `Could not parse expression with
acorn`. Wrapping it in a comment works, and is what this repository already does:
