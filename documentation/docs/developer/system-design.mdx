...
See [Integrated Code Lifecycle Setup Description](setup#set-up-artemis) on how to set up a single-node environment for developing purposes.
...
It provides endpoints so users can interact with these data structures, such as viewing and cancelling build jobs. It also receives the build job results, grades them, and notifies the user.
...
The service provides the functionality for an Artemis user to interact with build jobs and build agents. Build jobs can be viewed and cancelled.
...
As previously mentioned, most of the listeners are added in `LocalCIEventListenerService`.
...
Upon receiving a cancellation request through the `@DeleteMapping("build-jobs/{buildJobId}/cancel")` endpoint in either the `AdminBuildJobQueueResource` or `BuildJobQueueResource`,
...
This step involves cancelling the future and recording the job ID in `Set<String> cancelledBuildJobs`.
...
Modifications to `processingJobs` trigger WebSocket updates, consequently removing the cancelled job from the client's build overview display.
