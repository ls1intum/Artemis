...
```bash
docker compose --env-file .env -f docker/mailpit.yml down
```
...
```bash
docker compose --env-file .env -f docker/artemis-dev-mysql.yml -f docker/mailpit.yml up
```
...
1. Start Mailpit: `docker compose --env-file .env -f docker/mailpit.yml up -d`
...
- Verify Mailpit is running: `docker compose --env-file .env -f docker/mailpit.yml ps`
...
```bash
docker compose --env-file .env -f docker/mailpit.yml logs mailpit
```
