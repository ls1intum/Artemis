...
When using `bun start` (Angular dev server on port 9000), the SAML2 paths must be proxied to the Spring Boot backend. The proxy configuration in `proxy.conf.mjs` must include `/saml2/` and `/login/saml2/` — these are already configured in the project.
...
**Fix (Angular dev server):** Ensure the proxy config in `proxy.conf.mjs` includes `/saml2/` and `/login/saml2/`. Restart `bun start` after changes.
