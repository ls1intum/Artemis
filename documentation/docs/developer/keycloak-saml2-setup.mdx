---
id: keycloak-saml2-setup
title: Keycloak SAML2 Setup
sidebar_label: Keycloak SAML2 Setup
---
...
Replace `<ABSOLUTE_PATH>` with the actual absolute path from the `realpath` call under
[Generate SAML2 Signing Credentials](#generate-saml2-signing-credentials).
...
When using `pnpm start` (Angular dev server on port 9000), the SAML2 paths must be proxied to the Spring Boot server. The proxy configuration in `proxy.conf.mjs` must include `/saml2/` and `/login/saml2/` — these are already configured in the project.
...
```bash
docker compose --env-file .env -f docker/artemis-dev-mysql.yml -f docker/saml-test.yml up
```
...
<Callout variant={CalloutVariant.warning}>
    The containerized setup still requires signing credentials. Generate them as described in [Generate SAML2 Signing Credentials](#generate-saml2-signing-credentials),
...
## Test the SAML2 Login

1. Start Keycloak: `docker compose --env-file .env -f docker/saml-test.yml up -d`
2. Start Artemis with `artemis.user-management.saml2.enabled=true` and the configuration above
...
<Callout variant={CalloutVariant.info}>
    ...
    ```bash
    docker compose --env-file .env -f docker/saml-test.yml down
    docker compose --env-file .env -f docker/saml-test.yml up -d
    ```
</Callout>
...
**Fix:** Generate the signing credentials as described in [Generate SAML2 Signing Credentials](#generate-saml2-signing-credentials) and add the `signing.credentials` section to your `application-local.yml`.
...
```bash
docker compose --env-file .env -f docker/saml-test.yml logs keycloak
```
...
```bash
docker compose --env-file .env -f docker/saml-test.yml down -v
docker compose --env-file .env -f docker/saml-test.yml up -d
```
