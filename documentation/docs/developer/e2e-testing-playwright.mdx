...
sidebar_label: E2E Testing with Playwright
---
...
<Callout variant={CalloutVariant.info}>
    ...
    Testing Iris (the AI tutor) end to end against a **real Pyris** microservice — architecture, the mock
    LLM, and the nightly cross-repository monitor — is covered separately in
    [E2E Testing Iris (Real Pyris)](./e2e-testing-iris.mdx).
...
**Prerequisites:** Docker, Java 25+, Node.js 24+, pnpm (activate via `corepack enable`).

**Architecture:**

- **Docker:** PostgreSQL only (port 5432)
- **Host:** Spring Boot server via `./gradlew bootRun -x webapp` (port 8080)
- **Host:** Angular client via `pnpm start` (port 9000, proxies API calls to 8080)
...
Then install the dependencies (activate pnpm once via `corepack enable` if you haven't already):

```bash
pnpm install
```

### Customize Playwright configuration
...
### Setup Playwright package and its browser binaries
...
```bash
pnpm run playwright:setup-local
...
```bash
pnpm exec playwright test <path_to_test_file>
```
...
```bash
pnpm exec playwright test e2e/course/CourseManagement.spec.ts -g "Course creation"
```
...
### Use uniquely identifiable locators
...
<Callout variant={CalloutVariant.danger}>
    ...
    became a TUM AET UI button, the class disappeared and both tests waited about eleven minutes each for an element that
...
### Consider actionability of elements
...
### AI-assisted test authoring with a Playwright MCP server
