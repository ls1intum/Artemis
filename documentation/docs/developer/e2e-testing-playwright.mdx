...
**Prerequisites:** Docker, Java 25+, Node.js 24+, [Bun](https://bun.com) (install via `curl -fsSL https://bun.sh/install | bash` or `brew install oven-sh/bun/bun`).

**Architecture:**

- **Docker:** PostgreSQL only (port 5432)
- **Host:** Spring Boot server via `./gradlew bootRun -x webapp` (port 8080)
- **Host:** Angular client via `bun start` (port 9000, proxies API calls to 8080)
...
Then install the dependencies (install Bun first if you haven't already — see the [developer setup guide](/developer/setup#required-dependencies)):
...
```bash
bun run playwright:setup-local
bun run playwright:init
```
...
```bash
bunx playwright test <path_to_test_file>
```
...
```bash
bunx playwright test e2e/course/CourseManagement.spec.ts -g "Course creation"
```
