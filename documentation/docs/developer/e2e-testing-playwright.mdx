...
because they have no access to the running page state. With the MCP server active, the assistant can inspect the
current DOM, retrieve the `data-testid` attributes attached to elements, and generate locators that reference
those IDs directly.

To use it, start Artemis locally (via the fast runner or any other method) and enable the Playwright MCP
integration in your editor's AI assistant settings. The assistant can then inspect page structure and generate
