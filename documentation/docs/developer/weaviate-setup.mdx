...
<Callout variant={CalloutVariant.warning}>
  ...
  <code>Collection 'Artemis_Exercises' exists but its vectorizer configuration differs from the application config: open-ai-base-url: existing='...', configured='...'. Delete the collection and restart Artemis to apply the new settings: curl -X DELETE "http://localhost:8001/v1/schema/Artemis_Exercises"</code></p>
...
```yaml
        open-ai-base-url: https://gpu.aet.cit.tum.de/ollama/v1
