...
<Callout variant={CalloutVariant.info}>
  ...
  Use `http://host.docker.internal:11434` as the `api-base-url` in your Artemis configuration (no `/v1` suffix — Weaviate appends it automatically).
</Callout>
...
<Callout variant={CalloutVariant.warning}>
  <p><strong>Collections must be recreated when changing vectorizer settings.</strong> Weaviate does not update the vectorizer configuration of existing collections.
  When switching between vectorizer modules (e.g. `none` → `text2vec-openai`) or changing API settings (`api-base-url`, `api-embedding-model`, `api-key`),
  you must delete the existing collections first and restart Artemis so they are recreated with the new configuration.</p>

  <p>On startup, if a collection already exists and its vectorizer configuration differs from the application config, Artemis will log a warning like:<br/>
  <code>Collection 'Artemis_Exercises' exists but its vectorizer configuration differs from the application config: api-base-url: existing='...', configured='...'. Delete the collection and restart Artemis to apply the new settings: curl -X DELETE "http://localhost:8001/v1/schema/Artemis_Exercises"</code></p>

  <p>To delete all Artemis collections:</p>

  ```bash
  curl -X DELETE "http://localhost:8001/v1/schema/Artemis_Exercises"
  ```

  <p>Or reset the entire Weaviate volume (see <a href="#resetting-data">Resetting Data</a>).</p>
</Callout>
...
Add the `auth-admin-user` and `auth-api-key` properties to your Artemis configuration. The values must match `AUTHENTICATION_APIKEY_USERS` and `AUTHENTICATION_APIKEY_ALLOWED_KEYS` on the Weaviate server:

```yaml
artemis:
    weaviate:
        enabled: true
        auth-admin-user: admin@example.com  # Must match AUTHENTICATION_APIKEY_USERS on the Weaviate server
        auth-api-key: admin-key             # Must match AUTHENTICATION_APIKEY_ALLOWED_KEYS on the Weaviate server
```
...
<Callout variant={CalloutVariant.warning}>
    <p>Anonymous access is enabled by default for development convenience. For production deployments, configure API key authentication — see <a href="#authentication">Authentication</a>.</p>
</Callout>
...
artemis:
    weaviate:
        ...
        # auth-admin-user: admin@example.com    # Optional: admin username for Weaviate API key auth (see Authentication section)
        # auth-api-key: admin-key               # Optional: API key for Weaviate server authentication (see Authentication section)
```
...
artemis:
    weaviate:
        ...
        api-embedding-model: qwen3-embedding:8b                    # Embedding model name served by Ollama / OpenAI-compatible API
        api-base-url: http://host.docker.internal:11434            # Base URL for the API (no /v1 suffix); use host.docker.internal for local Ollama
        api-key: dummy                                             # API key (use any non-empty value for Ollama)
```

For a remote GPU server (e.g. TUM GPU server), use:

```yaml
        api-base-url: https://gpu.aet.cit.tum.de/ollama/v1
