...
<Callout variant={CalloutVariant.warning}>
  <p>The <code>vectorizer-module</code> setting must match your Docker setup. Using <code>text2vec-transformers</code> requires the transformer service from <code>weaviate-embeddings.yml</code> to be running, otherwise collection creation will fail.</p>
</Callout>
