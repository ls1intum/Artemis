...
Currently, the OpenAPI specs are only generated for the `globalsearch`, `tutorialgroup`, `hyperion`, `calendar` and (partially) `quiz` modules. We only support API-driven development for endpoints that are DTO-only and do not return domain objects.
...
- Performs some post-processing to make the client compile
