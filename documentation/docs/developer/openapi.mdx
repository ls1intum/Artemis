...
Currently, the OpenAPI specs are only generated for the `globalsearch`, `tutorialgroup`, `hyperion` and `calendar` modules. We only support API-driven development for endpoints that are DTO-only and do not return domain objects.
