...
| Staged programming-exercise code generation | `HyperionCodeGenerationResource`, `HyperionCodeGenerationService`, `HyperionCodeGenerationJobService`, `HyperionCodeGenerationTaskService`, and `HyperionCodeGenerationExecutionService` |

The staged code-generation services coordinate Artemis exercise and repository context, jobs, tasks, execution, and generated artifacts. They should not be documented as a direct external service call. For the detailed consistency-check flow, including its language-aware context filtering and verification pass, see [Consistency Check](./hyperion/consistency-check).
...
`AtlasAgentResource` accepts course chat requests and delegates them through `AtlasAgentService` and `AtlasAgentDelegationService`. Artemis owns the course context, authorization, agent-session history, and tool contract. These beans require `artemis.atlas.atlasllm.enabled`, which is disabled by default and additionally requires the Artemis Atlas module.

The agents, the orchestrator, the tool surfaces, and the Responses adapter all sit behind that one flag, because none of them can do anything without a configured chat model. An installation that leaves it off keeps competencies, learning paths, and learner profiles, and loads none of those beans, the content-change scheduler, or the autonomous exercise event listener. `artemis.atlas.enabled` therefore describes the competency feature set alone, and the two AI submodules, `atlasml` and `atlasllm`, are opted into separately.
...
Autonomous rounds and flavor stripping use an isolated Responses `ChatClient` by default; interactive Atlas and other modules retain the shared client. The adapter preserves ordered Responses output items, including encrypted reasoning, while the Spring AI tool advisor executes callbacks. Each run owns one shared tool budget and a `completeOrchestration(verified, message)` terminal decision. A successful index refresh after the latest work is required for verified completion. Budget exhaustion or missing/unverified completion is terminal and preserves applied actions without automatically replaying the run. See [Atlas model configuration](../admin/artemis-intelligence#atlas-model-configuration) for defaults and the explicit transport fallback.
