...
<Callout variant={CalloutVariant.info}>
    `docker-compose.yml` mirrors the production setup, so it needs a signing key and an admin password of your own before it starts. See [Other Docker Compose Setups](#other-docker-compose-setups) below. If you would rather not configure anything, `docker compose -f docker/artemis-dev-postgres.yml up` starts as it is.
</Callout>
...
# Both services
docker compose -f docker/local-development.yml up

# Or selectively from the combined file
docker compose -f docker/local-development.yml up mailpit
docker compose -f docker/local-development.yml up keycloak
```
