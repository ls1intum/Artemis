...
Every core and build-agent node also requires `INFO_OPERATORNAME`, `INFO_OPERATORADMINNAME`, and
`INFO_UNIVERSITYNAME` in the environment override. Use the installation's actual operator, administrator, and
university or institution. The shipped dev configurations provide local development identities; the development profile does not send
telemetry. Production configurations require identity values in the application configuration or environment override.
