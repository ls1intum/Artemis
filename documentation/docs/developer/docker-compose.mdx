...
Example commands to run such extended setups:

```bash
# Email testing only
docker compose -f docker/artemis-dev-mysql.yml -f docker/mailpit.yml up

# SAML2 testing only (add 'saml2' to your Spring profiles)
docker compose -f docker/artemis-dev-mysql.yml -f docker/saml-test.yml up

# Both services
docker compose -f docker/artemis-dev-mysql.yml -f docker/local-development.yml up
```

#### Mailpit

Mailpit is a drop-in email testing tool. All emails sent by Artemis will appear in the web UI at `http://localhost:8025`.

For **non-containerized Artemis** (e.g., running via IntelliJ or `./gradlew bootRun`), add the following to your `application-local.yml`:
...
This overrides the default mail port (25) from `application-dev.yml` to match Mailpit's SMTP port.

#### Keycloak (SAML2 Testing)

Keycloak provides a full-featured SAML2 Identity Provider pre-configured with an `artemis` realm containing test users and SAML attribute mappers.

For **containerized Artemis** (Docker Compose with an Artemis container), the `application-saml2.yml` is automatically mounted. You only need to add `saml2` to your Spring profiles (e.g., in `SPRING_PROFILES_ACTIVE`).

For **non-containerized Artemis**, create or update `src/main/resources/config/application-local.yml` with:

```yaml
spring:
    profiles:
        active: artemis,scheduling,core,dev,local,saml2
    security:
        saml2:
            relyingparty:
                registration:
...
1. Start Keycloak: `docker compose -f docker/saml-test.yml up keycloak`
2. Start Artemis with the `saml2` Spring profile
3. Click "SAML2 Login" on the Artemis login page
4. Authenticate with test user `saml2user1` / `password` (or `saml2user2` / `password`)
5. Keycloak admin console: `http://localhost:9080` (admin/admin)
