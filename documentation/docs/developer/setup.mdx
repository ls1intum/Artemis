...
4. **pnpm (≥ 11.1.0)**
   The version is pinned in `package.json` via the `packageManager` field.
   Activate it once via Corepack (shipped with Node.js 24):

   ```bash
   corepack enable
   ```

   Subsequent `pnpm install` / `pnpm run …` invocations will use the pinned version automatically. We migrated from npm to pnpm to get deterministic, content-addressable installs (faster cold installs locally and in CI, byte-identical `node_modules` across machines, stricter override semantics).
...
You need Node.js on your local machine, plus pnpm activated via Corepack (`corepack enable`). The pnpm version is pinned in `package.json` via the `packageManager` field.
