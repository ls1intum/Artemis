...
4. **pnpm (≥ 11.1.0) — the package manager Artemis uses for the Angular client**

   Artemis uses [pnpm](https://pnpm.io) instead of npm. We do **not** use `npm` / `npx` anywhere; please don't run `npm install` in this repo — it will create a `package-lock.json` that conflicts with the committed `pnpm-lock.yaml`. The exact pnpm version is pinned in `package.json` via the `packageManager` field, so every developer and every CI job uses the same version.

   You don't need to install pnpm manually. Node.js 24 ships with [Corepack](https://nodejs.org/api/corepack.html), a small shim that downloads and runs the pinned package manager on demand. Activate it **once** per machine:
