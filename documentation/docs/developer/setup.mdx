...
4. **[pnpm](https://pnpm.io) (≥ 11.19.0) — the package manager Artemis uses for the Angular client**
   ...
   ```bash
   pnpm --version          # should print 11.19.0 (matches packageManager in package.json)
...
- **Artemis (Server, LocalVC & LocalCI, Athena):** The server will be started separated from the client with the profiles `dev,localci,localvc,artemis,scheduling,buildagent,core,local`. Enable Athena via `artemis.athena.enabled=true` in your `application-local.yml`. After enabling, AI feedback features must also be activated per course by an instructor or administrator via the course settings. See [AI Assisted Feedback](/instructor/course-management/course-configuration#ai-assisted-feedback).
...
<Callout variant={CalloutVariant.info}>
    In case you encounter any problems regarding JavaScript heap memory leaks when executing `pnpm start` or any other scripts from `package.json`, you can adjust the [memory limit parameter](https://nodejs.org/docs/latest-v24.x/api/cli.html#--max-old-space-sizesize-in-megabytes) (`node-options=--max-old-space-size=6144`) which is set by default in the project-wide `.npmrc` file. pnpm 11 honours `node-options` from `.npmrc` for Node settings; pnpm's own install/resolution configuration lives in `pnpm-workspace.yaml` (e.g. `autoInstallPeers`, `allowBuilds`, `patchedDependencies`). If you still face the issue, you can try to set a lower/higher value than 6144 MB. Recommended values are 3072 (3GB), 4096 (4GB), 5120 (5GB), 6144 (6GB), 7168 (7GB), and 8192 (8GB). You can override the project-wide `.npmrc` file by [using a per-user `~/.npmrc`](https://pnpm.io/npmrc). Make sure to **not commit changes** in the project-wide `.npmrc` unless the GitHub build also needs these settings.
</Callout>
