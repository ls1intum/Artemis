...
- **Artemis (Server, LocalVC & LocalCI, Theia):** The server will be started separated from the client with the profiles `dev,localci,localvc,artemis,scheduling,buildagent,core,ldap,local` and the VM option `-Dartemis.theia.enabled=true`.
