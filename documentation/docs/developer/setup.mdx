...
4. **[Bun](https://bun.com) (≥ 1.3.0) — the package manager and script runner Artemis uses for the Angular client**

   The official Bun website is [bun.com](https://bun.com/); the CLI reference for `bun install` is at [bun.com/docs/pm/cli/install](https://bun.com/docs/pm/cli/install) and the installation guide at [bun.com/docs/installation](https://bun.com/docs/installation).
   Artemis uses Bun instead of npm or pnpm. We do **not** use `npm` / `npx` / `pnpm` / `corepack` anywhere; please don't run `npm install` or `pnpm install` in this repo — they will create a `package-lock.json` or `pnpm-lock.yaml` that conflicts with the committed `bun.lock`. The exact Bun version is pinned in `package.json` via the `packageManager` field, so every developer and every CI job uses the same version.

   **Install Bun** on macOS / Linux / WSL2:
   ...
   To pin the version explicitly (matches the `packageManager` field in `package.json`):

   ```bash
   curl -fsSL https://bun.sh/install | bash -s "bun-v1.3.14"
   ```

   Bun is a single self-contained binary — it does **not** require Node.js to be installed first, and it does **not** rely on Corepack. Node.js is still required separately, because the Angular CLI, Vitest, Jest, Playwright, and `prebuild.mjs` all run under Node.

   Verify the setup with:

   ```bash
   bun --version          # should print 1.3.14 (matches packageManager in package.json)
   which bun              # should point at the Bun binary, e.g. ~/.bun/bin/bun or /opt/homebrew/bin/bun
   ```

   **First-time install of dependencies** (after cloning the repo):

   ```bash
   bun install            # installs everything declared in package.json + bun.lock
   ```

   **Troubleshooting Bun installation:**
   - **`bun: command not found` after install:** Restart your shell so `~/.bun/bin` is picked up on `PATH`. The Bun installer adds the right line to your shell rc file, but it only takes effect on a fresh shell.
   - **macOS Gatekeeper warning ("cannot be opened"):** Run `xattr -d com.apple.quarantine ~/.bun/bin/bun` once, or install via Homebrew.
   - **Behind a corporate proxy:** The installer downloads the Bun binary from `github.com/oven-sh/bun/releases`. Set `HTTPS_PROXY` / `HTTP_PROXY` env vars if your network requires it.
   - **Windows native:** Bun on Windows is best-effort. Use **WSL2** for the Artemis dev environment; install Bun inside the WSL distro with the curl command above.

   **Why Bun (not npm / pnpm)?**
   - Warm-cache installs are roughly 2× faster than pnpm 11; full production build pipeline (`bun run webapp:prod`) measures ~2.5–3× faster than the previous pnpm setup on the same hardware.
   - The `bun.lock` lockfile is deterministic across machines and CI.
   - Patches are applied via the `patchedDependencies` block in `package.json` (same shape as the old `pnpm.patchedDependencies`); no `postinstall` patch-package step.
   - Dependency-script lifecycle hooks are allow-listed via the `trustedDependencies` array in `package.json` (replaces pnpm's `allowBuilds`).
...
- **Artemis (Client):** Will execute `bun install` and `bun start`. The client will be available at [http://localhost:9000/](http://localhost:9000/) with hot module replacement enabled (also see [Client Setup](#client-setup)).
...
You need Node.js on your local machine plus Bun on `PATH`. The Bun version is pinned in `package.json` via the `packageManager` field. If you haven't installed Bun yet, see [step 4 of Required Dependencies](#required-dependencies) above for the install command and troubleshooting tips.
...
- Select the `Artemis (Client)` configuration from the `npm section` (IntelliJ groups `package.json` scripts under "npm" even though we run Bun)
- Now you can run the configuration in the upper right corner of IntelliJ

### Using the command line

If this is your first time setting up the client, install Bun once (see [Required Dependencies, step 4](#required-dependencies)):

```bash
curl -fsSL https://bun.sh/install | bash      # macOS / Linux / WSL2
# or
...
<Callout variant={CalloutVariant.info}>
    In case you encounter any problems regarding JavaScript heap memory leaks when executing `bun start` or `bun run webapp:prod`, the heap budget is set explicitly inside the relevant scripts via `cross-env NODE_OPTIONS=--max-old-space-size=6144` (see `package.json`). Recommended values are 3072 (3GB), 4096 (4GB), 5120 (5GB), 6144 (6GB), 7168 (7GB), and 8192 (8GB). To override locally for a single run, prefix the command with `NODE_OPTIONS=--max-old-space-size=8192 bun run webapp:prod`. The Vitest and Jest CI scripts set their own (higher) `NODE_OPTIONS` via `cross-env`.
</Callout>
...
- When facing issues with deep dependencies and changes were made to the `package.json` file, deleting `node_modules` and running `bun install` again resolves most issues. To bypass the lockfile (e.g. after manually editing dependency ranges), drop the `--frozen-lockfile` flag and run plain `bun install`.
...
- **`bun: command not found`** after cloning the repo: install Bun (see [step 4 of Required Dependencies](#required-dependencies)) and restart your shell so `~/.bun/bin` is on `PATH`.
- **Accidentally ran `npm install` or `pnpm install` and got a stray `package-lock.json` / `pnpm-lock.yaml`:** delete the stray lockfile and `node_modules`, then run `bun install`. The repository's `.gitignore` blocks these stray lockfiles from being committed, but they still confuse tooling locally.
- **`bun install --frozen-lockfile` complains the lockfile is out of date:** the lockfile is authoritative; running plain `bun install` locally updates it. If your edit was intentional (e.g. you bumped a dependency in `package.json`), commit the updated `bun.lock`. If not, run `git checkout bun.lock` and rerun `bun install --frozen-lockfile`.
- **`bun test` runs Bun's built-in test runner, not Jest!** This is a gotcha — `bun test` is a Bun subcommand, not an alias for `bun run test`. To run the Artemis client Jest suite, always use `bun run test` (with the explicit `run`).
