...
3. **Set the installation metadata**
    - A server with the `core` profile, which every run configuration below includes, refuses to start until it knows who operates the installation. Add these three values to `application-local.yml`, replacing the examples with your own; they appear on the About page:
...
info:
    # The installation metadata from step 3 of the configuration setup
