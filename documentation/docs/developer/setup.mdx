...
- [Angular 22](https://angular.dev) (TypeScript) for the web application that serves the user interface.
...
3. **Node.js (LTS ≥ 24.18.0 < 25)**
   Download from [Node.js](https://nodejs.org/en/download).
   Required for compiling and running the Angular client.

4. **[pnpm](https://pnpm.io) (≥ 12.5.1) — the package manager Artemis uses for the Angular client**

   The official pnpm website is [pnpm.io](https://pnpm.io/); the CLI reference for `pnpm install` is at [pnpm.io/cli/install](https://pnpm.io/cli/install) and the installation guide at [pnpm.io/installation](https://pnpm.io/installation).
   Artemis uses pnpm instead of npm. We do **not** use `npm` / `npx` anywhere; please don't run `npm install` in this repo — it will create a `package-lock.json` that conflicts with the committed `pnpm-lock.yaml`. The exact pnpm version is pinned in `package.json` via the `packageManager` field, so every developer and every CI job uses the same version.

   You don't need to follow the official pnpm install steps manually — Node.js 24 ships with [Corepack](https://nodejs.org/api/corepack.html), a small shim that downloads and runs the pinned package manager on demand. This is also the [installation method recommended by the pnpm team](https://pnpm.io/installation#using-corepack). Activate it **once** per machine:

   ```bash
   corepack enable
   ```

   After that, any `pnpm` command run inside the Artemis repository automatically uses the pinned version. Verify the setup with:

   ```bash
   pnpm --version          # should print 12.5.1 (matches packageManager in package.json)
   which pnpm              # should point at the Corepack shim, e.g. ~/.nvm/.../bin/pnpm
   ```

   **First-time install of dependencies** (after cloning the repo):

   ```bash
   pnpm install            # installs everything declared in package.json + pnpm-lock.yaml
   ```

   **Troubleshooting `corepack enable`:**
   - **Permission denied / EACCES on macOS or Linux:** Corepack writes shim symlinks next to the Node.js binary. If Node.js was installed system-wide (e.g. via `apt`/`brew install node`), run `sudo corepack enable` once. With [nvm](https://github.com/nvm-sh/nvm), [fnm](https://github.com/Schniz/fnm), or [Volta](https://volta.sh), no `sudo` is needed.
   - **`corepack: command not found`:** Your Node.js install is too old or was built without Corepack. Reinstall Node.js 24+ from [nodejs.org](https://nodejs.org/en/download); Corepack is included by default.
   - **`pnpm: command not found` after `corepack enable`:** Restart your shell so the new shim is picked up on `PATH`.
   - **Behind a corporate proxy:** Corepack fetches the pinned pnpm tarball from npmjs.org on first use. Set `HTTPS_PROXY` / `HTTP_PROXY` env vars if your network requires it.

   We migrated from npm to pnpm to get deterministic, content-addressable installs (faster cold installs locally and in CI, byte-identical `node_modules` across machines, stricter override semantics, and patch application via `pnpm.patchedDependencies` instead of a `postinstall` `patch-package` step).
...
```bash
docker compose --env-file .env -f docker/mysql.yml up
```
...
3. **Set the installation metadata (optional)**
    - The About page shows who operates the installation. A development server starts without these values; only the production profile, as in the **Artemis (Server, Prod, LocalCI)** run configuration, requires them unless `info.testServer` is `true`. To set them, add them to `application-local.yml`, replacing the examples with your own:

    ```yaml
    info:
        operatorName: Research group of Applied Education Technologies
        operatorAdminName: Jane Doe
        universityName: Technical University of Munich
    ```

    - Empty values and template values such as `Admin`, `Your University` or `<name>` are rejected, and the error names the properties to fix. See [Telemetry](/admin/telemetry#required-configuration) for details.

4. **Activate the correct profile**
...
artemis:
    ...
    # Required module feature toggles
    atlas:
      enabled: true
      atlasllm:
        enabled: false   # Atlas chat agent and competency orchestration; needs a chat model under spring.ai (see artemis-intelligence.mdx). Competencies and learning paths work without it.
    exam:
      enabled: true
    fileupload:
      enabled: true
    ...
    hyperion:
      enabled: true
    weaviate:
      enabled: false   # global search; enable together with a running Weaviate instance (see weaviate-setup.mdx)
...
- **[Integrated Code Lifecycle Setup](/admin/production-setup/integrated-code-lifecycle-setup) (Recommended)**
- **[LocalVC + Jenkins Setup](/admin/jenkins-localvc)**
...
- **Artemis (Client):** Will execute `pnpm install` and `pnpm start`. The client will be available at [http://localhost:9000/](http://localhost:9000/) with hot module replacement enabled (also see [Client Setup](#client-setup)).
- **Artemis (Server, LocalVC & LocalCI):** The server will be started separated from the client with the profiles `dev,artemis,localci,localvc,scheduling,buildagent,core,local`. Enable LDAP via `artemis.user-management.ldap.enabled=true` in your `application-local.yml`.
...
- **Artemis (Server, LocalVC & LocalCI, Athena):** The server will be started separated from the client with the profiles `dev,localci,localvc,artemis,scheduling,buildagent,core,local`. Enable Athena via `artemis.athena.enabled=true` in your `application-local.yml`.
...
- **Artemis (Server, Prod, LocalCI):** The server will be started separated from the client with the profiles `prod,core,localvc,localci,scheduling,local`. Enable LDAP via `artemis.user-management.ldap.enabled=true` in your environment config. This configuration is used for production-like testing.
...
<Callout variant={CalloutVariant.info}>
  The Docker image requires several module properties to be explicitly configured. Without them, the server will fail to start with an error such as:
  *Property artemis.atlas.enabled not found in Artemis configuration.
  Make sure to add it to your application.yml-file.*
</Callout>

To provide the required configuration, one option is to create an `application-local.yml` and mount it into `/opt/artemis/config/`. Use the properties from [Common Configuration Options](#common-configuration-options) as a base, and add a `spring.datasource` block:

```yaml
# application-local.yml
# ... all other properties from Common Configuration Options
info:
    # Optional: the installation metadata from step 3 of the configuration setup
    operatorName: <operator>
    operatorAdminName: <administrator>
    universityName: <university>
spring:
    datasource:
        # If your database is running in another Docker container locally on your machine, replace `<db-host>` with `host.docker.internal` (macOS/Windows) or the host's IP address (Linux).
        url: "jdbc:mysql://<db-host>:3306/Artemis?createDatabaseIfNotExist=true&allowPublicKeyRetrieval=true&useUnicode=true&characterEncoding=utf8&useSSL=false&useLegacyDatetimeCode=false&serverTimezone=UTC"
        # Depending on your database setup, the following may be required:
        username: root
        password: ""
```

Then run the container using the following command and replace `<TAG/VERSION>` as described above:

```bash
docker run \
  -v "$PWD/application-local.yml:/opt/artemis/config/application-local.yml:ro" \
  -e SPRING_CONFIG_ADDITIONAL_LOCATION=file:/opt/artemis/config/ \
  -e SPRING_PROFILES_ACTIVE=dev,artemis,localci,localvc,scheduling,buildagent,core,local \
  -e ARTEMIS_USER_MANAGEMENT_LDAP_ENABLED=true \
  -e ARTEMIS_APOLLON_ENABLED=true \
  -p 8080:8080 \
  ghcr.io/ls1intum/artemis:<TAG/VERSION>
```

Alternatively, you can pass properties directly as Docker environment variables using [Spring's relaxed binding](https://github.com/spring-projects/spring-boot/wiki/Relaxed-Binding-2.0#environment-variables) (e.g. `artemis.atlas.enabled` → `ARTEMIS_ATLAS_ENABLED`). You can adapt the following to your needs:

```bash
docker run \
  -e ARTEMIS_ATLAS_ENABLED=true \
  -e INFO_OPERATORNAME="<operator>" \
  -e INFO_OPERATORADMINNAME="<administrator>" \
  -e INFO_UNIVERSITYNAME="<university>" \
  -e SPRING_DATASOURCE_URL="jdbc:mysql://<db-host>:3306/Artemis?..." \
  ...
  -p 8080:8080 \
  ghcr.io/ls1intum/artemis:<TAG/VERSION>
```
...
If you use IntelliJ Ultimate, add the following entry to the **Active Profiles** field (within **Spring Boot**) in the server run configuration:
...
You need Node.js on your local machine, plus pnpm activated via Corepack (`corepack enable`). The pnpm version is pinned in `package.json` via the `packageManager` field. If you haven't activated pnpm yet, see [step 4 of Required Dependencies](#required-dependencies) above for the full first-time setup and troubleshooting tips.
...
- Select the **Artemis (Client)** configuration from the **npm** section (IntelliJ groups `package.json` scripts under "npm" even though we run pnpm)
- Now you can run the configuration in the upper right corner of IntelliJ

### Using the command line

If this is your first time setting up the client, activate the pinned pnpm version once:

```bash
corepack enable
```

You should then be able to run the following command to install development tools and dependencies. You will only need to run this command when dependencies change in `package.json`.

```bash
pnpm install
```

In CI (and whenever you want the install to fail rather than silently update the lockfile), use:
...
<Callout variant={CalloutVariant.info}>
    In case you encounter any problems regarding JavaScript heap memory leaks when executing `pnpm start` or any other scripts from `package.json`, you can adjust the [memory limit parameter](https://nodejs.org/docs/latest-v24.x/api/cli.html#--max-old-space-sizesize-in-megabytes) (`node-options=--max-old-space-size=6144`) which is set by default in the project-wide `.npmrc` file. pnpm honours `node-options` from `.npmrc` for Node settings; pnpm's own install/resolution configuration lives in `pnpm-workspace.yaml` (e.g. `autoInstallPeers`, `allowBuilds`, `patchedDependencies`). If you still face the issue, you can try to set a lower/higher value than 6144 MB. Recommended values are 3072 (3GB), 4096 (4GB), 5120 (5GB), 6144 (6GB), 7168 (7GB), and 8192 (8GB). You can override the project-wide `.npmrc` file by [using a per-user `~/.npmrc`](https://pnpm.io/npmrc). Make sure to **not commit changes** in the project-wide `.npmrc` unless the GitHub build also needs these settings.
</Callout>
...
- When facing issues with deep dependencies and changes were made to the `package.json` file, deleting `node_modules` and running `pnpm install` again resolves most issues. To bypass the lockfile (e.g. after manually editing dependency ranges), use `pnpm install --no-frozen-lockfile`.
...
- **`pnpm: command not found`** after cloning the repo: run `corepack enable` once (see [step 4 of Required Dependencies](#required-dependencies)). If `corepack` itself is missing, your Node.js install is too old; reinstall Node.js 24+ from [nodejs.org](https://nodejs.org/en/download).
- **Accidentally ran `npm install` and got a `package-lock.json`:** delete `package-lock.json` and `node_modules`, then run `pnpm install`. The repository's `.gitignore` blocks `package-lock.json` from being committed, but a stray copy still confuses tooling.
- **`pnpm install` complains the lockfile is out of date:** the lockfile is authoritative; running `pnpm install` locally without `--frozen-lockfile` updates it. If your edit was intentional (e.g. you bumped a dependency in `package.json`), commit the updated `pnpm-lock.yaml`. If not, run `git checkout pnpm-lock.yaml` and rerun `pnpm install --frozen-lockfile`.
...
- **[Jenkins and LocalVC Setup](/admin/jenkins-localvc)** - Alternative CI/CD setup using Jenkins with LocalVC
