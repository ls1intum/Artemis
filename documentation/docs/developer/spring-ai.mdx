...
spring:
  ai:
    ...
    openai:
      ...
      # Local and OpenAI-compatible servers use Authorization: Bearer auth. Foundry auth is auto-detected
      # from an *.openai.azure.com base URL, so it stays off here; setting it true would send an Azure
      # api-key header instead and make auth-enforcing endpoints return 401.
