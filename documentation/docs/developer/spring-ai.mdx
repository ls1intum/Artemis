...
spring:
  ai:
    model:
      chat: openai
    openai:
      # The official OpenAI SDK appends /chat/completions itself, so the base URL must include the /v1 prefix
      base-url: http://localhost:1234/v1
      api-key: dummy-key # not required for local LLM service
      # Required for local / OpenAI-compatible servers: they use Authorization: Bearer auth. The shipped
      # default in application-artemis.yml is true (Azure/Foundry), which sends an Azure api-key header
      # instead and makes auth-enforcing endpoints return 401. Leave false here.
