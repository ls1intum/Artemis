...
async function expectReferenceTheme(page: Page, theme: 'light' | 'dark', story = 'introduction--docs') {
    await expect(page).toHaveURL(new RegExp(`/developer/tum-aet-ui/\\?path=/docs/${story}&globals=theme:${theme}$`));
...
test('connects the development guide and component reference with the selected theme', async ({ page }) => {
    ...
    await page.getByRole('main').getByRole('link', { name: 'TUM AET UI component reference', exact: true }).click();
    const darkReferenceBackground = await expectReferenceTheme(page, 'dark');
    const packageGuide = page.getByRole('link', { name: 'TUM AET UI' });
    await expect(packageGuide).toHaveAttribute('href', '/developer/guidelines/tum-aet-ui-kit');
    await packageGuide.click();
    await expect(page).toHaveURL(/\/developer\/guidelines\/tum-aet-ui-kit$/);
    await expect(page.getByRole('heading', { level: 1, name: 'TUM AET UI package' })).toBeVisible();
...
test('falls back to the introduction for a non-component heading', async ({ page }) => {
    await page.goto('./developer/tum-aet-ui-reference#available-components');

    await expectReferenceTheme(page, 'light');
});

test('redirects published URLs to the TUM AET UI reference', async ({ page }) => {
    await page.goto('./developer/guidelines/tum-ui-kit/#theme-contract');
    await expect(page).toHaveURL(/\/developer\/guidelines\/tum-aet-ui-kit#theme-contract$/);

    await page.goto('./developer/tum-ui-reference/#forms-radio-button');
    await expect(page).toHaveURL(/\/developer\/tum-aet-ui\/\?path=\/docs\/forms-radio-button--docs&globals=theme:light$/);

    await page.goto('./developer/tum-ui/?path=/docs/actions-button--docs&globals=theme:dark');
    await expect(page).toHaveURL(/\/developer\/tum-aet-ui\/\?path=\/docs\/actions-button--docs&globals=theme:dark$/);
});
