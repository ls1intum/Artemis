...
## Toolchain identity and upgrades

The shared `hyperion` library contains the wire protocol and agent runtime; `hyperion-worker`
is the deployable application. Protocol packages cannot depend on the runtime or Spring services.
The same dependency checks forbid database, application-grid and incoming web-server dependencies
in both modules.

Every authoring brief and worker event identifies its toolchain. Set
`artemis.hyperion.worker.toolchain` to the identifier of the qualified adapter and sandbox image
(default `java-gradle`). The worker checks both this identifier and the immutable image digest
before admitting an assignment. A different identifier does not install support for that language.
A worker uses one toolchain/image pair; use distinct worker IDs for different toolchain pools.

Protocol version 3 requires the toolchain field and a language-neutral `namespace` in the brief.
It does not accept version 2 messages. Drain authoring before updating core and workers together;
do not rely on a rolling mixed-protocol deployment. Keep the existing database and recovery
records, and verify worker discovery and a qualified generation after the update.
