...
test('aligns select-like overlays to their trigger', async ({ page }) => {
    for (const story of [
        { id: 'forms-autocomplete--default', name: 'Assignee', origin: '.tumaet-ui-autocomplete-container', panel: '.tumaet-ui-autocomplete-panel' },
        { id: 'forms-select--default', name: 'Course language', origin: '.tumaet-ui-select-trigger', panel: '.tumaet-ui-select-panel' },
    ]) {
...
test('ships a viewport backdrop that dismisses connected overlays', async ({ page }) => {
    ...
    const panel = page.locator('.tumaet-ui-select-panel');
...
test('keeps select text and controls separate at compact widths in both directions', async ({ page }) => {
    ...
    const root = page.locator('tumaet-ui-select');
...
test('renders the initial radio selection in AutoDocs', async ({ page }) => {
    ...
    const marker = box.locator('.tumaet-ui-radio-button-icon');
...
test('preserves component transitions in AutoDocs', async ({ page }) => {
    ...
    const handle = page.locator('.tumaet-ui-toggle-switch-handle').first();

    await expect(handle).toHaveCSS('transition-property', /inset-inline-start/);
    await expect(handle).toHaveCSS('transition-duration', /0\.2s/);
    await toggle.click();
    await expect(toggle).toBeChecked();

    await page.goto('./iframe.html?id=navigation-tabs--docs&viewMode=docs');
    const indicator = page.locator('.tumaet-ui-tab-indicator').first();
    const settings = page.getByRole('tab', { name: 'Settings' }).first();

    await settings.click();
    await expect(indicator).toHaveClass(/tumaet-ui-tab-indicator-animated/);
    await expect(indicator).toHaveCSS('transition-property', /transform/);
    await expect(indicator).toHaveCSS('transition-duration', /0\.25s/);
...
test('keeps input-group seams logical in both text directions', async ({ page }) => {
    ...
    const group = page.locator('tumaet-ui-input-group');
    const addons = page.locator('tumaet-ui-input-group-addon');
    ...
    await input.hover();
    const hoverColor = await tokenColor('--tumaet-ui-control-border-hover-color');
    await expect(input).toHaveCSS('border-color', hoverColor);
    await expect(addons.first()).toHaveCSS('border-color', hoverColor);
    await input.focus();
    const focusColor = await tokenColor('--tumaet-ui-focus-color');
    await expect(input).toHaveCSS('border-color', focusColor);
    await expect(addons.first()).toHaveCSS('border-color', focusColor);
    ...
    const numberGroup = page.locator('tumaet-ui-input-group');
    const numberInput = page.getByRole('spinbutton', { name: 'Capacity' });
    const increment = page.locator('.tumaet-ui-input-number-increment');
    const decrement = page.locator('.tumaet-ui-input-number-decrement');
