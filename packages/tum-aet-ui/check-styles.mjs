...
const allowedLayers = new Set(['properties', 'tumaet-ui-cdk', 'tumaet-ui-theme']);
...
const runtimeClasses = new Set(runtimeSources.flatMap((source) => [...source.matchAll(/tumaet:[^\s"'`$]+/g)].map((match) => match[0])));
...
const contrastRule = ruleFor('tumaet:text-primary-contrast');
...
const controlBackgroundRule = ruleFor('tumaet:bg-control-background');
...
const buttonFontRule = ruleFor('tumaet-ui-btn');
if (
...
stylesheet.walkDecls('font', (declaration) => {
    if (declaration.value === 'inherit' && declaration.parent.selector.includes('tumaet-ui-')) {
...
const focusRule = ruleFor('tumaet:focus-visible:outline-focus');
...
if ([...compiledClasses].some((className) => className.startsWith('tumaet:dark:'))) {
...
stylesheet.walkRules((rule) => {
    ...
    if (rule.parent.type === 'atrule' && rule.parent.name === 'layer' && rule.parent.params === 'tumaet-ui-theme') {
    ...
    for (const className of selectorClasses) {
        if (!className.startsWith('tumaet:') && !className.startsWith('tumaet-ui-') && !cdkOverlayClasses.has(className)) {
    ...
    if (globalSelector) {
        rule.walkDecls((declaration) => {
            if (!declaration.prop.startsWith('--tumaet-')) {
                errors.push(`global selector ${rule.selector} sets ${declaration.prop}`);
            }
        });
    } else if (selectorClasses.length === 0 && !rule.selector.includes('tumaet-ui-')) {
...
stylesheet.walkDecls(/^--tumaet-animate-/, (declaration) => {
    const name = declaration.value.split(/\s+/, 1)[0];
    if (!name.startsWith('tumaet-') || !keyframes.has(name)) {
...
if (errors.length > 0) {
    throw new Error(`Invalid TUM AET UI stylesheet:\n- ${[...new Set(errors)].join('\n- ')}`);
}
