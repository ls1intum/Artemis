...
const states: TumAetUiStatusDotState[] = ['queued', 'running', 'success', 'warning', 'danger', 'neutral', 'unknown'];

const stateWord: Record<TumAetUiStatusDotState, string> = {
    queued: 'Queued',
    running: 'Running',
    success: 'Succeeded',
...
const meta = {
    ...
    component: TumAetUiStatusDotComponent,
    args: {
        state: 'running',
        label: 'Running',
        showLabel: true,
...
} satisfies Meta<TumAetUiStatusDotComponent>;

export default meta;

type Story = StoryObj<TumAetUiStatusDotComponent>;
...
export const AllStates: Story = {
    decorators: [moduleMetadata({ imports: [TumAetUiStatusDotComponent] })],
    parameters: {
        layout: 'padded',
    },
    render: () => ({
        ...
        template: `
            <div class="tumaet-ui-story-legend">
                @for (row of rows; track row.state) {
                    <tumaet-ui-status-dot [state]="row.state" [label]="row.label" />
                    <code>{{ row.state }}</code>
                }
            </div>
        `,
...
export const StateShapes: Story = {
    ...
    render: () => ({
        ...
        template: `
            <div class="tumaet-ui-story-legend">
                @for (row of rows; track row.state) {
                    <tumaet-ui-status-dot [state]="row.state" [label]="row.label" [attr.data-testid]="'dot-' + row.state" />
                    <code>{{ row.state }}</code>
                }
            </div>
        `,
    ...
    play: async ({ canvas }) => {
        const indicator = (state: TumAetUiStatusDotState) => getComputedStyle(canvas.getByTestId(`dot-${state}`).querySelector('.tumaet-ui-status-dot-indicator')!);
...
export const NonTextContrast: Story = {
    ...
    play: async ({ canvas }) => {
        ...
        for (const state of states) {
            const indicator = canvas.getByTestId(`dot-${state}`).querySelector('.tumaet-ui-status-dot-indicator')!;
