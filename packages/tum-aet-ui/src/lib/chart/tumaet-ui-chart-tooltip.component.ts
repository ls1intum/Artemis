...
@Component({
    selector: 'tumaet-ui-chart-tooltip',
    changeDetection: ChangeDetectionStrategy.OnPush,
    host: {
        class: 'tumaet-ui-chart-tooltip tumaet:rounded-md tumaet:border tumaet:border-border tumaet:bg-overlay-background tumaet:px-2 tumaet:py-1.5 tumaet:text-text tumaet:shadow-lg',
        role: 'tooltip',
        '[style.left.px]': 'x()',
        '[style.top.px]': 'y()',
    ...
    styles: `
        ...
        .tumaet-ui-chart-tooltip-title {
            font-weight: 600;
        }
    `,
    template: `
        @if (title()) {
            <div class="tumaet-ui-chart-tooltip-title">{{ title() }}</div>
        }
...
export class TumAetUiChartTooltipComponent {
