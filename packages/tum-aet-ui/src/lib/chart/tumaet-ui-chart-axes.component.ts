...
import { ChartAxisTitle, ChartGridLine, ChartTick } from './tumaet-ui-chart.frame';
...
@Component({
    ...
    template: `
        @for (gridLine of gridLines(); track gridLine.key) {
            <svg:line class="tumaet-ui-chart-grid" [attr.x1]="gridLine.x1" [attr.y1]="gridLine.y1" [attr.x2]="gridLine.x2" [attr.y2]="gridLine.y2" />
        }
        @for (tick of ticks(); track tick.key) {
            <svg:text
                class="tumaet-ui-chart-tick"
                [attr.x]="tick.x"
                [attr.y]="tick.y"
        ...
        @for (title of titles(); track $index) {
            <svg:text
                class="tumaet-ui-chart-axis-title"
                [attr.x]="title.x"
                [attr.y]="title.y"
                text-anchor="middle"
...
export class TumAetUiChartAxesComponent {
