...
import { TumAetUiBarChartConfig, TumAetUiChartAxisConfig, TumAetUiChartDatumContext, TumAetUiChartSelectEvent, TumAetUiChartSeries } from './tumaet-ui-chart.types';
import { allIntegers, bandScale, finiteValues, linearScale, niceDomain } from './tumaet-ui-chart.scales';
import {
    CATEGORY_PADDING,
    ChartAxisTitle,
    ChartGridLine,
    ...
    placeTooltip,
    valueTickViews,
} from './tumaet-ui-chart.frame';
import { TumAetUiChartAxesComponent } from './tumaet-ui-chart-axes.component';
import { TumAetUiChartLegendComponent } from './tumaet-ui-chart-legend.component';
import { TumAetUiChartTooltipComponent } from './tumaet-ui-chart-tooltip.component';
import { TumAetUiChartDataTableComponent } from './tumaet-ui-chart-data-table.component';
...
interface BarView {
    ...
    height: number;
    color: string;
    context: TumAetUiChartDatumContext;
...
@Component({
    selector: 'tumaet-ui-bar-chart',
    templateUrl: './tumaet-ui-bar-chart.component.html',
    styleUrl: './tumaet-ui-bar-chart.component.scss',
    imports: [TumAetUiChartAxesComponent, TumAetUiChartLegendComponent, TumAetUiChartTooltipComponent, TumAetUiChartDataTableComponent],
    changeDetection: ChangeDetectionStrategy.OnPush,
    host: { class: 'tumaet-ui-bar-chart' },
})
export class TumAetUiBarChartComponent implements OnDestroy {
    ...
    readonly series = input.required<readonly TumAetUiChartSeries[]>();
    readonly config = input<TumAetUiBarChartConfig>({});
    ...
    readonly dataSelect = output<TumAetUiChartSelectEvent>();

    /** Names an interactive datum for assistive technology; see {@link datumAccessibleName}. */
    protected accessibleName(context: TumAetUiChartDatumContext): string {
    ...
    private readonly valueAxis = computed<TumAetUiChartAxisConfig | undefined>(() => (this.horizontal() ? this.config().xAxis : this.config().yAxis));
    private readonly categoryAxis = computed<TumAetUiChartAxisConfig | undefined>(() => (this.horizontal() ? this.config().yAxis : this.config().xAxis));
    ...
    protected readonly clipId = `tumaet-ui-bar-chart-clip-${nextBarChartId++}`;
    ...
    protected readonly bars = computed<BarView[]>(() => {
        ...
        series.forEach(({ entry, index: seriesIndex }, drawIndex) => {
            labels.forEach((label, index) => {
                ...
                const context: TumAetUiChartDatumContext = {
                    seriesIndex,
                    index,
                    label,
