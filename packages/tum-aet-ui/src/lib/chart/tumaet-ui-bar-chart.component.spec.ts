...
import { TumAetUiBarChartComponent } from './tumaet-ui-bar-chart.component';
import { TumAetUiBarChartConfig, TumAetUiChartSelectEvent, TumAetUiChartSeries } from './tumaet-ui-chart.types';

describe('TumAetUiBarChartComponent', () => {
    let fixture: ComponentFixture<TumAetUiBarChartComponent>;

    const labels = ['[0, 10)', '[10, 20)', '[20, 30)'];

    async function render(series: TumAetUiChartSeries[], config: TumAetUiBarChartConfig = {}): Promise<void> {
    ...
    function rects(): SVGRectElement[] {
        return fixture.debugElement.queryAll(By.css('rect.tumaet-ui-bar-chart-bar')).map((element) => element.nativeElement);
    }
    ...
    beforeEach(async () => {
        await TestBed.configureTestingModule({ imports: [TumAetUiBarChartComponent] }).compileComponents();
        fixture = TestBed.createComponent(TumAetUiBarChartComponent);
    ...
    it('should render category and value labels as real, selectable text nodes', async () => {
        ...
        const categories = texts('text.tumaet-ui-chart-tick');
    ...
    it('should render data labels using the configured formatter', async () => {
        ...
        expect(texts('text.tumaet-ui-bar-chart-data-label')).toEqual(['10%', '20%', '30%']);
    });
    ...
    it('should hide an axis that is configured as not displayed', async () => {
        ...
        expect(texts('text.tumaet-ui-chart-tick')).not.toContain(labels[0]);
    });

    it('should render the axis titles', async () => {
        await render([{ data: [1, 2, 3] }], { xAxis: { label: 'Score' }, yAxis: { label: 'Students' } });
        expect(texts('text.tumaet-ui-chart-axis-title')).toEqual(expect.arrayContaining(['Score', 'Students']));
    });
    ...
    it('should not render a tooltip when tooltips are disabled', async () => {
        ...
        rects()[0].dispatchEvent(new MouseEvent('mouseenter', { clientX: 40, clientY: 40 }));
        fixture.detectChanges();
        expect(fixture.debugElement.query(By.css('tumaet-ui-chart-tooltip'))).toBeNull();
    });
    ...
    it('should render a legend for labelled series when enabled', async () => {
        ...
        const items = fixture.debugElement.queryAll(By.css('.tumaet-ui-chart-legend-item')).map((element) => (element.nativeElement as HTMLElement).textContent!.trim());
    ...
    it('should drop a value it cannot place instead of blanking the chart', async () => {
        ...
        expect(texts('text.tumaet-ui-chart-tick').length).toBeGreaterThan(0);
    });
    ...
    it('should hide a series when its legend entry is switched off', async () => {
        ...
        const entries = fixture.debugElement.queryAll(By.css('.tumaet-ui-chart-legend-item'));
