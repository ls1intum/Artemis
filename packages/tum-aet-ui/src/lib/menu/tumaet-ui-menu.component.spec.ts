...
@Component({
    imports: [TumAetUiMenuComponent, TumAetUiMenuItemDirective, TumAetUiMenuTriggerDirective],
    template: `
        <button id="before">Before</button>
        <button id="trigger" [tumAetUiMenuTrigger]="actions" (menuOpened)="events.push('opened')" (menuClosed)="events.push('closed')">Actions</button>
        <button id="after">After</button>
        <ng-template #actions>
            <tumaet-ui-menu>
                <button tumAetUiMenuItem (triggered)="picked.set('students')">Add students</button>
                <button tumAetUiMenuItem [disabled]="tutorsDisabled()" (triggered)="picked.set('tutors')">Add tutors</button>
                <a tumAetUiMenuItem href="#editors" (click)="$event.preventDefault(); linkClicks = linkClicks + 1" (triggered)="picked.set('editors')">Add editors</a>
            </tumaet-ui-menu>
        </ng-template>
    `,
})
...
describe('TumAetUiMenuComponent', () => {
...
@Component({
    imports: [TumAetUiMenuComponent, TumAetUiMenuItemDirective, TumAetUiMenuTriggerDirective],
    template: `
        <button id="bound" [tumAetUiMenuTrigger]="actions" [disabled]="disabled()" (click)="clicks = clicks + 1">Bound</button>
        <button id="static" [tumAetUiMenuTrigger]="actions" disabled>Static</button>
        <ng-template #actions>
            <tumaet-ui-menu>
...
describe('TumAetUiMenuTriggerDirective (disabled)', () => {
