...
import { TumAetUiSkeletonComponent } from './tumaet-ui-skeleton.component';

const meta = {
    title: 'Feedback/Skeleton',
    component: TumAetUiSkeletonComponent,
    decorators: [moduleMetadata({ imports: [TumAetUiSkeletonComponent, TumAetUiCardComponent] })],
    args: {
        lines: 1,
    },
    parameters: {
...
} satisfies Meta<TumAetUiSkeletonComponent>;

export default meta;

type Story = StoryObj<TumAetUiSkeletonComponent>;

/** One line. It fills its container, so the box that reserves the space is the consumer's to size. */
export const Line: Story = {
    render: (args) => ({
        props: args,
        template: `<div style="width: 24rem;"><tumaet-ui-skeleton [lines]="lines" /></div>`,
    }),
};
...
export const Block: Story = {
    render: () => ({
        template: `<tumaet-ui-skeleton width="12rem" height="8rem" />`,
    }),
};
    ...
    render: () => ({
        template: `
            <div aria-busy="true" style="width: 24rem;">
                <span class="tumaet:sr-only">Loading changed files</span>
                <tumaet-ui-skeleton lines="3" />
            </div>
        `,
    }),
};
    ...
    render: () => ({
        template: `
            <tumaet-ui-card variant="outline" style="display: block; width: 26rem;">
                <div aria-busy="true">
                    <span class="tumaet:sr-only">Loading the run summary</span>
                    <tumaet-ui-skeleton width="9rem" height="1.25rem" />
...
export const NoShimmer: Story = {
    ...
    play: async ({ canvasElement }) => {
        const skeleton = canvasElement.querySelector('tumaet-ui-skeleton')!;
        const lines = [...skeleton.querySelectorAll('.tumaet-ui-skeleton-line')];
