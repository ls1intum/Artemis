...
interface StepperStoryArgs {
    orientation: TumAetUiStepperOrientation;
    ariaLabel: string;
...
const orientations: TumAetUiStepperOrientation[] = ['vertical', 'horizontal'];
const states: TumAetUiStepState[] = ['pending', 'current', 'complete', 'failed', 'skipped'];

const widthClass: Record<StepperStoryWidth, string> = {
    default: 'tumaet-ui-story-ladder',
    narrow: 'tumaet-ui-story-ladder-narrow',
    wide: 'tumaet-ui-story-ladder-wide',
};

const stages = ['Prepare workspace', 'Plan', 'Build and test', 'Review and repair', 'Publish'];

function ladder(...ladderStates: TumAetUiStepState[]): StepperStoryStep[] {
...
const meta = {
    ...
    component: TumAetUiStepperComponent,
    subcomponents: { Step: TumAetUiStepComponent },
    decorators: [
        moduleMetadata({
    ...
    render: ({ width, ...args }) => ({
        ...
        template: `
            <div class="${widthClass[width]}">
                <tumaet-ui-stepper [orientation]="orientation" [ariaLabel]="ariaLabel">
                    @for (step of steps; track step.label) {
                        <tumaet-ui-step [state]="step.state" [label]="step.label">
                            @if (step.detail) {
                                {{ step.detail }}
                            }
                        </tumaet-ui-step>
...
/**
 * A stage name that is more than a string: the projected `[tumAetUiStepLabel]` slot renders in the same line as the
 * `label` input, so a link or a count needs no second element and no wrapper around the step.
 */
export const ProjectedLabel: Story = {
    render: () => ({
        template: `
            <div class="tumaet-ui-story-ladder">
                <tumaet-ui-stepper ariaLabel="Run stages">
                    <tumaet-ui-step state="complete" label="Prepare workspace" />
                    <tumaet-ui-step state="failed">
                        <span tumAetUiStepLabel>Build and test — <a href="#log">open log</a></span>
                        2 of 18 checks failed.
                    </tumaet-ui-step>
                    <tumaet-ui-step label="Publish" />
                </tumaet-ui-stepper>
            </div>
        `,
    }),
...
const transitionTimeline: readonly TumAetUiStepState[][] = [
...
@Component({
    selector: 'tumaet-ui-stepper-transition-demo',
    imports: [TumAetUiButtonComponent, TumAetUiStepComponent, TumAetUiStepperComponent],
    template: `
        <div class="tumaet-ui-story-stack tumaet-ui-story-ladder">
            <div class="tumaet-ui-story-actions">
                <tumaet-ui-button size="small" [disabled]="atEnd()" (clicked)="advance()">Advance</tumaet-ui-button>
                <tumaet-ui-button size="small" severity="secondary" variant="outlined" [disabled]="atStart()" (clicked)="reset()">Reset</tumaet-ui-button>
            </div>
            <tumaet-ui-stepper ariaLabel="Run stages">
                @for (label of labels; track label; let index = $index) {
                    <tumaet-ui-step [state]="ladder()[index]" [label]="label" />
                }
            </tumaet-ui-stepper>
        </div>
    `,
...
export const StateTransitions: Story = {
    ...
    render: () => ({
        template: '<tumaet-ui-stepper-transition-demo />',
    }),
...
export const ConnectorAlignment: Story = {
    ...
    render: () => ({
        template: `
            <div class="tumaet-ui-story-stack tumaet-ui-story-ladder-wide">
                <tumaet-ui-stepper ariaLabel="Vertical alignment probe" data-testid="vertical-ladder">
                    <tumaet-ui-step state="complete" label="Prepare workspace">A detail line that makes this stage taller than its marker.</tumaet-ui-step>
                    <tumaet-ui-step state="current" label="Build and test" />
                </tumaet-ui-stepper>
                <tumaet-ui-stepper orientation="horizontal" ariaLabel="Horizontal alignment probe" data-testid="horizontal-ladder">
                    <tumaet-ui-step state="complete" label="Prepare workspace">A detail line that makes this stage taller than its marker.</tumaet-ui-step>
                    <tumaet-ui-step state="current" label="Build and test" />
                </tumaet-ui-stepper>
            </div>
    ...
    play: async ({ canvas }) => {
        for (const [ladder, axis] of [
            ...
            const step = canvas.getByTestId(ladder).querySelector('tumaet-ui-step')!;
            const marker = step.querySelector('.tumaet-ui-step-marker')!.getBoundingClientRect();
            const connector = step.querySelector('.tumaet-ui-step-connector')!.getBoundingClientRect();
        ...
        const [first, second] = Array.from(canvas.getByTestId('vertical-ladder').querySelectorAll('tumaet-ui-step'));
        const connector = first.querySelector('.tumaet-ui-step-connector')!.getBoundingClientRect();
        const nextMarker = second.querySelector('.tumaet-ui-step-marker')!.getBoundingClientRect();
        ...
        await expect(second.querySelector('.tumaet-ui-step-connector')!.getBoundingClientRect().height).toBe(0);
    },
};
...
export const NonTextContrast: Story = {
    ...
    render: () => ({
        template: `
            <tumaet-ui-stepper ariaLabel="Contrast probe" data-testid="contrast-ladder">
                <tumaet-ui-step state="complete" label="Complete" />
                <tumaet-ui-step state="failed" label="Failed" />
                <tumaet-ui-step state="skipped" label="Skipped" />
                <tumaet-ui-step state="current" label="Current" />
                <tumaet-ui-step state="pending" label="Pending" />
            </tumaet-ui-stepper>
        `,
    }),
    play: async ({ canvas }) => {
        const surface = getComputedStyle(document.body).backgroundColor;
        const steps = Array.from(canvas.getByTestId('contrast-ladder').querySelectorAll('tumaet-ui-step'));

        for (const step of steps) {
            const state = step.getAttribute('data-state');
            const marker = getComputedStyle(step.querySelector('.tumaet-ui-step-marker')!);
            ...
            const connector = step.querySelector('.tumaet-ui-step-connector')!;
