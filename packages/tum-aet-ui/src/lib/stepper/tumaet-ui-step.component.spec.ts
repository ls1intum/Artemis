...
import { TUM_AET_UI_TRANSLATOR, TumAetUiTranslator } from '../i18n/tumaet-ui-translations';
import { TumAetUiStepComponent, TumAetUiStepState } from './tumaet-ui-step.component';
import { TumAetUiStepperComponent, TumAetUiStepperOrientation } from './tumaet-ui-stepper.component';

@Component({
    template: `
        <tumaet-ui-stepper [orientation]="orientation()">
            <tumaet-ui-step state="complete" label="Prepare workspace" />
            <tumaet-ui-step state="current" label="Design" />
        </tumaet-ui-stepper>
    `,
    imports: [TumAetUiStepComponent, TumAetUiStepperComponent],
})
class TwoStepsHostComponent {
    readonly orientation = signal<TumAetUiStepperOrientation>('vertical');
}

describe('TumAetUiStepComponent', () => {
    let fixture: ComponentFixture<TumAetUiStepComponent>;
    let host: HTMLElement;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [TumAetUiStepComponent, FontAwesomeTestingModule],
        }).compileComponents();
        fixture = TestBed.createComponent(TumAetUiStepComponent);
        host = fixture.nativeElement as HTMLElement;
        fixture.componentRef.setInput('label', 'Build and test');
        fixture.detectChanges();
    });

    function withState(state: TumAetUiStepState): void {
    ...
    function runningIndicator(): HTMLElement | undefined {
        return fixture.debugElement.query(By.css('.tumaet-ui-step-running-indicator'))?.nativeElement;
    }
    ...
    it('keeps the marker out of the accessibility tree', () => {
        const track = fixture.debugElement.query(By.css('.tumaet-ui-step-marker-track')).nativeElement as HTMLElement;
        expect(track.getAttribute('aria-hidden')).toBe('true');
    });
});

describe('TumAetUiStepComponent state word translation', () => {
    it('resolves the state word through the host translator and follows its catalog', async () => {
        ...
        await TestBed.configureTestingModule({
            imports: [TumAetUiStepComponent, FontAwesomeTestingModule],
            providers: [{ provide: TUM_AET_UI_TRANSLATOR, useValue: translator }],
        }).compileComponents();

        const fixture = TestBed.createComponent(TumAetUiStepComponent);
        fixture.componentRef.setInput('label', 'Design');
        fixture.componentRef.setInput('state', 'failed');
        fixture.detectChanges();
        const host = fixture.nativeElement as HTMLElement;
        expect(host.textContent).toContain('tumAetUi.step.failed#0');

        revision.set(1);
        fixture.detectChanges();
        expect(host.textContent).toContain('tumAetUi.step.failed#1');
    });
});

describe('TumAetUiStepComponent label slot', () => {
    it('renders projected label markup in the same line as the label input', async () => {
        @Component({
            template: `<tumaet-ui-step state="complete" label="Build"><a tumAetUiStepLabel href="#log">log</a>detail line</tumaet-ui-step>`,
            imports: [TumAetUiStepComponent],
        })
        ...
        const label = fixture.debugElement.query(By.css('.tumaet-ui-step-label')).nativeElement as HTMLElement;
        const detail = fixture.debugElement.query(By.css('.tumaet-ui-step-detail')).nativeElement as HTMLElement;
...
describe('TumAetUiStepComponent connector', () => {
    ...
    function connectors(): HTMLElement[] {
        return fixture.debugElement.queryAll(By.css('.tumaet-ui-step-connector')).map((element) => element.nativeElement);
    }

    it('gives every step one connector element inside the marker track', () => {
        const tracks = fixture.debugElement.queryAll(By.css('.tumaet-ui-step-marker-track')).map((element) => element.nativeElement as HTMLElement);
