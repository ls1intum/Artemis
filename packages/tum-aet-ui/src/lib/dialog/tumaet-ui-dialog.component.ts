...
export type TumAetUiDialogSize = 'small' | 'medium' | 'large' | 'full';

const DIALOG_SIZE_CLASSES: Record<TumAetUiDialogSize, string> = {
    small: 'tumaet:w-[min(32rem,90dvw)]',
    medium: 'tumaet:w-[min(48rem,90dvw)]',
    large: 'tumaet:w-[min(72rem,90dvw)]',
    full: 'tumaet:h-[90dvh] tumaet:w-[90dvw]',
};
...
@Component({
    ...
    templateUrl: './tumaet-ui-dialog.component.html',
    imports: [NgTemplateOutlet, FaIconComponent, TumAetUiTranslatePipe],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TumAetUiDialogComponent implements OnDestroy {
    ...
    readonly size = input<TumAetUiDialogSize>();
    ...
    protected readonly titleId = `tumaet-ui-dialog-title-${nextDialogId++}`;
    ...
    private open(): void {
        ...
        if (!ariaLabel && !labelledBy) {
            throw new Error('tumaet-ui-dialog requires a visible header, a header template, or ariaLabel');
        }
        const ref = this.dialog.open(this.panel(), {
            panelClass: 'tumaet-ui-overlay',
