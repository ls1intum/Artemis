...
@Component({
    selector: 'tumaet-ui-toggle-switch',
    templateUrl: './tumaet-ui-toggle-switch.component.html',
    styleUrl: './tumaet-ui-toggle-switch.component.scss',
    host: {
        class: 'tumaet-ui-toggle-switch',
        '[class]': 'hostClasses()',
        '[attr.data-checked]': 'checked()',
        '[attr.data-disabled]': 'effectiveDisabled() || null',
    },
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => TumAetUiToggleSwitchComponent), multi: true }],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TumAetUiToggleSwitchComponent implements ControlValueAccessor {
    ...
    protected readonly hostClasses = computed(() => {
        const track = this.checked() ? 'tumaet:bg-primary' : 'tumaet:bg-control-border';
        return `${track} ${this.effectiveDisabled() ? 'tumaet:opacity-60' : ''}`.trim();
    });
