...
import { TumAetUiToggleSwitchComponent } from './tumaet-ui-toggle-switch.component';

describe('TumAetUiToggleSwitchComponent', () => {
    let fixture: ComponentFixture<TumAetUiToggleSwitchComponent>;
    let component: TumAetUiToggleSwitchComponent;
    let host: HTMLElement;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [TumAetUiToggleSwitchComponent],
        }).compileComponents();
        fixture = TestBed.createComponent(TumAetUiToggleSwitchComponent);
    ...
    it('keeps its identity class while the state classes change around it', () => {
        expect(host.classList.contains('tumaet-ui-toggle-switch')).toBe(true);
        expect(host.classList.contains('tumaet:bg-control-border')).toBe(true);

        component.writeValue(true);
        fixture.detectChanges();

        expect(host.classList.contains('tumaet-ui-toggle-switch')).toBe(true);
        expect(host.classList.contains('tumaet:bg-primary')).toBe(true);
        expect(host.classList.contains('tumaet:bg-control-border')).toBe(false);
    });
...
@Component({
    template: `
        ...
        <tumaet-ui-toggle-switch inputId="notifications" aria-labelledby="notifications-label" [formControl]="control" />
    `,
    imports: [TumAetUiToggleSwitchComponent, ReactiveFormsModule],
})
class ReactiveHostComponent {
    readonly control = new FormControl(false);
}

describe('TumAetUiToggleSwitchComponent (reactive forms)', () => {
...
describe('TumAetUiToggleSwitchComponent accessible name', () => {
    @Component({
        imports: [TumAetUiToggleSwitchComponent],
        template: `
            <tumaet-ui-toggle-switch aria-label="Static name" />
            <tumaet-ui-toggle-switch [ariaLabel]="dynamicName()" />
        `,
    })
