...
import { TumAetUiTableSortableColumnComponent } from './tumaet-ui-table-sortable-column.component';

@Component({
    template: `
        <table tumAetUiTable [sortField]="sortField()" [sortOrder]="sortOrder()" (sortChange)="events.push($event)">
            <thead>
                <tr>
                    <th tumAetUiSortableColumn="login">Login</th>
                    <th tumAetUiSortableColumn="email">Email</th>
                    <th tumAetUiSortableColumn="frozen" [disabled]="true">Frozen</th>
                </tr>
            </thead>
        </table>
    `,
    imports: [TumAetUiTableDirective, TumAetUiTableSortableColumnComponent],
})
class HostComponent {
    readonly sortField = signal<string | undefined>('login');
    readonly sortOrder = signal(1);
    events: TumAetUiTableSortEvent[] = [];
}

describe('TumAetUiTableSortableColumnComponent', () => {
    ...
    it('renders a sort icon after the projected header text', () => {
        ...
        expect(login.querySelector('fa-icon.tumaet-ui-sort-icon')).not.toBeNull();
    });
