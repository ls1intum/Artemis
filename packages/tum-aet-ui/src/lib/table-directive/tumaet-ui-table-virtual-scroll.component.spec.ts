...
@Component({
    template: `
        <tumaet-ui-table-virtual-scroll [items]="items" [itemSize]="40" [rowTemplate]="rowTpl" scrollHeight="20rem" minWidth="30rem" ariaDescribedBy="desc">
        ...
        >
    `,
    imports: [TumAetUiTableVirtualScrollComponent],
})
class HostComponent {
    items: Row[] = Array.from({ length: 50 }, (_, i) => ({ name: `logger-${i}` }));
}

describe('TumAetUiTableVirtualScrollComponent', () => {
