...
import { buildMonthMatrix } from './tumaet-ui-date-picker.util';
import { TUM_AET_UI_TRANSLATOR } from '../i18n/tumaet-ui-translations';

@Component({
    selector: 'tumaet-ui-calendar',
    templateUrl: './tumaet-ui-calendar.component.html',
    imports: [FaIconComponent],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TumAetUiCalendarComponent {
    ...
    protected readonly previousMonthLabel = computed(() =>
        this.translate('tumAetUi.datePicker.previousMonth', { month: this.formatDate(this.activeMonth().subtract(1, 'month'), { month: 'long', year: 'numeric' }) }),
    );
    protected readonly nextMonthLabel = computed(() =>
        this.translate('tumAetUi.datePicker.nextMonth', { month: this.formatDate(this.activeMonth().add(1, 'month'), { month: 'long', year: 'numeric' }) }),
    );
    ...
    protected dayButtonClasses(day: dayjs.Dayjs): string {
        const base =
            'tumaet:appearance-none tumaet:border-0 tumaet:h-8 tumaet:w-8 tumaet:rounded-full tumaet:focus-visible:outline tumaet:focus-visible:outline-2 tumaet:focus-visible:outline-focus';
        let color: string;
        if (this.isSelected(day)) {
            color = 'tumaet:bg-primary tumaet:text-primary-contrast';
        } else if (this.isOtherMonth(day)) {
            color = 'tumaet:bg-transparent tumaet:text-muted tumaet:hover:bg-hover-background';
        } else {
            color = 'tumaet:bg-transparent tumaet:text-text tumaet:hover:bg-hover-background';
        }
        const today = this.isToday(day) && !this.isSelected(day) ? 'tumaet:ring-1 tumaet:ring-primary' : '';
