 ...
 *
 * ```html
 * <button tumAetUiButton [tumAetUiDisabledReason]="saveBlockedReason()" (click)="save()">Save</button>
 * ```
 *
 * Menu items are the one place not to use it: `[tumAetUiMenuItem]` activates through the CDK menu's own key handling, so
 * set its `disabled` input and render the reason as a second line of the item instead.
 */
@Directive({
    selector: '[tumAetUiDisabledReason]',
    hostDirectives: [{ directive: TumAetUiTooltipDirective, inputs: ['tumAetUiTooltip: tumAetUiDisabledReason', 'tumAetUiTooltipPlacement'] }],
    host: {
        '[attr.aria-disabled]': "reason() ? 'true' : null",
    },
})
export class TumAetUiDisabledReasonDirective {
    private readonly tooltip = inject(TumAetUiTooltipDirective);
