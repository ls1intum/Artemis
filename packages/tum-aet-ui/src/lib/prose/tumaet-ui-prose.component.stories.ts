...
import { TumAetUiProseComponent, TumAetUiProseDensity } from './tumaet-ui-prose.component';

interface ProseStoryArgs {
    density: TumAetUiProseDensity;
    html: string;
}
...
const meta = {
    ...
    component: TumAetUiProseComponent,
    parameters: {
        layout: 'padded',
    },
    args: {
    ...
    argTypes: {
        density: {
            ...
            options: ['comfortable', 'compact'] satisfies TumAetUiProseDensity[],
        },
        html: {
            control: 'text',
        },
    ...
    render: ({ density, html }) => ({
        props: { density, html },
        template: `<tumaet-ui-prose [density]="density" [innerHTML]="html"></tumaet-ui-prose>`,
    }),
...
export const InsideAPanel: Story = {
    ...
    decorators: [moduleMetadata({ imports: [TumAetUiPanelComponent, TumAetUiProseComponent] })],
    render: ({ density, html }) => ({
        props: { density, html },
        template: `
            <tumaet-ui-panel header="Retention policy" toggleable style="display: block; width: min(40rem, 100%);">
                <tumaet-ui-prose [density]="density" [innerHTML]="html"></tumaet-ui-prose>
            </tumaet-ui-panel>
        `,
    }),
 ...
 * page's size and weight. `.tumaet-ui-prose h2` is a class selector at specificity (0,1,1) in the same unlayered
    ...
    render: ({ html }) => ({
        ...
        template: `
            <div>
                ...
                <tumaet-ui-prose [innerHTML]="html" data-testid="prose"></tumaet-ui-prose>
            </div>
        `,
    }),
    play: async ({ canvas }) => {
...
export const CodeSurface: Story = {
    ...
    render: ({ density, html }) => ({
        props: { density, html },
        template: `<tumaet-ui-prose [density]="density" [innerHTML]="html" data-testid="prose"></tumaet-ui-prose>`,
    }),
    play: async ({ canvas }) => {
