...
import { TumAetUiProseComponent } from './tumaet-ui-prose.component';

@Component({
    imports: [TumAetUiProseComponent],
    template: `
        <tumaet-ui-prose [density]="density()">
            <h2>Heading</h2>
            <p>Body</p>
        </tumaet-ui-prose>
        <article tumAetUiProse [innerHTML]="html"></article>
    `,
})
class HostComponent {
...
describe('TumAetUiProseComponent', () => {
    ...
    it('projects the document untouched and adds no markup of its own', () => {
        const prose = element('tumaet-ui-prose');
    ...
    it('exposes its slot and density on the host', () => {
        expect(element('tumaet-ui-prose').getAttribute('data-slot')).toBe('prose');
        expect(element('tumaet-ui-prose').getAttribute('data-density')).toBe('comfortable');

        fixture.componentInstance.density.set('compact');
        fixture.detectChanges();
        expect(element('tumaet-ui-prose').getAttribute('data-density')).toBe('compact');
    });

    it('styles an element the consumer chose, so a document keeps its own semantics', () => {
        const article = element('article[tumAetUiProse]');
        expect(article.tagName).toBe('ARTICLE');
        expect(article.classList).toContain('tumaet-ui-prose');
    ...
    describe('cascade contract', () => {
        // The component's whole justification is that `.tumaet-ui-prose h2` (0,1,1) outranks a host page's bare `h2`
        ...
        function proseStylesheet(): string {
            return [...document.querySelectorAll('style')].map((style) => style.textContent ?? '').find((text) => text.includes('.tumaet-ui-prose')) ?? '';
        }
        ...
        it('scopes every heading rule behind the class, so it outranks a bare element rule', () => {
            ...
            for (const heading of ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']) {
                expect(css).toContain(`.tumaet-ui-prose ${heading}`);
            }
        });
