...
export type TumAetUiSelectSize = 'small' | 'large';
const TRIGGER_SIZE: Record<'small' | 'default' | 'large', string> = {
    small: 'tumaet:min-h-8 tumaet:py-1.5 tumaet:ps-2.5 tumaet:text-sm',
    default: 'tumaet:min-h-10 tumaet:py-2 tumaet:ps-3 tumaet:text-base',
    large: 'tumaet:min-h-12 tumaet:py-2.5 tumaet:ps-3.5 tumaet:text-lg',
};
...
@Component({
    selector: 'tumaet-ui-select',
    templateUrl: './tumaet-ui-select.component.html',
    styleUrl: './tumaet-ui-select.component.scss',
    imports: [FaIconComponent, TumAetUiTranslatePipe],
    host: {
        class: 'tumaet-ui-select',
    },
    providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => TumAetUiSelectComponent), multi: true }],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TumAetUiSelectComponent implements ControlValueAccessor {
    ...
    readonly size = input<TumAetUiSelectSize>();
    ...
    private readonly fallbackInputId = `tumaet-ui-select-${nextSelectId++}`;
    ...
    protected readonly listboxId = `tumaet-ui-select-listbox-${nextSelectId++}`;
    ...
    protected readonly triggerClasses = computed(() => `${this.buildTriggerClasses()} ${this.showClearButton() ? 'tumaet:pe-17' : 'tumaet:pe-10'}`);
    ...
    private buildTriggerClasses(): string {
        const base = 'tumaet-ui-select-trigger tumaet:box-border tumaet:flex tumaet:w-full tumaet:items-center tumaet:border tumaet:text-start tumaet:transition-colors';
    ...
    protected optionClasses(option: unknown, index: number): string {
        const base = 'tumaet-ui-select-option tumaet:flex tumaet:cursor-pointer tumaet:items-center tumaet:px-3 tumaet:py-2';
