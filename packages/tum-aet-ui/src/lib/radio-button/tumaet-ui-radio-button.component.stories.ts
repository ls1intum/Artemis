...
import { TumAetUiRadioButtonComponent, TumAetUiRadioButtonSelectEvent } from './tumaet-ui-radio-button.component';
...
interface RadioButtonStoryArgs {
    ...
    disabled: boolean;
    selected: (event: TumAetUiRadioButtonSelectEvent) => void;
}
...
const meta = {
    ...
    component: TumAetUiRadioButtonComponent,
    decorators: [
        formStoryDecorator,
        moduleMetadata({
            imports: [FormsModule],
    ...
    render: (args) => ({
        ...
        template: `
            <div style="display: flex; align-items: center; gap: 8px; width: 120px;">
                <tumaet-ui-radio-button inputId="tight" name="tight" value="tight" data-testid="tight-radio-button" />
...
export const ShowsTheSelectedDotWithoutTheSpacingToken: Story = {
    ...
    play: async ({ canvas }) => {
        ...
        const indicator = radioButton.querySelector('.tumaet-ui-radio-button-icon')!.getBoundingClientRect();
