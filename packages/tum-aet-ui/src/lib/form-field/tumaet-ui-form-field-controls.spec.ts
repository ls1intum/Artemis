...
describe('tumaet-ui-form-field with package controls', () => {
    ...
    it('describes a select by the field hint', () => {
        ...
        const hintId = (fixture.debugElement.queryAll(By.css('.tumaet-ui-form-field-hint'))[0].nativeElement as HTMLElement).id;
    ...
    it('describes an input number by the field error without repeating it', () => {
        ...
        const errorId = (fixture.debugElement.queryAll(By.css('.tumaet-ui-form-field-error'))[1].nativeElement as HTMLElement).id;
        expect(numberInput().getAttribute('aria-describedby')).toBe(errorId);
    });

    it('marks a select invalid from the field', () => {
        expect(selectTrigger().className).not.toContain('tumaet:border-state-danger');

        host.invalid.set(true);
        fixture.detectChanges();

        expect(selectTrigger().className).toContain('tumaet:border-state-danger');
    });

    it('marks an input number invalid from the field', () => {
        expect(numberInput().className).not.toContain('tumaet:border-state-danger');

        host.invalid.set(true);
        fixture.detectChanges();

        expect(numberInput().className).toContain('tumaet:border-state-danger');
    });
