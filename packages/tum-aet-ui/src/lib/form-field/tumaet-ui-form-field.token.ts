...
/**
 * Contract a {@link TumAetUiFormFieldComponent} exposes to the control it wraps, so the control can adopt the
 * field's label target, description, and validity without the consumer repeating any of them.
 *
 * A control opts in by injecting {@link TUM_AET_UI_FORM_FIELD} optionally. A component that renders another
 * TUM AET UI control inside its own view must shadow the token in `viewProviders` and forward what it needs, so the
 ...
 */
export interface TumAetUiFormFieldContext {
...
export const TUM_AET_UI_FORM_FIELD = new InjectionToken<TumAetUiFormFieldContext | null>('TumAetUiFormField');
