...
@Component({
    imports: [TumAetUiFormFieldComponent, TumAetUiInputDirective],
    template: `
        <tumaet-ui-form-field [label]="label()" [controlId]="controlId()" [required]="required()" [hint]="hint()" [invalid]="invalid()" [error]="error()">
            <input tumAetUiInput [tumAetUiInputId]="explicitId()" />
...
describe('TumAetUiFormFieldComponent', () => {
    ...
    const hintEl = () => fixture.debugElement.query(By.css('.tumaet-ui-form-field-hint'))?.nativeElement as HTMLElement | undefined;
    const errorEl = () => fixture.debugElement.query(By.css('.tumaet-ui-form-field-error'))?.nativeElement as HTMLElement | undefined;
    ...
    it('marks a required field with a marker hidden from assistive technology', () => {
        expect(fixture.debugElement.query(By.css('.tumaet-ui-form-field-required'))).toBeNull();

        host.required.set(true);
        fixture.detectChanges();

        const marker = fixture.debugElement.query(By.css('.tumaet-ui-form-field-required')).nativeElement as HTMLElement;
    ...
    it('propagates the invalid state to the projected control', () => {
        expect(control().className).not.toContain('tumaet:border-state-danger');

        host.invalid.set(true);
        fixture.detectChanges();

        expect(control().className).toContain('tumaet:border-state-danger');
    });
