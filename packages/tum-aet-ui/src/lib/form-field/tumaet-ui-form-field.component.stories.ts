...
const meta = {
    ...
    component: TumAetUiFormFieldComponent,
    decorators: [moduleMetadata({ imports: [TumAetUiInputDirective, TumAetUiSelectComponent] }), formStoryDecorator],
    args: {
        label: 'Login',
        required: false,
        hint: undefined,
    ...
    render: (args) => ({
        props: args,
        template: `
            <tumaet-ui-form-field ${argsToTemplate(args)} style="width: min(24rem, 100%);">
                <input tumAetUiInput placeholder="ab12cde" />
...
export const WrappingASelect: Story = {
    ...
    render: (args) => ({
        ...
        template: `
            <tumaet-ui-form-field ${argsToTemplate(args)} style="width: min(24rem, 100%);">
                <tumaet-ui-select [options]="['English', 'German']" placeholder="Select a language" />
            </tumaet-ui-form-field>
        `,
    }),
};
