...
import { TumAetUiTableComponent } from './tumaet-ui-table.component';
import { ColumnDef } from './tumaet-ui-table.types';
...
@Component({
    imports: [TumAetUiTableComponent],
    template: `
        <tumaet-ui-table [columns]="columns" [rows]="rows" [totalRecords]="2" [rowActions]="actions" />
...
describe('TumAetUiTableComponent', () => {
    let component: TumAetUiTableComponent<Row>;
    let fixture: ComponentFixture<TumAetUiTableComponent<Row>>;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [TumAetUiTableComponent, ActionsHostComponent, FontAwesomeTestingModule],
        }).compileComponents();
        fixture = TestBed.createComponent<TumAetUiTableComponent<Row>>(TumAetUiTableComponent);
    ...
    it('marks only the rows the highlight predicate accepts', async () => {
        ...
        fixture.detectChanges();
        await fixture.whenStable();
        const highlighted = fixture.debugElement.queryAll(By.css('tr[cdk-row]')).map((d) => (d.nativeElement as HTMLElement).classList.contains('tumaet-ui-table-row-highlighted'));
    ...
    it('marks no row when no highlight predicate is given', async () => {
        ...
        expect(rows.every((d) => !(d.nativeElement as HTMLElement).classList.contains('tumaet-ui-table-row-highlighted'))).toBe(true);
    });
