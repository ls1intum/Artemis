...
const tumUiGuideUrl = process.env.STORYBOOK_TUM_UI_GUIDE_URL || 'https://docs.artemis.tum.de/developer/guidelines/tum-ui-kit';

function managerTheme(theme: unknown) {
    const baseTheme = theme === 'dark' ? themes.dark : theme === 'light' ? themes.light : themes.normal;
    return {
        ...baseTheme,
        brandTitle: 'TUM UI package guide',
        brandUrl: tumUiGuideUrl,
        brandTarget: '_self',
    };
}
