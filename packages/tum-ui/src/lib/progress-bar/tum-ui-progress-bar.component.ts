 ...
 * content-border-color background, 6px radius), the same animated `bg-tum-ui-primary` fill, and the same label
...
export class TumUiProgressBarComponent {
    ...
    /** CSS color for the fill background; overrides the default `bg-tum-ui-primary` (drop-in for p-progressbar `[color]`). */
    ...
    protected readonly hostClasses = computed(() => `tum-ui-progress-bar bg-tum-ui-surface-200 dark:bg-tum-ui-surface-800 ${this.styleClass()}`.trim());
}
