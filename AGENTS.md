...
- Tests live in `src/test/java` (server), `src/main/webapp` (Jest or Vitest, co-located to TS components), and `src/test/playwright` (e2e). Docs are in `documentation/`; deployment helpers sit in `docker/`. API spec is generated at runtime by springdoc (served at `/v3/api-docs` and `/swagger-ui`); `src/main/webapp/app/openapi/` contains generated TypeScript client code.
...
- Testing: `npm test` (Jest + coverage), `npm run vitest:coverage` (Vitest + coverage); server `./gradlew test`. Java formatting: `./gradlew spotlessCheck` / `spotlessApply`.
...
- Run individual client jest tests like `npm run test:one -- --test-path-pattern='src/main/webapp/app/communication/directive/posting\.directive\.spec\.ts$'` (just adapt the path)
- Run individual client vitests like `npm run vitest -- src/main/webapp/app/fileupload/manage/update/file-upload-exercise-update.component.spec.ts` (just adapt the path)
