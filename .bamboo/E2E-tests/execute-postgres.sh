...
docker compose -f cypress-E2E-tests-postgres.yml pull artemis-cypress postgresql nginx
docker compose -f cypress-E2E-tests-postgres.yml build --build-arg WAR_FILE_STAGE=external_builder --no-cache --pull artemis-app
docker compose -f cypress-E2E-tests-postgres.yml up --exit-code-from artemis-cypress
exitCode=$?
cd ..
