...
class WorkerContextTest {
    ...
    void serverProfilesCannotLoadServerModules(String profiles) {
        new ApplicationContextRunner().withInitializer(new ConfigDataApplicationContextInitializer()).withUserConfiguration(AiWorkerApplication.class, ExternalServices.class)
                .withInitializer(context -> context.getBeanFactory().setConversionService(ApplicationConversionService.getSharedInstance()))
                .withPropertyValues("spring.profiles.active=" + profiles, "artemis.aiworker.workload=hyperion-generation", "artemis.aiworker.profile=java-gradle",
                        "artemis.aiworker.id=context-test", "artemis.aiworker.image=sha256:" + "a".repeat(64), "artemis.aiworker.max-concurrent-executions=4",
                        "spring.artemis.broker-url=tcp://broker.invalid:61617?sslEnabled=true", "spring.artemis.user=test", "spring.artemis.password=test",
                        "spring.ai.model.chat=openai", "spring.ai.openai.api-key=test-only", "management.tracing.export.otlp.enabled=false",
