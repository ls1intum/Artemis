...
/**
 * Unit tests for the mechanical per-stage gates of the SPEC, TESTS and STATEMENT stages. The differential ({@link DifferentialVerifier}) is mocked throughout; its own
 * build-and-parse behaviour is covered by {@code DifferentialVerifierTest}.
 */
class StageChecksTest {
    ...
    private static final class FakeSandbox extends FakeInteractiveSandbox {
        ...
        private String declarationProbe(String[] command, boolean solutionRepo) {
            ...
            boolean scaffoldType = StageChecks.specScaffoldTypes(effective == null ? "" : effective).contains(type);
    ...
    private static String specWithDesign(String designRows) {
        ...
        String publicApi = java.util.Arrays.stream(designRows.lines().toArray(String[]::new)).filter(line -> line.startsWith("|")).map(line -> line.split("\\|")[1].strip())
                .map(StageChecksTest::publicApiFor).collect(java.util.stream.Collectors.joining("\n"));
        return """
                # Exercise
    ...
    void setUp() {
        ...
        service = new StageChecks(verifier, approvedSpecs, true);
    ...
    void productionServiceRejectsCandidateAuthoredSpecAuthorityOutsideTheSpecStage() {
        service = new StageChecks(verifier, approvedSpecs);
    ...
    class HiddenVariantDeclaration {
        ...
        void readsTheDecisionFromTheTablesLastCell() {
            assertThat(StageChecks.specDeclaresHiddenVariants(specWithDesign("| Calculator | computes | stubbed |\n"))).isFalse();
            assertThat(StageChecks.specDeclaresHiddenVariants(specWithDesign("| Calculator | computes | stubbed |\n").replace("| S1 | Calculator | typical; zero | 3 | no |",
        ...
        void aSectionThatDeclinesHiddenVariantsInProseIsNotReadAsDeclaringThem() {
            ...
            assertThat(StageChecks.specDeclaresHiddenVariants(spec)).isFalse();
        }
    ...
    class Spec {
        ...
        void acceptsMarkdownEmphasisAroundAnOtherwiseExactTemplateStatusToken() {
            ...
            assertThat(StageChecks.specStudentCreatedTypes(sandbox.spec)).containsExactly("Calculator");
        }
        ...
        void preservesJavaIdentifierUnderscoresAndAcceptsAnOptionalTrailingTablePipe() {
            ...
            assertThat(StageChecks.hiddenVariantSeamIds(sandbox.spec)).containsExactly("S1");
        }
