...
class WorkerModelConfigurationTest {
    ...
    void createsTheActualProviderModelAndEngineWithoutAWebApplicationStack() {
        new ApplicationContextRunner().withInitializer(context -> context.getBeanFactory().setConversionService(ApplicationConversionService.getSharedInstance()))
                ...
                }).withBean(SandboxApi.class, () -> mock(SandboxApi.class)).withPropertyValues("artemis.aiworker.profile=java-gradle",
                        "spring.ai.openai.api-key=test-only-not-used", "spring.ai.openai.chat.options.model=test-model", "spring.ai.openai.max-retries=0")
                .run(context -> {
