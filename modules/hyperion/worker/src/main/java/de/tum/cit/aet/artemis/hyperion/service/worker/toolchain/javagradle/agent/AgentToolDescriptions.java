...
final class AgentToolDescriptions {
    ...
    static final String SEARCH = "Search a UTF-8 workspace file or directory recursively for an exact text fragment. Returns paths, line numbers, and matching lines without changing the workspace. Use an empty path to search the entire workspace. Use this instead of bash/grep.";

    static final String SEARCH_PATH = "workspace-relative file or directory path to search, or an empty string to search the entire workspace";
    ...
    static final String BASH = "Run a shell command in the workspace, e.g. {\"command\":\"ls -R\"}. Send the command as a single string (NOT a JSON array). Returns its exit code plus combined stdout/stderr. Use it for workspace inspection and targeted diagnostics when the structured tools do not expose enough detail. Do not treat raw Gradle or verify.sh commands as an acceptance verdict: use the dedicated verify tool, which runs the grader-equivalent check and parses its reports. Long output is truncated to the LAST 10000 characters; the COMPLETE output is saved in the sandbox to /tmp/hyperion/bash-<n>.log, so inspect earlier parts with sed/grep/head/tail on that file. Prefer search or targeted grep/sed over re-reading whole files.";
