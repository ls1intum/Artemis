 ...
 * stops, the turn budget is reached, cancellation is requested, or an error occurs. Manual scheduling preserves Hyperion's turn, compaction and submit-veto semantics; Spring AI
 * supplies provider and tool primitives. Artifact correctness is decided separately by the authoritative verifier.
 * <p>
...
public class AgentLoopRunner {
     ...
     * {@link ChatModel}'s own options. Resolved once so the model id, the reasoning effort, and the output-token limit cannot read from
     * different sources.
     */
    @Nullable
    ...
    /** Derivation constructor for {@link #forSettings}; shares the wrapped model, tool manager, and cooldown state of the deployment-wide runner. */
     ...
     * @param tools             the tools object whose {@code @Tool} methods are exposed to the model (supplied by the worker)
    ...
    private AgentLoopSession runSessionWithCallbacks(String systemPrompt, @Nullable List<Message> priorConversation, String userPrompt, @Nullable SubmitVetoAware tools,
        ...
        for (int turn = 1; turn <= maxTurns; turn++) {
            ...
            try {
                ...
                if (hasCause(e, SandboxUnavailableException.class)) {
    ...
    private static boolean hasOnlyRecoverableFileToolRejections(List<Message> conversation) {
        for (int i = conversation.size() - 1; i >= 0; i--) {
            if (conversation.get(i) instanceof ToolResponseMessage toolResponse) {
                List<ToolResponseMessage.ToolResponse> rejected = toolResponse.getResponses().stream()
                        .filter(response -> !"verify".equals(response.name()) && isErrorResponse(response.responseData())).toList();
    ...
    private ChatResponse callModel(Prompt prompt, int turn, BooleanSupplier cancelled, @Nullable Consumer<ChatResponse> usageSink, @Nullable Consumer<String> stepListener,
        ...
        for (int sample = 1; sample <= EMPTY_RESPONSE_SAMPLES; sample++) {
            ...
            try {
                ...
                if (response == null) {
                    emit(stepListener, "The AI service returned no response; usage could not be determined.");
                    return null;
                }
