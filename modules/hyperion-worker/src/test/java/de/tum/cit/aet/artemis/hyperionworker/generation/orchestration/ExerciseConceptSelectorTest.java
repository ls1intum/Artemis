...
class ExerciseConceptSelectorTest {
    ...
    @Test
    void conceptAdmissionRejection_stillReportsTheAgentTurnsItSpent() {
        ChatModel model = mock(ChatModel.class);
        when(model.call(any(Prompt.class))).thenReturn(new ChatResponse(List.of(new Generation(new AssistantMessage(THREE_CANDIDATES)))));
        AgentLoopRunner loop = new AgentLoopRunner(List.of(model), 128_000, Duration.ofMinutes(5), ProviderFailureCooldown.disabled());
        SpecFidelityCriticService critic = mock(SpecFidelityCriticService.class);
        when(critic.reviewConceptCandidates(eq("RAW BRIEF"), anyMap(), any(), any()))
                .thenReturn(new SpecFidelityCriticService.ConceptSelectionReview(true, null, List.of("No candidate passed.")));
        var events = new java.util.ArrayList<de.tum.cit.aet.artemis.hyperion.protocol.ProviderUsageUpdate>();
        var usage = new de.tum.cit.aet.artemis.hyperionworker.generation.WorkerUsageRecorder(100_000, 0.5, true, events::add);
    ...
    private static final class RecordingProgressSink implements GenerationProgressSink {
        ...
        private final List<GenerationActivity> activities = new java.util.ArrayList<>();
