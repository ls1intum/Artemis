...
import de.tum.cit.aet.artemis.hyperionworker.generation.verification.ContractWitnessOutcome.Disposition;
...
class ContractWitnessExecutionTest {
    ...
    private static GenerationInput javaExercise() {
        return new GenerationInput(42, "de.tum", "", false, Set.of());
    }
    ...
    private static InteractiveSandbox sandbox(TarArchiveInputStream first, TarArchiveInputStream... remaining) {
        ...
        when(sandbox.exec(anyString(), any(), any(String[].class))).thenReturn(new SandboxExecResult(0, "", "", false));
