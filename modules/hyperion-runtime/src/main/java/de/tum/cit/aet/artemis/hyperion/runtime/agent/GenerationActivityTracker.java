 ...
 * that crosses a thread boundary is the immutable {@link GenerationActivity} a snapshot produces, safely published through the distributed transcript map.
 */
public final class GenerationActivityTracker {

    /** The substep of context-isolated concept discovery, which runs before the staged authoring phases and therefore has no authoring stage of its own. */
     ...
     */
    @Nullable
    public GenerationActivity snapshot() {
     ...
     */
    public GenerationActivity waitingOnModel() {
        this.started = true;
        return activity(true);
    }

    private GenerationActivity activity(boolean waitingOnModel) {
        return new GenerationActivity(step, attempt, turn, waitingOnModel, modelCalls, toolCalls, filesWritten);
    }
}
