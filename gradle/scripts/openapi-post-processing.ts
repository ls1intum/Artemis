...
const replaceOneOfModelsWithUnionTypes = (project: Project, openApiSpecification: OpenApiSpecification) => {
    ...
    for (const [schemaName, referencedSchemaNames] of referencedUnionSchemas(openApiSpecification)) {
        ...
        for (const staleVariableStatement of sourceFile
            .getVariableStatements()
            .filter((statement) => statement.getDeclarations().every((declaration) => declaration.getName().startsWith(schemaName)))) {
...
const main = async () => {
    ...
    // generateApiDocs owns this shape. The postprocessor consumes only optional component fields.
        ...
        for (const importDeclaration of sourceFile.getImportDeclarations()) {
            for (const namedImport of importDeclaration.getNamedImports()) {
                ...
                const isUsed = refs.some((refGroup) =>
                    refGroup.getReferences().some((usage) => usage.getNode().getSourceFile() === sourceFile && usage.getNode() !== id),
                );
            ...
            const isEmpty =
                importDeclaration.getNamedImports().length === 0 && !importDeclaration.getDefaultImport() && !importDeclaration.getNamespaceImport();
            if (isEmpty) {
