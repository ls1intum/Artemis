...
* The operator's name  (e.g. university) and the operators contact information (administrator email address and name) can be specified in the ``application-{dev,prod}.yml`` configuration file under the keys ``info.operatorName``, ``info.contact`` and ``info.operatorAdminName``. These values are also displayed on the ``/about`` page. The university name is required, while the administrator's name is optional. Artemis uses this information for the :ref:`telemetry` service.
