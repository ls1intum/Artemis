...
To start the Artemis application server from the development environment, first import the project into IntelliJ and then make sure
to install the Spring Boot plugins to run the main class ``de.tum.in.www1.artemis.ArtemisApp``.

Before the application runs, you have to change some configuration options.
You can view the options directly in the configuration files in the folder ``src/main/resources/config``.
However, you have to be careful that you do not accidentally commit your passwords or configuration overrides which
are specific to your local setup.
Therefore, we strongly recommend, to create a new **override configuration file** called ``application-local.yml``
in the folder ``src/main/resources/config`` which is not version-controlled.
You can do so by using the ``application-local.yml.sample`` as a template.
This template already contains preset passwords/secrets for development purposes and the most important configuration
values you might have to override depending on your setup.
For a plain Artemis Instance without **version control** and **build** systems you just have to copy the template.

.. note::
   Be careful that you do not commit changes to ``application*.yml``.
   ...
   2) Copy the contents of ``application-local.yml.sample`` into the new file.
   ...
   ``local`` at the end to make sure the ``application-local.yml`` is considered.
   If you use different setups, you can create multiple files like ``application-local-atlassian.yml`` and
   ``application-local-gitlab-jenkins.yml`` with the overrides for your local setups.
   Depending on which setup you want to use, just copy the whole file to ``application-local.yml``.

.. hint::
    If you use a Database password, you need to adapt it in ``gradle/liquibase.gradle``.
...
``src/main/resources/config/application-local.yml`` like so:
...
``application-local.yml`` correctly, then you should be able to login
...
The Athene service is running on a dedicated machine and is addressed via HTTP.
The default *Athene* configuration values from ``src/main/resources/config/application-athene.yml`` are loaded,
if you activated the ``athene`` spring profile.
...
The Apollon conversion service is running on a dedicated machine and is addressed via HTTP.
The default *Apollon* configuration values from ``src/main/resources/config/application-apollon.yml`` are loaded,
if you activated the ``apollon`` spring profile.
...
2. Configure the credentials in ``application-local.yml`` in the folder ``src/main/resources/config`` as described above
