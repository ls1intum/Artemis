...
5. Saving and Deleting Entities with Cascade and OrphanRemoval
===============================================================

When using ``CascadeType.REMOVE`` and ``orphanRemoval = true`` on ``@OneToMany`` relationships, special care must be taken
to properly save new child entities and delete parent entities, especially when Hibernate L2 cache is enabled.
...
1. Set the child's reference to the parent: ``child.setParent(parent)``
2. Save the child entity via its repository: ``childRepository.save(child)``
3. Add to the parent's collection for in-memory consistency: ``parent.addChild(child)``

The foreign key is on the child side, so the parent does NOT need to be saved again after adding a child.
...
**Deleting Parent Entities with Cached Collections**
----------------------------------------------------

When explicitly deleting child entities before deleting a parent entity, you must reload the parent
with its collection to refresh the L2 cache. Without this step, Hibernate's cascade mechanism will
load stale references from the L2 cache and throw ``EntityNotFoundException``.

**The Problem:**

1. You explicitly delete child entities (e.g., via ``childRepository.deleteById()``)
2. The L2 cache still contains the old collection with references to deleted children
3. When you delete the parent, cascade tries to process the collection
4. Hibernate loads the collection from L2 cache (stale data)
5. ``EntityNotFoundException`` is thrown for already-deleted children

**The Solution:**

After deleting children explicitly, reload the parent entity with its collection before deleting:
...
.. code-block:: java
    ...
    public void delete(long participationId) {
        ...
        // Step 2: Reload parent WITH the collection to refresh L2 cache
        // The collection will now be empty since children were deleted
...
* ``ParticipationDeletionService.delete()`` - reloads participation with submissions
* ``ExampleParticipationService.deleteById()`` - reloads example participation with submissions
* ``ExerciseDeletionService`` - reloads exercise with participations
* ``ProgrammingExerciseDeletionService`` - reloads exercise with participations

.. warning::
    Do NOT skip the reload step when using ``CascadeType.REMOVE`` with L2 cache. The stale cache
    references will cause ``EntityNotFoundException`` during cascade processing.
