...
::
   ...
   --spring.profiles.active=dev,jenkins,gitlab,artemisscheduling
...
* Expanding the ``Environment`` section to reveal ``VM Options`` and setting them to
  ``-Dspring.profiles.active=dev,jenkins,gitlab,artemisscheduling``
...
.. code:: bash
   ...
   ./gradlew bootRun --args='--spring.profiles.active=dev,jenkins,gitlab,artemisscheduling'
