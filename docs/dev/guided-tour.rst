...
The optional ``course-group-students`` property is used to automatically add the given tutorial's course group to all the new created users.
This functionality can be extended to users with *instructor* or *teaching assistant* roles, adding the optional ``course-group-instructors`` and/or ``course-group-tutors`` properties.
