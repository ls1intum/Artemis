...
INSERT INTO participant_score(user_id, exercise_id, last_result_id, last_score, discriminator)
WITH last_submission AS (
    SELECT DISTINCT e.id as "exercise_id", p.student_id as "student_id", max(s.id) as "id_of_last_submission"
    FROM exercise e
    ...
    GROUP BY e.id, p.student_id
     ...
     last_result AS (
         SELECT DISTINCT ls.exercise_id, ls.student_id, r.id AS "last_result_id", r.score as "last_result_score"
...
SELECT lr.student_id, lr.exercise_id, lr.last_result_id, lr.last_result_score, 'SS'
FROM last_result lr;
...
INSERT INTO participant_score(team_id, exercise_id, last_result_id, last_score, discriminator)
WITH last_submission AS (
    SELECT DISTINCT e.id as "exercise_id", p.team_id as "team_id", max(s.id) as "id_of_last_submission"
    FROM exercise e
             JOIN participation p ON p.exercise_id = e.id
             JOIN submission s ON p.id = s.participation_id
    WHERE e.mode = 'TEAM'
    ...
    GROUP BY e.id, p.team_id
     ...
     last_result AS (
         SELECT DISTINCT ls.exercise_id, ls.team_id, r.id AS "last_result_id", r.score as "last_result_score"
         FROM last_submission ls
                  JOIN result r ON ls.id_of_last_submission = r.submission_id
         WHERE r.score IS NOT NULL
           AND r.completion_date IS NOT NULL
...
SELECT lr.team_id, lr.exercise_id, lr.last_result_id, lr.last_result_score, 'TS'
FROM last_result lr;
