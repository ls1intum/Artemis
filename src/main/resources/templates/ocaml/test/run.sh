...
if [ "$ONLY" != "all" ] && [ "$ONLY" != "compile" ] && [ "$ONLY" != "test" ]; then
...
find ../${studentParentWorkingDirectoryName}/ -type l | grep -q . && echo "Cannot build with symlinks in submission." && exit 1

# shellcheck disable=SC2046
eval $(opam env)

if [ "$ONLY" != "test" ] || $RUN_HIDDEN; then
