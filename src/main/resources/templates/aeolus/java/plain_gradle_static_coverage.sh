...
setupworkingdirectoryforcleanup () {
  ...
  local _sources="build/reports/testwise-coverage/tiaTests/tiaTests.json"
  local _directory
  _directory=$(dirname "${_sources}")
  mkdir -p /var/tmp/aeolus-results/"${_directory}"
  cp -a "${_sources}" /var/tmp/aeolus-results/build/reports/testwise-coverage/tiaTests/tiaTests.json
}
...
main () {
  ...
  bash -c "source ${_script_name} aeolus_sourcing;tests ${_current_lifecycle}"
  cd "${AEOLUS_INITIAL_DIRECTORY}"
}
