...
const numberMax =
    ...
    (control: AbstractControl): ValidationErrors | null => {
        const value = parseNumber(control.value);
        return value !== undefined && value > max ? { max: true } : null;
    };

const wholeNumber: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    const value = parseNumber(control.value);
    return value !== undefined && !Number.isInteger(value) ? { wholeNumber: true } : null;
};
...
export class PresentationAssessmentFormDialogComponent {
    ...
    protected readonly minPresentationDate = dayjs('1970-01-01T00:00:00');
    ...
    editForm = this.formBuilder.group(
        {
            ...
            maxPoints: [undefined as number | string | undefined, [numberRequired, wholeNumber, numberMin(1), numberMax(MAX_POINTS_UPPER_BOUND)]],
            resultPoints: [undefined as number | string | undefined, [optionalNumber, wholeNumber, numberMin(0), numberMax(MAX_POINTS_UPPER_BOUND)]],
