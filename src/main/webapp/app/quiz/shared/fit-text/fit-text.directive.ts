import { AfterViewInit, Directive, ElementRef, HostListener, OnChanges, OnDestroy, OnInit, Renderer2, SimpleChanges, inject, input } from '@angular/core';
...
export class FitTextDirective implements AfterViewInit, OnInit, OnChanges, OnDestroy {
    ...
    readonly fitText = input(true);
    readonly compression = input(1);
    readonly activateOnResize = input(true);
    readonly minFontSize = input<(number | 'inherit') | undefined>(0);
    readonly maxFontSize = input<(number | 'inherit') | undefined>(Number.POSITIVE_INFINITY);
    readonly delay = input(100);
    readonly innerHTML = input<any>();
    readonly fontUnit = input<'px' | 'em' | string>('px');
