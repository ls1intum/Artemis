...
/**
 * v6 relation graph. Improves on the v2 graph by auto-fitting the layout to the viewport (the v2 graph let
 * nodes overflow off-screen) and by rendering a legible legend. Otherwise identical: edit form, relation
 * list, prerequisite (solid) vs. harder-than (dashed) edges.
 */
@Component({
    selector: 'jhi-exercise-relation-graph-autofit',
    templateUrl: './exercise-relation-graph-autofit.component.html',
    styleUrl: './exercise-relation-graph-autofit.component.scss',
    imports: [FormsModule, NgxGraphModule, SelectModule, FaIconComponent],
})
export class ExerciseRelationGraphAutofitComponent {
    ...
    constructor() {
        // Re-fit the graph whenever the set of nodes changes (initial render, add/remove relation).
        effect(() => {
            this.nodes();
            // eslint-disable-next-line no-undef
            queueMicrotask(() => this.zoomToFit$.next({ autoCenter: true }));
        });
    }
