    ...
    public getTitle(type: EntityType, ids: number[]): Observable<string> {
        ...
        try {
            if (!type || !ids?.length || ids.some((id) => !id && id !== 0)) {
                captureException(new Error(`Supplied invalid parameters to getTitle() of EntityTitleService: Type=${type}, ids=${ids.join(', ')}`));
                return EMPTY;
            }
    ...
    public setTitle(type: EntityType, ids: (number | undefined)[], title: string | undefined) {
        ...
        try {
            if (!ids?.length || ids.some((id) => !id && id !== 0) || !title) {
                captureException(new Error(`Supplied invalid parameters to setTitle() of EntityTitleService: Type=${type}, ids=${ids.join(', ')}, title=${title}`));
                return;
            }
