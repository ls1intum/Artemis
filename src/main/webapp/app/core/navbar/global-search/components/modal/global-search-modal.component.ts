...
export class GlobalSearchModalComponent implements OnDestroy {
                    ...
                    return timer(SEARCH_DEBOUNCE_MS).pipe(
                        switchMap(() => {
                            ...
                            const search$ = useContentSearch
                                ? this.lectureSearchService.search(searchQuery, 10, courseIdsParam).pipe(
     ...
     * gestures rather than from applyTokens on purpose, so that only a user-initiated clear returns home.
     */
