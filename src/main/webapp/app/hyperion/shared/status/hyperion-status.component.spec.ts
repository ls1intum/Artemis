...
import { HyperionStatusComponent, HyperionStatusState } from './hyperion-status.component';

describe('HyperionStatusComponent', () => {
    let fixture: ComponentFixture<HyperionStatusComponent>;
    let host: HTMLElement;

    beforeEach(async () => {
        await TestBed.configureTestingModule({
            imports: [HyperionStatusComponent],
        }).compileComponents();
        fixture = TestBed.createComponent(HyperionStatusComponent);
    ...
    function withState(state: HyperionStatusState): void {
        fixture.componentRef.setInput('state', state);
        fixture.detectChanges();
    }

    function dot(): HTMLElement {
        return fixture.debugElement.query(By.css('.hyperion-status-indicator')).nativeElement;
    }
    ...
    it('keeps the accessible name when the label is hidden visually', () => {
        ...
        fixture.detectChanges();
        const label = fixture.debugElement.query(By.css('.hyperion-status-label')).nativeElement as HTMLElement;
    ...
    it('keeps one indicator element whatever the state is', () => {
        for (const state of ['queued', 'running', 'success', 'warning', 'danger', 'neutral', 'unknown'] as const) {
            withState(state);
            expect(fixture.debugElement.queryAll(By.css('.hyperion-status-indicator'))).toHaveLength(1);
            expect(dot().getAttribute('aria-hidden')).toBe('true');
        }
    });
});
