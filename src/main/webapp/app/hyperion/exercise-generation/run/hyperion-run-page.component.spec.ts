...
class MockGenerationService {
    ...
    readonly subscribeToStream = vi.fn((): Observable<HyperionGenerationEvent> => EMPTY);
...
describe('HyperionRunPageComponent', () => {
    ...
    it('acknowledges the same job only when it finishes while the page is open', () => {
        const stream = new Subject<HyperionGenerationEvent>();
        service.subscribeToStream.mockReturnValue(stream);
        render(status({ running: true, events: [event({ type: 'STARTED', phase: 'PREPARING' })] }));
        expect(registry.markSeen).not.toHaveBeenCalled();

        stream.next(event({ type: 'DONE', phase: 'SAVING', completionStatus: 'SUCCESS' }));
        fixture.detectChanges();
