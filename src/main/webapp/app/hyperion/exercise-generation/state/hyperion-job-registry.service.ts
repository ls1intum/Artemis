        ...
        from(exerciseIds)
            .pipe(
                mergeMap((exerciseId) => {
                    ...
                    return defer(() => this.generationService.getStatus(exerciseId)).pipe(
...
function isPersistedEntry(candidate: unknown): candidate is HyperionJobEntry {
    ...
    return (
        ...
        Number.isSafeInteger(entry.exerciseId) &&
        entry.exerciseId > 0 &&
        typeof entry.courseId === 'number' &&
        Number.isSafeInteger(entry.courseId) &&
        entry.courseId > 0 &&
        typeof entry.exerciseTitle === 'string' &&
        typeof entry.startedAt === 'string' &&
        typeof entry.status === 'string' &&
        (TERMINAL_STATUSES.has(entry.status) || entry.status === 'queued' || entry.status === 'running' || entry.status === 'cancelling') &&
        (entry.mode === 'GENERATE' || entry.mode === 'ADAPT') &&
