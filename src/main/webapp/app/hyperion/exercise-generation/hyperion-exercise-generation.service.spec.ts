...
describe('HyperionExerciseGenerationService', () => {
    ...
    it('asks the course for the derived metadata, because titles and short names are unique per course rather than per exercise', () => {
        service.suggestMetadata(7, 'a bounded stack exercise', ProjectType.GRADLE_GRADLE).subscribe((suggestion) => {
        ...
        expect(request.request.method).toBe('POST');
        expect(request.request.body).toEqual({ prompt: 'a bounded stack exercise', projectType: 'GRADLE_GRADLE' });
