...
describe('ResultHistoryDropdownComponent', () => {
                ...
                translateService.setTranslation('en', {
                    artemisApp: {
                        result: {
                            ...
                            aiFeedbackTooltip: 'This result was generated by Athena, an AI-powered assessment system.',
                            instructorFeedback: 'Instructor Feedback',
                            instructorFeedbackTooltip: 'This result was assessed by an instructor.',
                            resultString: {
    ...
    describe('AI feedback indicator', () => {
        it('should render an "AI Feedback" badge for Athena results', () => {
            const result = createResult(1, 50);
            ...
            expect(indicator?.textContent).toContain('AI Feedback');
            const tooltip = fixture.debugElement.query(By.css('[data-testid="ai-feedback-indicator"]')).injector.get(Tooltip);
            expect(tooltip.content).toBe('AI-based feedback can include mistakes. Consider checking important information.');
        });
        ...
        it('should use the failed tooltip for failed Athena results', () => {
            ...
            const tooltip = fixture.debugElement.query(By.css('[data-testid="ai-feedback-indicator"]')).injector.get(Tooltip);
            expect(tooltip.content).toBe('AI feedback generation failed.');
        });
        ...
        it('should use the in-progress tooltip for Athena results still being generated', () => {
            ...
            const tooltip = fixture.debugElement.query(By.css('[data-testid="ai-feedback-indicator"]')).injector.get(Tooltip);
            expect(tooltip.content).toBe('AI feedback is being generated.');
        });

        it('should not render the AI badge for normal automatic results', () => {
            const result = createResult(1, 50);
        ...
        it('should render an "Instructor Feedback" badge for manually assessed results', () => {
            const result = createResult(1, 50);
            ...
            const indicator = document.querySelector<HTMLElement>('[data-testid="instructor-feedback-indicator"]');
            expect(indicator).toBeTruthy();
            expect(indicator?.textContent).toContain('Instructor Feedback');
            expect(document.querySelector('[data-testid="ai-feedback-indicator"]')).toBeNull();
        });

        it('should render the "Instructor Feedback" badge for semi-automatic results too', () => {
            const result = createResult(1, 50);
            ...
            expect(document.querySelector('[data-testid="instructor-feedback-indicator"]')).toBeTruthy();
        });

        it('should not render the instructor badge for normal automatic results', () => {
            const result = createResult(1, 50);
            result.assessmentType = AssessmentType.AUTOMATIC;
            fixture.componentRef.setInput('sortedHistoryResults', [result]);
            fixture.detectChanges();
            ...
            expect(document.querySelector('[data-testid="instructor-feedback-indicator"]')).toBeNull();
        });
    });
    ...
    describe('template rendering', () => {
        ...
        it('should render unfinished Athena feedback as pending instead of a scored result', () => {
            ...
            // The AI origin badge survives the unfinished state so students can still tell this row apart from an
            // instructor-graded one, but the graded/practice metadata tag (which needs a completion date) stays hidden.
            expect(row?.querySelector('[data-testid="ai-feedback-indicator"]')).toBeTruthy();
            expect(row?.querySelectorAll('p-tag')).toHaveLength(1);
