...
export class CourseDashboardStudentPageComponent {
    ...
    readonly irisEnabled = signal<boolean>(false);
    ...
    private async loadIrisSettings(courseId: number): Promise<void> {
        if (this.profileInfo()?.activeProfiles.includes(PROFILE_IRIS)) {
            const combinedCourseSettings = await lastValueFrom(this.irisSettingsService.getCombinedCourseSettings(courseId));
            this.irisEnabled.set(!!combinedCourseSettings?.irisChatSettings?.enabled);
        }
    }
}
