...
export class FileUploadExamSubmissionComponent implements OnInit, ComponentCanDeactivate {
    ...
    ngOnInit() {
        ...
        this.fileUploadSubmissionService.getDataForFileUploadEditor(participationId).subscribe(
            (submission: FileUploadSubmission) => {
                ...
                this.exercise.studentParticipations = [this.participation];
                ...
                this.isLate =
                    ...
                    moment(this.participation.initializationDate).isAfter(this.exercise.dueDate);

                this.acceptedFileExtensions = this.exercise
                    .filePattern!.split(',')
    ...
    public submitExercise() {
        ...
        this.isSaving = true;
        this.fileUploadSubmissionService.update(this.submission!, this.exercise.id!, file).subscribe(
            (response) => {
    ...
    get isActive(): boolean {
        return !this.examMode && this.exercise && (!this.exercise.dueDate || moment(this.exercise.dueDate).isSameOrAfter(moment()));
    }
    ...
    get submitButtonTooltip(): string {
        ...
        if (!this.isLate) {
            if (this.isActive && !this.exercise.dueDate) {
