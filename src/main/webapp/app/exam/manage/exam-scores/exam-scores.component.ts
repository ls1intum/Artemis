...
export class ExamScoresComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        const getExamScoresObservable = this.examService.getExamScores(this.courseId(), this.examId());
        // alternative exam scores calculation using participant scores table
        const findExamScoresObservable = this.participantScoresService.findExamScores(this.examId()).pipe(catchError(() => of(new HttpResponse<ScoresDTO[]>())));
