...
// Namespaces the generated tab/panel element ids so several tab groups on one page never collide.
let nextGroupId = 0;
 ...
 * `aria-labelledby` links between a tab and its panel. Kept framework-neutral (no PrimeNG dependency),
 * part of the tum-aet-ui kit (future `@tumaet/ui-angular`).
 */
@Injectable()
export class TumUiTabsService {
    private readonly groupId = `tum-ui-tabs-${nextGroupId++}`;

    // The container's `value` model, registered once on construction. Wrapping the signal in a signal keeps
    // `active` fully reactive: swapping the source (register) and the source's own emissions both re-run it.
