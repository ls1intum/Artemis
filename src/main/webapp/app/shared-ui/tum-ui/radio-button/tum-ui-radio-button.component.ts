...
// Distinguishes "a form binding (ngModel/formControl) is driving selection" from "not": writeValue may be
// called with `undefined` (a legitimate deselected value), so a plain undefined can't tell the two apart.
const UNSET = Symbol('tum-ui-radio-unset');

/**
 * Owned radio button, part of the tum-aet-ui kit (future @tumaet/ui-angular).
 *
...
export class TumUiRadioButtonComponent implements ControlValueAccessor {
    ...
    // Selected value delivered through the CVA (ngModel / formControl). UNSET until a form binding writes.
    private readonly cvaValue = signal<unknown>(UNSET);
    // A form binding wins over the explicit input; otherwise fall back to the [modelValue] input.
    ...
    // Circle color set (bg/border) via sanctioned token utilities, toggled by state — same approach as the
    // checkbox. Structure, the inner dot scale, hover overlay, and focus ring live in the stylesheet.
    ...
    // The inner dot: primary-contrast (white) when checked, muted when a checked radio is disabled (Aura).
    protected readonly iconClasses = computed(() => (this.isDisabled() ? 'bg-surface-500 dark:bg-surface-400' : 'bg-surface-0'));
