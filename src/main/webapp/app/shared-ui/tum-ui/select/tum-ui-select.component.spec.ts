...
// the ARIA semantics, the CVA value flow, and the emitted values — never pixel positions (see the kit README).
describe('TumUiSelectComponent', () => {
    ...
    afterEach(() => {
        // Dispose any open overlay so the portaled panel does not leak into the next spec.
        fixture.destroy();
        vi.restoreAllMocks();
    });
    ...
    it('keyboard: ArrowDown advances the active option and Enter selects it', () => {
        const emitSpy = vi.spyOn(component.onChange, 'emit');
        openPanel();
        // Panel opens with the first option active; ArrowDown → second (Bravo).
...
describe('TumUiSelectComponent with reactive formControl', () => {
    ...
    it('reflects and updates the FormControl value, and honors control.disable()', async () => {
        ...
        // Selecting writes the resolved value into the control.
        fixture.debugElement.query(By.css('[data-testid="tum-ui-select-trigger"]')).nativeElement.click();
        fixture.detectChanges();
        ...
        // control.disable() flows through setDisabledState → the trigger is disabled.
