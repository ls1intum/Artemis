...
/**
 * Owned segmented single-select control, part of the tum-aet-ui kit (future @tumaet/ui-angular).
 *
 ...
 * selected segment is highlighted with `bg-primary` per the kit house style, dark-mode-correct for free.
 *
...
export class TumUiSelectButtonComponent implements ControlValueAccessor {
    ...
    // Roving tabindex anchor: the selected option is tab-reachable, or the first option when nothing is selected.
    protected readonly focusableIndex = computed(() => {
        const index = this.normalizedOptions().findIndex((option) => option.selected);
        return index >= 0 ? index : 0;
    });

    // Selected segment is highlighted with bg-primary (kit house style); unselected rides the surface ramp with a
    // muted label. Padding / borders / radius are structural (see the stylesheet); size only scales the font, as
    // in Aura togglebutton.
    protected optionClasses(selected: boolean): string {
        const sizeClass = this.size() === 'small' ? 'text-sm' : this.size() === 'large' ? 'text-lg' : 'text-base';
        const state = selected ? 'bg-primary text-surface-0 border-primary' : 'bg-surface-100 text-muted-color border-surface-200 dark:bg-surface-800 dark:border-surface-700';
    ...
    protected select(option: NormalizedOption): void {
        ...
        let next: unknown;
        if (option.selected) {
            // Re-clicking the selected option clears it, unless allowEmpty is disabled (then it is a no-op).
            if (!this.allowEmpty()) {
                return;
            }
            next = undefined;
    ...
    protected onKeydown(event: KeyboardEvent, index: number): void {
        ...
        switch (event.key) {
            ...
            case 'End':
                ...
                break;
            default:
                // Space / Enter activate the focused option natively (it is a real <button>), so they are not
                // handled here — doing so would fire the selection twice in a real browser.
                return;
        }
