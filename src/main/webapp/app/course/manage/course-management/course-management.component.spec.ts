...
import { CourseManagementComponent } from 'app/course/manage/course-management/course-management.component';
...
describe('CourseManagementComponent', () => {
    ...
    it('should initialize', () => {
        vi.spyOn(service, 'getCourseOverview').mockReturnValue(of(new HttpResponse({ body: [courseDetails187, courseDetails188] })));
        vi.spyOn(service, 'getExercisesForManagementOverview').mockReturnValue(of(new HttpResponse({ body: [courseWithExercises187, courseWithExercises188] })));
        vi.spyOn(service, 'getStatsForManagementOverview').mockReturnValue(of(new HttpResponse({ body: [] })));
        vi.spyOn(service, 'getWithUserStats').mockReturnValue(of(new HttpResponse({ body: [course187, course188] })));
    ...
    it('should correctly sort unique semester names', () => {
        ...
        component.courses.set([course1, course2, course3, course4, course5]);
    ...
    it('should correctly sort courses into semesters', () => {
        ...
        component.courses.set([course1, course2, course3, course4, course5, course6]);
        component.courseSemesters.set(['SS20', 'WS19', 'SS19', '']);

        // Simulate that course1 and course2 were recently accessed
        vi.spyOn(courseAccessStorageService, 'getLastAccessedCourses').mockReturnValue([1, 2]);
