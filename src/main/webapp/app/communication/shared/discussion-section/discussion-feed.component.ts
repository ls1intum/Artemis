import { AfterViewInit, Component, ElementRef, OnDestroy, effect, inject, input, output, viewChild, viewChildren } from '@angular/core';
...
import { faArrowLeft, faLongArrowRight } from '@fortawesome/free-solid-svg-icons';
...
export class DiscussionFeedComponent extends CourseDiscussionDirective implements AfterViewInit, OnDestroy {
    ...
    readonly noChannelAvailable = output<void>();
    ...
    /**
     * Set the channel for the discussion feed, either for a lecture or an exercise
     * @param courseId
     */
        ...
        const getChannel = () => {
            return {
                next: (channel: ChannelDTO) => {
                    ...
                    if (!this.channel?.id) {
                        this.isNoChannelAvailable = true;
                        this.noChannelAvailable.emit();
                        return;
                    }
    ...
    handleScrollOnNewMessage = () => {
        const containerHeight = this.content()?.nativeElement.clientHeight;
        if ((this.posts.length > 0 && this.content()?.nativeElement.scrollTop === 0 && this.page === 1) || this.previousScrollDistanceFromTop === containerHeight) {
