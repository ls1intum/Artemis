(function() {
    ...
    JhiWebsocketService.$inject = ['$rootScope', '$window', '$cookies', '$http', '$q', '$injector'];
    ...
    function JhiWebsocketService ($rootScope, $window, $cookies, $http, $q, $injector) {
        ...
        /**
         * Connects to a websocket. The param angStompClient is the stompClient given by the Angular 5 websocket
         * service. It is called when the Angular 5 websocket service connects.
         * @param angStompClient
         */
        function connect (angStompClient) {
            if (connectionStatus === ConnectionStatus.CONNECTED) {
                return; // don't connect, if already connected
            }
            connectionStatus = ConnectionStatus.CONNECTING;
            if (typeof angStompClient == 'undefined' || angStompClient == null) {
                //building absolute path so that websocket doesn't fail when deploying with a context path
                var loc = $window.location;
                var url = '//' + loc.host + loc.pathname + 'websocket/tracker';
                const authToken = $injector.get('AuthServerProvider').getToken();
                if (authToken) {
            ...
            var stateChangeStart;
            stompClient = angStompClient;
            if (stompClient.connected) {
                // angularjs got a stompclient from the angular 5 app
