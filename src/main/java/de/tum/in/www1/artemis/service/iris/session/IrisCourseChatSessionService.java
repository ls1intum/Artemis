...
public class IrisCourseChatSessionService implements IrisChatBasedFeatureInterface<IrisCourseChatSession>, IrisRateLimitedFeatureInterface {
    ...
    public void requestAndHandleResponse(IrisCourseChatSession session) {
        ...
        var course = courseRepository.findById(chatSession.getCourse().getId()).orElseThrow();
    ...
    public void handleStatusUpdate(CourseChatJob job, PyrisCourseChatStatusUpdateDTO statusUpdate) {
        var session = (IrisCourseChatSession) irisSessionRepository.findByIdWithMessagesAndContents(job.getSessionId());
