...
public class QuizExerciseResource {
    ...
    * @return the ResponseEntity with status 201 (Created) and with body the new quizExercise, or
    *     with status 400 (Bad Request) if the quizExercise has already an ID
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
    @PostMapping("/quiz-exercises")
    @PreAuthorize("hasAnyRole('TA', 'INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<QuizExercise> createQuizExercise(@RequestBody QuizExercise quizExercise)
            ...
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME, "idexists", "A new quizExercise cannot already have an ID"))
                    .body(null);
        }
            ...
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME,
                                    "courseNotFound",
                                    "The course belonging to this quiz exercise does not exist"))
                    .body(null);
        }
        ...
        if (!quizExercise.isValid()) {
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME, "invalidQuiz", "The quiz exercise is invalid"))
                    .body(null);
        }
    ...
    * @return the ResponseEntity with status 200 (OK) and with body the updated quizExercise, or with
    *     status 400 (Bad Request) if the quizExercise is not valid, or with status 500 (Internal
    *     Server Error) if the quizExercise couldn't be updated
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
        ...
        if (originalQuiz == null) {
            return ResponseEntity.notFound()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME,
                                    "quizExerciseNotFound",
                                    "The quiz exercise does not exist yet. Use POST to create a new quizExercise."))
                    .build();
        }
    ...
    * @return the ResponseEntity with status 200 (OK) and with body the quizExercise, or with status
    *     404 (Not Found)
    */
    ...
    public ResponseEntity<QuizExercise> getQuizExercise(@PathVariable Long quizExerciseId) {
        ...
        QuizExercise quizExercise =
                quizExerciseService.findOneWithQuestionsAndStatistics(quizExerciseId);
    ...
    /**
    * GET /quiz-exercises/:quizExerciseId/recalculate-statistics : recalculate all statistics in case
    * something went wrong with them
    *
    * @param quizExerciseId the id of the quizExercise for which the statistics should be
    *     recalculated
    * @return the ResponseEntity with status 200 (OK) and with body the quizExercise, or with status
    *     404 (Not Found)
    */
    ...
    /**
    * GET /quiz-exercises/:id/for-student : get the "id" quizExercise. (information filtered for
    * students)
    *
    ...
    *     "open-for-practice")
    * @return the response entity with status 204 if quiz was started, appropriate error code
    *     otherwise
    */
    @PostMapping("/quiz-exercises/{id}/{action}")
    @PreAuthorize("hasAnyRole('TA', 'INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<String> performActionForQuizExercise(
        ...
        switch (action) {
            case "start-now":
                ...
                if (quizExercise.isStarted()) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body("{\"message\": \"Quiz has already started.\"}");
                }
            ...
            case "set-visible":
                ...
                if (quizExercise.isVisibleToStudents()) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body("{\"message\": \"Quiz is already visible to students.\"}");
                }
            ...
            case "open-for-practice":
                ...
                if (!quizExercise.isStarted() || quizExercise.getRemainingTime() > 0) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body("{\"message\": \"Quiz hasn't ended yet.\"}");
                }
                // check if quiz is already open for practice
                if (quizExercise.isIsOpenForPractice()) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body("{\"message\": \"Quiz is already open for practice.\"}");
                }
            ...
            default:
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("{\"message\": \"Unknown action: " + action + "\"}");
        }
    ...
    *
    * <p>1. reset not allowed changes and set flag updateResultsAndStatistics if a recalculation of
    * results and statistics is necessary 2. save changed quizExercise 3. if flag is set: -> change
    * results if an answer or a question is set invalid -> recalculate statistics and results and
    * save them.
    *
    * @param quizExercise the quizExercise to re-evaluate
    * @return the ResponseEntity with status 200 (OK) and with body the re-evaluated quizExercise, or
    *     with status 400 (Bad Request) if the quizExercise is not valid, or with status 500
    *     (Internal Server Error) if the quizExercise couldn't be re-evaluated
    */
    @PutMapping("/quiz-exercises-re-evaluate")
    @PreAuthorize("hasAnyRole('INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<QuizExercise> reEvaluateQuizExercise(
            @RequestBody QuizExercise quizExercise) {
        log.debug("REST request to re-evaluate QuizExercise : {}", quizExercise);
        if (quizExercise.getId() == null) {
            return ResponseEntity.notFound()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME, "quizExerciseWithoutId", "The quiz exercise doesn't have an ID."))
                    .build();
        }
        QuizExercise originalQuizExercise =
                quizExerciseService.findOneWithQuestionsAndStatistics(quizExercise.getId());
        ...
        boolean updateOfResultsAndStatisticsNecessary =
                quizExercise.checkIfRecalculationIsNecessary(originalQuizExercise);
