...
public class BuildPlanResource {
    ...
    /**
     * Returns the build plan for a given programming exercise.
     * This is for the members of this course to see the build plan for the exercise if this has been enabled in
    ...
    public ResponseEntity<String> getBuildPlanForStudent(@PathVariable Long exerciseId) {
        log.debug("REST request to get build plan for programming exercise with id {}", exerciseId);
        ...
        if (!programmingExercise.isPublishBuildPlanUrl()) {
            throw new AccessForbiddenException("build plan of the exercise", exerciseId);
        }

        return ResponseEntity.ok().contentType(MediaType.TEXT_PLAIN).body(buildPlan.getBuildPlan());
    }
