...
public class ProgrammingSubmissionService extends SubmissionService {
    ...
    /**
     * See {@link SubmissionService#filterOutTestRunSubmissions}
     * Programming exercises have multiple submissions. We can only be sure that the initial submission of the
     * participation has been marked as a test run submission ({@link StudentParticipation#getStudent()} equals {@link Result#getAssessor()}).
     * Therefore we need to remove all participations which contain at least one submission marked as a test run.
     *
    ...
    public <T extends Submission> List<T> filterOutTestRunSubmissions(List<T> submissions, Exercise exercise) {
        if (submissions.size() > 0 && submissions.get(0) instanceof ProgrammingSubmission) {
            var instructors = super.userService.getInstructors(exercise.getExerciseGroup().getExam().getCourse());
            final var immutableSubmissionsList = List.copyOf(submissions);
            var groupedByParticipation = immutableSubmissionsList.stream().collect(Collectors.groupingBy(Submission::getParticipation));
            for (var participation : groupedByParticipation.keySet()) {
                final var student = ((StudentParticipation) participation).getStudent().get();
                if (groupedByParticipation.get(participation).stream()
                        .noneMatch(submission -> student.equals(submission.getResult().getAssessor()) && instructors.contains(student))) {
                    groupedByParticipation.remove(participation);
                }
            }
            groupedByParticipation.keySet().stream().map(groupedByParticipation::get).forEach(submissions::removeAll);
            return submissions;
        }
        return super.filterOutTestRunSubmissions(submissions, exercise);
    }

    /**
     * See {@link SubmissionService#getTestRunSubmissionsOfInstructor}.
     * Calls {@link ProgrammingSubmissionService#linkLatestProgrammingSubmissionToTestRunResult} to display the correct test run result and submission.
     * @param exercise the exercise for which we request the submissions
     * @param instructor the owner of the test run
     * @param <T> Placeholder for subclass of {@link Submission}, {@link ProgrammingSubmission}
     * @return the list of programming exercise test run submissions
     */
    @Override
    public <T extends Submission> List<T> getTestRunSubmissionsOfInstructor(Exercise exercise, User instructor) {
        var submissions = super.getTestRunSubmissionsOfInstructor(exercise, instructor);
        return (List<T>) linkLatestProgrammingSubmissionToTestRunResult(exercise, submissions, instructor);
    }

    /**
     * ProgrammingParticipations can have more than one submission.
     * In order to display the latest submission and result for the test run, we need to link those accordingly.
     * This is done by connecting the latest submission with the latest result.
     * The latest result can either be the initial result we create for test run submissions or a different one from the test suite.
     * In that case we need to mark it as a test run result by adding the student as the assesor.
     * @param exercise the exercise for which we request the submissions
     * @param instructor the owner of the test run
     * @return the list of programming exercise test run submissions
     */
    private List<Submission> linkLatestProgrammingSubmissionToTestRunResult(Exercise exercise, List<Submission> submissions, User instructor) {
        Submission latestSubmission;
        if (!submissions.isEmpty()) {
            var participations = participationService.findByExerciseAndStudentIdWithEagerResultsAndSubmissions(exercise, instructor.getId());
            if (!participations.isEmpty()) {
                var participation = participations.get(0);
                var latestResult = participation.findLatestResult();
                if (latestResult.getAssessor() == null || !instructor.equals(latestResult.getAssessor())) {
                    latestResult.setAssessor(instructor);
                    resultRepository.save(latestResult);
                }
                var latestSubmissionOptional = participation.findLatestSubmission();
                if (latestSubmissionOptional.isPresent()) {
                    latestSubmission = latestSubmissionOptional.get();
                    if (!latestSubmission.getResult().equals(latestResult)) {
                        latestSubmission.setResult(latestResult);
                        submissionRepository.save(latestSubmission);
                    }
                    submissions = List.of(latestSubmission);
                }
            }
        }
        return submissions;
