...
public abstract class Submission extends DomainObject implements Comparable<Submission> {
    ...
    private List<Result> filterNonAutomaticResults() {
        return results.stream().filter(result -> result == null || !(result.isAutomatic() || result.isAi())).toList();
    }
    ...
    public void removeAutomaticResults() {
        this.results = this.results.stream().filter(result -> result == null || !(result.isAutomatic() || result.isAi())).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public List<Result> getManualResults() {
        return results.stream().filter(result -> result != null && !result.isAutomatic() && !result.isAi()).collect(Collectors.toCollection(ArrayList::new));
    }
