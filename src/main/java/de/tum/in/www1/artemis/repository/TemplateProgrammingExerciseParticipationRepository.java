...
public interface TemplateProgrammingExerciseParticipationRepository
        extends JpaRepository<TemplateProgrammingExerciseParticipation, Long>, JpaSpecificationExecutor<TemplateProgrammingExerciseParticipation> {
    ...
    default TemplateProgrammingExerciseParticipation findByExerciseIdWithDynamicFetchElseThrow(long exerciseId, Collection<TemplateParticipationFetchOptions> fetchOptions)
        ...
        final Specification<TemplateProgrammingExerciseParticipation> specification = (root, query, criteriaBuilder) -> {
            for (var option : fetchOptions) {
                if (option == TemplateParticipationFetchOptions.SubmissionsAndResults) {
                    // Handle nested fetch
