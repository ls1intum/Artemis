...
public class GitlabService implements VersionControlService {
    ...
    public VcsRepositoryUrl copyRepository(String sourceProjectKey, String sourceRepositoryName, String targetProjectKey, String targetRepositoryName) {
        // createRepositoryInternal(getNamespaceFromUrl(baseRepositoryUrl), getProjectNameFromUrl(baseRepositoryUrl), username);
        // TODO
        return null;
    }
    ...
    public void configureRepository(URL repositoryUrl, String username) {
        ...
        if (username.startsWith(USER_PREFIX)) {
            ...
            if (!userExists(username) && (!LDAP_DN_PREFIX.equals("") || !LDAP_DN_SUFFIX.equals(""))) {
                ...
                createUserExternalProvider(username, user.getEmail(), displayName, "ldapmain", LDAP_DN_PREFIX + username + LDAP_DN_SUFFIX); // This will create a User linked to the
    ...
    public URL getRepositoryWebUrl(ProgrammingExerciseParticipation participation) {
        try {
            return new URL(GITLAB_SERVER_URL + "/" + getNamespaceFromUrl(participation.getRepositoryUrlAsUrl()) + "/" + getProjectNameFromUrl(participation.getRepositoryUrlAsUrl())
                    + "/tree/master"); // TODO: Don't hardcode master branch
        }
    ...
    public void createBambooService(String namespace, String project, String bambooUrl, String buildKey, String bambooUsername, String bambooPassword) {
        ...
        try {
            restTemplate.exchange(baseUri, HttpMethod.PUT, entity, Map.class);
        }
     ...
     */
    private void createRepositoryInternal(String namespace, String baseProjectName, String username) throws GitlabException {
        /*
         * In Gitlab, you cannot fork an existing repository with another name. Therefor we create a new repository and specify the base repository as import_url. We authenticate
         * with the private token and the according username.
         */
        ...
        body.put("import_url", GITLAB_SERVER_URL.getProtocol() + "://" + GITLAB_USER + ":" + GITLAB_PRIVATE_TOKEN + "@" + GITLAB_SERVER_URL.getAuthority()
                + GITLAB_SERVER_URL.getPath() + "/" + namespace + "/" + baseProjectName + ".git");
        ...
        try {
            response = restTemplate.exchange(GITLAB_SERVER_URL + API_PATH + "projects/", HttpMethod.POST, entity, Map.class);
        }
        catch (HttpClientErrorException e) {
    ...
    private void giveWritePermission(URL repositoryUrl, String username) throws GitlabException {
        ...
        RestTemplate restTemplate = new RestTemplate();
        try {
            restTemplate.exchange(baseUri, HttpMethod.POST, entity, Map.class);
        }
        catch (HttpClientErrorException e) {
    ...
    private Map<Integer, String> getExistingWebHooks(String urlEncodedIdentifier) {
        ...
        try {
            response = restTemplate.exchange(baseUri, HttpMethod.GET, entity, List.class);
        }
        catch (Exception e) {
    ...
    private Integer getUserId(String username, boolean expectExistence) throws GitlabException {
        ...
        ResponseEntity<List> response;
        try {
            response = restTemplate.exchange(GITLAB_SERVER_URL + API_PATH + "users?username=" + username, HttpMethod.GET, entity, List.class);
        }
        catch (Exception e) {
    ...
    private Integer getNamespaceId(String namespace) throws GitlabException {
        ...
        try {
            /*
             * In Gitlab, a namespace can either be an user or a group. As all participations of a specified exercise should be in the same namespace, only a group-namespace is
             * applicable.
             */
            response = restTemplate.exchange(GITLAB_SERVER_URL + API_PATH + "groups/" + namespace, HttpMethod.GET, entity, Map.class);
        }
        catch (Exception e) {
    ...
    private URL buildCloneUrl(String namespace, String projectName, String username) {
        URL cloneUrl = null;
        try {
            cloneUrl = new URL(GITLAB_SERVER_URL.getProtocol() + "://" + username + "@" + GITLAB_SERVER_URL.getAuthority() + GITLAB_SERVER_URL.getPath() + "/" + namespace + "/"
                    + projectName + ".git");
        }
        catch (MalformedURLException e) {
