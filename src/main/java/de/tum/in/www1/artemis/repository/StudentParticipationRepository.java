...
public interface StudentParticipationRepository extends JpaRepository<StudentParticipation, Long> {
    ...
    @Query("select distinct participation from StudentParticipation participation left join fetch Submission submission left join fetch submission.result r where participation.exercise.exerciseGroup.exam.id = :#{#examId} and r.rated = true")
