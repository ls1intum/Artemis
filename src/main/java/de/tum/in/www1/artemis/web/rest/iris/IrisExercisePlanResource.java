...
public class IrisExercisePlanResource {
    ...
    public IrisExercisePlanResource(IrisSessionService irisSessionService,
    ...
    public ResponseEntity<Void> executePlan(
            @PathVariable Long sessionId, @PathVariable Long messageId, @PathVariable Long planId
    ) {
    ...
    @EnforceAtLeastEditor
    public ResponseEntity<IrisExercisePlanMessageContent> updatePlan(
            @PathVariable Long sessionId, @PathVariable Long messageId, @PathVariable Long planId,
    ...
    private static IrisSession checkOwnershipAndReturnSession(Long sessionId, Long messageId, IrisMessageContent existingPlan) {
        ...
        if (!Objects.equals(message.getId(), messageId)) {
            throw new ConflictException("The exercise plan does not belong to the message",
                    "IrisExercisePlanMessageContent", "irisMessageContentConflict");
        }
        var session = message.getSession();
        if (!Objects.equals(session.getId(), sessionId)) {
            throw new ConflictException("The message does not belong to the session",
                    "IrisMessage", "irisMessageSessionConflict");
        }
        return session;
    }
}
