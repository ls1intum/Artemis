...
public class ExerciseService {
    ...
    public StatsForDashboardDTO populateCommonStatistics(Exercise exercise, boolean examMode) {
        ...
        if (examMode) {
            numberOfComplaints = complaintRepository.countByResultSubmissionParticipationExerciseIdAndComplaintTypeIgnoreTestRuns(exerciseId, ComplaintType.COMPLAINT);
        }
        else {
    ...
    public void filterForCourseDashboard(Exercise exercise, Set<StudentParticipation> participations, boolean isStudent) {
        ...
        relevantParticipations.forEach(participation -> {
            ...
            if (latestSubmissionWithRatedResult != null && latestSubmissionWithRatedResult.getLastResult() != null) {
                results = Set.of(latestSubmissionWithRatedResult.getLastResult());
                // filter sensitive information about the assessor if the current user is a student
                if (isStudent) {
                    latestSubmissionWithRatedResult.getLastResult().filterSensitiveInformation();
                }
            }

            // filter sensitive information in submission's result
            if (isStudent && submission != null && submission.getLastResult() != null) {
                submission.getLastResult().filterSensitiveInformation();
            }
    ...
    public void reEvaluateExercise(Exercise exercise, boolean deleteFeedbackAfterGradingInstructionUpdate) {
        ...
        List<Result> results = resultRepository.findWithEagerSubmissionAndFeedbackBySubmission_ParticipationExerciseId(exercise.getId());
