...
public class CompassStatistics {
    ...
    public CompassStatistics(ModelIndex modelIndex, AutomaticAssessmentController automaticAssessmentController) {
    ...
    public JsonObject getStatistics() {
        ...
        for (Map.Entry<String, Long> umlElementEntry : modelIndex.getUniqueElementEntries()) {
            String jsonElementId = umlElementEntry.getKey();
            JsonObject uniqueElement = new JsonObject();
            // TODO: Melih Oezbeyli Add functionality to get element name from unique element list
            // uniqueElement.addProperty("name", umlElement.toString());
            uniqueElement.addProperty("apollonId", jsonElementId);
            boolean hasConflict = hasConflict(modelIndex.getSimilarityId(jsonElementId));
            if (hasConflict) {
                numberOfConflicts++;
            }
            uniqueElement.addProperty("conflicts", hasConflict);
            uniqueElements.add(String.valueOf(modelIndex.getSimilarityId(jsonElementId)), uniqueElement);
        }
        ...
        for (Map.Entry<Long, UMLDiagram> modelEntry : modelIndex.getModelEntries()) {
    ...
    public void printStatistic(long exerciseId, List<Result> finishedResults) {
        ...
        long numberOfModelElements = 0;// modelElementMapping.size();
