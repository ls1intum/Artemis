...
public class JSONParser {
    ...
    /**
    * Process a json object retrieved from a json formatted file to retrieve an UML model TODO adapt
    * the parser to support different UML diagrams
    *
    ...
    */
    // TODO CZ: refactor this (extract buildModelFromJSON to specific UML classes, e.g.
    // UMLClass.buildModelFromJSON() to get the parsed class)
    public static UMLClassModel buildModelFromJSON(JsonObject root, long modelId) throws IOException {
            ...
            if (UMLClassType.getTypesAsList().contains(elementType)) {
                ...
                for (JsonElement attributeId : element.getAsJsonArray(JSONMapping.elementAttributes)) {
                    ...
                    String[] attributeNameArray =
                            attribute.get(JSONMapping.elementName).getAsString().replaceAll(" ", "").split(":");
                    ...
                    UMLAttribute newAttr =
                            new UMLAttribute(
                                    attributeName, attributeType, attribute.get(JSONMapping.elementID).getAsString());
                ...
                if (element.has(JSONMapping.elementOwner)
                        && !element.get(JSONMapping.elementOwner).isJsonNull()) {
        ...
        for (JsonElement rel : relationships) {
            ...
            String sourceJSONID =
                    relationshipSource.get(JSONMapping.relationshipEndpointID).getAsString();
            String targetJSONID =
                    relationshipTarget.get(JSONMapping.relationshipEndpointID).getAsString();

            UMLClass source = umlClassMap.get(sourceJSONID);
            UMLClass target = umlClassMap.get(targetJSONID);

            JsonElement relationshipSourceRole =
                    relationshipSource.has(JSONMapping.relationshipRole)
                            ? relationshipSource.get(JSONMapping.relationshipRole)
                            : JsonNull.INSTANCE;
            JsonElement relationshipTargetRole =
                    relationshipTarget.has(JSONMapping.relationshipRole)
                            ? relationshipTarget.get(JSONMapping.relationshipRole)
                            : JsonNull.INSTANCE;
            JsonElement relationshipSourceMultiplicity =
                    relationshipSource.has(JSONMapping.relationshipMultiplicity)
                            ? relationshipSource.get(JSONMapping.relationshipMultiplicity)
                            : JsonNull.INSTANCE;
            JsonElement relationshipTargetMultiplicity =
                    relationshipTarget.has(JSONMapping.relationshipMultiplicity)
                            ? relationshipTarget.get(JSONMapping.relationshipMultiplicity)
                            : JsonNull.INSTANCE;
            ...
            if (source != null && target != null) {
                UMLClassRelationship newRelation =
                        new UMLClassRelationship(
                                source,
                                target,
                                relationshipType,
                                ...
                                relationshipSourceMultiplicity.isJsonNull()
                                        ? ""
                                        : relationshipSourceMultiplicity.getAsString(),
                                relationshipTargetMultiplicity.isJsonNull()
                                        ? ""
                                        : relationshipTargetMultiplicity.getAsString());
        ...
        return new UMLClassModel(
                new ArrayList<>(umlPackageMap.values()),
                new ArrayList<>(umlClassMap.values()),
                umlAssociationList,
                modelId);
    }
    ...
    private static Map<String, JsonObject> generateJsonElementMap(JsonArray elements) {
        ...
        elements.forEach(
                element ->
                        jsonElementMap.put(
                                element.getAsJsonObject().get("id").getAsString(), element.getAsJsonObject()));
