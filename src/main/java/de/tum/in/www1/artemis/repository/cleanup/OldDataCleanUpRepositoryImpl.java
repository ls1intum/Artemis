...
public class OldDataCleanUpRepositoryImpl implements OldDataCleanUpRepository {
    ...
    @Override
    @Transactional
    public void deletePlagiarismComparisons(ZonedDateTime keepAllLaterThan) {
        entityManager
                .createQuery("DELETE FROM PlagiarismComparison pc " + "WHERE pc.id IN (" + "    SELECT pc.id " + "    FROM PlagiarismComparison pc " + "    JOIN pc.submissionA s1 "
                        ...
                        + "    JOIN p2.exercise e2 " + "    JOIN e1.course c1 " + "    JOIN e2.course c2 " + "    WHERE GREATEST(c1.endDate, c2.endDate) < :keepAllLaterThan" + ")")
                .setParameter("keepAllLaterThan", keepAllLaterThan).executeUpdate();
    }

    @Override
    @Transactional
    public void deleteNonRatedResults(ZonedDateTime keepAllLaterThan) {
        entityManager.createQuery("DELETE FROM Result r " + "WHERE r.rated = false " + "AND r.participation.exercise.course.endDate < :keepAllLaterThan")
                .setParameter("keepAllLaterThan", keepAllLaterThan).executeUpdate();
    }

    @Override
    @Transactional
    public void deleteOldRatedResults(ZonedDateTime keepAllLaterThan) {
        entityManager
                .createQuery("DELETE FROM Result r " + "WHERE r.id IN (" + "    SELECT r.id " + "    FROM Result r " + "    WHERE r.rated = true "
                        + "    AND r.participation.exercise.course.endDate < :keepAllLaterThan " + "    AND r.id NOT IN (" + "        SELECT r1.id " + "        FROM Result r1 "
    ...
    @Override
    @Transactional
    public void deleteOldSubmissionVersions(ZonedDateTime keepAllLaterThan) {
        entityManager.createQuery("DELETE FROM SubmissionVersion sv " + "WHERE sv.submission.id IN (" + "    SELECT s.id " + "    FROM Submission s "
                + "    JOIN s.participation p " + "    JOIN p.exercise e " + "    JOIN e.exerciseGroup eg " + "    JOIN eg.exam ex " + "    JOIN ex.course c "
                + "    WHERE c.endDate < :keepAllLaterThan" + ")").setParameter("keepAllLaterThan", keepAllLaterThan).executeUpdate();
    }

    @Override
    @Transactional
    public void deleteOldFeedback(ZonedDateTime keepAllLaterThan) {
        entityManager
                .createQuery("DELETE FROM Feedback f " + "WHERE f.result.id NOT IN (" + "    SELECT MAX(r.id) " + "    FROM Result r "
                        + "    WHERE r.participation = f.result.participation" + ") " + "AND f.result.participation.exercise.course.endDate < :keepAllLaterThan")
                .setParameter("keepAllLaterThan", keepAllLaterThan).executeUpdate();
    }

    @Override
    @Transactional
    public boolean existsDataForCleanup(ZonedDateTime keepAllLaterThan) {
                ...
                SELECT
                    CASE
                        ...
                        WHEN (
                            ...
                            JOIN e1.course c1
                            JOIN e2.course c2
                            WHERE GREATEST(c1.endDate, c2.endDate) < :keepAllLaterThan
                        ) > 0 THEN 1
                        WHEN (
                            SELECT COUNT(r)
                            FROM Result r
                            WHERE r.rated = false
                            AND r.participation.exercise.course.endDate < :keepAllLaterThan
                        ) > 0 THEN 1
                        WHEN (
                            SELECT COUNT(r.id)
                            FROM Result r
                        ...
                        WHEN (
                            ...
                            WHERE sv.submission.id IN (
                                ...
                                JOIN eg.exam ex
                                JOIN ex.course c
                                WHERE c.endDate < :keepAllLaterThan
                            )
                        ) > 0 THEN 1
                        WHEN (
                            SELECT COUNT(f)
                            ...
                            AND f.result.participation.exercise.course.endDate < :keepAllLaterThan
                        ) > 0 THEN 1
                        ELSE 0
                    END
                """).setParameter("keepAllLaterThan", keepAllLaterThan).getSingleResult();
