...
public class CompassService {
    ...
    */
    ...
    public List<ModelAssessmentConflict> getConflicts(
            long exerciseId, Result result, List<Feedback> modelingAssessment) {
        CompassCalculationEngine engine = getCalculationEngine(exerciseId);
        Map<String, List<Feedback>> elementConflictingFeedbackMapping =
                engine.getConflictingFeedbacks(result.getSubmission().getId(), modelingAssessment);
        List<ModelAssessmentConflict> conflicts = new LinkedList<>();
        elementConflictingFeedbackMapping.forEach(
                (elementID, feedbacks) -> {
                    HashMap<String, Result> elementResultMap = new HashMap<>();
                    feedbacks.forEach(
                            feedback ->
                                    elementResultMap.put(feedback.getReferenceElementId(), feedback.getResult()));
