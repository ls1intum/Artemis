...
public class CompassService {
    ...
    /**
     * TODO: adjust documentation Get the assessment for a given model from the calculation engine, create an automatic result from it and save it to the database. This is done
     * only if the submission is not manually assessed already, i.e. the assessment type is not MANUAL and the assessor is not set. Note, that Compass tries to automatically assess
     * every model as much as possible, but does not submit any automatic assessment to the student. A user has to review every(!) automatic assessment before completing and
     * submitting the assessment manually, even if Compass could assess 100% of the model automatically.
     *
     * @param submissionId the id of the modeling submission that should be updated with an automatic assessment
     * @param exerciseId   the id of the corresponding exercise
     */
    private void assessAutomatically(long submissionId, long exerciseId) {// Workaround for ignoring automatic assessments of unsupported modeling exercise types TODO remove this
                                                                          // after adapting compass
    ...
    // TODO: documentation
    private void assessAllAutomatically(Collection<Long> modelIds, long exerciseId) {// Workaround for ignoring automatic assessments of unsupported modeling exercise types TODO
                                                                                     // remove this after adapting compass
        if (!isSupported(exerciseId)) {
            return;
        }

        CalculationEngine engine = compassCalculationEngines.get(exerciseId);
        List<ModelingSubmission> modelingSubmissions = modelingSubmissionRepository.findWithEagerResultAndFeedbackAndAssessorAndParticipationResultsByIdIn(modelIds);
    ...
    private void generateAutomaticResult(long submissionId, Result result, CalculationEngine engine) {
        ...
        if (result.getAssessmentType() != AssessmentType.MANUAL && result.getAssessor() == null) {
            ...
            List<Feedback> automaticFeedbackAssessments = engine.convertToFeedback(grade, submissionId, result);
