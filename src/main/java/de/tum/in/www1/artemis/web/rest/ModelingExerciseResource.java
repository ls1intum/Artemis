...
public class ModelingExerciseResource {
    ...
    * @return the ResponseEntity with status 201 (Created) and with body the new modelingExercise, or
    *     with status 400 (Bad Request) if the modelingExercise has already an ID
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
    @PostMapping("/modeling-exercises")
    @PreAuthorize("hasAnyRole('TA', 'INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<ModelingExercise> createModelingExercise(
            @RequestBody ModelingExercise modelingExercise) throws URISyntaxException {
        log.debug("REST request to save ModelingExercise : {}", modelingExercise);
        if (modelingExercise.getId() != null) {
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME, "idexists", "A new modelingExercise cannot already have an ID"))
                    .body(null);
        }
    ...
    @Nullable
    private ResponseEntity<ModelingExercise> checkModelingExercise(
            @RequestBody ModelingExercise modelingExercise) {
            ...
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME,
                                    "courseNotFound",
                                    "The course belonging to this modeling exercise does not exist"))
                    .body(null);
        }
    ...
    *     with status 400 (Bad Request) if the modelingExercise is not valid, or with status 500
    *     (Internal Server Error) if the modelingExercise couldn't be updated
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
    @PutMapping("/modeling-exercises")
    @PreAuthorize("hasAnyRole('TA', 'INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<ModelingExercise> updateModelingExercise(
            @RequestBody ModelingExercise modelingExercise) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
                .headers(
                        HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, modelingExercise.getId().toString()))
                .body(result);
    }
    ...
    */
    ...
    public ResponseEntity<List<ModelingExercise>> getModelingExercisesForCourse(
    ...
    * @return the ResponseEntity with status 200 (OK) and with body the modelingExercise, or with
    ...
    */
    ...
    public ResponseEntity<Void> deleteModelingExercise(@PathVariable Long exerciseId) {
        ...
        return ResponseEntity.ok()
                .headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, exerciseId.toString()))
                .build();
    }

    /**
    * Returns the data needed for the modeling editor, which includes the participation,
    * modelingSubmission with model if existing and the assessments if the submission was already
    * submitted.
    *
    ...
    */
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<ModelingSubmission> getDataForModelingEditor(
        ...
        if (authCheckService.isOwnerOfParticipation(participation)
                || courseService.userHasAtLeastTAPermissions(modelingExercise.getCourse())) {
            // continue
        } else {
            return forbidden();
        }
        ...
        Optional<ModelingSubmission> optionalModelingSubmission =
                participation.findLatestModelingSubmission();
