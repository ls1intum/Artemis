...
public class ModelingSubmissionResource {
    ...
    public ModelingSubmissionResource(
    ...
    /**
    * POST /courses/{courseId}/exercises/{exerciseId}/modeling-submissions : Create a new
    * modelingSubmission. This is called when a student saves his model the first time after starting
    * the exercise or starting a retry.
    *
    * @param exerciseId the id of the exercise for which to init a participation
    * @param principal the current user principal
    * @param modelingSubmission the modelingSubmission to create
    * @return the ResponseEntity with status 200 (OK) and the Result as its body, or with status 4xx
    ...
    */
    ...
    public ResponseEntity<ModelingSubmission> createModelingSubmission(
        ...
        if (modelingSubmission.getId() != null) {
            throw new BadRequestAlertException(
                    "A new modelingSubmission cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ModelingExercise modelingExercise = modelingExerciseService.findOne(exerciseId);
        Optional<Participation> participation =
                participationService.findOneByExerciseIdAndStudentLoginAnyState(
                        exerciseId, principal.getName());
        if (!participation.isPresent()) {
            throw new ResponseStatusException(
                    HttpStatus.FAILED_DEPENDENCY,
                    "No participation found for " + principal.getName() + " in exercise " + exerciseId);
        }
        checkAuthorization(modelingExercise);
        modelingSubmission =
                modelingSubmissionService.save(modelingSubmission, modelingExercise, participation.get());
        hideDetails(modelingSubmission);
        return ResponseEntity.ok(modelingSubmission);
    }

    /**
    * PUT /courses/{courseId}/exercises/{exerciseId}/modeling-submissions : Updates an existing
    * modelingSubmission. This function is called by the modeling editor for saving and submitting
    * modeling submissions. The submit specific handling occurs in the
    * ModelingSubmissionService.save() function.
    *
    * @param exerciseId the id of the exercise for which to init a participation
    * @param principal the current user principal
    ...
    *     or with status 400 (Bad Request) if the modelingSubmission is not valid, or with status 500
    *     (Internal Server Error) if the modelingSubmission couldn't be updated
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
    @PutMapping("/exercises/{exerciseId}/modeling-submissions")
    @PreAuthorize("hasAnyRole('USER', 'TA', 'INSTRUCTOR', 'ADMIN')")
    @Transactional
    public ResponseEntity<ModelingSubmission> updateModelingSubmission(
            @PathVariable Long exerciseId,
            Principal principal,
            @RequestBody ModelingSubmission modelingSubmission) {
    ...
    @ApiResponses({
        @ApiResponse(
                code = 200,
                message = GET_200_SUBMISSIONS_REASON,
                response = ModelingSubmission.class,
    ...
    public ResponseEntity<List<ModelingSubmission>> getAllModelingSubmissions(
            @PathVariable Long exerciseId, @RequestParam(defaultValue = "false") boolean submittedOnly) {
        ...
        List<ModelingSubmission> submissions =
                modelingSubmissionService.getModelingSubmissions(exerciseId, submittedOnly);
        return ResponseEntity.ok(submissions);
    }

    /**
    * GET /modeling-submissions/{submissionId} : Gets an existing modelingSubmission with result. If
    * no result exists for this submission a new Result object is created and assigned to the
    * submission.
    *
    * @param submissionId the id of the modelingSubmission to retrieve
    * @return the ResponseEntity with status 200 (OK) and with body the modelingSubmission for the
    *     given id, or with status 404 (Not Found) if the modelingSubmission could not be found
    */
    ...
    public ResponseEntity<ModelingSubmission> getModelingSubmission(@PathVariable Long submissionId) {
        ...
        ModelingExercise modelingExercise =
                (ModelingExercise) modelingSubmission.getParticipation().getExercise();
        ...
        if (modelingSubmission.getResult().getAssessor() == null) {
            ...
            // we set the assessor and save the result to soft lock the assessment (so that it cannot be
            // edited by another tutor)
    ...
    private void hideDetails(ModelingSubmission modelingSubmission) {
        ...
        if (modelingSubmission.getParticipation() != null) {
            ...
            if (modelingSubmission.getParticipation().getExercise() != null
                    && modelingSubmission.getParticipation().getExercise() instanceof ModelingExercise) {
                // make sure the solution is not sent to the client
                ModelingExercise modelingExerciseForClient =
                        (ModelingExercise) modelingSubmission.getParticipation().getExercise();
    ...
    private void checkAuthorization(ModelingExercise exercise) throws AccessForbiddenException {
        ...
        if (!courseService.userHasAtLeastStudentPermissions(course)) {
            throw new AccessForbiddenException(
                    "Insufficient permission for course: " + exercise.getCourse().getTitle());
        }
    }
}
