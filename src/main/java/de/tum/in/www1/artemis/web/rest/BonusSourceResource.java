...
public class BonusSourceResource {
    ...
    public ResponseEntity<BonusSource> getBonusSourceForCourse(@PathVariable Long courseId) {
    ...
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<BonusSource> getBonusSourcesForExam(@PathVariable Long courseId, @PathVariable Long examId) {
    ...
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<BonusSource> createBonusSourceForExam(@PathVariable Long courseId, @PathVariable Long examId, @RequestBody BonusSource bonusSource)
        ...
        bonusSource.setSourceGradingScale(exam);

        BonusSource savedBonusSource = bonusSourceService.saveBonusSource(bonusSource);
        return ResponseEntity.created(new URI("/api/courses/" + courseId + "/exams/" + examId + "/bonus-sources/"))
     ...
     */
    @PutMapping("/courses/{courseId}/bonus-sources")
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<BonusSource> updateBonusSourceForCourse(@PathVariable Long courseId, @RequestBody BonusSource bonusSource) {
        log.debug("REST request to update a bonus source for course: {}", courseId);
        Course course = courseRepository.findByIdElseThrow(courseId);
        BonusSource oldBonusSource = bonusSourceRepository.findByCourseIdOrElseThrow(courseId);
        authCheckService.checkHasAtLeastRoleInCourseElseThrow(Role.INSTRUCTOR, course, null);
     ...
     */
    @DeleteMapping("/courses/{courseId}/bonus-sources")
    @PreAuthorize("hasRole('INSTRUCTOR')")
    public ResponseEntity<Void> deleteBonusSourceForCourse(@PathVariable Long courseId) {
        log.debug("REST request to delete the bonus source for course: {}", courseId);
        Course course = courseRepository.findByIdElseThrow(courseId);
        BonusSource bonusSource = bonusSourceRepository.findByCourseIdOrElseThrow(courseId);
        authCheckService.checkHasAtLeastRoleInCourseElseThrow(Role.INSTRUCTOR, course, null);
