...
public class TextExerciseResource {
    ...
    */
    ...
    public ResponseEntity<TextExercise> createTextExercise(@RequestBody TextExercise textExercise)
            throws URISyntaxException {
        log.debug("REST request to save TextExercise : {}", textExercise);
        if (textExercise.getId() != null) {
            throw new BadRequestAlertException(
                    "A new textExercise cannot already have an ID", ENTITY_NAME, "idexists");
        }

        if (textExercise.getTitle() == null) {
            throw new BadRequestAlertException(
                    "A new textExercise needs a title", ENTITY_NAME, "missingtitle");
        }

        if (textExercise.getMaxScore() == null) {
            throw new BadRequestAlertException(
                    "A new textExercise needs a max score", ENTITY_NAME, "missingmaxscore");
        }
            ...
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ENTITY_NAME,
                                    "courseNotFound",
                                    "The course belonging to this text exercise does not exist"))
                    .body(null);
        }
    ...
    * @return the ResponseEntity with status 200 (OK) and with body the updated textExercise, or with
    *     status 400 (Bad Request) if the textExercise is not valid, or with status 500 (Internal
    ...
    public ResponseEntity<TextExercise> updateTextExercise(@RequestBody TextExercise textExercise)
        ...
        if (textExercise.getExampleSubmissions().size() != 0) {
            result
                    ...
                    .forEach(exampleSubmission -> exampleSubmission.setExercise(null));
            result
                    .getExampleSubmissions()
                    .forEach(exampleSubmission -> exampleSubmission.setTutorParticipation(null));
        }
    ...
    * @return the ResponseEntity with status 200 (OK) and with body the textExercise, or with status
    *     404 (Not Found)
    */
    ...
    public ResponseEntity<TextExercise> getTextExercise(@PathVariable Long exerciseId) {
        ...
        Set<ExampleSubmission> exampleSubmissions =
                new HashSet<>(this.exampleSubmissionRepository.findAllByExerciseId(exerciseId));
        optionalTextExercise.ifPresent(
                textExercise -> textExercise.setExampleSubmissions(exampleSubmissions));
    ...
    */
    ...
    public ResponseEntity<Participation> getDataForTextEditor(@PathVariable Long participationId) {
            ...
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    ...
                                    "No participation was found for the given ID."))
                    .body(null);
        }
                ...
                return ResponseEntity.badRequest()
                        .headers(
                                HeaderUtil.createFailureAlert(
                                        "textExercise",
                                        "exerciseEmpty",
                                        "The exercise belonging to the participation is null."))
                        .body(null);
            }
        } else {
            return ResponseEntity.badRequest()
                    .headers(
                            HeaderUtil.createFailureAlert(
                                    "textExercise",
                                    "wrongExerciseType",
                                    "The exercise of the participation is not a modeling exercise."))
                    .body(null);
        }
        ...
        if (!authCheckService.isOwnerOfParticipation(participation)
                && !courseService.userHasAtLeastTAPermissions(textExercise.getCourse())) {
        ...
        if (participation.getResults().size() <= 0) {
            List<Result> results =
                    resultRepository.findByParticipationIdOrderByCompletionDateDesc(participation.getId());
        ...
        if (textSubmission.isPresent()) {
            ...
            if (textSubmission.get().isSubmitted()
                    && result != null
