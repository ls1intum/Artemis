...
public class PyrisStatusUpdateService {
    ...
    public PyrisStatusUpdateService(PyrisJobService pyrisJobService, IrisChatSessionService irisChatSessionService, IrisCourseChatSessionService irisCourseChatSessionService) {
     ...
     * @param statusUpdate the status update
     */
    public void handleStatusUpdate(CourseChatJob job, PyrisCourseChatStatusUpdateDTO statusUpdate) {
        irisCourseChatSessionService.handleStatusUpdate(job, statusUpdate);
        var isDone = statusUpdate.getStages().stream().map(PyrisStageDTO::state)
                .allMatch(state -> state == PyrisStageStateDTO.DONE || state == PyrisStageStateDTO.ERROR || state == PyrisStageStateDTO.SKIPPED);
        if (isDone) {
            pyrisJobService.removeJob(job.getId());
