...
public class SystemNotificationResource {
    ...
    public SystemNotificationResource(
    ...
    * @return the ResponseEntity with status 201 (Created) and with body the new system notification,
    *     or with status 400 (Bad Request) if the system notification has already an ID
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
    @PostMapping("/system-notifications")
    @PreAuthorize("hasAnyRole('INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<Notification> createSystemNotification(
            @RequestBody SystemNotification systemNotification) throws URISyntaxException {
        log.debug("REST request to save SystemNotification : {}", systemNotification);
        if (systemNotification.getId() != null) {
            throw new BadRequestAlertException(
                    "A new system notification cannot already have an ID", ENTITY_NAME, "idexists");
        }
    ...
    *     status 400 (Bad Request) if the system notification is not valid, or with status 500
    *     (Internal Server Error) if the system notification couldn't be updated
    * @throws URISyntaxException if the Location URI syntax is incorrect
    */
    @PutMapping("/system-notifications")
    @PreAuthorize("hasAnyRole('INSTRUCTOR', 'ADMIN')")
    public ResponseEntity<SystemNotification> updateSystemNotification(
            @RequestBody SystemNotification systemNotification) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
                .headers(
                        HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, systemNotification.getId().toString()))
                .body(result);
    }
    ...
    */
    ...
    public SystemNotification getActiveSystemNotification() {
        ...
        SystemNotification systemNotification =
                systemNotificationService.findActiveSystemNotification();
