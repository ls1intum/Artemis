...
public class SecurityConfiguration {
    ...
    protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        .authorizeHttpRequests(auth -> auth
            ...
            .requestMatchers(HttpMethod.GET, "/api/{v\\d*}" + SYSTEM_NOTIFICATIONS_RESOURCE_PATH + "active").permitAll()
            .requestMatchers(HttpMethod.POST, NEW_RESULT_RESOURCE_API_PATH).permitAll()
            .requestMatchers(HttpMethod.POST, "/api/{v\\d*}/" + NEW_RESULT_RESOURCE_PATH).permitAll()
