...
public interface QuizExerciseRepository extends JpaRepository<QuizExercise, Long> {
    ...
    @Query("select e from QuizExercise left join fetch e.questions where e.id = :#{#exerciseId}")
    QuizExercise findOneByIdWithEagerQuestions(@Param("exerciseId") Long exerciseId);
