...
public interface IrisMessageRepository extends ArtemisJpaRepository<IrisMessage, Long> {
     ...
     * reason as {@link #findListIndex}. A locking read, because a plain one is answered from the transaction's
     * snapshot on MySQL and would miss a row committed since, handing out an index that row already holds. The
     * {@code NULL} filter skips a row between its insert and its {@link #setListIndex}, where the engines disagree
     * over how it sorts.
     *
    ...
    /**
     * Write an inserted message's position in its session's ordered list. An append inserts through the message side
     * rather than through the collection that owns {@code iris_message_order}, so the column is set here.
     *
