...
public class IrisExerciseChatSessionService extends AbstractIrisChatSessionService<IrisProgrammingExerciseChatSession> implements IrisRateLimitedFeatureInterface {
    ...
    public void checkIsFeatureActivatedFor(IrisProgrammingExerciseChatSession session) {
        var exercise = exerciseRepository.findByIdElseThrow(session.getExerciseId());
        anyEnabledOrElseThrow(exercise);
    }

    @Override
    public void sendOverWebsocket(IrisProgrammingExerciseChatSession session, IrisMessage message, String event) {
        irisChatWebsocketService.sendMessage(session, message, null, event);
    }
    ...
    public void requestAndHandleResponse(IrisProgrammingExerciseChatSession session) {
        if (session.isInPromptingModePipeline()) {
            ...
            requestAndHandleResponseExerciseChat(session, Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty());
        }
    }
     ...
     */
    public void requestAndHandleResponseExerciseChat(IrisProgrammingExerciseChatSession session, Optional<String> event,
     ...
     * @param latestSubmission Optional latest submission to use if already loaded elsewhere. If not provided, the latest submission will be fetched
     */
    public void requestAndHandleResponsePromptUser(IrisProgrammingExerciseChatSession session, Optional<String> event, Optional<IrisCombinedPromptUserSubSettingsDTO> settings,
        ...
        var actualSettings = settings.orElseGet(() -> irisSettingsService.getCombinedIrisSettingsFor(exercise, false).irisPromptUserSettings());
        if (!actualSettings.enabled()) {
    ...
    private void onBuildFailure(ProgrammingExerciseStudentParticipation studentParticipation, ProgrammingSubmission submission) {
        ...
        CompletableFuture.runAsync(() -> requestAndHandleResponseExerciseChat(session, Optional.of(IrisEventType.BUILD_FAILED.name().toLowerCase()), Optional.of(settings),
                Optional.of(user), Optional.of(submission)));
    }
    ...
    public IrisProgrammingExerciseChatSession startInClassPromptingModeForCurrentSession(ProgrammingExercise exercise, User user) {
     ...
     * @param exercise The exercise to check
     */
    private void anyEnabledOrElseThrow(Exercise exercise) {
