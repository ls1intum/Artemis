...
import de.tum.cit.aet.artemis.hyperion.runtime.agent.TurnAware;
...
public class FileChangeEmittingAgentTools implements TurnAware, SubmitVetoAware {
    ...
    private final Consumer<FileChange> changeSink;
    ...
    public FileChangeEmittingAgentTools(SandboxAgentTools delegate, Consumer<FileChange> changeSink) {
        this(delegate, changeSink, null);
    }

    public FileChangeEmittingAgentTools(SandboxAgentTools delegate, Consumer<FileChange> changeSink, @Nullable GenerationActivityTracker activityTracker) {
    ...
    public String writeFile(@ToolParam(description = AgentToolDescriptions.WRITE_FILE_PATH) String path,
        ...
        return emitOnSuccess(delegate.writeFile(path, content), WRITE_SUCCESS_PREFIX, path, "write");
    }
    ...
    public String editFile(@ToolParam(description = AgentToolDescriptions.EDIT_FILE_PATH) String path,
        ...
        return emitOnSuccess(delegate.editFile(path, oldText, newText), EDIT_SUCCESS_PREFIX, path, "edit");
    }
    ...
    public String deleteFile(@ToolParam(description = AgentToolDescriptions.DELETE_FILE_PATH) String path) {
        return emitOnSuccess(delegate.deleteFile(path), DELETE_SUCCESS_PREFIX, path, "delete");
    }
    ...
    private static @Nullable String boundedPreview(@Nullable String content) {
        return content == null || content.length() <= 65_536 ? content : content.substring(0, 65_536);
    }

    private void emit(String path, String action) {
        try {
            changeSink.accept(new FileChange(path, action, currentTurn, Instant.now(), boundedPreview(delegate.latestMutationContent())));
        }
