...
public record ProgrammingExerciseStudentParticipationDTO(Long id, String type, InitializationState initializationState, ZonedDateTime initializationDate,
        ...
        TeamDTO team, ProgrammingExerciseResponseDTO exercise, List<ProgrammingSubmissionWithResultsDTO> submissions) implements Serializable {
    ...
    /**
     * Converts a student participation and nests the exercise it belongs to. Use this when the participation is the
     * response root; the nested exercise is only mapped when it is already initialized.
     *
     * @param participation the participation to convert (may be {@code null})
     * @return the converted DTO, or {@code null} if the input was {@code null}
     */
    public static ProgrammingExerciseStudentParticipationDTO ofWithExercise(ProgrammingExerciseStudentParticipation participation) {
        if (participation == null || !Hibernate.isInitialized(participation)) {
            return null;
        }
        var exercise = participation.getProgrammingExercise();
        ProgrammingExerciseResponseDTO exerciseDTO = exercise != null && Hibernate.isInitialized(exercise) ? ProgrammingExerciseResponseDTO.of(exercise) : null;
        return of(participation, exerciseDTO);
    }
    ...
    public static ProgrammingExerciseStudentParticipationDTO of(ProgrammingExerciseStudentParticipation participation, ProgrammingExerciseResponseDTO exercise,
        ...
        return new ProgrammingExerciseStudentParticipationDTO(participation.getId(), TYPE, participation.getInitializationState(), participation.getInitializationDate(),
                ...
                participation.getParticipantName(), participation.getParticipantIdentifier(), team, exercise, submissions);
    }
