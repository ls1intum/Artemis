...
public interface IrisExerciseChatSessionRepository extends ArtemisJpaRepository<IrisProgrammingExerciseChatSession, Long> {
    ...
    /**
     * Finds a list of {@link IrisProgrammingExerciseChatSession} based on the exercise and user IDs with messages.
     *
     * @param exerciseId The ID of the exercise.
     * @param userId     The ID of the user.
     * @return A list of chat sessions with messages sorted by creation date in descending order.
     */
    @EntityGraph(type = LOAD, attributePaths = "messages")
    @Query("""
            SELECT s
                ...
                ORDER BY s.creationDate DESC
            """)
    List<IrisProgrammingExerciseChatSession> findByExerciseIdAndUserIdWithMessages(@Param("exerciseId") Long exerciseId, @Param("userId") Long userId);

    /**
     * Finds the latest chat session containing at least one prompting mode message that is not in prompting mode anymore.
     *
     * @param exerciseId The ID of the exercise.
     * @param userId     The ID of the user.
     ...
     */
    @NotNull
    default IrisProgrammingExerciseChatSession findLatestFinishedPromptingModeSessionByExerciseIdAndUserIdElseThrow(long exerciseId, long userId) throws EntityNotFoundException {
        var result = findByExerciseIdAndUserIdWithMessages(exerciseId, userId).stream()
                .filter(s -> !s.isInPromptingModePipeline() && s.getMessages().stream().anyMatch(m -> Boolean.TRUE.equals(m.getInPromptingMode()))).findFirst();

        if (result.isEmpty()) {
            throw new EntityNotFoundException("Iris Exercise Chat Session with Prompting Mode");
        }
        return result.get();
    }
}
