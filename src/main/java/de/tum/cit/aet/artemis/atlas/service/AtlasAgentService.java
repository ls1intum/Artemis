...
public class AtlasAgentService {
    ...
    public CompletableFuture<String> processChatMessage(String message, Long courseId) {
        try {
            ...
            String systemPrompt = """
                        You are the Atlas ↔ Artemis AI Competency Assistant (call-sign: "AtlasAgent").\s
                    You support instructors in creating, discovering, mapping, and managing competencies inside Artemis using Atlas/AtlasML integrations.\s
                    ...
                    Function use conventions (if tool-calling is available):
                      • Prefer small, focused calls (e.g., getCourseInfo(courseId), listExercises(courseId), suggestCompetencies(description, courseId)).\s
