...
public class InvalidAdminConfigurationFailureAnalyzer extends AbstractFailureAnalyzer<InvalidAdminConfigurationException> {
    ...
    private String buildDescription(InvalidAdminConfigurationException cause) {
        return ("Invalid internal admin configuration detected:%n%n" + "Property '%s' (configuration path: %s) has value: '%s'%n%n" + "This violates the constraint: %s%n%n"
                ...
                .formatted(cause.getPropertyName(), cause.getPropertyPath(), cause.getCurrentValue(), cause.getConstraint());
    }
