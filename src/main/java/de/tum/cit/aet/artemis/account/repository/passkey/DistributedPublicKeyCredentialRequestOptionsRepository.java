...
import de.tum.cit.aet.artemis.account.security.passkey.DistributedPasskeyMapAccess;

/**
 * A distributed implementation of {@link PublicKeyCredentialRequestOptionsRepository} using a
 * {@link DistributedPasskeyMapAccess} to store and synchronize WebAuthn authentication request options across multiple nodes.
 *
 * <p>
 ...
 * key for looking up challenge options in the distributed map. This ensures that the challenge
 ...
 *
 * <p>
 * The repository stores options with a time-to-live of 5 minutes to accommodate
...
public class DistributedPublicKeyCredentialRequestOptionsRepository implements PublicKeyCredentialRequestOptionsRepository {
    ...
    public DistributedPublicKeyCredentialRequestOptionsRepository(DistributedPasskeyMapAccess passkeyMapAccess, Environment environment) {
     ...
     * distributed map key, and set as a cookie on the response. When {@code options} is {@code null}
     * (cleanup after authentication), the existing challenge lookup ID is read from the cookie, the
     * map entry is removed, and the cookie is deleted.
     * </p>
     *
    ...
    public void save(HttpServletRequest request, HttpServletResponse response, PublicKeyCredentialRequestOptions options) {
        ...
        if (storeNewChallenge) {
            ...
            passkeyMapAccess.getDistributedPasskeyAuthOptionsMap().put(challengeLookupId, options, AUTH_OPTIONS_TIME_TO_LIVE_SECONDS, TimeUnit.SECONDS);
            ...
            log.debug("Saved PublicKeyCredentialRequestOptions with challengeLookupId: {}", challengeLookupId);
        }
        else {
            // clear old challenge
            Cookie existingCookie = WebUtils.getCookie(request, WEBAUTHN_CHALLENGE_COOKIE_NAME);
            if (existingCookie != null) {
                log.debug("Removing PublicKeyCredentialRequestOptions for challengeLookupId: {}", existingCookie.getValue());
                passkeyMapAccess.getDistributedPasskeyAuthOptionsMap().remove(existingCookie.getValue());
            }
    ...
    public PublicKeyCredentialRequestOptions load(HttpServletRequest request) {
        ...
        PublicKeyCredentialRequestOptions options = passkeyMapAccess.getPasskeyAuthOptionsMap().get(cookie.getValue());
        if (options != null) {
            log.debug("Loaded PublicKeyCredentialRequestOptions for challengeLookupId: {}", cookie.getValue());
        }
        else {
            log.warn("No PublicKeyCredentialRequestOptions found for challengeLookupId: {}. The entry may have expired.", cookie.getValue());
        }
        return options;
    }
