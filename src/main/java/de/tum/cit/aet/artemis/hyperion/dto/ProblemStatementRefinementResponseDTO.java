 ...
 * Contract:
 * <ul>
 * <li><strong>Success case:</strong> {@code refinedProblemStatement} contains
 * the generated text (never null or empty),
 * and {@code originalProblemStatement} is null.</li>
 * <li><strong>Error case:</strong> {@code refinedProblemStatement} is an empty
 * string (""),
 * and {@code originalProblemStatement} contains the original problem statement
 * (never null).</li>
 * </ul>
 * <p>
 * Note: The {@code refinedProblemStatement} field is never null - it will be
 * either a non-empty string (success)
 * or an empty string (error). Callers should check if
 * {@code originalProblemStatement} is null to determine success/failure,
 * or check if {@code refinedProblemStatement.isEmpty()} for the error case.
 *
 * @param refinedProblemStatement  the refined problem statement text (non-empty
 *                                     on success, empty string on error, never
 *                                     null)
