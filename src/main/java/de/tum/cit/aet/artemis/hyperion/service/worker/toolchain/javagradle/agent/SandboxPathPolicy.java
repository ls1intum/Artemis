...
final class SandboxPathPolicy {
    ...
    private static final Pattern MANAGED_BUILD_PATH = Pattern.compile(
    ...
    static boolean mutatesManagedBuildInfrastructure(String command) {
        ...
        if (!MANAGED_BUILD_PATH.matcher(lower).matches()) {
            return false;
        }
    ...
    static @Nullable String workspaceRelativePath(@Nullable String path) {
        ...
        return trimmed.startsWith("/") || trimmed.contains("..") || !SAFE_RELATIVE_PATH.matcher(trimmed).matches() ? null : trimmed;
    }
}
