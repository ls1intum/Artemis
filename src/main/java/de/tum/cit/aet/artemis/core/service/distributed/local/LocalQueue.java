...
public class LocalQueue<T> implements DistributedQueue<T> {
    ...
    /**
     * Ordering used to report the queue contents in dequeue order, or {@code null} for an unordered (FIFO) queue.
     * A {@link java.util.PriorityQueue} only guarantees that its <em>head</em> is the smallest element; iterating it
     * yields internal heap order. {@link #getAll()} therefore has to sort explicitly, because callers such as
     * {@code DistributedDataAccessService#getQueuedJobs} present the result as the order jobs will be dequeued in.
     */
    @Nullable
    ...
    public LocalQueue(Queue<T> queue, String name, @Nullable Comparator<? super T> ordering) {
        this.queue = queue;
        this.name = name;
    ...
    public List<T> getAll() {
        ...
        try {
            ...
            return queue.stream().sorted(ordering).toList();
        }
        finally {
            lock.unlock();
        }
