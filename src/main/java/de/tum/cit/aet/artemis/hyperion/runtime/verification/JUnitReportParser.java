...
public final class JUnitReportParser {
    ...
    public record TestCaseResult(String name, List<String> testMessages) {
    }
    ...
    /**
     * @param maxFeedbackLength maximum number of characters retained per failure message
     */
    public JUnitReportParser(int maxFeedbackLength) {
        if (maxFeedbackLength < 0) {
            throw new IllegalArgumentException("Feedback length must not be negative");
        }
     ...
     */
    private void processTopLevelTestSuite(List<TestCaseResult> failedTests, List<TestCaseResult> successfulTests, TestSuite suite) {
     ...
     */
    private void processInnerTestSuite(TestSuite testSuite, List<TestCaseResult> failedTests, List<TestCaseResult> successfulTests, String outerNamePrefix) {
     ...
     */
    private void processTestSuiteWithNamePrefix(TestSuite testSuite, List<TestCaseResult> failedTests, List<TestCaseResult> successfulTests, String namePrefix) {
        for (TestCase testCase : testSuite.testCases()) {
            ...
            if (failure != null) {
                ...
                failedTests.add(new TestCaseResult(namePrefix + testCase.name(), List.of(truncatedFeedbackMessage)));
            }
            else {
                successfulTests.add(new TestCaseResult(namePrefix + testCase.name(), List.of()));
            }
        }
     ...
     */
    private String truncateFeedbackMessage(String message) {
