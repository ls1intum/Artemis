...
public class ApiProfileNotPresentException extends RuntimeException {
    ...
    public ApiProfileNotPresentException(Class<? extends AbstractApi> api, String featureOrProfile) {
        super("Api %s is not enabled because '%s' is not active. Enable the corresponding module feature or activate the required Spring profile.".formatted(api.getName(),
                featureOrProfile));
    }
}
