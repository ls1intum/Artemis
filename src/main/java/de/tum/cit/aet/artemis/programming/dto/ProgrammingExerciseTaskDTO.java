...
public record ProgrammingExerciseTaskDTO(Long id, String taskName, List<ProgrammingExerciseTestCaseResponseDTO> testCases) {
     ...
     * collection. {@code ProgrammingExerciseTaskService.getTasksWithoutInactiveTestCases} achieves the same result
     * by calling {@code Set.removeIf} on the managed collection backing the {@code programming_exercise_task_test_case}
     * join table, which risks a destructive write on that join table; this factory filters at mapping time instead
     * and is intentionally not used with that service method's result.
     *
