 ...
 */
public class GenerationWorkspace {

    private static final Logger log = LoggerFactory.getLogger(GenerationWorkspace.class);
    ...
    public GenerationWorkspace(SandboxBuildCommands sandboxBuildCommandService, GenerationResources resourceLoaderService) {
    ...
    public void stageBuildReadinessFixture(InteractiveSandbox sandbox, String sessionId, GenerationInput exercise) {
        SandboxExecResultDTO preparation = sandbox.exec(sessionId, SANDBOX_READ_TIMEOUT, "sh", "-c", "find " + SandboxBuildCommands.READINESS_FIXTURE_DIR + " -mindepth 1 -delete");
        if (!preparation.isSuccess()) {
            throw new IllegalStateException("Could not prepare the build-readiness fixture directory: " + preparation.combinedOutput());
        }
        sandbox.copyIn(sessionId, SandboxBuildCommands.READINESS_FIXTURE_DIR, WorkspaceArchive.buildWorkspaceTarStream(readBuildReadinessFixture(exercise), Map.of()));
    }
    ...
    public void materializeRepositoryFiles(InteractiveSandbox sandbox, String sessionId, GenerationInput exercise, Mode mode,
        ...
        if (verifyScript != null) {
            workspaceFiles.put(SandboxBuildCommands.VERIFY_SCRIPT_NAME, verifyScript);
        }
