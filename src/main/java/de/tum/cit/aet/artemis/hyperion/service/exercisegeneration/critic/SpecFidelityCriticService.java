...
public class SpecFidelityCriticService {
    ...
    /**
     * The model id comes from the configured {@link ChatModel} bean's options, the same place {@code AgentLoopRunner} reads it, rather than from the
     * {@code spring.ai.openai.chat.model} property: a deployment that configures the model anywhere else would otherwise run its critics against a different model than its agent.
     */
    @Autowired
    ...
    /**
     * Languages whose default assertion failure does not name the behaviour — a bare {@code assertEquals(600L, actual)} reports only "expected 600 but was 500". Other frameworks
     * self-describe (Go {@code t.Errorf} format strings, Jest's auto-diff plus descriptive {@code it()} names, Catch2/gtest expression expansion) and so fail open here.
     */
