...
class CriticVerdictParser {
    ...
    private static final Pattern PASSAGE_ID = Pattern.compile("P[1-9][0-9]*");
    ...
    private static boolean unsupportedSourceRequirement(String requirement, @Nullable String sourceQuote, String authoritativeSource) {
        ...
        String evidenceId = CLOSE_BRACKET.matcher(OPEN_BRACKET.matcher(sourceQuote.strip()).replaceFirst("")).replaceFirst("");
        String citedPassage = sourceQuote;
        if (PASSAGE_ID.matcher(evidenceId).matches()) {
    ...
    private static boolean sourceQuoteIsGrounded(@Nullable String sourceQuote, String authoritativeSource) {
        ...
        String evidenceId = CLOSE_BRACKET.matcher(OPEN_BRACKET.matcher(sourceQuote.strip()).replaceFirst("")).replaceFirst("");
        if (PASSAGE_ID.matcher(evidenceId).matches() && EvidenceSource.from("P", authoritativeSource).passages().containsKey(evidenceId)) {
            return true;
        }
