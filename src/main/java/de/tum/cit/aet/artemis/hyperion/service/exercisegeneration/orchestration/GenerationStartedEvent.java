 ...
 * @param variantPreparation       optional source-copy preparation for a new variant destination
 * @param settings                 the effort profile this run resolved to, or {@code null} to run the deployment default. Carried on the event rather than re-resolved on the
 *                                     worker, so admission, the deadline it computed, and the run itself can never disagree about which configuration was admitted
 */
public record GenerationStartedEvent(String jobId, User user, ProgrammingExercise exercise, String userPrompt, GenerationMode mode, String expectedProblemStatement,
        ...
        @Nullable GenerationVariantPreparation variantPreparation) {
    ...
    public GenerationStartedEvent(String jobId, User user, ProgrammingExercise exercise, String userPrompt, GenerationMode mode, String expectedProblemStatement,
            String expectedTitle, @Nullable Instant deadlineAt, @Nullable String budgetReservationId, @Nullable String sourceBrief, @Nullable HyperionGenerationSettings settings) {
        this(jobId, user, exercise, userPrompt, mode, expectedProblemStatement, expectedTitle, deadlineAt, budgetReservationId, sourceBrief, settings, null);
    }
