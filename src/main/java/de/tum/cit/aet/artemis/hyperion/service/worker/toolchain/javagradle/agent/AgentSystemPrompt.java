...
public class AgentSystemPrompt {
    ...
    private final SandboxBuildCommands sandboxBuildCommandService;

    public AgentSystemPrompt(SandboxBuildCommands sandboxBuildCommandService) {
    ...
    private String buildContextSection(GenerationInput exercise) {
        SandboxBuildCommands.BuildContextSummary context;
        try {
    ...
    private static String capCommand(String command) {
        String oneLine = WHITESPACE.matcher(command).replaceAll(" ").trim();
