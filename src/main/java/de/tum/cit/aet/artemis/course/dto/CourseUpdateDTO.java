...
public record CourseUpdateDTO(
        ...
        @JsonDeserialize(using = StrictIntegerDeserializer.class) Integer maxPoints, @Min(0) @Max(5) Integer accuracyOfScores, boolean athenaFormativeEnabled,
        boolean athenaGradingEnabled, String timeZone, CourseInformationSharingConfiguration courseInformationSharingConfiguration, boolean onboardingDone,
    ...
    public static CourseUpdateDTO of(Course course) {
        return new CourseUpdateDTO(course.getId(), course.getTitle(), course.getShortName(), course.getDescription(), course.getSemester(), course.getStartDate(),
                ...
                course.getCourseInformationSharingConfiguration(), course.isOnboardingDone(), course.isGradeRelevant(), course.isDataRetentionHold());
    }
}
