...
/**
 * This class represents repositories cloned from the VC system to Artemis to then be used in the online editor. These
 * repositories are cloned from the remote VCS and are saved in the folder defined in the application properties at
 * artemis.repo-clone-path. Note: This class does not represent local VCS repositories. The local VCS is treated as a
 * remote VCS in code and is represented by the {@link LocalVCRepositoryUri} class. Its repositories are saved as bare
 * repositories in the folder defined in the application properties at artemis.version-control.local-vcs-repo-path.
 */
public class Repository extends org.eclipse.jgit.internal.storage.file.FileRepository {
    ...
    /**
     * Check if the file is allowed in this repository. This checks if the path of the file is a subdirectory of the
     * repository directory. Also checks that the ../ operator is not used to traverse up directories on the server.
     * Also checks that the file is not using the ".git" folder
     *
