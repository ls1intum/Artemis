 ...
 * <p>
 * {@code hasVideo} exists because a missing {@code videoVersion} has two very different causes: the video is gone, or the video is still there while its transcription is
 * not — the row is deleted and rewritten whenever the video changes. Only the first is material that no longer exists; the second is a video the client cannot verify the
 * citation against, and telling a student that a video which visibly plays no longer exists would be wrong.
 *
