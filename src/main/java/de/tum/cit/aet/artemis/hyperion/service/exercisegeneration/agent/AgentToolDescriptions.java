...
final class AgentToolDescriptions {
    ...
    static final String VERIFY = "Run the mechanical precheck. In an unstaged/legacy session this always builds the solution and template, parses reports with the SAME production parser, and returns exact test names, pass/fail results, bounded failure evidence, binding problems, and mechanical gate results. In staged generation, verify checks the CURRENT stage's artifact at the right depth instead: DESIGN scans DESIGN.md's required sections (no build), SOLUTION and TEMPLATE each run one pristine build (TEMPLATE also rejects a degenerate copy of the solution), TESTS runs the full solution/template differential exactly like the unstaged path, and STATEMENT resolves [task] bindings against the TESTS stage's exact test names (no build). Use the failure evidence to spot assertion, exception, or shared setup failures. A passing precheck does not prove semantic relevance; authoritative post-loop verification determines save eligibility, while quality review may request repairs or flag instructor review. Call it after changes and iterate until the mechanical precheck passes before submitting. Each call re-runs its check (no cache).";
