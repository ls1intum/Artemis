...
public class GenerationResources {
    ...
    private static final Pattern FIXTURE_AUTHOR = Pattern.compile("(?m)^[ \\t]*\\*[ \\t]*@(author|version)\\b[^\\r\\n]*\\R?");

    private static final Pattern TEST_LABEL = Pattern.compile("\\b(test[\\w$]+)\\(\\)");
    ...
    public static String javaGradleFixture(String source) {
        return FIXTURE_AUTHOR.matcher(source).replaceAll("").replace("@WhitelistPath(\"target\")", "@WhitelistPath(\"build\")")
    ...
    public static String javaGradleStatementFixture(String source) {
        return TEST_LABEL.matcher(source).replaceAll("$1");
    }
