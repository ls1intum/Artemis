...
public class ExerciseVersionResource {
    ...
    public ResponseEntity<List<ExerciseVersionDTO>> getExerciseVersions(@PathVariable Long exerciseId, Pageable pageable) {
        log.debug("REST request to get versions for Exercise : {}", exerciseId);
        Page<ExerciseVersionDTO> versions = exerciseVersionRepository.findAllByExerciseIdOrderByCreatedDateDesc(exerciseId, pageable);
    ...
    @EnforceAtLeastTutor
    public ResponseEntity<ExerciseSnapshotDTO> getExerciseSnapshot(@PathVariable Long versionId) {
