 ...
 * <p>
 * An Iris citation or point-out is pinned to these values when it is written, so that following it later can tell whether the referenced page or timestamp still refers to
 * the same content. They are only reported once processing reached {@code DONE} — while a unit is being reprocessed there is nothing trustworthy to pin, because the vector
 * database still serves the previous version. A {@code null} value means that kind of material was never ingested.
 * <p>
