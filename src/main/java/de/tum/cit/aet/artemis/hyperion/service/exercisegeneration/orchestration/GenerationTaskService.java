...
/**
 * Runs core-side generation or adaptation asynchronously, streams progress, and persists mechanically verified snapshots for instructor review.
 * Sandbox cleanup belongs to the worker; unverified output is never persisted.
 */
@Service
@Lazy
...
public class GenerationTaskService {
    ...
    private void runObserved(GenerationStartedEvent event, Observation observation) {
        ...
        try {
            ...
            if (!LanguageGenerationProfile.isSupported(exercise, buildConfigRepository.getProgrammingExerciseBuildConfigElseThrow(exerciseId),
                    !auxiliaryRepositoryRepository.findByExerciseId(exerciseId).isEmpty())) {
                emitter.milestone(
            ...
            BooleanSupplier cancelled = () -> jobService.isCancelled(jobId) || deadlineExceeded.get() || tokenBudgetExceeded.get() || tokenAccountingFailed.get()
                    || heartbeatLost.get();
            GenerationOutcome outcome = orchestrator.generate(exercise, user, userPrompt, jobId, event.mode(), cancelled, emitter, fileChangeSink, usageSink, event.sourceBrief(),
                    effectiveDeadlineAt(event.deadlineAt()), event.settings());
            ...
            try {
                ...
                if (!savedToExercise) {
                    retainUnsavedCandidate(exerciseId, jobId, user, outcome);
                }
            }
        }
        catch (RuntimeException | LinkageError e) {
    ...
    private ProgrammingExercise reloadDraftExerciseBeforeLiveMutation(long exerciseId) {
        ...
        if (exercise.isExamExercise() && !isStillDraftWithoutParticipations(exerciseId)) {
            throw new IllegalStateException("Hyperion generation cannot modify an exercise assigned to a student exam or test run.");
        }
