...
final class GenerationAttemptSupport {
    ...
    static void addIfExtractionFailed(Set<String> extractionFailed, GenerationWorkspace.RepositoryExtraction extraction, RepositoryRole type) {
        if (extraction.extractionFailed()) {
            extractionFailed.add(GenerationWorkspace.directoryFor(type));
        }
    }
}
