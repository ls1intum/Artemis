    ...
    private static final class WebsocketEventPublisher implements HyperionCodeGenerationEventPublisher {
        ...
        public void newFile(String path, RepositoryType repoType, int iteration) {
            websocket.send(login, topicSuffix,
                    new HyperionCodeGenerationEventDTO(HyperionCodeGenerationEventDTO.Type.NEW_FILE, jobId, exercise.getId(), iteration, repoType, path, null, null, null, null));
        }

        @Override
        public void done(HyperionCodeGenerationEventDTO.CompletionStatus completionStatus, int attemptsUsed, String message) {
            ...
            websocket.send(login, topicSuffix, new HyperionCodeGenerationEventDTO(HyperionCodeGenerationEventDTO.Type.DONE, jobId, exercise.getId(), null, repositoryType, null,
                    success, completionStatus, attemptsUsed, message));
        }

        @Override
        public void error(String message) {
            websocket.send(login, topicSuffix,
                    new HyperionCodeGenerationEventDTO(HyperionCodeGenerationEventDTO.Type.ERROR, jobId, exercise.getId(), null, repositoryType, null, null, null, null, message));
        }
    }
}
