...
public interface CourseRepository extends ArtemisJpaRepository<Course, Long> {
    ...
    @Query("""
            SELECT new de.tum.cit.aet.artemis.core.dto.ExistingCourseExerciseDetails(
                array_agg(e.title), array_agg(e.shortName), array_agg(con.name))
            FROM Course c
                LEFT JOIN Exercise e ON c.id = :courseId
                LEFT JOIN Conversation con ON con.exercise.id = e.id
            """)
    ExistingCourseExerciseDetails findExistingCourseExerciseDetails(@Param("courseId") long courseId);
