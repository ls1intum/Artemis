...
final class AuthoringStageCapture {
    ...
    private static final List<RepositoryRole> CANDIDATE_REPOSITORIES = List.of(RepositoryRole.TESTS, RepositoryRole.TEMPLATE, RepositoryRole.SOLUTION);

    /** A run's workspace as it stood at one point in time, either read live or replayed from the last authoring-stage boundary. */
    record CapturedWorkspace(Map<RepositoryRole, Map<String, String>> files, String problemStatement, @Nullable String specDocument, @Nullable String testPlanJson,
    ...
    record CandidateReadBack(Map<RepositoryRole, GenerationWorkspace.RepositoryExtraction> repositories, String problemStatement, @Nullable String testPlanJson) {
    }
    ...
    AuthoringStageCapture(GenerationOrchestrator service, GenerationWorkspace workspace, InteractiveSandbox sandbox, String sessionId, SandboxSessionLifecycle lifecycle,
            GenerationWorkspace.WorkspaceSeed workspaceSeed, Map<String, String> placeholderReplacements, Map<RepositoryRole, Map<String, String>> baselineRepositoryFiles,
            @Nullable String baselineProblemStatement) {
    ...
    void recordStageBoundary(GenerationStage stage) {
        ...
        try {
            Map<RepositoryRole, Map<String, String>> files = GenerationOrchestrator.changedCapturedRepositoryFiles(baselineRepositoryFiles,
            ...
            latestStageSnapshot = new CapturedWorkspace(files, statement, GenerationOrchestrator.readSpecDocument(sandbox, sessionId),
                    GenerationOrchestrator.readWorkspaceRootFile(sandbox, sessionId, "test-plan.json"), stage);
    ...
    CandidateReadBack readBackCandidate() {
        ...
        try {
            Map<RepositoryRole, GenerationWorkspace.RepositoryExtraction> repositories = new EnumMap<>(RepositoryRole.class);
            for (RepositoryRole type : CANDIDATE_REPOSITORIES) {
                ...
                repositories.put(type, GenerationOrchestrator.replacePlaceholders(extraction, placeholderReplacements));
            }
            ...
            return new CandidateReadBack(repositories, workspace.extractProblemStatement(sandbox, sessionId).trim(),
                    GenerationOrchestrator.readWorkspaceRootFile(sandbox, sessionId, "test-plan.json"));
        }
        finally {
            lifecycle.endCapture();
        }
    ...
    CapturedWorkspace captureNow() {
        ...
        try {
            ...
            Map<RepositoryRole, Map<String, String>> files = Map.of();
            try {
                files = GenerationOrchestrator.changedCapturedRepositoryFiles(baselineRepositoryFiles,
                        service.captureRepositoryFiles(sandbox, sessionId, workspaceSeed, placeholderReplacements));
            }
            ...
            return new CapturedWorkspace(files, statement, GenerationOrchestrator.readSpecDocument(sandbox, sessionId),
                    GenerationOrchestrator.readWorkspaceRootFile(sandbox, sessionId, "test-plan.json"), null);
        }
        finally {
            lifecycle.endCapture();
        }
    ...
    GenerationOutcome partialOutcome(AgentLoopResult loopResult, String reviewNote) {
        ...
        return new GenerationOutcome(loopResult, null, captured.files(), captured.problemStatement(), SpecFidelityReport.qualityReviewUnavailable(reviewNote),
                captured.specDocument(), captured.testPlanJson());
    }
