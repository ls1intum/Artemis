...
public class VariantAgentLoopService {
    ...
    /**
     * Reads the total token count from the response metadata; 0 when the provider reported no usage. Spring AI
     * accumulates usage across the round's whole tool loop, so this is the round's full cost.
     */
