...
import static de.tum.cit.aet.artemis.hyperion.service.worker.toolchain.javagradle.critic.ReviewGuardrails.MAX_ARTIFACT_EVIDENCE_CHARS;
import static de.tum.cit.aet.artemis.hyperion.service.worker.toolchain.javagradle.critic.ReviewGuardrails.MAX_REQUIREMENT_CHARS;
import static de.tum.cit.aet.artemis.hyperion.service.worker.toolchain.javagradle.critic.ReviewGuardrails.MAX_REVIEW_FINDINGS;
...
/**
 * Semantic exercise-quality reviewer for the contract risks the differential oracle ({@link DifferentialVerifier}) is structurally blind to.
 * <p>
 ...
 * </ul>
 * Contract-risk findings drive the bounded repair loop and, if unresolved, are attached to the saved candidate for instructor review; presentation findings stay advisory.
 */
public class SpecFidelityCritic {

    private static final Pattern BLOCK_COMMENT = Pattern.compile("(?s)/\\*.*?\\*/");

    private static final Pattern LINE_COMMENT = Pattern.compile("(?m)//.*$");

    private static final Logger log = LoggerFactory.getLogger(SpecFidelityCritic.class);
    ...
    /**
     * The best candidate a <em>completed</em> review saw, recorded even when that candidate was not admitted. Never softens {@link ConceptSelectionReview#accepted()}; it only
     * names which candidate a caller should fall back to if it proceeds with findings attached rather than producing nothing at all.
     *
     * @param candidate          the candidate number, in the reviewer's own numbering
     * @param failedRequiredAxes how many of the selection reviewer's required axes this candidate failed; zero means only a later, narrower pass objected
     */
    ...
    /** Phrases that describe how the grader and template are rigged rather than the task, so their presence in a student-facing statement means grader internals leaked. */
    ...
    public SpecFidelityCritic(@Nullable ChatClient chatClient, JsonMapper objectMapper, PromptTemplates templateService, String configuredModel,
    ...
    SpecFidelityCritic(@Nullable ChatClient chatClient, JsonMapper objectMapper, PromptTemplates templateService, String configuredModel, Duration providerHardFailureCooldown,
    ...
    /**
     * A critic configured for one run's effort profile, so every reviewer pass runs on the same model and context window as the authoring agent rather than reviewing one
     * configuration's output with another's judgement.
     *
     ...
     */
    public SpecFidelityCritic forSettings(@Nullable HyperionGenerationSettings settings) {
        ...
        return new SpecFidelityCritic(chatClient, objectMapper, templateService, modelOf(profileOptions), providerHardFailureCooldown, providerFailureCooldown,
    ...
    public SpecFidelityCritic(@Nullable ChatClient chatClient, JsonMapper objectMapper) {
    ...
    /** Keeps a missed exact-technique mandate from becoming a known-but-ungraded requirement downstream: a frozen correctness rule must be enforceable by the assessment mode. */
     ...
     *
     * @param specificationContract the approved specification, the sole rule authority
     ...
     * @param usageSink             token-usage observer, or {@code null}
     * @param cancelled             cancellation signal
     * @return at most four structurally valid proposals, none with environment evidence yet
     */
    ...
    public SpecFidelityReport critique(@Nullable String brief, @Nullable String problemStatement, List<String> testNames, Map<RepositoryRole, Map<String, String>> artifacts,
     ...
     * instructor-visible, so "no test covers this spec rule" is reportable even against a one-line brief. The produced statement is excluded from that authority, so the final
     * artifact can never authorize its own additions.
     *
     ...
     * @param usageSink               reviewer usage observer, or {@code null}
     * @param cancelled               cancellation signal
     ...
     * @param cancelled         cancellation signal
     * @param previousReport    the preceding review, threaded into the reviewer prompt for continuity, or {@code null}
    ...
    public SpecFidelityReport critiqueAdaptation(@Nullable String brief, @Nullable String problemStatement, List<String> testNames, String adaptationChanges,
            Map<RepositoryRole, Map<String, String>> artifacts, @Nullable Consumer<ChatResponse> usageSink, BooleanSupplier cancelled,
    ...
    private static void requireReviewInputsSafe(@Nullable String brief, @Nullable String problemStatement, List<String> testNames,
            @Nullable Map<RepositoryRole, Map<String, String>> artifacts, @Nullable String adaptationChanges) {
        ...
        for (Map.Entry<RepositoryRole, Map<String, String>> repository : artifacts.entrySet()) {
    ...
    private static boolean hasCompleteArtifactSet(Map<RepositoryRole, Map<String, String>> artifacts) {
        return artifacts != null && List.of(RepositoryRole.SOLUTION, RepositoryRole.TEMPLATE, RepositoryRole.TESTS).stream()
    ...
    private List<SpecFidelityReport.Finding> reviewArtifacts(@Nullable String brief, @Nullable String problemStatement, List<String> testNames,
            Map<RepositoryRole, Map<String, String>> artifacts, @Nullable String adaptationChanges, @Nullable Consumer<ChatResponse> usageSink, BooleanSupplier cancelled,
        ...
        // Keeping brief and spec visibly separate prevents a model-authored SPEC from laundering its own invented requirement into "primary source" authority.
        ...
        boolean expectApiChecks = artifacts.getOrDefault(RepositoryRole.SOLUTION, Map.of()).values().stream().filter(Objects::nonNull)
        ...
        if (!cancelled.getAsBoolean() && oracleReviewInvalid && userPrompt.length() + ORACLE_REVIEW_CORRECTION.length() <= MAX_REVIEW_INPUT_CHARS) {
            ...
            if (correctedOracleFindings != null && !CriticVerdictParser.hasUngroundedOracleReview(correctedOracleFindings)
                ...
                // leave the corrected response unable to retract a semantically false one.
        ...
        // Keep at least one verdict from each independent pass even when the other floods the shared cap: silently dropping a whole pass would make the saved instructor review
        // falsely look as though that axis was clean.
        ...
        // Blockers from both passes before any advisory, alternating so neither pass can consume the shared cap alone.
    ...
    /** Requires the reviewer to adjudicate each preceding finding before reporting anything new, so a repair attempt is reviewed against its history rather than re-rolled. */
    ...
    static Map<String, String> designTemplateStatuses(String specification) {
        ...
        for (String rawLine : specification.lines().toList()) {
            ...
            String status = StageChecks.normalizeTemplateStatus(cells[cells.length - 1]);
    ...
    private static Map<String, String> downstreamEvidenceByArtifact(@Nullable String problemStatement, @Nullable String testPlanJson,
            Map<RepositoryRole, Map<String, String>> artifacts) {
    ...
    /**
     * A JVM assertion carrying a string literal in the same statement. A match may be a real failure message or merely a string expected-value
    ...
    /**
     * Flags rules that mandate an implementation technique, which behavioural tests cannot observe: no assertion over the public API can tell a recursive implementation from an
     * iterative one that returns the same values (see {@link SpecFidelityReport.Kind#UNENFORCEABLE_TECHNIQUE_RULE}).
     * <p>
     * Only a mandate naming a control-flow or API technique matches, and it must stay that tight. A rule like "must delegate to the injected collaborator" IS observable through
     * a recording fake; flagging it would attach noise to nearly every exercise and dilute the real finding.
     *
    ...
    public List<SpecFidelityReport.Finding> detectUnenforceableTechniqueRules(@Nullable String specificationContract) {
        ...
        // Shared with the specification gate that rejects these outright; this pass is the backstop for a mandate that reaches a later stage anyway.
    ...
    /**
     * Advisory-only check: a failing student sees only the raw value mismatch when a graded assertion carries no message. File-level by design — only a wholly message-less file
     * is flagged, which sidesteps per-assertion argument parsing and leaves a mixed file alone. Never affects acceptance.
     *
     ...
     */
    public List<SpecFidelityReport.Finding> detectMessagelessAssertions(@Nullable Map<String, String> producedTestsFiles) {
        ...
        for (Map.Entry<String, String> entry : producedTestsFiles.entrySet()) {
            ...
            if (content == null || !path.endsWith(".java")) {
                continue;
            }
     ...
     *
     * @param report the spec-fidelity report whose findings drive the rendered guidance
    ...
    public String renderForRetryPrompt(SpecFidelityReport report) {
        ...
        if (report.hasBlockingFindings()) {
            ...
            // TEMPLATE_QUALITY_GAP findings only polish the scaffold, so they sort last: a bounded repair attempt must not spend its turns on placement nits while a design
            // defect survives.
