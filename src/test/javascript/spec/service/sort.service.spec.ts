...
describe('Sort Service', () => {
    ...
    describe('Service methods', () => {
        it('should sort basic array ascending', repeatWithRandomArray(10, (arr) => {
            service.sortByProperty(arr, 'a', true);
            expect(arr).toEqual([e3, e6, e1, e5, e2, e4]);
        }));

        it('should sort basic array descending', repeatWithRandomArray(10, (arr) => {
            service.sortByProperty(arr, 'b', false);
            expect(arr).toEqual([e6, e3, e4, e5, e1, e2]);
        }));

        it('should sort array with moment properties', repeatWithRandomArray(10, (arr) => {
            service.sortByProperty(arr, 'c', true);
            expect(arr).toEqual([e4, e1, e2, e6, e3, e5]);
        }));

        it('should sort array with null properties', repeatWithRandomArray(10, (arr) => {
            service.sortByProperty(arr, 'd', true);
            expect(arr.slice(0, 4)).toEqual([e4, e1, e5, e2]);
        }));

        it('should sort array of nested maps', repeatWithRandomArray(10, (arr) => {
