...
import {
    EXAM_LIVE_STATISTICS_ACTION_TOPIC,
    EXAM_LIVE_STATISTICS_ACTIONS_TOPIC,
    EXAM_LIVE_STATISTICS_STATUS_TOPIC,
    ExamActionService,
} from 'app/exam/statistics/exam-action.service';
...
describe('ExamActionService', () => {
    ...
    // Notify exam live statistics update subscribers
    it.each([true, false])('should notify exam live statistics update subscribers', (status: boolean) => {
        const examLiveStatisticsStatusObservables = new Map<number, BehaviorSubject<boolean>>();
        expect(examActionService.examLiveStatisticsStatusObservables).toEqual(examLiveStatisticsStatusObservables);

        examActionService.notifyExamLiveStatisticsUpdateSubscribers(exam, status);

        examLiveStatisticsStatusObservables.set(exam.id!, new BehaviorSubject(status));

        expect(examActionService.examLiveStatisticsStatusObservables).toEqual(examLiveStatisticsStatusObservables);

        examActionService.notifyExamLiveStatisticsUpdateSubscribers(exam, !status);

        examLiveStatisticsStatusObservables.get(exam.id!)!.next(!status);
    ...
    it.each(createActions())('should call websocket send', (action: ExamAction) => {
        ...
        expect(spy).toHaveBeenCalledOnce();
        expect(spy).toHaveBeenCalledWith(EXAM_LIVE_STATISTICS_ACTIONS_TOPIC(exam.id!), action);
    });

    // open exam live statistics websocket subscription if not existing
    it.each(createActions())('should open exam live statistics websocket subscription if not existing', (action: ExamAction) => {
        ...
        examActionService.openExamLiveStatisticsWebsocketSubscriptionIfNotExisting(exam);

        expect(examActionService.openExamLiveStatisticsWebsocketSubscriptions).toEqual(new Map([[exam.id, topic]]));
    ...
    // open exam live statistics update websocket subscription if not existing
    it.each([true, false])('should open exam live statistics update websocket subscription if not existing', (status: boolean) => {
        ...
        const notifyExamLiveStatisticsUpdateSubscribersSpy = jest.spyOn(examActionService, 'notifyExamLiveStatisticsUpdateSubscribers');

        examActionService.openExamLiveStatisticsUpdateWebsocketSubscriptionIfNotExisting(exam);

        expect(examActionService.openExamLiveStatisticsStatusWebsocketSubscriptions).toEqual(new Map([[exam.id, topic]]));
        ...
        expect(notifyExamLiveStatisticsUpdateSubscribersSpy).toHaveBeenCalledOnce();
        expect(notifyExamLiveStatisticsUpdateSubscribersSpy).toHaveBeenCalledWith(exam, status);
    });

    // topics
    it('should get correct exam live statistics action topic', () => {
        expect(EXAM_LIVE_STATISTICS_ACTION_TOPIC(exam.id!)).toBe(`/topic/exams/${exam.id!}/live-statistics-action`);
    });

    it('should get correct exam live statistics actions topic', () => {
        expect(EXAM_LIVE_STATISTICS_ACTIONS_TOPIC(exam.id!)).toBe(`/topic/exams/${exam.id!}/live-statistics-actions`);
    });

    it('should get correct exam live statistics status topic', () => {
        expect(EXAM_LIVE_STATISTICS_STATUS_TOPIC(exam.id!)).toBe(`/topic/exams/${exam.id!}/live-statistics-update`);
    });
});
