...
describe('IrisSessionService', () => {
    ...
    it('should dispatch an error if getCurrentSession returns an error', async () => {
        ...
        // then
        expect(dispatchSpy).toHaveBeenCalledWith(new ConversationErrorOccurredAction(errorMessages[IrisErrorMessageKey.SESSION_LOAD_FAILED]));
    });
    ...
    it('should dispatch an error if getCurrentSession is successful. but getMessages returned an error', async () => {
        ...
        expect(dispatchSpy).toHaveBeenCalledWith(new ConversationErrorOccurredAction(errorMessages[IrisErrorMessageKey.HISTORY_LOAD_FAILED]));
    });
    ...
    it('should dispatch an error if getCurrentSession is 404 and createSession returned an error', async () => {
        ...
        expect(dispatchSpy).toHaveBeenCalledWith(new ConversationErrorOccurredAction(errorMessages[IrisErrorMessageKey.SESSION_CREATION_FAILED]));
    });
});
