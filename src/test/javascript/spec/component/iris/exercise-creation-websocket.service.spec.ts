...
import {
    mockExerciseCreationWebsocketClientMessage,
    mockExerciseCreationWebsocketServerMessage,
    mockServerMessage,
...
describe('IrisExerciseCreationWebsocketService', () => {
    let irisWebsocketService: IrisExerciseCreationWebsocketService;
    ...
    const newMessageObservable = of(mockExerciseCreationWebsocketServerMessage);

    beforeEach(() => {
        TestBed.configureTestingModule({
            imports: [HttpClientTestingModule],
            providers: [IrisExerciseCreationWebsocketService, MockProvider(JhiWebsocketService), IrisStateStore, { provide: AccountService, useClass: MockAccountService }],
        });
        irisWebsocketService = TestBed.inject(IrisExerciseCreationWebsocketService);
    ...
    it('should receive a new student message and emit an action', fakeAsync(() => {
        const websocketReceiveMock = jest.spyOn(jhiWebsocketService, 'receive').mockReturnValue(of(mockExerciseCreationWebsocketClientMessage));
        const dispatchSpy = jest.spyOn(irisStateStore, 'dispatch');
        ...
        expect(dispatchSpy).toHaveBeenNthCalledWith(2, {
            ...new StudentMessageSentAction(
                mockExerciseCreationWebsocketClientMessage.message as IrisUserMessage,
                setTimeout(() => {}),
            ),
