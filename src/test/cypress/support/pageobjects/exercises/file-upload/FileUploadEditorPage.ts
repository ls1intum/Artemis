...
export class FileUploadEditorPage {
     ...
     */
    saveAndContinue() {
        cy.intercept(PUT, BASE_API + 'exercises/*/file-upload-submissions').as('savedSubmission');
        cy.get('#save').click();
        return cy.wait('@savedSubmission');
    }

    submit() {
        cy.intercept(POST, BASE_API + 'exercises/*/file-upload-submissions').as('fileUploadSubmission');
        cy.get('#submit').click();
