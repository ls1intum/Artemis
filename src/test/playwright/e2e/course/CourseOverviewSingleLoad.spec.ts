...
/**
 * Entering a course must load the course shell and its tab availability exactly once each.
 *
...
test.describe('Course overview navigation', { tag: '@fast' }, () => {
    test('loads the course shell and tabs once each', async ({ page, login, courseManagementAPIRequests }) => {
        await login(admin);
        ...
        const seen: Record<string, string[]> = { forOverview: [], availableTabs: [], exercisesForOverview: [] };
        const canceled: string[] = [];
        const track = (url: string, collect: (key: string) => void) => {
            if (url.includes(exercisesForOverview)) collect('exercisesForOverview');
