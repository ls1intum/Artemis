...
class WorkerMonitoringServiceTest {
    ...
    void sendsCurrentSnapshotOnlyToTheAdminTopic() {
        ...
        when(subscribers.findSubscriptions(any())).thenAnswer(invocation -> {
            ...
            when(subscription.getDestination()).thenReturn(WORKERS.template());
        ...
        service.publish();
        verify(messaging).sendMessage(WORKERS.at(), snapshot);
        when(source.workerStatuses()).thenReturn(List.of());
        service.publish();
        verify(messaging).sendMessage(WORKERS.at(), List.of());
    }
}
