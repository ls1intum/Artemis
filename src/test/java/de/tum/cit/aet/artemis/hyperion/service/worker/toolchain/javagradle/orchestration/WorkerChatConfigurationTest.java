...
class WorkerChatConfigurationTest {
    ...
    void configuredTimeoutReachesTheActualProviderRequest(boolean chatOverride) {
        ...
        var contextRunner = new ApplicationContextRunner().withInitializer(new ConfigDataApplicationContextInitializer())
                .withInitializer(context -> context.getEnvironment().setActiveProfiles("aiworker"))
                ...
                .withUserConfiguration(JavaGradleGenerationAdapterService.class).withBean(SandboxApi.class, () -> mock(SandboxApi.class))
                .withBean(ObservationRegistry.class, () -> ObservationRegistry.NOOP)
                .withPropertyValues("artemis.aiworker.workload=hyperion-generation", "spring.ai.model.chat=openai", "spring.ai.openai.api-key=test-key",
                        "spring.ai.openai.chat.model=test-model", "spring.ai.openai.timeout=10m", "spring.ai.openai.max-retries=0")
        ...
        contextRunner.run(context -> {
            ...
            var settings = context.getBean(JavaGradleGenerationAdapterService.class).settings(parameters);
            ...
            var critic = new SpecFidelityCritic(ChatClient.create(context.getBean(OpenAiChatModel.class)), JsonMapper.builder().build()).forSettings(settings);
