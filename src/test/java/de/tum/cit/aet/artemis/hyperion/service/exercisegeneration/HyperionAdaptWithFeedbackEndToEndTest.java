 ...
 * categories (additive, difficulty up/down, bug-fix, refactor, requirement-change, contradictory) and several languages. Each case asserts the run CONVERGED (differential
 * oracle still accepts), the FEEDBACK was actually APPLIED (a scenario-specific check, proving the baseline was not re-submitted unchanged), and VERIFIABILITY was preserved.
 * Gated behind {@code HYPERION_E2E_GPU}; {@code HYPERION_ADAPT_SCENARIOS} narrows the run for parallel isolated JVMs.
 * <p>
 * This drives {@link ExerciseGenerationOrchestrationService#generate} directly in {@link GenerationMode#ADAPT} against a fully-scaffolded, already-buildable exercise (the exact
 * production precondition for an adapt run — the orchestrator seeds the sandbox from the exercise's current live repository contents). It intentionally stops short of the
 * production persist/revert path ({@code GenerationPersistenceService} / {@code ExerciseAdaptationRevertService}): those are exercised separately, and asserting only on the
 * still-open {@link GenerationOutcome} keeps this test focused on the adapt engine itself (agent + differential oracle), mirroring the sibling
 * {@link HyperionExerciseGenerationEndToEndTest}.
 */
...
class HyperionAdaptWithFeedbackEndToEndTest extends AbstractSpringIntegrationLocalCILocalVCTest {
    ...
    void adaptsWithFeedback(String key, ProgrammingLanguage language, ProjectType projectType, String shortName, String feedback, AdaptCheck check) throws Exception {
        ...
        try (var outcome = orchestrator.generate(exercise, instructor(), feedback, "adaptfb-" + key, GenerationMode.ADAPT, () -> false, line -> {
