...
class PyrisWebhookServiceTest {
    ...
    void setUp() {
        ...
        slideApi = mock(SlideApi.class);
        videoSourceResolver = mock(VideoSourceResolverService.class);

        service = new PyrisWebhookService(pyrisConnectorService, mock(PyrisJobService.class), irisSettingsService, Optional.empty(), Optional.empty(), Optional.empty(),
                Optional.of(slideApi), videoSourceResolver);
    ...
    void updateLectureUnitMetadataInPyrisSendsLightweightMetadataWithoutReadingAttachmentFile() {
        ...
        verify(slideApi, never()).findAllByAttachmentVideoUnitId(unit.getId());
    }
    ...
    void updateLectureUnitVisibilityInPyrisFetchesSlidesAndSendsSortedLightweightVisibility() {
        ...
        when(irisSettingsService.isEnabledForCourse(unit.getLecture().getCourse())).thenReturn(true);
        when(slideApi.findAllByAttachmentVideoUnitId(unit.getId())).thenReturn(List.of(slide(3, null), slide(1, hiddenUntil), slide(2, null)));

        String token = service.updateLectureUnitVisibilityInPyris(unit);
    ...
    private static Slide slide(int slideNumber, ZonedDateTime hiddenUntil) {
