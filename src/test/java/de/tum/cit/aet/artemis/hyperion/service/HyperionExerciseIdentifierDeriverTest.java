...
class HyperionExerciseIdentifierDeriverTest {
    ...
    void buildsThePackageOnTheInstitutionalPrefix() {
        assertThat(HyperionExerciseIdentifierDeriver.derivePackageName("Bounded Stack", ProjectType.GRADLE_GRADLE)).isEqualTo("de.tum.cit.aet.boundedstack");
    }
    ...
    void namesThePackageAfterTheTitleRatherThanAfterTheAbbreviatedShortName() {
        ...
        assertThat(HyperionExerciseIdentifierDeriver.derivePackageName("Grade Classification with Enum Outcomes", ProjectType.GRADLE_GRADLE))
    ...
    void keepsTheConventionsTheClientProposalEstablished(String title, String expected) {
        assertThat(HyperionExerciseIdentifierDeriver.derivePackageName(title, ProjectType.GRADLE_GRADLE)).isEqualTo(expected);
    }

    @Test
    void dropsStopWordsFromThePackageJustAsItDropsThemFromTheShortName() {
        assertThat(HyperionExerciseIdentifierDeriver.derivePackageName("Traversal of the Binary Search Trees", ProjectType.GRADLE_GRADLE))
    ...
    void escapesATitleThatIsAJavaKeywordInsteadOfProducingAnInvalidPackage() {
        ...
        assertThat(HyperionExerciseIdentifierDeriver.derivePackageName("Enum", ProjectType.GRADLE_GRADLE)).isEqualTo("de.tum.cit.aet.enumexercise");
    }

    @Test
    void fallsBackToAWordForATitleWithNoUsableLettersAtAll() {
        assertThat(HyperionExerciseIdentifierDeriver.derivePackageName("42 !!!", ProjectType.GRADLE_GRADLE)).isEqualTo("de.tum.cit.aet.exercise");
    }

    @Test
    void keepsThePackageWithinTheRuleThatValidatesItEvenForAnAbsurdTitle() {
        String packageName = HyperionExerciseIdentifierDeriver.derivePackageName("a".repeat(200), ProjectType.GRADLE_GRADLE);
    ...
    void stopsAtAWordBoundaryRatherThanCuttingAWordInHalfToFillTheBudget() {
        ...
        String packageName = HyperionExerciseIdentifierDeriver.derivePackageName("%s %s".formatted("a".repeat(50), "b".repeat(50)), ProjectType.GRADLE_GRADLE);
