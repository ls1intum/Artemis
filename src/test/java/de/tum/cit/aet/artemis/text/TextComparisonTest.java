...
class TextComparisonTest {
    ...
    void compareSubmittedAnswers() {
        ...
        assertThat(SubmissionComparisonUtil.isContentEqualTo(submission1, submission2)).isTrue();  // submission with same text
        assertThat(SubmissionComparisonUtil.isContentEqualTo(submission1, submission3)).isFalse(); // submission with different text
        assertThat(SubmissionComparisonUtil.isContentEqualTo(submission3, submission4)).isFalse();
        assertThat(SubmissionComparisonUtil.isContentEqualTo(submission4, submission5)).isTrue();  // both submission with null text

        assertThat(SubmissionComparisonUtil.isContentEqualTo(submission2, null)).isFalse(); // one submission null
        assertThat(SubmissionComparisonUtil.isContentEqualTo(null, submission4)).isFalse();  // one submission null, other null text
        assertThat(SubmissionComparisonUtil.isContentEqualTo((TextSubmission) null, null)).isTrue(); // both submissions null
    }
}
