...
class UserJenkinsLocalVCIntegrationTest extends AbstractSpringIntegrationJenkinsLocalVCTest {
    ...
    void updateUser_asInstructor_forbidden() throws Exception {
        request.put("/api/account/admin/users", new ManagedUserVM(userTestService.getStudent()), HttpStatus.FORBIDDEN);
    }
    ...
    void shouldBlockUserIfAccountNotActivated() throws Exception {
        ...
        request.put("/api/account/admin/users", new ManagedUserVM(user, password), HttpStatus.OK);
    ...
    @Test
    @WithMockUser(username = TEST_PREFIX + "student1", roles = "USER")
    void initializeUserDeactivated() throws Exception {
        userTestService.initializeUserDeactivated();
    }
