...
/**
 * Builds the {@code copyOut} report tars the verifier reads from the sandbox's verifier-owned reports directory, so {@link DifferentialVerifierTest} can drive the
 * copyOut plus production-parser flow without Docker. Each collected file is routed by the {@code <seq>__<canonical>} name the pristine {@code verify.sh} assigns: JUnit reports
 * carry the {@link SandboxBuildCommands#COLLECTED_JUNIT_TOKEN} token, SCA reports keep their per-tool canonical name. Entries are prefixed with the assignment directory the
 * way Docker prefixes copied-out entries, so {@link CollectedReports} strips it.
 */
final class ReportTarFixtures {
    ...
    static TarArchiveInputStream junitReports(String assignment, List<String> allNames, List<String> failedNames) {
        return tar(assignment, Map.of("0001" + SandboxBuildCommands.COLLECTED_NAME_SEPARATOR + SandboxBuildCommands.COLLECTED_JUNIT_TOKEN,
    ...
    static TarArchiveInputStream junitAndScaReports(String assignment, List<String> allNames, List<String> failedNames, Map<String, String> scaReportsByCanonicalName) {
        ...
        entries.put("0001" + SandboxBuildCommands.COLLECTED_NAME_SEPARATOR + SandboxBuildCommands.COLLECTED_JUNIT_TOKEN,
                junitXml(allNames, failedNames).getBytes(StandardCharsets.UTF_8));
        int seq = 2;
        for (Map.Entry<String, String> sca : scaReportsByCanonicalName.entrySet()) {
            entries.put("%04d".formatted(seq++) + SandboxBuildCommands.COLLECTED_NAME_SEPARATOR + sca.getKey(), sca.getValue().getBytes(StandardCharsets.UTF_8));
        }
