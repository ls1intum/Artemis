...
class PyrisLectureIngestionTest extends AbstractIrisIntegrationTest {
    ...
    @Test
    @WithMockUser(username = TEST_PREFIX + "instructor1", roles = "INSTRUCTOR")
    void ingestionCompleteSavesSlidePageNumberMap() throws Exception {
        activateIrisFor(lecture1.getCourse());
        ...
        PyrisStageDTO doneStage = new PyrisStageDTO("doneStage", 1, PyrisStageState.DONE, "Completed successfully.", false, null);
        Map<String, Integer> slidePageNumberMap = Map.of("0", 1, "1", 2, "2", -1, "3", 4);
        PyrisLectureIngestionStatusUpdateDTO statusUpdate = new PyrisLectureIngestionStatusUpdateDTO("Success", List.of(doneStage), unit.getId(), null, slidePageNumberMap);

        var headers = new HttpHeaders(new LinkedMultiValueMap<>(Map.of(HttpHeaders.AUTHORIZATION, List.of(Constants.BEARER_PREFIX + jobToken))));
        request.postWithoutResponseBody("/api/iris/internal/webhooks/ingestion/runs/" + jobToken + "/status", statusUpdate, HttpStatus.OK, headers);

        AttachmentVideoUnit updatedUnit = attachmentVideoUnitTestRepository.findByIdElseThrow(unit.getId());
        assertThat(updatedUnit.getSlidePageNumberMap()).isNotNull().hasSize(4).containsEntry(0, 1).containsEntry(1, 2).containsEntry(2, -1).containsEntry(3, 4);
    }
}
