...
/**
 * Unit tests for {@link BareGitRepositoryService#verifyRefUpdateResult(RefUpdate.Result, String, LocalVCRepositoryUri)}.
 * <p>
 ...
 */
class BareGitRepositoryRefUpdateResultTest {
    ...
    void shouldAcceptSuccessfulRefUpdateResults(RefUpdate.Result result) {
        assertThatCode(() -> BareGitRepositoryService.verifyRefUpdateResult(result, REF_NAME, TARGET_REPO_URI)).doesNotThrowAnyException();
    }
    ...
    void shouldRejectFailedRefUpdateResults(RefUpdate.Result result) {
        assertThatExceptionOfType(IOException.class).isThrownBy(() -> BareGitRepositoryService.verifyRefUpdateResult(result, REF_NAME, TARGET_REPO_URI))
                .withMessageContaining(result.name()).withMessageContaining(REF_NAME);
    }
}
