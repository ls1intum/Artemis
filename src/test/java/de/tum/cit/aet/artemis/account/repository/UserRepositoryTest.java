...
class UserRepositoryTest extends AbstractSpringIntegrationIndependentTest {
     ...
     * read in the endpoint, so without the condition each would rotate the password and return a plaintext one, of which
     * only the last saved would still match the stored hash.
     */
    @Test
    void testInitializeLtiPasswordSucceedsExactlyOnce() {
        User user = eligibleLtiUser(TEST_PREFIX + "lticlaim");

        assertThat(userRepository.initializeLtiPassword(user.getId(), "hash-one")).as("the first caller initialises it").isEqualTo(1);
        assertThat(userRepository.initializeLtiPassword(user.getId(), "hash-two")).as("a second caller changes nothing").isZero();

        User reloaded = userRepository.findById(user.getId()).orElseThrow();
        assertThat(reloaded.isLtiInitialized()).isTrue();
        assertThat(reloaded.getPassword()).as("the password of the caller that won is the one that is stored").isEqualTo("hash-one");

        userRepository.delete(user);
    }

    /**
     * The account state is part of the writing statement rather than checked beforehand, so an administrator deactivating the
     * account in between cannot be overwritten and no password is handed out.
     */
    @Test
    void testInitializeLtiPasswordDoesNothingForADeactivatedAccount() {
        User user = eligibleLtiUser(TEST_PREFIX + "ltideact");
        user.setActivated(false);
        userRepository.save(user);

        assertThat(userRepository.initializeLtiPassword(user.getId(), "hash")).isZero();

        User reloaded = userRepository.findById(user.getId()).orElseThrow();
        assertThat(reloaded.getActivated()).as("the deactivation stands").isFalse();
        assertThat(reloaded.isLtiInitialized()).isFalse();
        assertThat(reloaded.getPassword()).isNotEqualTo("hash");

        userRepository.delete(user);
    }

    @Test
    void testInitializeLtiPasswordDoesNothingForASoftDeletedAccount() {
        User user = eligibleLtiUser(TEST_PREFIX + "ltideleted");
    ...
    @Test
    void testInitializeLtiPasswordDoesNothingForAnAccountTheLaunchDidNotCreate() {
        User user = eligibleLtiUser(TEST_PREFIX + "ltinotcreated");
    ...
    @Test
    void testInitializeLtiPasswordDoesNothingForAnExternalAccount() {
        User user = eligibleLtiUser(TEST_PREFIX + "ltiexternal");
    ...
    @Test
    void testInitializeLtiPasswordDoesNothingForAnUnknownUser() {
        assertThat(userRepository.initializeLtiPassword(-1L, "hash")).isZero();
    }

    private User eligibleLtiUser(String login) {
