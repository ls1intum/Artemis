...
class DatabaseQueryCountTest extends AbstractSpringIntegrationIndependentTest {
    ...
    void testGetAllCoursesForDashboardRealisticQueryCount() throws Exception {
        ...
        // potentially, we might get a course that has faqs disabled, in which case we would have 15 calls instead of 16
        int numberOfCounts = course.isFaqEnabled() ? 16 : 15;
        assertThatDb(() -> {
        ...
        // 1 DB call to check Iris course settings (SELECT only, defaults used if not found)
