...
class SharedQueueProcessingServicePauseTest {
    ...
    void setUp() {
        ...
        runningJob = new BuildJobQueueItem(JOB_ID, "job being submitted", new BuildAgentDTO(AGENT_NAME, "address", "display name"), 1, 2, 3, 0, 1, null, null,
                new JobTimingInfo(now, now, null, null, 60), null, null);
        lenient().when(processingJobs.get(JOB_ID)).thenReturn(runningJob);
        lenient().when(processingJobs.getAll(anySet())).thenReturn(Map.of(JOB_ID, runningJob));
    }

    private SharedQueueProcessingService.BuildAttemptState registerActiveAttempt() {
        var attemptState = new SharedQueueProcessingService.BuildAttemptState(runningJob);
        ReflectionTestUtils.setField(service, "activeBuildAttempts", new ConcurrentHashMap<>(Map.of(JOB_ID, attemptState)));
        return attemptState;
    }
    ...
    void cancelsAndRequeuesAJobThatIsStillBeingSubmitted() {
        ...
        var attemptState = registerActiveAttempt();
        ...
        ReflectionTestUtils.invokeMethod(service, "finishInternallyRequeuedAttempt", runningJob, attemptState);
        ArgumentCaptor<BuildJobQueueItem> requeued = ArgumentCaptor.captor();
        verify(buildJobQueue).add(requeued.capture());
        assertThat(requeued.getValue().id()).isEqualTo(JOB_ID);
        assertThat(requeued.getValue().retryCount()).isEqualTo(1);
    ...
    void doesNotRequeueAJobThatFinishedBeforeItCouldBeCancelled() {
        ...
        assertThat(attemptState.beginCompletion()).as("the internal requeue claim was rolled back").isFalse();
