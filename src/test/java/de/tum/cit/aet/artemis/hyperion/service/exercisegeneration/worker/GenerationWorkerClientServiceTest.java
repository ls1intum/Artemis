...
class GenerationWorkerClientServiceTest {
    ...
    private final GenerationWorkerClientService client = new GenerationWorkerClientService(new de.tum.cit.aet.artemis.aiworker.api.AiWorkerApi(mock(),
            new de.tum.cit.aet.artemis.aiworker.service.WorkerClientService(factory, new de.tum.cit.aet.artemis.aiworker.api.WorkerMessageCodecApi())), codec);

    @BeforeEach
    void setup() throws Exception {
        when(factory.createConnection()).thenReturn(connection);
        when(connection.createSession(true, Session.AUTO_ACKNOWLEDGE)).thenReturn(session);
        when(session.createQueue("aiworker.worker.events")).thenReturn(queue);
