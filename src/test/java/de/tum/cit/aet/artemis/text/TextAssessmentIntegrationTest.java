...
class TextAssessmentIntegrationTest extends AbstractSpringIntegrationIndependentBatchTest {
    ...
    void retrieveParticipationForSubmission_resultOfComplaintResponse_ok() throws Exception {
        ...
        assertThat(resultRepository.existsBySubmissionIdAndCorrectionRound(textSubmission.getId(), 1)).as("the complaint response is stored as the result of round 1").isTrue();
        ...
        assertThat(resultRepository.existsBySubmissionIdAndCorrectionRound(textSubmission.getId(), 2)).as("opening the existing result creates no further result").isFalse();
    }
