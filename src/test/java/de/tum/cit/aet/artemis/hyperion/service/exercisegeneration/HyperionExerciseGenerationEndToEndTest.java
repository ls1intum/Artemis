...
class HyperionExerciseGenerationEndToEndTest extends AbstractSpringIntegrationLocalCILocalVCTest {
    ...
    @Autowired
    private HyperionProblemStatementGenerationService problemStatementGenerationService;
    ...
    void setUp() {
        ...
        // Drive the real agent loop with the real GPU model through the actual production transport: the Spring AI OpenAI starter's OpenAiChatModel, wrapped in the same
        // HarmonyScrubbingChatModel decorator production wires via HyperionChatModelConfiguration. The injected bean is the (mocked) test ChatModel, so make it delegate.
        OpenAiChatOptions options = OpenAiChatOptions.builder().baseUrl(GPU_BASE_URL).apiKey(GPU_API_KEY).model(GPU_MODEL).build();
        OpenAiChatModel realOpenAi = OpenAiChatModel.builder().options(options).build();
        ChatModel wrapped = new HarmonyScrubbingChatModel(realOpenAi);
        // The loop pins the request model from chatModel.getDefaultOptions().getModel() (see AgentLoopRunner#configuredModel()); the injected bean is the MOCK, so stub it too.
    ...
    void generatesExerciseWithStructuralTests_endToEnd() throws Exception {
        ...
        try (var outcome = orchestrator.generate(exercise, instructor(), prompt, "e2e-structural", GenerationMode.GENERATE, () -> false, line -> {
    ...
    private void runAndAssertAccepted(ProgrammingExercise exercise, String prompt, String mode, Consumer<GenerationOutcome> extraChecks) {
        StringBuilder transcript = new StringBuilder();
        try (var outcome = orchestrator.generate(exercise, instructor(), prompt, "e2e-" + mode, GenerationMode.GENERATE, () -> false, line -> {
            transcript.append(line).append('\n');
