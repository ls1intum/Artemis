...
class Lti13InitiationIntegrationTest extends AbstractLtiIntegrationTest {
    ...
    // No @AfterEach cleanup: each test uses a UUID-keyed registrationId so there are no intra-class collisions,
    // and several later-running LTI tests (notably LtiIntegrationTest.getAllConfiguredLtiPlatformsAsAdmin and
    // updateLtiPlatformConfigurationAsAdmin) implicitly rely on auto-generated IDs from prior LTI tests existing
    // in the shared DB — wiping rows here causes those tests to fail with optimistic-locking exceptions.
