 ...
 * {@code @JdbcTypeCode(SqlTypes.JSON)} columns. This test pins that behavior so a Jackson upgrade or a custom {@code ObjectMapper}/{@code CoercionConfig} that disables it fails
...
class QuizJsonBooleanCoercionTest {
    ...
    // A default ObjectMapper mirrors the mapper Hibernate uses for @JdbcTypeCode(SqlTypes.JSON) columns: ALLOW_COERCION_OF_SCALARS is enabled by default.
