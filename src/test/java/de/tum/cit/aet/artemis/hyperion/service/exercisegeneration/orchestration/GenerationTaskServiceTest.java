...
class GenerationTaskServiceTest {
    ...
    void setUp() {
        ...
        when(programmingExerciseRepository.findWithTemplateAndSolutionParticipationAndBuildConfigById(EXERCISE_ID)).thenReturn(Optional.of(exercise));
    }
    ...
    void acceptedRun_persistsExactlyOnceAndDestroysTheSandbox() {
        when(persistenceService.persist(any(), any(), any())).thenReturn(Map.of());
        run(GenerationMode.GENERATE, outcomeWith(AgentLoopResult.Status.COMPLETED, new VerificationResult(true, true, true, 3, List.of())));
        ...
        verify(adaptationRevertService, never()).recordBaseline(any(), anyString(), any());
    ...
    void acceptedAdaptRun_recordsARevertBaseline() {
        when(persistenceService.persist(any(), any(), any())).thenReturn(Map.of(RepositoryType.SOLUTION, "head-sha"));
        ...
        verify(adaptationRevertService).recordBaseline(eq(exercise), eq(JOB_ID), any());
    }

    @Test
    void nonAcceptedButRecoverableRun_savesDraftAndEmitsNeedsReview() {
        when(recoveryService.recover(any(), any(), any(), anyString())).thenReturn(new GenerationRecoveryService.RecoveryResult(2, false, null));
    ...
    void deletedExercise_failsClosedWithError_withoutRunningTheOrchestrator() {
        when(programmingExerciseRepository.findWithTemplateAndSolutionParticipationAndBuildConfigById(EXERCISE_ID)).thenReturn(Optional.empty());
