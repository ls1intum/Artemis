...
class SingleUserNotificationServiceTest extends AbstractSpringIntegrationIndependentTest {
    ...
    void shouldCreateNewCpcPlagiarismCaseNotificationWhenCourseSpecificNotificationsEnabled() {
        ...
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            Optional<CourseNotification> newCpcPlagiarismCaseNotification = notifications.stream()
                    .filter(notification -> notification.getCourse().getId().equals(course.getId()) && notification.getType() == 13).findFirst();
    ...
    void shouldCreateNewPlagiarismCaseNotificationWhenCourseSpecificNotificationsEnabled() {
        ...
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            Optional<CourseNotification> newPlagiarismCaseNotification = notifications.stream()
                    .filter(notification -> notification.getCourse().getId().equals(course.getId()) && notification.getType() == 14).findFirst();
    ...
    void shouldCreatePlagiarismCaseVerdictNotificationWhenCourseSpecificNotificationsEnabled() {
        ...
        await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            var newPlagiarismVerdictNotification = notifications.stream()
                    .filter(notification -> notification.getCourse().getId().equals(course.getId()) && notification.getType() == 17).findFirst();
