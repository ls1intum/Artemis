...
class AuthoritativeVerificationServiceTest {
    ...
    private static SandboxBuildCommandService buildCommandFactory() {
        ...
        when(phases.getDefaultBuildPlanPhasesFor(any())).thenReturn(java.util.List.of());
    ...
    private static SandboxExecResult result(int tests, int failures, int errors, int exit) {
        ...
        // satisfied.
        java.util.List<String> failed = (failures + errors) > 0 ? names : java.util.List.of();
    ...
    private static SandboxExecResult resultWithFailsAndCounts(int tests, int failures, int errors, int exit, java.util.List<String> allNames, java.util.List<String> failedNames) {
     ...
     */
    private static SandboxExecResult resultWithFails(int tests, int exit, java.util.List<String> allNames, java.util.List<String> failedNames) {
        StringBuilder body = new StringBuilder("build log...\n");
        for (String name : allNames) {
            body.append("HYPERION_TESTNAME ").append(TEST_NONCE).append(' ').append(name).append('\n');
        }
    ...
    private static VerificationResult verifyWithSeededStructural(SandboxExecResult solution, SandboxExecResult template, String problemStatement, Set<String> seededStructural) {
        return newVerifier().verify(new ScriptedSandbox(solution, template, problemStatement), "s", new ProgrammingExercise(), java.util.Map.of(), java.util.Map.of(),
                java.util.Map.of(), java.util.Map.of(), Set.of(), seededStructural);
    }

    // ----------------------------------------------------------------------------------------------------------------
    // Defect A (CRITICAL): the verdict must NOT be forgeable. The verifier re-seeds and runs a PRISTINE verify.sh OUTSIDE
    // /workspace, so an agent that overwrote its own /workspace/verify.sh to echo a perfect marker set with no real tests
    // is graded by the pristine script (which finds no real reports) and rejected.
    // ----------------------------------------------------------------------------------------------------------------

    /**
     * A sandbox that returns DIFFERENT output for the agent's {@code /workspace/verify.sh} (a forged stub) versus the verifier-owned {@code /opt/hyperion/verify.sh} (the pristine
     * script). It models the real container behaviour: whichever script the caller actually invokes determines what runs. Records every exec command so the test can assert that
     * the
     * verifier ran the PRISTINE path and never the agent's copy.
     */
    private static final class PathDispatchingSandbox implements InteractiveSandbox {
        ...
        private final java.util.List<String> execCommands = new java.util.ArrayList<>();
    ...
    @Test
    void exploit_forgedWorkspaceVerifyScript_isIgnored_pristineScriptFindsNoTests_rejected() {
        // THE HEADLINE EXPLOIT. The agent overwrote /workspace/verify.sh to echo a PERFECT passing solution / failing template marker set (5 tests, the right names, the right
        // fails)
        // with NO real tests present. The pristine /opt/hyperion/verify.sh the verifier re-seeds and runs finds no real reports: it reports tests=0 for both runs. The verifier
        // must
        // grade by the PRISTINE script and REJECT — never trusting the agent's forged copy. (With the old code, which ran the agent's /workspace/verify.sh, this exact scenario was
        // ACCEPTED — a fully forged verdict.)
        java.util.List<String> names = java.util.List.of("sortsUnsortedArray", "sortsArrayWithDuplicates", "t3", "t4", "t5");
        SandboxExecResult forgedSolution = resultWithFails(5, 0, names, java.util.List.of());
        SandboxExecResult forgedTemplate = resultWithFails(5, 1, names, names);
        // The pristine build finds no reports at all (no real tests) -> tests=0 both runs.
        ...
        // Prove the verifier actually ran the PRISTINE script and never the agent's forged /workspace copy.
    ...
    @Test
    void exploit_pristineScriptStillGradesCorrectly_whenTheRealBuildIsGood() {
        // The mirror of the exploit: when the pristine script reports a genuinely-good build (the agent did NOT forge), the verdict is accepted. Confirms the re-seed/pristine-run
        // path does not break the happy case.
        java.util.List<String> names = java.util.List.of("sortsUnsortedArray", "sortsArrayWithDuplicates");
        PathDispatchingSandbox sandbox = new PathDispatchingSandbox(result(0, 0, 0, 0), result(0, 0, 0, 0), resultWithFails(2, 0, names, java.util.List.of()),
    ...
    @Test
    void accepts_whenSolutionPassesAndTemplateFailsSameTests() {
     ...
     */
    @Test
    void accepts_whenTaskBindingTestNamesCarryParentheses() {
        String problemStatement = "# Sort\n[task][Sort an array](testBubbleSort(),testMergeSort())\n";
        java.util.List<String> names = java.util.List.of("testBubbleSort()", "testMergeSort()");
        SandboxExecResult solution = resultWithFails(2, 0, names, java.util.List.of());
    ...
    @Test
    void accepts_whenTaskBindingOmitsParenthesesButTestNameHasThem() {
        String problemStatement = "# Sort\n[task][Sort an array](testBubbleSort,testMergeSort)\n";
        java.util.List<String> names = java.util.List.of("testBubbleSort()", "testMergeSort()");
        SandboxExecResult solution = resultWithFails(2, 0, names, java.util.List.of());
        SandboxExecResult template = resultWithFails(2, 1, names, names);
    ...
    private static VerificationResult verifyWithFiles(SandboxExecResult solution, SandboxExecResult template, java.util.Map<String, String> seedTests,
            java.util.Map<String, String> producedTests, java.util.Map<String, String> producedTemplate, java.util.Map<String, String> producedSolution) {
    ...
    void integrityGates_acceptWhenHarnessUnchangedAndNoLeak() {
        ...
        var seedTests = java.util.Map.of("test.cabal", SEED_CABAL);
        var producedTests = java.util.Map.of("test.cabal", SEED_CABAL.replace("${solutionWorkingDirectory}", "assignment"));
        var producedTemplate = java.util.Map.of("src/Exercise.hs", "factorial _ = error \"todo: implement factorial here\"\n");
        var producedSolution = java.util.Map.of("src/Exercise.hs", SOLUTION_BODY);
    ...
    void integrityGates_rejectWhenHarnessBuildLayoutTampered() {
        ...
        var seedTests = java.util.Map.of("test.cabal", SEED_CABAL);
        var producedTests = java.util.Map.of("test.cabal", SEED_CABAL.replace("${solutionWorkingDirectory}/src", "assignment/solution/src"));
        VerificationResult result = verifyWithFiles(result(5, 0, 0, 0), result(5, 3, 0, 1), seedTests, producedTests, java.util.Map.of(), java.util.Map.of());
    ...
    void integrityGates_rejectWhenTemplateLeaksSolutionToANonGradedPath() {
        ...
        var producedTemplate = java.util.Map.of("src/Exercise.hs", "factorial _ = error \"todo: implement the factorial function here\"\n", "doc/reference_solution.hs",
                SOLUTION_BODY);
        var producedSolution = java.util.Map.of("src/Exercise.hs", SOLUTION_BODY);
        VerificationResult result = verifyWithFiles(result(5, 0, 0, 0), result(5, 3, 0, 1), java.util.Map.of(), java.util.Map.of(), producedTemplate, producedSolution);
        assertThat(result.accepted()).isFalse();
    ...
    void integrityGates_failOpenWhenNoFilesProvided() {
        ...
        VerificationResult result = verifyWithFiles(result(5, 0, 0, 0), result(5, 3, 0, 1), java.util.Map.of(), java.util.Map.of(), java.util.Map.of(), java.util.Map.of());
        assertThat(result.accepted()).isTrue();
    }

    @Test
    void accepts_catch2_whenTestCaseCountsMatchEvenThoughAssertionCountsWouldNot() {
        ...
        // gate.)
        java.util.List<String> names = java.util.List.of("stack_initially_empty", "push_then_pop", "size_tracks_elements");
        VerificationResult result = verify(resultWithFails(3, 0, names, java.util.List.of()), resultWithFails(3, 1, names, names),
    ...
    @Test
    void accepts_whenTemplateFailsButBuildExitCodeIsZero() {
    ...
    @Test
    void accepts_whenTemplateFailsWithErrors() {
    ...
    @Test
    void rejects_whenTemplateDoesNotCompile() {
    ...
    @Test
    void rejects_whenTemplatePasses() {
    ...
    @Test
    void rejects_whenSolutionFails() {
    ...
    @Test
    void rejects_whenSolutionHasNoTests() {
    ...
    @Test
    void rejects_whenTemplateRunsFewerTestsThanSolution() {
    ...
    @Test
    void rejects_whenBuildHelperEmittedNoSummary() {
    ...
    @Test
    void rejects_whenSolutionTimesOut() {
    ...
    @Test
    void rejects_whenTemplateFailsTooFewTests() {
    ...
    @Test
    void rejects_whenProblemStatementHasNoTaskBindings() {
    ...
    @Test
    void accepts_whenTaskBindingsResolveToRealTestNames() {
        ...
        java.util.List<String> names = java.util.List.of("sortsUnsortedArray()", "sortsArrayWithDuplicates()");
        VerificationResult result = verify(resultWithFails(2, 0, names, java.util.List.of()), resultWithFails(2, 1, names, names));
        assertThat(result.accepted()).isTrue();
    }

    @Test
    void rejects_whenATaskBindingReferencesADisplayNameInsteadOfTheMethodName() {
    ...
    @Test
    void rejects_whenSolutionRanTestsButEmittedNoTestNames() {
    ...
    @Test
    void rejects_whenFewerTestNamesThanTestsAreEmitted() {
    ...
    @Test
    void accepts_whenEveryTaskBoundTestFailsOnTemplate() {
    ...
    @Test
    void rejects_whenATaskBoundBehaviouralTestPassesOnTemplate_rustFibonacciZero() {
        ...
        java.util.List<String> all = java.util.List.of("test_factorial_of_0", "test_factorial_of_5", "test_factorial_of_20", "test_fibonacci_of_0", "test_fibonacci_of_1",
                "test_fibonacci_of_10", "test_fibonacci_of_50");
        java.util.List<String> failedOnTemplate = java.util.List.of("test_factorial_of_5", "test_factorial_of_20", "test_fibonacci_of_1", "test_fibonacci_of_10",
        ...
        java.util.List<String> failed2 = new java.util.ArrayList<>(failedOnTemplate);
        ...
        VerificationResult result = verify(resultWithFails(7, 0, all, java.util.List.of()), resultWithFails(7, 1, all, failed2), ps);
        assertThat(result.accepted()).isFalse();
        assertThat(result.reasons()).anyMatch(r -> r.contains("PASS on the template") && r.contains("test_fibonacci_of_0"));
    }

    @Test
    void rejects_whenATaskBoundBehaviouralTestPassesOnTemplate_tsPopUndefined() {
        ...
        java.util.List<String> all = java.util.List.of("Stack_push_pop_cycle", "Stack_peek_returns_top_without_removal", "Stack_isEmpty_behaviour",
                "Stack_pop_on_empty_returns_undefined");
        java.util.List<String> failedOnTemplate = java.util.List.of("Stack_push_pop_cycle", "Stack_peek_returns_top_without_removal", "Stack_isEmpty_behaviour");
        String ps = "# Stack\n[task][Push/pop](Stack_push_pop_cycle)\n[task][Pop on empty returns undefined](Stack_pop_on_empty_returns_undefined)\n";
        VerificationResult result = verify(resultWithFails(4, 0, all, java.util.List.of()), resultWithFails(4, 1, all, failedOnTemplate), ps);
        assertThat(result.accepted()).isFalse();
        assertThat(result.reasons()).anyMatch(r -> r.contains("PASS on the template") && r.contains("Stack_pop_on_empty_returns_undefined"));
    }

    @Test
    void accepts_whenAnUnboundHarnessTestPassesOnTemplateButAllTaskBoundTestsFail() {
        ...
        java.util.List<String> all = java.util.List.of("CompileStack", "stack_initially_empty", "stack_push_top", "stack_pop");
        // The template fails the three behavioural tests but the harness CompileStack passes on both.
        java.util.List<String> failedOnTemplate = java.util.List.of("stack_initially_empty", "stack_push_top", "stack_pop");
    ...
    @Test
    void gap1_rejects_whenAnUNBOUNDBehaviourTestPassesOnTemplate() {
        ...
        java.util.List<String> all = java.util.List.of("reverse_non_empty", "reverse_empty_string", "is_palindrome_true", "is_palindrome_false");
        // The template fails three behaviour tests but the unbound reverse_empty_string PASSES on it (placeholder returns "").
        java.util.List<String> failedOnTemplate = java.util.List.of("reverse_non_empty", "is_palindrome_true", "is_palindrome_false");
    ...
    @Test
    void gap1_accepts_whenOnlyABuildConfigureGatePassesOnTemplate_everyBehaviourTestFails() {
        ...
        java.util.List<String> all = java.util.List.of("TestConfigure", "CompileSort", "sorts_ascending", "sorts_with_duplicates", "stable_on_equal_keys");
        java.util.List<String> failedOnTemplate = java.util.List.of("sorts_ascending", "sorts_with_duplicates", "stable_on_equal_keys");
        String ps = "# Sort\n[task][Ascending](sorts_ascending)\n[task][Duplicates](sorts_with_duplicates)\n[task][Stable](stable_on_equal_keys)\n";
        VerificationResult result = verify(resultWithFails(5, 0, all, java.util.List.of()), resultWithFails(5, 1, all, failedOnTemplate), ps);
    ...
    @Test
    void gap1_accepts_whenAFRAMEWORKPREFIXEDBuildGatePassesOnTemplate() {
        ...
        java.util.List<String> all = java.util.List.of("GBS-Tester-1.36.TestConfigure", "GBS-Tester-1.36.CompileSort", "sort-test.empty_initial", "sort-test.push_top");
        java.util.List<String> failedOnTemplate = java.util.List.of("sort-test.empty_initial", "sort-test.push_top");
    ...
    @Test
    void gap1_rejects_whenABuildGateNamedTestIsActuallyTheOnlyFailingDiscriminator_unboundBehaviourStillPasses() {
        ...
        java.util.List<String> all = java.util.List.of("CompileSort", "push_grows", "peek_returns_top");
        java.util.List<String> failedOnTemplate = java.util.List.of("push_grows");
        String ps = "# Stack\n[task][Push](push_grows)\n";
        VerificationResult result = verify(resultWithFails(3, 0, all, java.util.List.of()), resultWithFails(3, 1, all, failedOnTemplate), ps);
        assertThat(result.accepted()).isFalse();
    ...
    void rejects_whenABoundStructuralTestPassesOnTemplate_rubyStackStructure() {
        ...
        java.util.List<String> all = java.util.List.of("test_new_stack_is_empty", "test_push_makes_not_empty", "test_peek_returns_last_without_removing",
                "test_pop_returns_last_and_removes", "test_pop_on_empty_raises", "test_peek_on_empty_raises", "test_stack_structure");
        java.util.List<String> behaviouralFailOnTemplate = java.util.List.of("test_new_stack_is_empty", "test_push_makes_not_empty", "test_peek_returns_last_without_removing",
        ...
        VerificationResult result = verify(resultWithFails(7, 0, all, java.util.List.of()), resultWithFails(7, 1, all, behaviouralFailOnTemplate), ps);
        assertThat(result.accepted()).isFalse();
        assertThat(result.reasons()).anyMatch(r -> r.contains("PASS on the template") && r.contains("test_stack_structure"));
    }

    @Test
    void rejects_whenTemplateReportsFailuresButEmitsNoFailNames() {
    ...
    @Test
    void rejects_whenARepositoryThatWasSeededNonEmptyExtractsEmpty() {
        ...
        VerificationResult result = newVerifier().verify(
                new ScriptedSandbox(resultWithFails(2, 0, names, java.util.List.of()), resultWithFails(2, 1, names, names), PROBLEM_STATEMENT_WITH_TASK), "s",
                new ProgrammingExercise(), java.util.Map.of(), java.util.Map.of(), java.util.Map.of(), java.util.Map.of(), Set.of("tests"));
        assertThat(result.accepted()).isFalse();
    ...
    private static SandboxExecResult forgedTemplateRun(java.util.List<String> allNames, java.util.List<String> reallyFailed, String forgedLuckyName) {
    ...
    @Test
    void exploit_forgedUnsignedTestFailMarkerPrintedByAgentTest_isIgnored_brokenTemplateRejected() {
        ...
        java.util.List<String> all = java.util.List.of("push_grows", "peek_returns_top");
        java.util.List<String> reallyFailedOnTemplate = java.util.List.of("push_grows");
        String ps = "# Stack\n[task][Push](push_grows)\n[task][Peek](peek_returns_top)\n";
        SandboxExecResult solution = resultWithFails(2, 0, all, java.util.List.of());
    ...
    @Test
    void exploit_forgedUnsignedResultLine_doesNotInflateFailureCount() {
        ...
        java.util.List<String> all = java.util.List.of("a", "b", "c", "d");
        String ps = "# X\n[task][A](a)\n";
        SandboxExecResult solution = resultWithFails(4, 0, all, java.util.List.of());
    ...
    class StaticCodeAnalysisParityGate {
        ...
        private SandboxExecResult solutionWithScaFindings(String... findings) {
            java.util.List<String> names = java.util.List.of(DEFAULT_BOUND_NAMES);
            StringBuilder body = new StringBuilder("build log...\n");
        ...
        private de.tum.cit.aet.artemis.programming.domain.StaticCodeAnalysisCategory category(String name, de.tum.cit.aet.artemis.assessment.domain.CategoryState state,
                double penalty) {
            var c = new de.tum.cit.aet.artemis.programming.domain.StaticCodeAnalysisCategory();
            c.setName(name);
            c.setState(state);
        ...
        private VerificationResult verifyScaExercise(Integer maxPenalty, Boolean scaEnabled, Set<de.tum.cit.aet.artemis.programming.domain.StaticCodeAnalysisCategory> categories,
                SandboxExecResult solution, SandboxExecResult template) {
            ...
            var repo = mock(de.tum.cit.aet.artemis.programming.repository.StaticCodeAnalysisCategoryRepository.class);
        ...
        @Test
        void rejects_whenScaEnabledAndSolutionHasGradedViolation() {
            ...
            // score.
            var categories = Set.of(category("Code Style", de.tum.cit.aet.artemis.assessment.domain.CategoryState.GRADED, 0.2));
        ...
        @Test
        void accepts_whenScaEnabledButSolutionIsScaClean() {
            var categories = Set.of(category("Code Style", de.tum.cit.aet.artemis.assessment.domain.CategoryState.GRADED, 0.2));
        ...
        @Test
        void accepts_whenScaDisabled_evenIfFindingsLeakIntoOutput() {
        ...
        @Test
        void accepts_whenMaxPenaltyIsZero_soScaCannotAffectTheScore() {
            var categories = Set.of(category("Code Style", de.tum.cit.aet.artemis.assessment.domain.CategoryState.GRADED, 0.2));
        ...
        @Test
        void accepts_whenFindingIsInANonGradedCategory_noOverRejection() {
            ...
            var categories = Set.of(category("Code Style", de.tum.cit.aet.artemis.assessment.domain.CategoryState.FEEDBACK, 0.2),
                    category("Security", de.tum.cit.aet.artemis.assessment.domain.CategoryState.GRADED, 2.5));
            VerificationResult result = verifyScaExercise(50, true, categories, solutionWithScaFindings("SPOTBUGS|STYLE"), failingTemplate());
        ...
        @Test
        void accepts_whenGradedCategoryHasZeroPenalty_soItDeductsNothing() {
            // A GRADED category with penalty 0 contributes 0 points (size * 0). Production deducts nothing, so the oracle must not reject.
            var categories = Set.of(category("Code Style", de.tum.cit.aet.artemis.assessment.domain.CategoryState.GRADED, 0.0));
            VerificationResult result = verifyScaExercise(50, true, categories, solutionWithScaFindings("SPOTBUGS|STYLE"), failingTemplate());
            assertThat(result.accepted()).as("a graded category with zero penalty deducts nothing => no rejection").isTrue();
        }

        @Test
        void rejects_onUnknownCategoryFinding_whenTheToolHasAGradedCategory() {
    ...
    class StructuralBindingExemption {
        ...
        @Test
        void accepts_whenBehaviourTestsBoundButAutoSeededStructuralTestsAreNot() {
            ...
            java.util.List<String> all = java.util.List.of("sortsUnsortedArray", "sortsArrayWithDuplicates", "testClass[Sorter]", "testMethods[Sorter]");
            ...
            VerificationResult result = verify(resultWithFails(4, 0, all, java.util.List.of()), resultWithFails(4, 1, all, all));
            assertThat(result.accepted()).as("auto-seeded structural tests need no [task] binding; they remain in the differential").isTrue();
        }

        @Test
        void accepts_whenAStructuralTaskBindingResolvesToNothing_becauseSeederDeclinedToSeedIt() {
            ...
            java.util.List<String> all = java.util.List.of("sortsUnsortedArray", "sortsArrayWithDuplicates");
            String ps = "# Sort\n[task][Sort](sortsUnsortedArray,sortsArrayWithDuplicates)\n[task][Helper structure](testClass[Helper],testMethods[Helper])\n";
            VerificationResult result = verify(resultWithFails(2, 0, all, java.util.List.of()), resultWithFails(2, 1, all, all), ps);
        ...
        void resolvesStructuralBinding_whenItIsInTheAuthoritativeSeededSet_evenIfExtractionMomentarilyLagsBindingValue() {
            ...
            VerificationResult result = verifyWithSeededStructural(resultWithFails(4, 0, all, java.util.List.of()), resultWithFails(4, 1, all, all), ps,
        ...
        void forgeryResistance_aRealBehaviourTestNamedStructurallyThatPassesOnTemplate_isStillRejected() {
            ...
            java.util.List<String> all = java.util.List.of("realBehaviour", "testClass[Evil]");
            // realBehaviour fails on the template (good); testClass[Evil] PASSES on the template (the forged free point).
            java.util.List<String> failedOnTemplate = java.util.List.of("realBehaviour");
            String ps = "# X\n[task][Real](realBehaviour)\n[task][Disguised](testClass[Evil])\n";
            VerificationResult result = verify(resultWithFails(2, 0, all, java.util.List.of()), resultWithFails(2, 1, all, failedOnTemplate), ps);
        ...
        void differentialStillEnforced_whenAnAutoSeededStructuralTestPassesOnTemplate() {
            ...
            java.util.List<String> all = java.util.List.of("sortsUnsortedArray", "sortsArrayWithDuplicates", "testClass[Sorter]");
            // testClass[Sorter] PASSES on the template (not in the failed set); the two behaviour tests fail.
            java.util.List<String> failedOnTemplate = java.util.List.of("sortsUnsortedArray", "sortsArrayWithDuplicates");
            VerificationResult result = verifyWithSeededStructural(resultWithFails(3, 0, all, java.util.List.of()), resultWithFails(3, 1, all, failedOnTemplate),
                    PROBLEM_STATEMENT_WITH_TASK, Set.of("testClass[Sorter]"));
