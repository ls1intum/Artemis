...
class AgentLoopToolFailureTest {
    ...
    @Test
    void verificationFailuresDoNotMakeRejectedEditsTerminal() {
        ChatModel model = mock(ChatModel.class);
        var rejected = new ChatResponse(List.of(new Generation(AssistantMessage.builder().content("")
                .toolCalls(List.of(new AssistantMessage.ToolCall("edit", "function", "edit_file", "{}"), new AssistantMessage.ToolCall("check", "function", "verify", "{}")))
                .build())));
        when(model.call(any(Prompt.class))).thenReturn(rejected, rejected, rejected, rejected, rejected, rejected, text());
        ...
        assertThat(result.status()).isEqualTo(AgentLoopResult.Status.COMPLETED);
        verify(model, times(7)).call(any(Prompt.class));
    }

    @Test
    void cancellationDuringLargeToolResultDoesNotFailContextBudget() {
        ChatModel model = mock(ChatModel.class);
        when(model.call(any(Prompt.class))).thenReturn(call("read_file"));
        var cancelled = new AtomicBoolean();

        var result = runner(model).run("system", "brief", new CancellingTools(cancelled), 10, cancelled::get, null, null);
    ...
    static class CancellingTools implements SubmitVetoAware {
        ...
        CancellingTools(AtomicBoolean cancelled) {
        ...
        public String read() {
            cancelled.set(true);
            return "oversized result ".repeat(200_000);
        }
    }
    ...
    static class RejectedTools implements SubmitVetoAware {
        ...
        @Tool(name = "verify", description = "Verify the exercise")
        public String verifyExercise() {
            return "ERROR: solution tests failed";
        }
