...
@Component
@Profile("localci")
public class LocalCIRequestMockProvider {

    @Value("${artemis.continuous-integration.vcs-application-link-name}")
    private String vcsApplicationLinkName;

    @Autowired
    private LocalVCRequestMockProvider localVCRequestMockProvider;

    @Autowired
    private ObjectMapper mapper;

    public LocalCIRequestMockProvider() {
    }
     ...
     *
     * @param exercise the programming ex ercise that already exists
     */
     ...
     */
    public void mockCheckIfProjectExists(ProgrammingExercise exercise, final boolean exists, boolean shouldFail) {
        // Hier wird für Bamboo der mockServer.expect() konfiguriert, der den Request zu .../search/projects abfängt und je nach exists und shouldFail eine andere Antwort
        // zurückgibt.
        // Für den LocalCIService ist die "checkIfProjectExists()"-Methode noch nicht implementiert, deswegen gibt es auch nichts zu mocken bisher.
        // Hier noch irgendeinen Kommentar rein, dass ein Mock implementiert werden muss.
        // Wobei unter Umständen genau wie für den LocalVCRequestMockProvider.mockGetDefaultBranch die Tests später gar keinen Mock aufrufen müssen,
        // sondern tatsächlich Ordner erstellen und darauf testen.
    }

    public void mockRemoveAllDefaultProjectPermissions(ProgrammingExercise exercise) {
        // Nicht implementiert für local CI also auch nichts zu mocken.
    }

    public void mockGiveProjectPermissions(ProgrammingExercise exercise) {
        // Nicht implementiert für local CI also auch nichts zu mocken.
    }

    private URI buildGivePermissionsURIFor(String projectKey, String groupName) throws URISyntaxException {
        // return
        // UriComponentsBuilder.fromUri(bambooServerUrl.toURI()).path("/rest/api/latest/permissions/project/").pathSegment(projectKey).path("/groups/").pathSegment(groupName)
        // .build().toUri();
        return null;
    }
    ...
    public void mockGetBuildPlanRepositoryList(String buildPlanKey) throws IOException, URISyntaxException {
        ...
        // var uri = UriComponentsBuilder.fromUri(bambooServerUrl.toURI()).path("/chain/admin/config/editChainRepository.action").queryParam("buildKey",
        // buildPlanKey).build().toUri();
    ...
    public void mockDeleteBambooBuildProject(String projectKey) throws URISyntaxException, JsonProcessingException {
        ...
        // 1) get all plans
        // var uri = UriComponentsBuilder.fromUri(bambooServerUrl.toURI()).path("/rest/api/latest/project/" + projectKey).queryParam("expand", "plans").queryParam("max-results",
        // 5000)
        // .build().toUri();
        // mockServer.expect(requestTo(uri)).andExpect(method(HttpMethod.GET))
