...
import de.tum.in.www1.artemis.connector.LocalCIRequestMockProvider;
import de.tum.in.www1.artemis.connector.LocalVCRequestMockProvider;
...
@ActiveProfiles({ SPRING_PROFILE_TEST, "artemis", "localci", "localvc", "jira", "ldap", "scheduling", "athene", "apollon" })
public abstract class AbstractSpringIntegrationLocalCILocalVCJiraTest extends AbstractArtemisIntegrationTest {
    ...
    @Autowired
    protected LocalCIRequestMockProvider localCIRequestMockProvider;

    @Autowired
    protected LocalVCRequestMockProvider localVCRequestMockProvider;
    ...
    protected void resetSpyBeans() {
        Mockito.reset(ldapUserService, continuousIntegrationService, versionControlService, textBlockService);
        super.resetSpyBeans();
    }
    ...
    public void mockConnectorRequestsForSetup(ProgrammingExercise exercise, boolean failToCreateCiProject) throws Exception {
        ...
        localCIRequestMockProvider.mockCheckIfProjectExists(exercise, false, false);
        localVCRequestMockProvider.mockCheckIfProjectExists(exercise, false);
        localVCRequestMockProvider.mockCreateProjectForExercise(exercise);
        localVCRequestMockProvider.mockCreateRepository(exercise, exerciseRepoName);
        localVCRequestMockProvider.mockCreateRepository(exercise, testRepoName);
        localVCRequestMockProvider.mockCreateRepository(exercise, solutionRepoName);
        localVCRequestMockProvider.mockDefaultBranch(defaultBranch, exercise.getProjectKey());
        localVCRequestMockProvider.mockAddWebHooks(exercise);
        mockLocalCIBuildPlanCreation(exercise, failToCreateCiProject);

        // ? Die Methode wird nur in GitServiceTest (extends AbstractSpringIntegrationBambooBitbucketJiraTest) aufgerufen.
        // Keine Ahnung, warum sie dann z.B. für createProgrammingExercise_mode_validExercise_created stumm geschalten werden muss.
        doNothing().when(gitService).pushSourceToTargetRepo(any(), any());
    }

    private void mockLocalCIBuildPlanCreation(ProgrammingExercise exercise, boolean failToCreateCiProject) {
        if (!failToCreateCiProject) {
            // TODO: check the actual plan and plan permissions that get passed here
            // Local CI is not implemented yet. Accordingly, build plan creation does not have to be mocked.
            localCIRequestMockProvider.mockRemoveAllDefaultProjectPermissions(exercise);
            localCIRequestMockProvider.mockGiveProjectPermissions(exercise);
        }
        else {
            // Not implemented yet what will happen, if CI project creation fails.
            // TODO: throw the expected error here.
        }
    }
    ...
    public void mockConnectorRequestsForImport(ProgrammingExercise sourceExercise, ProgrammingExercise exerciseToBeImported, boolean recreateBuildPlans, boolean addAuxRepos)
        ...
        if (!recreateBuildPlans) {
            ...
            mockLocalCIBuildPlanCreation(exerciseToBeImported, false);
        }
        if (addAuxRepos) {
    ...
    public void resetMockProvider() {
        localVCRequestMockProvider.reset();
        localCIRequestMockProvider.reset();
    }
