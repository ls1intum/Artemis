...
public class QueryCountAssert<T, E extends Exception> extends AbstractAssert<QueryCountAssert<T, E>, ThrowingProducer<T, E>> {
    ...
    protected QueryCountAssert(HibernateQueryInterceptor interceptor, ThrowingProducer<T, E> throwingProducer, Class<?> selfType) {
        super(throwingProducer, selfType);
     ...
     */
    public CallResult<T> hasBeenCalledTimes(long times) throws E {
     ...
     */
    public CallResult<T> hasBeenCalledAtMostTimes(long times) throws E {
        var result = performCall();
        if (result.callCount > times) {
    ...
    public record CallResult<T> (T result, long callCount) {
    }
