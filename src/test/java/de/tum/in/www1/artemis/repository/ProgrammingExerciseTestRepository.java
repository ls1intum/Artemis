...
public interface ProgrammingExerciseTestRepository extends JpaRepository<ProgrammingExercise, Long> {
    ...
    @Query("select p from ProgrammingExercise p left join fetch p.studentParticipations left join fetch p.attachments left join fetch p.categories "
            ...
            + "left join fetch p.tutorParticipations left join fetch p.posts left join fetch p.testCases left join fetch p.staticCodeAnalysisCategories "
            + "left join fetch p.auxiliaryRepositories left join fetch p.tasks t left join t.testCases where p.id = :#{#exerciseId}")
