...
public class ExamQuizServiceTest extends AbstractSpringIntegrationBambooBitbucketJiraTest {
    ...
    public void init() {
        ...
        quizExercise = database.createQuizForExam(exerciseGroup, ZonedDateTime.now().minusHours(3), ZonedDateTime.now().minusMinutes(2));
    ...
    public void evaluateQuiz() throws Exception {
        ...
        for (int i = 0; i < numberOfParticipants; i++) {
            ...
            quizSubmissionService.saveSubmissionForExamMode(quizExercise, quizSubmission, users.get(i).getLogin());
        }
