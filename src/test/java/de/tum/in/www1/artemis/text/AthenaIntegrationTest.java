...
class AthenaIntegrationTest extends AbstractSpringIntegrationBambooBitbucketJiraTest {
    ...
    void testProcessingClusterAddedDistances() throws Exception {
        ...
        final var athenaResultBuilder = AtheneResponse.newBuilder();
        ...
        athenaResultBuilder.addAllSegments(segments);

        List.of(0, 1, 2).forEach(cid -> {
            var clusterBuilder = athenaResultBuilder.addClustersBuilder();
        ...
        List<Long> runningAthenaTasks = new ArrayList<>();
        runningAthenaTasks.add(exercise.getId());
        ReflectionTestUtils.setField(athenaService, "runningAthenaTasks", runningAthenaTasks);

        final HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.set("Authorization", athenaApiSecret);
        AtheneResponse athenaResponse = athenaResultBuilder.build();
        request.postWithoutLocation("/api/athena-result/" + exercise.getId(), athenaResponse.toByteArray(), HttpStatus.OK, httpHeaders, "application/x-protobuf");
        ...
        for (int clusterIndex = 0; clusterIndex < clusters.size(); clusterIndex++) {
            ...
            for (int blockIndex = 0; blockIndex < blocks.size(); blockIndex++) {
                ...
                Segment segment = athenaResponse.getClusters(clusterIndex).getSegmentsList().get(blockIndex);
