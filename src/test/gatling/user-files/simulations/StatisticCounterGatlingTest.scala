...
class StatisticCounterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the QuizStatisticCounter entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all statisticCounters")
            .get("/api/statistic-counters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created statisticCounter")
                .get("${new_statisticCounter_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created statisticCounter")
            .delete("${new_statisticCounter_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
