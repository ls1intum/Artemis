...
class StatisticGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Statistic entity")
        ...
        .repeat(2) {
            ...
            .post("/api/statistics")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "released":null, "participantsRated":"0", "participantsUnrated":"0"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_statistic_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created statistic")
                .get("${new_statistic_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created statistic")
            .delete("${new_statistic_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
